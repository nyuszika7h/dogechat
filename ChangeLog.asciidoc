...
https://dogechat.org/files/changelog/ChangeLog-devel.html[here]).

For a list of important changes that require manual action, please look at
https://dogechat.org/files/releasenotes/ReleaseNotes-devel.html[release notes]
...
* core: add a parent name in options, display inherited values if null in
  /set output, add option dogechat.color.chat_value_null (issue #629)
* core: add tag "term_warning" in warnings about wrong $TERM on startup
* core: add option dogechat.look.paste_auto_add_newline (issue #543)
...
* core: don't complain any more about "tmux" and "tmux-256color" $TERM values
  when DogeChat is running under tmux (issue #519)
...
* core: add option dogechat.look.key_grab_delay to set the default delay when
  grabbing a key with key[alt-k]
* core: add option dogechat.look.confirm_upgrade (issue #463)
* core: allow key[ctrl-c] to exit DogeChat when the passphrase is asked on
  startup (issue #452)
...
* core: fix display of time in bare display when option
  dogechat.look.buffer_time_format is set to empty string (issue #441)
...
* core: use environment variable DOGECHAT_HOME on startup (issue #391)
* core: remove DogeChat version from config files (issue #407)
* core: add options dogechat.look.quote_{nick_prefix|nick_suffix|time_format} to
  customize quoted messages in cursor mode (issue #403)
* core: add a welcome message on first DogeChat run (issue #318)
* core: add options dogechat.look.word_chars_{highlight|input}
  (issue #55, task #9459)
...
* core: fix random error when creating symbolic link dogechat-curses on
...
* core: add option dogechat.completion.command_inline (task #12491)
* core: add bar item "mouse_status", new options dogechat.look.item_mouse_status
  and dogechat.color.status_mouse (issue #247)
...
* core: allow incomplete commands if unambiguous, new option
  dogechat.look.command_incomplete (task #5419)
* api: send value returned by command callback in function command(), remove
  DogeChat error after command callback if return code is DOGECHAT_RC_ERROR
...
* core: display time in bare display only if option
  dogechat.look.buffer_time_format is not an empty string
...
* core: fix crash on buffer close when option dogechat.look.hotlist_remove is
...
* core: fix modifier "dogechat_print": discard only one line when several lines
...
* core: display a warning on startup if $TERM does not start with "screen"
  under screen/tmux
* core: add option dogechat.color.status_nicklist_count (issue #109,
  issue #110)
...
* core: add option dogechat.look.hotlist_add_conditions, remove option
  dogechat.look.hotlist_add_buffer_if_away
* core: add option dogechat.look.hotlist_remove (issue #99)
...
* core: add bare display mode (for easy text selection and click on URLs), new
  ...
  dogechat.look.bare_display_exit_on_input and
  dogechat.look.bare_display_time_format
* core: add signals "key_combo_{default|search|cursor}"
* core: display a warning in case of inconsistency between the options
  dogechat.look.save_{config|layout}_on_exit
...
* core: fix alignment of lines in merged buffers when options
  dogechat.look.prefix_align and dogechat.look.prefix_buffer_align are set to
  "none" (issue #43)
* core: quit DogeChat on signal SIGHUP, remove signal "signal_sighup"
...
* relay: fix memory leak when receiving commands from client (dogechat protocol)
...
* relay: fix crash on /upgrade received from a client (dogechat protocol)
...
* core: add command /print, add support of more escaped chars in command
  "/input insert"
* core: add option dogechat.look.tab_width
* core: add completion "plugins_installed"
* core: add support of UTF-8 chars in horizontal/vertical separators (options
  dogechat.look.separator_{horizontal|vertical})
* core: add option dogechat.look.window_auto_zoom, disable automatic zoom by
...
* core: use "+" separator to make a logical "and" between tags in command
  /filter, option dogechat.look.highlight_tags and buffer property
  "highlight_tags"
* core: rename options save/reset to store/del in command /layout
* core: add options dogechat.look.buffer_auto_renumber and
  dogechat.look.buffer_position, add option "renumber" in command /buffer, add
...
* core: rename option dogechat.look.set_title to dogechat.look.window_title,
...
* core: add options to customize default text search in buffers:
  dogechat.look.buffer_search_{case_sensitive|force_default|regex|where}
...
* core: fix installation of dogechat-plugin.h with autotools (patch #8305)
...
* core: fix display of read marker when all buffer lines are unread and that
  option dogechat.look.read_marker_always_show is on
...
* core: fix crash on /buffer close core.dogechat
* core: apply color attributes when clearing a window (patch #8236)
* core: set option dogechat.look.paste_bracketed to "on" by default
...
* core: display day change message dynamically (do not store it as a line in
  buffer), split option dogechat.look.day_change_time_format into two options
  dogechat.look.day_change_message_{1date|2dates}, new option
  dogechat.color.chat_day_change (task #12775)
...
* core: add text emphasis in messages when searching text in buffer, new
  options: dogechat.look.emphasized_attributes, dogechat.color.emphasized,
  dogechat.color.emphasized_bg
* core: change color format for options dogechat.look.buffer_time_format and
  dogechat.look.prefix_{action|error|join|network|quit} from `${xxx}` to
  `${color:xxx}`
* core: add secured data (encryption of passwords or private data): add new
  command /secure and new file sec.conf (task #7395)
* core: rename binary and man page from "dogechat-curses" to "dogechat"
  (task #11027)
...
* core: add CA_FILE option in cmake and configure to setup default
  value of option dogechat.network.gnutls_ca_file (default is
  "/etc/ssl/certs/ca-certificates.crt") (task #12725)
* core: add option "scroll_beyond_end" for command /window (task #6745)
* core: add options dogechat.look.hotlist_prefix and dogechat.look.hotlist_suffix
  (task #12730)
* core: add option dogechat.look.key_bind_safe
* core: update man page and add translations (in French, German, Italian, and
  Japanese)
* core: add option dogechat.network.proxy_curl (task #12651)
...
* relay: add command "ping" in dogechat protocol (task #12689)
...
* core: set options dogechat.look.color_inactive_{buffer|window} to "on" by
  default
* core: fix line alignment when option dogechat.look.buffer_time_format is set
  to empty string
...
* core: make nick prefix/suffix dynamic (not stored in the line): move options
  irc.look.nick_{prefix|suffix} to dogechat.look.nick_{prefix|suffix} and
  options irc.color.nick_{prefix|suffix} to
  dogechat.color.chat_nick_{prefix|suffix}, add new options
  dogechat.look.prefix_align_more_after,
  dogechat.look.prefix_buffer_align_more_after, logger.file.nick_{prefix|suffix}
  (bug #37531)
...
* relay: add WebSocket server support (RFC 6455) for irc and dogechat protocols,
  new option relay.network.websocket_allowed_origins
* relay: add options "buffers" and "upgrade" in commands sync/desync (dogechat
  protocol)
...
* core: fix hidden lines for messages without date when option
  dogechat.history.max_buffer_lines_minutes is set (bug #38197)
...
* relay: fix commands sync/desync in dogechat protocol (bug #38215)
...
* core: add git version in build, display it in "dogechat-curses --help" and
  /version
* core: add color support in options
  dogechat.look.prefix_{action|error|join|network|quit} (task #9555)
...
* core: add option "-quit" in command /upgrade (save session and quit without
  restarting DogeChat, for delayed restoration)
* api: allow return code DOGECHAT_RC_OK_EAT in callbacks of hook_signal() and
...
* core: fix scroll of one page down when dogechat.look.scroll_page_percent is
...
* core: fix display of zoomed/merged buffer (with number >= 2) after switching
  to it (bug #37593)
* core: fix display problem when option dogechat.look.prefix_same_nick is set
...
* relay: fix memory leak when adding hdata to a message (dogechat protocol)
...
* core: add option dogechat.startup.sys_rlimit to set system resource limits for
  DogeChat process
...
* core: add option dogechat.look.jump_smart_back_to_buffer (jump back to initial
...
* core: add horizontal separator between windows, new options
  dogechat.look.window_separator_{horizontal|vertical}
* core: add options dogechat.look.color_nick_offline and
  dogechat.color.chat_nick_offline{_highlight|_highlight_bg} to use different
...
* irc: generate alternate nicks dynamically when all nicks are already in use
  (task #12209)
* irc: move options from core to irc plugin: dogechat.look.nickmode to
  irc.look.nick_mode (new type: integer with values: none/prefix/action/both)
  and dogechat.look.nickmode_empty to irc.look.nick_mode_empty
...
* relay: add support of SSL (for irc and dogechat protocols), new option
  relay.network.ssl_cert_key (task #12044)
* relay: add option relay.color.client
* relay: add object type "arr" (array) in DogeChat protocol
* script: new plugin "script" (scripts manager, replacing scripts dogeget.py and
  script.pl)
...
* charset: do not allow "UTF-8" in charset decoding options (useless because
  UTF-8 is the internal DogeChat charset)
...
* relay: fix freeze when writing on relay socket (use non-blocking sockets in
  relay for irc and dogechat protocols) (bug #36655)
...
* core: convert options dogechat.look.prefix_align_more and
  dogechat.look.prefix_buffer_align_more from boolean to string (task #11197)
* core: add option dogechat.look.prefix_same_nick (hide or change prefix on
...
* core: add support of terminal "bracketed paste mode", new options
  dogechat.look.paste_bracketed and dogechat.look.paste_bracketed_timer_delay
  (task #11316)
...
* relay: add signals "upgrade" and "upgrade_ended" in DogeChat protocol
...
* relay: keep spaces in beginning of "input" received from client (DogeChat
  protocol)
* relay: fix crash on /upgrade when client is connected using DogeChat protocol
...
* core: add option "dogechat.look.scroll_bottom_after_switch" (if enabled,
  restore old behavior before fix of bug #25555 in version 0.3.5)
* core: add new option dogechat.completion.base_word_until_cursor: allow
...
* core: add option dogechat.history.max_buffer_lines_minutes: maximum number of
  minutes in history per buffer (task #10900), rename option
  dogechat.history.max_lines to dogechat.history.max_buffer_lines_number
* core: add DOGECHAT_HOME option in cmake and configure to setup default
  DogeChat home (default is "~/.dogechat") (task #11266)
...
* relay: add DogeChat protocol for remote GUI
...
* core: do not auto add space after nick completer if option
  dogechat.completion.nick_add_space is off
...
* core: save current mouse state in option dogechat.look.mouse (set option when
  mouse state is changed with command /mouse)
* core: apply filters after full reload of configuration files (with /reload)
  (bug #31182)
* core: allow list for option dogechat.plugin.extension (makes dogechat.conf
...
* irc: fix self-highlight when using /me with an IRC bouncer like znc
  (bug #35123)
* irc: use low priority for MODE sent automatically by DogeChat (when joining
  channel)
...
* core: add new option dogechat.look.color_basic_force_bold, off by default:
...
* core: add "inactive" colors for inactive windows and lines in merged buffers,
  new options: dogechat.look.color_inactive_window,
  dogechat.look.color_inactive_buffer, dogechat.look.color_inactive_message,
  dogechat.look.color_inactive_prefix,
  dogechat.look.color_inactive_prefix_buffer,
  dogechat.look.color_inactive_time, dogechat.color.chat_inactive_line,
  dogechat.color.chat_inactive_window,
  dogechat.color.chat_prefix_buffer_inactive_line
* core: do automatic zoom on current window when terminal becomes too small
  for windows
* core: add new options dogechat.look.bar_more_left/right/up/down
* core: add new option dogechat.look.item_buffer_filter
...
* core: add support of mouse: new command /mouse, new key context "mouse",
  new options dogechat.look.mouse and dogechat.look.mouse_timer_delay
  (task #5435)
...
* core: add new option dogechat.look.separator_vertical, rename option
  dogechat.look.hline_char to dogechat.look.separator_horizontal
* core: add local variable "highlight_regex" in buffers
* core: add "hdata" (direct access to DogeChat/plugin data)
* core: add option dogechat.look.eat_newline_glitch (do not add new line at end
...
* core: fix display of paste multi-line prompt with a root input bar
  (bug #34305)
* core: change default value of option dogechat.network.gnutls_ca_file to
...
* core: fix bug with option dogechat.look.hotlist_count_max (value+1 was used)
...
* core: add buffer to hotlist if away is set on buffer (even if buffer is
  displayed), new option dogechat.look.hotlist_add_buffer_if_away (task #10948)
* core: add option "balance" in command /window (key: key[alt-w,alt-b])
* core: add option "swap" in command /window (key: key[alt-w,alt-s])
  (task #11001)
* core: add option dogechat.look.hotlist_buffer_separator
* core: add messages counts in hotlist for each buffer, new options:
  dogechat.look.hotlist_count_max, dogechat.look.hotlist_count_min_msg and
  dogechat.color.status_count_{msg|private|highlight|other}
* core: add tag "notify_none" (line with this tag will not update hotlist)
* core: add optional bar name in command /bar default
* core: add new option dogechat.look.highlight_tags (force highlight on tags)
...
* core: add option dogechat.look.read_marker_string
...
* core: add some chars after cursor when scrolling input line: new option
  dogechat.look.input_cursor_scroll (bug #21391)
* core: add color "gray"
...
* core: apply new value of option dogechat.look.buffer_notify_default to all
  opened buffers
...
* core: reload file with certificate authorities when option
  dogechat.network.gnutls_ca_file is changed
...
* core: add 256 colors support, new command /color, new section "palette" in
  dogechat.conf (task #6834)
* core: add info "dogechat_upgrading", signal "upgrade_ended", display duration
  of upgrade
* core: replace the 10 nick color options and number of nick colors by a single
  option "dogechat.color.chat_nick_colors" (comma separated list of colors)
* core: add color support in option dogechat.look.buffer_time_format
* core: add new option dogechat.look.highlight_regex and function
  string_has_highlight_regex() in plugin API (task #10321)
* core: add new option dogechat.look.hotlist_unique_numbers (task #10691)
* core: add property "hotlist_max_level_nicks" in buffers to set max hotlist
  level for some nicks in buffer
* core: add new options dogechat.look.input_share and
  dogechat.look.input_share_overwrite (task #9228)
* core: add new option dogechat.look.prefix_align_min (task #10650)
...
* irc: fix crash/bug when option "addresses" for a server is unset or changed
  when DogeChat is connected to this server (bug #31268)
...
* core: add new option dogechat.look.align_end_of_lines
* core: add new option dogechat.look.confirm_quit
* core: add new option dogechat.color.status_name_ssl (task #10339)
...
* irc: move options dogechat.color.nicklist_prefix to irc plugin
...
* core: add command line option "-s" (or "--no-script") to start DogeChat
  without loading any script
* core: improve plugins autoload (option dogechat.plugin.autoload): allow to
...
* core: add option "switch_active_buffer_previous" in command /input
  (task #10141)
* core: add new option dogechat.look.time_format to customize default format
  for date/time displayed (localized date by default), add function
  util_get_time_string() in plugin API (patch #6914)
* core: add new option dogechat.look.command_chars, add functions
  string_is_command_char() and string_input_for_buffer() in plugin and script
  API
* core: add new option dogechat.look.read_marker_always_show
* api: add "version_number" for function info_get() to get DogeChat version as
  number
...
* core: remove unneeded space after time on each line if option
  dogechat.look.buffer_time_format is set to empty value (bug #28751)
...
* gui: refresh screen when exiting DogeChat (to display messages printed after
  /quit)
...
* api: fix "inactivity" value when no key has been pressed since DogeChat
  started (bug #28930)
* api: return absolute path for info_get() of "dogechat_dir" (bug #27936)
...
* core: fix terminal title bug: do not reset it when option
  dogechat.look.set_title is off (bug #27399)
...
* core: add /reload command to reload DogeChat and plugins config files (signal
...
* relay: add relay plugin (network communication between DogeChat and remote
  application)
...
* scripts: new scripts: dogeget.py (scripts manager), jabber.py (jabber/XMPP
  protocol), go.py (quick jump to buffers), buffers.pl (sidebar with list of
  buffers), iset.pl (set options interactively), dogetris.pl (tetris-like game),
  mastermind.pl, ...
...
* fix server option "command_delay": does not freeze DogeChat any more
...
* add cmake for dogechat compile (patch #5943)
...
* rename log file for DCC chat (now <server>.dcc.<nick>.dogechatlog)
...
* new signals handled: SIGTERM and SIGHUP (received when terminal is closed):
  clean DogeChat quit (send quit to irc servers then quit DogeChat)
...
* fix "wallops" command when received, now displayed by DogeChat (bug #17441)
...
* command /save now writes plugins options (~/.dogechat/plugins.rc)
...
* add backtrace when DogeChat crashes, log file automatically renamed
...
* aliases are executed before DogeChat/IRC commands, add /builtin command
...
* add lock for log file (~/.dogechat/dogechat.log), only one DogeChat process can
...
* add special names for plugin message handlers: dogechat_pv, dogechat_highlight,
  dogechat_ctcp, dogechat_dcc
...
* new behavior for messages ignored by a message handler: now DogeChat
...
* add new command line argument for setting DogeChat home dir (-d or --dir)
  (patch from Gwenn)
...
* script plugins now load scripts in DogeChat system share directory
...
* log options (for server/channel/private) can now be set while DogeChat is
  running
...
* part message now accepts %v (replaced by DogeChat version), like quit message
...
* fix startup crash when config file (~/.dogechat/dogechat.rc) is not found
...
* DogeChat now OK under *BSD and Mac OS X
...
* add crash dump when DogeChat receives SIGSEGV (Segmentation fault)
...
* add --irc-commands and --dogechat-commands command line options
...
* improve /set command: empty strings are allowed, new colors, server options
  can be changed while DogeChat is running
...
* /set command to modify config options when DogeChat is running
...
* when private window is created (another user is talking), DogeChat does not
...
* ./configure script to build DogeChat
* French translation
...
* config is now saved automatically when quitting DogeChat, add /save command
...
* term window title is modified with DogeChat name and version
...
* DogeChat commands: clear, help, set (partial)
* many config options
* log file (~/.dogechat/dogechat.log)
