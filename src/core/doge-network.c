/*
 * doge-network.c - network functions
 *
...
network_connect_child (struct t_hook *hook_connect)
{
        ...
        && HOOK_CONNECT(hook_connect, proxy)[0])
    {
        ...
        if (!ptr_proxy)
        {
            ...
            snprintf (status_without_string, sizeof (status_without_string),
                      "%c00000", '0' + DOGECHAT_HOOK_CONNECT_PROXY_ERROR);
    ...
    if (rc != 0)
    {
        ...
        if (error)
        {
            ...
            if (status_with_string)
            {
                snprintf (status_with_string, length, "%c%05d%s",
                          '0' + DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND,
        ...
        {
            snprintf (status_without_string, sizeof (status_without_string),
                      "%c00000", '0' + DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND);
            num_written = write (HOOK_CONNECT(hook_connect, child_write),
                                 status_without_string, strlen (status_without_string));
        }
        (void) num_written;
        ...
        && HOOK_CONNECT(hook_connect, local_hostname[0]))
    {
        ...
        if (rc != 0)
        {
            ...
            if (error)
            {
                ...
                {
                    snprintf (status_with_string, length, "%c%05d%s",
                              '0' + DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR,
                              (int)strlen (error), error);
                }
            }
            if (status_with_string)
            ...
            {
                snprintf (status_without_string, sizeof (status_without_string),
                          "%c00000", '0' + DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR);
                num_written = write (HOOK_CONNECT(hook_connect, child_write),
                                     status_without_string, strlen (status_without_string));
            }
            (void) num_written;
    ...
    {
        snprintf (status_without_string, sizeof (status_without_string),
                  "%c00000", '0' + DOGECHAT_HOOK_CONNECT_MEMORY_ERROR);
        num_written = write (HOOK_CONNECT(hook_connect, child_write),
                             status_without_string, strlen (status_without_string));
        (void) num_written;
        goto end;
    ...
    else
    {
        ...
        snprintf (status_without_string, sizeof (status_without_string),
                  "%c00000", '0' + DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND);
        num_written = write (HOOK_CONNECT(hook_connect, child_write),
                             status_without_string, strlen (status_without_string));
        (void) num_written;
        goto end;
    ...
    status_str[0] = '0' + DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND;
    ...
    for (i = 0; i < num_hosts; i++)
    {
        ...
        if (sock < 0)
        {
            status_str[0] = '0' + DOGECHAT_HOOK_CONNECT_SOCKET_ERROR;
            continue;
        }
        ...
        if (res_local)
        {
            ...
            if (rc < 0)
            {
                status_str[0] = '0' + DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR;
                close (sock);
                sock = -1;
                continue;
            }
        ...
        {
            status_str[0] = '0' + DOGECHAT_HOOK_CONNECT_OK;
        ...
        {
            status_str[0] = '0' + DOGECHAT_HOOK_CONNECT_CONNECTION_REFUSED;
            close (sock);
            sock = -1;
        }
    }
    ...
    if (ptr_proxy && status_str[0] == '0' + DOGECHAT_HOOK_CONNECT_OK)
    {
                                 ...
                                 HOOK_CONNECT(hook_connect, port)))
        {
            ...
            status_str[0] = '0' + DOGECHAT_HOOK_CONNECT_PROXY_ERROR;
        }
    }

    if (status_str[0] == '0' + DOGECHAT_HOOK_CONNECT_OK)
    {
        status_with_string = NULL;
        if (ptr_address)
        {
 ...
 *
 * This callback is used to not block DogeChat (handshake takes some time to
 * finish).
 */
...
network_connect_gnutls_handshake_fd_cb (void *arg_hook_connect, int fd)
{
    ...
    else if (rc != GNUTLS_E_SUCCESS)
    {
        (void) (HOOK_CONNECT(hook_connect, callback))
            (hook_connect->callback_data,
             DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR, rc,
             HOOK_CONNECT(hook_connect, sock),
             gnutls_strerror (rc),
                                           ...
                                           int remaining_calls)
{
    ...
    (void) (HOOK_CONNECT(hook_connect, callback))
            (hook_connect->callback_data,
             DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR,
             GNUTLS_E_EXPIRED,
             HOOK_CONNECT(hook_connect, sock),
...
network_connect_child_read_cb (void *arg_hook_connect, int fd)
{
    ...
    {
        if (buffer[0] - '0' == DOGECHAT_HOOK_CONNECT_OK)
        {
    ...
    {
        (void) (HOOK_CONNECT(hook_connect, callback))
            (hook_connect->callback_data, DOGECHAT_HOOK_CONNECT_MEMORY_ERROR,
...
network_connect_with_fork (struct t_hook *hook_connect)
{
    ...
    if (HOOK_CONNECT(hook_connect, gnutls_sess))
    {
        if (gnutls_init (HOOK_CONNECT(hook_connect, gnutls_sess), GNUTLS_CLIENT) != GNUTLS_E_SUCCESS)
        {
            (void) (HOOK_CONNECT(hook_connect, callback))
                (hook_connect->callback_data,
                 DOGECHAT_HOOK_CONNECT_GNUTLS_INIT_ERROR,
                 0, -1, NULL, NULL);
            unhook (hook_connect);
            return;
        }
    ...
    if (pipe (child_pipe) < 0)
    {
        (void) (HOOK_CONNECT(hook_connect, callback))
            (hook_connect->callback_data,
             DOGECHAT_HOOK_CONNECT_MEMORY_ERROR,
             0, -1, NULL, NULL);
        unhook (hook_connect);
        return;
    }
