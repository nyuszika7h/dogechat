...
struct t_proxy *dogechat_proxies = NULL;    /* first proxy                   */
struct t_proxy *last_dogechat_proxy = NULL; /* last proxy                    */

struct t_proxy *dogechat_temp_proxies = NULL;    /* proxies used when        */
struct t_proxy *last_dogechat_temp_proxy = NULL; /* reading configuration    */
    ...
    switch (index_option)
    {
        case PROXY_OPTION_TYPE:
            ptr_option = config_file_new_option (
                dogechat_config_file, dogechat_config_section_proxy,
                option_name, "integer",
                        ...
                        struct t_config_option *password)
{
    ...
    new_proxy->prev_proxy = last_dogechat_proxy;
    if (dogechat_proxies)
        last_dogechat_proxy->next_proxy = new_proxy;
    else
        dogechat_proxies = new_proxy;
    last_dogechat_proxy = new_proxy;
    new_proxy->next_proxy = NULL;
...
proxy_use_temp_proxies ()
{
    ...
    while (dogechat_temp_proxies)
    {
        next_temp_proxy = dogechat_temp_proxies->next_proxy;

        if (dogechat_temp_proxies->name)
            free (dogechat_temp_proxies->name);
        free (dogechat_temp_proxies);

        dogechat_temp_proxies = next_temp_proxy;
    }
    last_dogechat_temp_proxy = NULL;
}
...
proxy_free (struct t_proxy *proxy)
{
    ...
    if (dogechat_proxies == proxy)
        dogechat_proxies = proxy->next_proxy;
    if (last_dogechat_proxy == proxy)
        last_dogechat_proxy = proxy->prev_proxy;
...
proxy_free_all ()
{
    while (dogechat_proxies)
    {
        proxy_free (dogechat_proxies);
    }
}
...
proxy_hdata_proxy_cb (void *data, const char *hdata_name)
{
    ...
    if (hdata)
    {
        ...
        HDATA_LIST(dogechat_proxies, DOGECHAT_HDATA_LIST_CHECK_POINTERS);
        HDATA_LIST(last_dogechat_proxy, 0);
    }
    return hdata;
}
...
/*
 * Prints proxies in DogeChat log file (usually for crash dump).
 */
