/*
 * doge-config-file.c - configuration files/sections/options management
 *
...
struct t_config_file *
config_file_new (struct t_dogechat_plugin *plugin, const char *name,
                        ...
                        void *callback_delete_data)
{
    ...
    if (new_option)
    {
        ...
        switch (var_type)
        {
            ...
            case CONFIG_OPTION_TYPE_COLOR:
                new_option->min = min;
                new_option->max = gui_color_get_dogechat_colors_number () - 1;
                if (default_value)
                {
                    new_option->default_value = malloc (sizeof (int));
                    if (!new_option->default_value)
 ...
 *
 * Returns:
 *   DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED: OK, value has been changed
 *   DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE: OK, value not changed
 *   DOGECHAT_CONFIG_OPTION_SET_ERROR: error
 */
...
config_file_option_reset (struct t_config_option *option, int run_callback)
{
            ...
            case CONFIG_OPTION_TYPE_BOOLEAN:
                if (!option->value)
                {
                    ...
                    if (option->value)
                    {
                        ...
                        rc = DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED;
                    }
                }
                else
                {
    ...
    if ((rc == DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED)
 ...
 *
 * Returns:
 *   DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET: OK, value has not been reset
 *   DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET: OK, value has been reset
 *   DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED: OK, value has been removed
 *   DOGECHAT_CONFIG_OPTION_UNSET_ERROR: error
 */
...
config_file_option_unset (struct t_config_option *option)
{
    ...
    rc = DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET;
    ...
    if (option->section && option->section->user_can_delete_options)
    {
        ...
        else
        {
            ...
            rc = DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED;
        }
    ...
    else
    {
        ...
        switch (config_file_option_reset (option, 1))
        {
            case DOGECHAT_CONFIG_OPTION_SET_ERROR:
                rc = DOGECHAT_CONFIG_OPTION_UNSET_ERROR;
                break;
            case DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE:
                rc = DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET;
                break;
            case DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
                rc = DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET;
                break;
        }
    }
 ...
 *   DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE: OK, value not changed
 *   DOGECHAT_CONFIG_OPTION_SET_ERROR: error
 *   DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND: option not found
 */
...
config_file_option_set_with_string (const char *option_name, const char *value)
{
    ...
    rc = DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND;
                            ...
                            int default_options)
{
    ...
    /* build filename */
    filename_length = strlen (dogechat_home) +
        strlen (config_file->filename) + 2;
    filename = malloc (filename_length);
    if (!filename)
        return DOGECHAT_CONFIG_WRITE_MEMORY_ERROR;
    snprintf (filename, filename_length, "%s%s%s",
              dogechat_home, DIR_SEPARATOR, config_file->filename);
    ...
    snprintf (filename2, filename_length + 32, "%s.dogechattmp", filename);
    ...
    /* write header with name of config file and DogeChat version */
         ...
         ptr_section = ptr_section->next_section)
    {
        ...
        if (default_options && ptr_section->callback_write_default)
        {
            if ((ptr_section->callback_write_default) (ptr_section->callback_write_default_data,
                                                       config_file,
                                                       ptr_section->name) != DOGECHAT_CONFIG_WRITE_OK)
                goto error;
        }
...
config_file_read_internal (struct t_config_file *config_file, int reload)
{
    ...
    /* build filename */
    filename_length = strlen (dogechat_home) + strlen (DIR_SEPARATOR) +
        strlen (config_file->filename) + 1;
    filename = malloc (filename_length);
    if (!filename)
        return DOGECHAT_CONFIG_READ_MEMORY_ERROR;
    snprintf (filename, filename_length, "%s%s%s",
              dogechat_home, DIR_SEPARATOR, config_file->filename);
    ...
    {
        if (strcmp (config_file->name, DOGECHAT_CONFIG_NAME) == 0)
            dogechat_first_start = 1;
                ...
                && (ptr_line[0] != '\n'))
            {
                ...
                else
                {
                    ...
                    if (pos)
                    {
                        ...
                        if (pos[0]
                            && string_strcasecmp (pos, DOGECHAT_CONFIG_OPTION_NULL) != 0)
                        {
                            undefined_value = 0;
    ...
    return DOGECHAT_CONFIG_READ_OK;
}
...
void
config_file_free_all_plugin (struct t_dogechat_plugin *plugin)
{
...
config_file_hdata_config_file_cb (void *data, const char *hdata_name)
{
    ...
    if (hdata)
    {
        ...
        HDATA_LIST(config_files, DOGECHAT_HDATA_LIST_CHECK_POINTERS);
...
/*
 * Prints configuration file in DogeChat log file (usually for crash dump).
 */
