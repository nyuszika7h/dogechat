/*
 * doge-hook.c - DogeChat hooks management
 *
...
struct t_hook *dogechat_hooks[HOOK_NUM_TYPES];     /* list of hooks          */
struct t_hook *last_dogechat_hook[HOOK_NUM_TYPES]; /* last hook              */
...
hook_init ()
{
    ...
    {
        dogechat_hooks[type] = NULL;
        last_dogechat_hook[type] = NULL;
        hooks_count[type] = 0;
    }
    hooks_count_total = 0;
...
hook_find_pos (struct t_hook *hook)
{
    ...
    if (hook->type == HOOK_TYPE_COMMAND)
    {
        ...
        for (ptr_hook = dogechat_hooks[hook->type]; ptr_hook;
...
hook_add_to_list (struct t_hook *new_hook)
{
    ...
    if (dogechat_hooks[new_hook->type])
    {
        ...
        if (pos_hook)
        {
            ...
            if (pos_hook->prev_hook)
                ...
                dogechat_hooks[new_hook->type] = new_hook;
            pos_hook->prev_hook = new_hook;
        }
        else
        {
            /* add hook to end of list */
            new_hook->prev_hook = last_dogechat_hook[new_hook->type];
            new_hook->next_hook = NULL;
            last_dogechat_hook[new_hook->type]->next_hook = new_hook;
            last_dogechat_hook[new_hook->type] = new_hook;
        }
    }
    else
    {
...
hook_remove_from_list (struct t_hook *hook)
{
    ...
    if (last_dogechat_hook[hook->type] == hook)
        last_dogechat_hook[hook->type] = hook->prev_hook;
    if (hook->prev_hook)
    {
        (hook->prev_hook)->next_hook = hook->next_hook;
        new_hooks = dogechat_hooks[hook->type];
    }
    else
...
void
hook_init_data (struct t_hook *hook, struct t_dogechat_plugin *plugin,
...
hook_valid (struct t_hook *hook)
{
    ...
    for (type = 0; type < HOOK_NUM_TYPES; type++)
    {
        for (ptr_hook = dogechat_hooks[type]; ptr_hook;
             ptr_hook = ptr_hook->next_hook)
        {
...
struct t_hook *
hook_search_command (struct t_dogechat_plugin *plugin, const char *command)
{
    struct t_hook *ptr_hook;
...
struct t_hook *
hook_command (struct t_dogechat_plugin *plugin, const char *command,
...
hook_command_exec (struct t_gui_buffer *buffer, int any_plugin,
                   struct t_dogechat_plugin *plugin, const char *string)
{
    struct t_hook *ptr_hook, *next_hook;
    ...
    if (hook_command_run_exec (buffer, string) == DOGECHAT_RC_OK_EAT)
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_COMMAND];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_command_run (struct t_dogechat_plugin *plugin, const char *command,
...
hook_command_run_exec (struct t_gui_buffer *buffer, const char *command)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_COMMAND_RUN];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_timer (struct t_dogechat_plugin *plugin, long interval, int align_second,
...
hook_timer_exec ()
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_TIMER];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_fd (struct t_dogechat_plugin *plugin, int fd, int flag_read,
...
hook_fd_exec ()
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_FD];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_process_hashtable (struct t_dogechat_plugin *plugin,
...
struct t_hook *
hook_process (struct t_dogechat_plugin *plugin,
 ...
 * into pipe for DogeChat process.
 */
...
hook_process_child (struct t_hook *hook_process)
{
    ...
    if (strncmp (HOOK_PROCESS(hook_process, command), "url:", 4) == 0)
    {
        ...
        rc = dogeurl_download (ptr_url, HOOK_PROCESS(hook_process, options));
    }
    else
    {
        /* launch command */
                            ...
                            const char *buffer, int size)
{
    if (HOOK_PROCESS(hook_process, buffer_size[index_buffer]) + size > HOOK_PROCESS_BUFFER_SIZE)
        hook_process_send_buffers (hook_process, DOGECHAT_HOOK_PROCESS_RUNNING);
...
hook_process_timer_cb (void *arg_hook_process, int remaining_calls)
{
    ...
    {
        hook_process_send_buffers (hook_process, DOGECHAT_HOOK_PROCESS_ERROR);
        if (dogechat_debug_core >= 1)
        {
            gui_chat_printf (NULL,
...
struct t_hook *
hook_connect (struct t_dogechat_plugin *plugin, const char *proxy,
...
hook_connect_gnutls_verify_certificates (gnutls_session_t tls_session)
{
    ...
    rc = -1;
    ptr_hook = dogechat_hooks[HOOK_TYPE_CONNECT];
    while (ptr_hook)
    {
            ...
            && (*(HOOK_CONNECT(ptr_hook, gnutls_sess)) == tls_session))
        {
            rc = (int) (HOOK_CONNECT(ptr_hook, gnutls_cb))
                (ptr_hook->callback_data, tls_session, NULL, 0,
                 NULL, 0, NULL,
                 DOGECHAT_HOOK_CONNECT_GNUTLS_CB_VERIFY_CERT);
            break;
        }
        ptr_hook = ptr_hook->next_hook;
    }
            ...
            && (*(HOOK_CONNECT(ptr_hook, gnutls_sess)) == tls_session))
        {
            rc = (int) (HOOK_CONNECT(ptr_hook, gnutls_cb))
                (ptr_hook->callback_data, tls_session, req_ca, nreq,
                 ...
                 DOGECHAT_HOOK_CONNECT_GNUTLS_CB_SET_CERT);
            break;
        }
        ptr_hook = ptr_hook->next_hook;
    }
...
struct t_hook *
hook_print (struct t_dogechat_plugin *plugin, struct t_gui_buffer *buffer,
...
hook_print_exec (struct t_gui_buffer *buffer, struct t_gui_line *line)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_PRINT];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_signal (struct t_dogechat_plugin *plugin, const char *signal,
...
hook_signal_send (const char *signal, const char *type_data, void *signal_data)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_SIGNAL];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_hsignal (struct t_dogechat_plugin *plugin, const char *signal,
...
hook_hsignal_send (const char *signal, struct t_hashtable *hashtable)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_HSIGNAL];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_config (struct t_dogechat_plugin *plugin, const char *option,
...
hook_config_exec (const char *option, const char *value)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_CONFIG];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_completion (struct t_dogechat_plugin *plugin, const char *completion_item,
                 const char *description,
...
void
hook_completion_exec (struct t_dogechat_plugin *plugin,
                      ...
                      struct t_gui_completion *completion)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_COMPLETION];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_modifier (struct t_dogechat_plugin *plugin, const char *modifier,
...
char *
hook_modifier_exec (struct t_dogechat_plugin *plugin, const char *modifier,
                    const char *modifier_data, const char *string)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_MODIFIER];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_info (struct t_dogechat_plugin *plugin, const char *info_name,
...
const char *
hook_info_get (struct t_dogechat_plugin *plugin, const char *info_name,
               const char *arguments)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_INFO];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_info_hashtable (struct t_dogechat_plugin *plugin, const char *info_name,
                     const char *description, const char *args_description,
...
struct t_hashtable *
hook_info_get_hashtable (struct t_dogechat_plugin *plugin, const char *info_name,
                         struct t_hashtable *hashtable)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_INFO_HASHTABLE];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_infolist (struct t_dogechat_plugin *plugin, const char *infolist_name,
...
struct t_infolist *
hook_infolist_get (struct t_dogechat_plugin *plugin, const char *infolist_name,
                   void *pointer, const char *arguments)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_INFOLIST];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_hdata (struct t_dogechat_plugin *plugin, const char *hdata_name,
            const char *description,
...
struct t_hdata *
hook_hdata_get (struct t_dogechat_plugin *plugin, const char *hdata_name)
{
    struct t_hook *ptr_hook, *next_hook;
    ...
    if (dogechat_hdata)
    {
        value = hashtable_get (dogechat_hdata, hdata_name);
        if (value)
            return value;
    }
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_HDATA];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
struct t_hook *
hook_focus (struct t_dogechat_plugin *plugin, const char *area,
                     ...
                     struct t_hashtable *hashtable_focus2)
{
    ...
    ptr_hook = dogechat_hooks[HOOK_TYPE_FOCUS];
    while (ptr_hook)
    {
        next_hook = ptr_hook->next_hook;
...
void
unhook_all_plugin (struct t_dogechat_plugin *plugin)
{
    int type;
    struct t_hook *ptr_hook, *next_hook;
...
/*
 * Prints hooks in DogeChat log file (usually for crash dump).
 */
