/*
 * doge-string.c - string functions
 *
 ...
 *   1. "%h" (at beginning of string) by DogeChat home directory.
                       ...
                       struct t_hashtable *options)
{
    ...
    /* replace "%h" by Dogechat home */
    if (strncmp (path, "%h", 2) == 0)
    {
        length = strlen (dogechat_home) + strlen (path + 2) + 1;
        path1 = malloc (length);
        if (path1)
            snprintf (path1, length, "%s%s", dogechat_home, path + 2);
    }
    else
        path1 = strdup (path);
    if (!path1)
 ...
 *
 * The word chars are customizable with options "dogechat.look.word_chars_*".
 *
 * Returns:
 ...
 *
 * The argument "regex" is a pointer to a regex compiled with DogeChat function
...
/*
 * Converts a string to DogeChat internal storage charset (UTF-8).
 *
 * Note: result must be freed after use.
 */
...
string_iconv_to_internal (const char *charset, const char *string)
{
    ...
    output = string_iconv (0,
                           (charset && charset[0]) ?
                           charset : dogechat_local_charset,
                           DOGECHAT_INTERNAL_CHARSET,
                           input);
    if (!output)
        return input;
