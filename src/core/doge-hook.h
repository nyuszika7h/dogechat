...
#ifndef DOGECHAT_HOOK_H
#define DOGECHAT_HOOK_H 1
...
struct t_hook
{
    ...
    struct t_dogechat_plugin *plugin;   /* plugin which created this hook    */
                                       /* (NULL for hook created by DogeChat)*/
    char *subplugin;                   /* subplugin which created this hook */
                                       /* (commonly a script name, NULL for */
                                       /* hook created by DogeChat or by     */
                                       /* plugin itself)                    */
...
extern struct t_hook *dogechat_hooks[];
extern struct t_hook *last_dogechat_hook[];
...
extern struct t_hook *hook_command (struct t_dogechat_plugin *plugin,
                                    const char *command,
...
extern int hook_command_exec (struct t_gui_buffer *buffer, int any_plugin,
                              ...
                              const char *string);
extern struct t_hook *hook_command_run (struct t_dogechat_plugin *plugin,
                                        const char *command,
                                        t_hook_callback_command_run *callback,
                                        void *callback_data);
extern int hook_command_run_exec (struct t_gui_buffer *buffer,
                                  const char *command);
extern struct t_hook *hook_timer (struct t_dogechat_plugin *plugin,
                                  long interval, int align_second,
                                  int max_calls,
                                  t_hook_callback_timer *callback,
                                  void *callback_data);
extern void hook_timer_exec ();
extern struct t_hook *hook_fd (struct t_dogechat_plugin *plugin, int fd,
                               int flag_read, int flag_write,
                               int flag_exception,
                               t_hook_callback_fd *callback,
                               void *callback_data);
extern void hook_fd_exec ();
extern struct t_hook *hook_process (struct t_dogechat_plugin *plugin,
                                    const char *command,
                                    int timeout,
                                    t_hook_callback_process *callback,
                                    void *callback_data);
extern struct t_hook *hook_process_hashtable (struct t_dogechat_plugin *plugin,
                                              const char *command,
                                              struct t_hashtable *options,
                                              int timeout,
                                              t_hook_callback_process *callback,
                                              void *callback_data);
extern struct t_hook *hook_connect (struct t_dogechat_plugin *plugin,
...
#endif /* LIBGNUTLS_VERSION_NUMBER >= 0x020b00 */
#endif /* HAVE_GNUTLS */
extern struct t_hook *hook_print (struct t_dogechat_plugin *plugin,
                                  struct t_gui_buffer *buffer,
...
extern void hook_completion_list_add (struct t_gui_completion *completion,
                                      ...
                                      const char *where);
extern void hook_completion_exec (struct t_dogechat_plugin *plugin,
                                  const char *completion_item,
                                  struct t_gui_buffer *buffer,
                                  struct t_gui_completion *completion);
extern struct t_hook *hook_modifier (struct t_dogechat_plugin *plugin,
                                     const char *modifier,
                                     t_hook_callback_modifier *callback,
                                     void *callback_data);
extern char *hook_modifier_exec (struct t_dogechat_plugin *plugin,
                                 const char *modifier,
                                 const char *modifier_data,
                                 const char *string);
extern struct t_hook *hook_info (struct t_dogechat_plugin *plugin,
                                 const char *info_name,
                                 const char *description,
                                 const char *args_description,
                                 t_hook_callback_info *callback,
                                 void *callback_data);
extern const char *hook_info_get (struct t_dogechat_plugin *plugin,
                                  const char *info_name,
                                  const char *arguments);
extern struct t_hook *hook_info_hashtable (struct t_dogechat_plugin *plugin,
                                           const char *info_name,
                                           const char *description,
                                           const char *args_description,
                                           ...
                                           void *callback_data);
extern struct t_hashtable *hook_info_get_hashtable (struct t_dogechat_plugin *plugin,
                                                    const char *info_name,
                                                    struct t_hashtable *hashtable);
extern struct t_hook *hook_infolist (struct t_dogechat_plugin *plugin,
                                     const char *infolist_name,
                                     const char *description,
                                     ...
                                     void *callback_data);
extern struct t_infolist *hook_infolist_get (struct t_dogechat_plugin *plugin,
                                             const char *infolist_name,
                                             void *pointer,
                                             const char *arguments);
extern struct t_hook *hook_hdata (struct t_dogechat_plugin *plugin,
                                  const char *hdata_name,
                                  const char *description,
                                  t_hook_callback_hdata *callback,
                                  void *callback_data);
extern struct t_hdata *hook_hdata_get (struct t_dogechat_plugin *plugin,
                                       const char *hdata_name);
extern struct t_hook *hook_focus (struct t_dogechat_plugin *plugin,
                                  const char *area,
...
extern void unhook_all_plugin (struct t_dogechat_plugin *plugin);
