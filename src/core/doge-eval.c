/*
 * doge-eval.c - evaluate expressions with references to internal vars
 *
...
eval_hdata_get_value (struct t_hdata *hdata, void *pointer, const char *path)
{
     ...
     */
    if ((type == DOGECHAT_HDATA_POINTER) && pos)
    {
 ...
 *
 * See /help in DogeChat for examples.
 *
 * Note: result must be freed after use.
 */
 ...
 *   >> ${window.buffer.full_name} == irc.freenode.#dogechat
 *   == [1]
