/*
 * doge-util.c - some useful functions
 *
...
/*
 * Sets resource limits using value of option "dogechat.startup.sys_rlimit".
 */
...
/*
 * Converts date to a string, using format of option "dogechat.look.time_format"
 * (can be localized).
 */
...
/*
 * Creates a directory in DogeChat home.
 *
 * Returns:
 *   1: OK
 *   0: error
...
util_mkdir_home (const char *directory, int mode)
{
    ...
    dir_length = strlen (dogechat_home) + strlen (directory) + 2;
    dir_name = malloc (dir_length);
    if (!dir_name)
        return 0;

    snprintf (dir_name, dir_length, "%s/%s", dogechat_home, directory);
...
/*
 * Searches for the full name of a DogeChat library with name and extension
 * (searches first in DogeChat user's dir, then DogeChat global lib directory).
 *
                               ...
                               const char *plugins_dir)
{
    ...
    length = strlen (dogechat_home) + strlen (name_with_ext) +
    ...
    /* try DogeChat global lib dir */
    length = strlen (DOGECHAT_LIBDIR) + strlen (name_with_ext) +
        strlen (plugins_dir) + 16;
    final_name = malloc (length);
    if (!final_name)
    {
...
/*
 * Searches for the full name of a DogeChat library with name.
 *
 * All extensions listed in option "dogechat.plugin.extension" are tested.
 *
 * Note: result must be freed after use (if not NULL).
 */
