/*
 * doge-backtrace.c - backtrace after a segfault
 *
...
/*
 * Displays a backtrace line on standard error output and in DogeChat log.
 */

void
dogechat_backtrace_printf (const char *message, ...)
{
...
void
dogechat_backtrace_addr2line (int number, void *address, const char *symbol)
{
#ifdef HAVE_BACKTRACE
    int rc;
    Dl_info info;
    ...
    {
        dogechat_backtrace_printf ("%03d  %s", number, symbol);
        return;
    }
    ...
    while (!feof (output))
    {
        ...
        if (ptr_line && ptr_line[0])
        {
            ...
            {
                if (function_name[0])
                    dogechat_backtrace_printf ("%03d  %s",
...
dogechat_backtrace ()
{
    ...
    dogechat_backtrace_printf ("======= DogeChat backtrace =======");
    dogechat_backtrace_printf ("(written by DogeChat %s, compiled on %s %s)",
    ...
    {
        dogechat_backtrace_addr2line (i + 1, trace[i], symbols[i]);
    }
#else
    dogechat_backtrace_printf ("  No backtrace info (no debug info available "
                              "or no backtrace possible on your system).");
#endif /* HAVE_BACKTRACE */

    dogechat_backtrace_printf ("======= End of  backtrace =======");
}
