/*
 * doge-upgrade.c - save/restore session data of DogeChat core
 *
...
/*
 * Saves history in DogeChat upgrade file (from last to first, to restore it in
 * good order).
 *
 * Returns:
 *   1: OK
...
int
upgrade_dogechat_save_history (struct t_upgrade_file *upgrade_file,
...
/*
 * Saves buffers in DogeChat upgrade file.
 *
 * Returns:
 *   1: OK
 *   0: error
...
int
upgrade_dogechat_save_buffers (struct t_upgrade_file *upgrade_file)
{
    struct t_infolist *ptr_infolist;
         ...
         ptr_buffer = ptr_buffer->next_buffer)
    {
        ...
        {
            rc = upgrade_dogechat_save_history (upgrade_file,
...
/*
 * Saves miscellaneous info in DogeChat upgrade file.
 *
 * Returns:
 *   1: OK
 *   0: error
...
int
upgrade_dogechat_save_misc (struct t_upgrade_file *upgrade_file)
{
    struct t_infolist *ptr_infolist;
    struct t_infolist_item *ptr_item;
    int rc;
    ...
    if (!infolist_new_var_time (ptr_item, "start_time", dogechat_first_start_time))
    {
        infolist_free (ptr_infolist);
        return 0;
    }
    if (!infolist_new_var_integer (ptr_item, "upgrade_count", dogechat_upgrade_count))
    {
        infolist_free (ptr_infolist);
        return 0;
    }
...
/*
 * Saves hotlist in DogeChat upgrade file.
 *
 * Returns:
 *   1: OK
 *   0: error
...
int
upgrade_dogechat_save_hotlist (struct t_upgrade_file *upgrade_file)
{
    struct t_infolist *ptr_infolist;
...
/*
 * Saves tree with layout for windows in DogeChat upgrade file.
 *
 * Returns:
 *   1: OK
 *   0: error
...
int
upgrade_dogechat_save_layout_window_tree (struct t_upgrade_file *upgrade_file,
                                         struct t_gui_layout_window *layout_window)
{
    ...
    {
        if (!upgrade_dogechat_save_layout_window_tree (upgrade_file,
...
/*
 * Saves layout for windows in DogeChat upgrade file.
 *
 * Returns:
 *   1: OK
 *   0: error
...
int
upgrade_dogechat_save_layout_window (struct t_upgrade_file *upgrade_file)
{
    ...
    if (ptr_layout)
    {
        ...
        rc = upgrade_dogechat_save_layout_window_tree (upgrade_file, ptr_layout->layout_windows);
...
int
upgrade_dogechat_save ()
{
    int rc;
    struct t_upgrade_file *upgrade_file;

    upgrade_file = upgrade_file_new (DOGECHAT_UPGRADE_FILENAME, 1);
    if (!upgrade_file)
        return 0;

    rc = 1;
    rc &= upgrade_dogechat_save_history (upgrade_file, last_gui_history);
    rc &= upgrade_dogechat_save_buffers (upgrade_file);
    rc &= upgrade_dogechat_save_misc (upgrade_file);
    rc &= upgrade_dogechat_save_hotlist (upgrade_file);
    rc &= upgrade_dogechat_save_layout_window (upgrade_file);
...
void
upgrade_dogechat_read_buffer (struct t_infolist *infolist)
{
    struct t_gui_buffer *ptr_buffer;
...
void
upgrade_dogechat_read_buffer_line (struct t_infolist *infolist)
{
...
void
upgrade_dogechat_read_nicklist (struct t_infolist *infolist)
{
...
void
upgrade_dogechat_read_hotlist (struct t_infolist *infolist)
{
...
/*
 * Reads DogeChat upgrade file.
 */

int
upgrade_dogechat_read_cb (void *data,
                         struct t_upgrade_file *upgrade_file,
                         int object_id,
                         struct t_infolist *infolist)
{
    ...
    while (infolist_next (infolist))
    {
        switch (object_id)
        {
            case UPGRADE_DOGECHAT_TYPE_HISTORY:
                if (upgrade_current_buffer)
                {
                ...
                break;
            case UPGRADE_DOGECHAT_TYPE_BUFFER:
                upgrade_dogechat_read_buffer (infolist);
                break;
            case UPGRADE_DOGECHAT_TYPE_BUFFER_LINE:
                upgrade_dogechat_read_buffer_line (infolist);
                break;
            case UPGRADE_DOGECHAT_TYPE_NICKLIST:
                upgrade_dogechat_read_nicklist (infolist);
                break;
            case UPGRADE_DOGECHAT_TYPE_MISC:
                dogechat_first_start_time = infolist_time (infolist, "start_time");
                dogechat_upgrade_count = infolist_integer (infolist, "upgrade_count");
                upgrade_set_current_window = infolist_integer (infolist, "current_window_number");
                break;
            case UPGRADE_DOGECHAT_TYPE_HOTLIST:
                upgrade_dogechat_read_hotlist (infolist);
                break;
            case UPGRADE_DOGECHAT_TYPE_LAYOUT_WINDOW:
...
upgrade_dogechat_load ()
{
    ...
    upgrade_file = upgrade_file_new (DOGECHAT_UPGRADE_FILENAME, 0);
    if (!upgrade_file)
        return 0;
    rc = upgrade_file_read (upgrade_file, &upgrade_dogechat_read_cb, NULL);
    upgrade_file_close (upgrade_file);
...
/*
 * Removes a .upgrade file (callback called for each .upgrade file in DogeChat
 * home directory).
 */

void
upgrade_dogechat_remove_file_cb (void *data, const char *filename)
{
    /* make C compiler happy */
    (void) data;
...
/*
 * Removes *.upgrade files after upgrade and send signal "dogechat_upgrade_done".
 */

void
upgrade_dogechat_end ()
{
    struct timeval tv_now;
    long long time_diff;

    /* remove .upgrade files */
    util_exec_on_files (dogechat_home,
                        0,
                        NULL,
                        &upgrade_dogechat_remove_file_cb);

    /* display message for end of /upgrade with duration */
    gettimeofday (&tv_now, NULL);
    time_diff = util_timeval_diff (&dogechat_current_start_timeval, &tv_now);
    ...
    (void) hook_signal_send ("upgrade_ended", DOGECHAT_HOOK_SIGNAL_STRING, NULL);
}
