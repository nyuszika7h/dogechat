/*
 * doge-hashtable.c - implementation of hashtable
 *
...
{ DOGECHAT_HASHTABLE_INTEGER, DOGECHAT_HASHTABLE_STRING,
  DOGECHAT_HASHTABLE_POINTER, DOGECHAT_HASHTABLE_BUFFER,
                                ...
                                const void *key, const void *value)
{
    ...
    dogelist_add (list, hashtable_to_string (hashtable->type_keys, key),
...
{
    struct t_dogelist *dogelist;

    dogelist = dogelist_new ();
    if (dogelist)
        hashtable_map (hashtable, &hashtable_get_list_keys_map_cb, dogelist);
    return dogelist;
}
...
/*
 * Prints hashtable in DogeChat log file (usually for crash dump).
 */
