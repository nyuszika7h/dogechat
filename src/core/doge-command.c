...
COMMAND_CALLBACK(buffer)
{
    ...
    struct t_gui_buffer *dogechat_buffer;
        ...
        if (CONFIG_BOOLEAN(config_look_buffer_auto_renumber))
        {
            gui_chat_printf (NULL,
                             _("%sError: renumbering is allowed only if option "
                               "dogechat.look.buffer_auto_renumber is off"),
                             gui_chat_prefix[GUI_CHAT_PREFIX_ERROR]);
            return DOGECHAT_RC_OK;
        }
    ...
    {
        dogechat_buffer = gui_buffer_search_main();

        if (argc < 3)
        {
            if (buffer == dogechat_buffer)
            {
                gui_chat_printf (NULL,
                                 _("%sError: DogeChat main buffer can't be "
                                   "closed"),
                                 gui_chat_prefix[GUI_CHAT_PREFIX_ERROR]);
            }
            else
        ...
        else
        {
            ...
            else
            {
                ...
                if (ptr_buffer)
                {
                    ...
                    {
                        gui_chat_printf (NULL,
                                         _("%sError: DogeChat main buffer can't "
                                           "be closed"),
                                         gui_chat_prefix[GUI_CHAT_PREFIX_ERROR]);
                    }
                    else
    ...
    if (string_strcasecmp (argv[1], "notify") == 0)
    {
        ...
        if (!config_dogechat_notify_set (buffer, argv_eol[2]))
        {
            gui_chat_printf (NULL,
...
COMMAND_CALLBACK(color)
{
    ...
    /* switch DogeChat/terminal colors */
...
/*
 * Callback for command "/command": launches explicit DogeChat or plugin command.
 */
...
COMMAND_CALLBACK(debug)
{
    ...
    if (string_strcasecmp (argv[1], "dump") == 0)
    {
        if (argc > 2)
            ...
            log_printf ("Dump request for DogeChat core and plugins");
        dogechat_log_use_time = 0;
        (void) hook_signal_send ("debug_dump", DOGECHAT_HOOK_SIGNAL_STRING,
                                 (argc > 2) ? argv_eol[2] : NULL);
        dogechat_log_use_time = 1;
        return DOGECHAT_RC_OK;
    }
    ...
    if (string_strcasecmp (argv[1], "libs") == 0)
    {
        ...
        (void) hook_signal_send ("debug_libs", DOGECHAT_HOOK_SIGNAL_STRING, NULL);
        return DOGECHAT_RC_OK;
    }
    ...
    if (string_strcasecmp (argv[1], "set") == 0)
    {
        ...
        if (strcmp (argv[3], "0") == 0)
        {
            ...
            ptr_option = config_dogechat_debug_get (argv[2]);
            if (ptr_option)
            {
        ...
        else
        {
            ...
            if (config_dogechat_debug_set (argv[2], argv[3]) != DOGECHAT_CONFIG_OPTION_SET_ERROR)
            {
                ptr_option = config_dogechat_debug_get (argv[2]);
                if (ptr_option)
                {
...
void
command_help_list_plugin_commands (struct t_dogechat_plugin *plugin,
                                   int verbose)
{
             ...
             ptr_hook = ptr_hook->next_hook)
        {
                ...
                && HOOK_COMMAND(ptr_hook, command)[0])
            {
                ...
                if (length > max_length)
                    max_length = length;
                dogelist_add (list, HOOK_COMMAND(ptr_hook, command),
                             DOGECHAT_LIST_POS_SORT, NULL);
            }
        }

        /* use list to display commands, sorted by columns */
        list_size = dogelist_size (list);
        if ((max_length > 0) && (list_size > 0))
        {
            ...
            for (line = 0; line < lines; line++)
            {
                ...
                for (col = 0; col < cols; col++)
                {
                    ...
                    if (index < list_size)
                    {
                        item = dogelist_get (list, index);
                        if (item)
                        {
                            if (strlen (str_line) + strlen (dogelist_string (item)) + 1 < (int)sizeof (str_line))
                            {
                                snprintf (str_command, sizeof (str_command),
                                          str_format, dogelist_string (item));
    ...
    if (ptr_option)
    {
        ...
        switch (ptr_option->type)
        {
            ...
            case CONFIG_OPTION_TYPE_COLOR:
                ...
                gui_chat_printf (NULL, "  %s: %s",
                                 _("values"),
                                 _("a DogeChat color name (default, black, "
...
COMMAND_CALLBACK(input)
{
    ...
    /* not used any more in DogeChat >= 1.0 (replaced by "/buffer ++") */
...
COMMAND_CALLBACK(mute)
{
    ...
    {
        /*
         * action ignored in DogeChat >= 1.0 (mute on all buffers is default)
         ...
         ptr_plugin = ptr_plugin->next_plugin)
    {
        if (!name || (string_strcasestr (ptr_plugin->name, name)))
        {
            ...
            if (full)
            {
                ...
                /* command_run hooked */
                hook_found = 0;
                for (ptr_hook = dogechat_hooks[HOOK_TYPE_COMMAND_RUN]; ptr_hook;
                     ptr_hook = ptr_hook->next_hook)
                {
                    if (!ptr_hook->deleted && (ptr_hook->plugin == ptr_plugin))
                    {
                ...
                /* timers hooked */
                hook_found = 0;
                for (ptr_hook = dogechat_hooks[HOOK_TYPE_TIMER]; ptr_hook;
                     ptr_hook = ptr_hook->next_hook)
                {
                    if (!ptr_hook->deleted && (ptr_hook->plugin == ptr_plugin))
                    {
                ...
                /* fd hooked */
                hook_found = 0;
                for (ptr_hook = dogechat_hooks[HOOK_TYPE_FD]; ptr_hook;
                     ptr_hook = ptr_hook->next_hook)
                {
                    if (!ptr_hook->deleted && (ptr_hook->plugin == ptr_plugin))
                    {
                ...
                /* process hooked */
                hook_found = 0;
                for (ptr_hook = dogechat_hooks[HOOK_TYPE_PROCESS]; ptr_hook;
                     ptr_hook = ptr_hook->next_hook)
                {
                    if (!ptr_hook->deleted && (ptr_hook->plugin == ptr_plugin))
                    {
                ...
                /* connect hooked */
                hook_found = 0;
                for (ptr_hook = dogechat_hooks[HOOK_TYPE_CONNECT]; ptr_hook;
                     ptr_hook = ptr_hook->next_hook)
                {
                    if (!ptr_hook->deleted && (ptr_hook->plugin == ptr_plugin))
                    {
                ...
                /* prints hooked */
                hook_found = 0;
                for (ptr_hook = dogechat_hooks[HOOK_TYPE_PRINT]; ptr_hook;
                     ptr_hook = ptr_hook->next_hook)
                {
                    if (!ptr_hook->deleted && (ptr_hook->plugin == ptr_plugin))
                    {
                ...
                /* signals hooked */
                hook_found = 0;
                for (ptr_hook = dogechat_hooks[HOOK_TYPE_SIGNAL]; ptr_hook;
                     ptr_hook = ptr_hook->next_hook)
                {
                    if (!ptr_hook->deleted && (ptr_hook->plugin == ptr_plugin))
                    {
                ...
                hook_found = 0;
                for (ptr_hook = dogechat_hooks[HOOK_TYPE_CONFIG]; ptr_hook;
                     ptr_hook = ptr_hook->next_hook)
                {
                    if (!ptr_hook->deleted && (ptr_hook->plugin == ptr_plugin))
                    {
                ...
                /* completion hooked */
                hook_found = 0;
                for (ptr_hook = dogechat_hooks[HOOK_TYPE_COMPLETION]; ptr_hook;
                     ptr_hook = ptr_hook->next_hook)
                {
                    if (!ptr_hook->deleted && (ptr_hook->plugin == ptr_plugin))
                    {
                ...
                /* modifier hooked */
                hook_found = 0;
                for (ptr_hook = dogechat_hooks[HOOK_TYPE_MODIFIER]; ptr_hook;
                     ptr_hook = ptr_hook->next_hook)
                {
                    if (!ptr_hook->deleted && (ptr_hook->plugin == ptr_plugin))
                    {
...
/*
 * Callback for command "/plugin": lists/loads/unloads DogeChat plugins.
 */
...
/*
 * Callback for command "/quit": quits DogeChat.
 */
...
COMMAND_CALLBACK(quit)
{
     ...
     * from servers
     */
    (void) hook_signal_send ("quit", DOGECHAT_HOOK_SIGNAL_STRING, pos_args);

    /* force end of DogeChat main loop */
    dogechat_quit = 1;
...
COMMAND_CALLBACK(set)
{
    ...
    if ((argc > 1) && (string_strcasecmp (argv[1], "env") == 0))
    {
        if (argc == 2)
        {
            ...
            for (i = 0; environ[i]; i++)
            {
                dogelist_add (list, environ[i], DOGECHAT_LIST_POS_SORT, NULL);
            }
            list_size = dogelist_size (list);
            for (i = 0; i < list_size; i++)
            {
                item = dogelist_get (list, i);
                if (item)
                {
                    ptr_string = dogelist_string (item);
                    if (ptr_string)
    ...
    config_file_search_with_string (argv[1], NULL, NULL, &ptr_option_before,
                                    NULL);
    value = (string_strcasecmp (argv_eol[2], DOGECHAT_CONFIG_OPTION_NULL) == 0) ?
    ...
    switch (rc)
    {
        case DOGECHAT_CONFIG_OPTION_SET_ERROR:
            gui_chat_printf (NULL,
                             _("%sError: failed to set option \"%s\""),
                             gui_chat_prefix[GUI_CHAT_PREFIX_ERROR],
                             argv[1]);
            return DOGECHAT_RC_OK;
        case DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND:
            gui_chat_printf (NULL,
                      ...
                      int *number_reset, int *number_removed)
{
    ...
    {
        case DOGECHAT_CONFIG_OPTION_UNSET_ERROR:
            gui_chat_printf (NULL,
                             _("%sFailed to unset option \"%s\""),
                             gui_chat_prefix[GUI_CHAT_PREFIX_ERROR],
                             option_full_name);
            break;
        case DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET:
            break;
        case DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET:
            command_set_display_option (option, _("Option reset: "));
            if (number_reset)
                (*number_reset)++;
            break;
        case DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED:
            gui_chat_printf (NULL,
...
/*
 * Callback for command "/upgrade": upgrades DogeChat.
 */
...
COMMAND_CALLBACK(upgrade)
{
     ...
     */
    if (dogechat_hooks[HOOK_TYPE_PROCESS] || dogechat_hooks[HOOK_TYPE_CONNECT])
    {
        gui_chat_printf (NULL,
    ...
    if (argc > index_args)
    {
        ...
        else
        {
            ...
            if (ptr_binary)
            {
                /* check if dogechat binary is here and executable by user */
                rc = stat (ptr_binary, &stat_buf);
                if ((rc != 0) || (!S_ISREG(stat_buf.st_mode)))
                {
                    gui_chat_printf (NULL,
                                     _("%sCan't upgrade: DogeChat binary \"%s\" "
                                       "does not exist"),
                                     gui_chat_prefix[GUI_CHAT_PREFIX_ERROR],
                                     ptr_binary);
                    free (ptr_binary);
    ...
    if (!ptr_binary && !quit)
    {
        ptr_binary = (dogechat_argv0) ? strdup (dogechat_argv0) : NULL;
        if (!ptr_binary)
        {
            gui_chat_printf (NULL,
    ...
    {
        gui_chat_printf (NULL,
                         _("Upgrading DogeChat with binary file: \"%s\"..."),
                         ptr_binary);
    }

    /* send "upgrade" signal to plugins */
    (void) hook_signal_send ("upgrade", DOGECHAT_HOOK_SIGNAL_STRING,
                             (quit) ? "quit" : NULL);

    if (!upgrade_dogechat_save ())
    {
        gui_chat_printf (NULL,
    ...
    exec_args[3] = strdup (dogechat_home);
...
/*
 * Callback for command "/uptime": displays DogeChat uptime.
 */
...
COMMAND_CALLBACK(uptime)
{
    ...
    running_time = time (NULL) - dogechat_first_start_time;
    ...
    if ((argc >= 2) && (string_strcasecmp (argv[1], "-o") == 0))
    {
        snprintf (string, sizeof (string),
                  "DogeChat uptime: %d %s %02d:%02d:%02d, started on %s",
                  day,
                  (day != 1) ? "days" : "day",
                  hour,
                  min,
                  sec,
                  ctime (&dogechat_first_start_time));
    ...
    else if ((argc >= 2) && (string_strcasecmp (argv[1], "-ol") == 0))
    {
        snprintf (string, sizeof (string),
                  ...
                  _("DogeChat uptime: %d %s %02d:%02d:%02d, started on %s"),
                  day,
                  NG_("day", "days", day),
                  hour,
                  min,
                  sec,
                  util_get_time_string (&dogechat_first_start_time));
        (void) input_data (buffer, string);
    }
    else
    {
                         ...
                         int display_git_version)
{
    ...
    if (send_to_buffer_as_input)
    {
        if (translated_string)
        {
            ...
            (void) input_data (buffer, string);
            if (dogechat_upgrade_count > 0)
            {
                snprintf (string, sizeof (string),
                          _("Upgraded %d %s, first start: %s"),
                          dogechat_upgrade_count,
                          /* TRANSLATORS: text is: "upgraded xx times" */
                          NG_("time", "times", dogechat_upgrade_count),
                          util_get_time_string (&dogechat_first_start_time));
                (void) input_data (buffer, string);
            }
        }
        else
        {
            ...
            if (dogechat_upgrade_count > 0)
            {
                snprintf (string, sizeof (string),
                          ...
                          dogechat_upgrade_count,
                          (dogechat_upgrade_count > 1) ? "times" : "time",
                          ctime (&dogechat_first_start_time));
                string[strlen (string) - 1] = '\0';
                (void) input_data (buffer, string);
            }
    ...
    {
        gui_chat_printf (NULL, "%sDogeChat %s %s[%s%s %s %s%s]",
                         GUI_COLOR(GUI_COLOR_CHAT_BUFFER),
                         (display_git_version) ? version_get_version_with_git () : version_get_version (),
                         GUI_COLOR(GUI_COLOR_CHAT_DELIMITERS),
                         GUI_COLOR(GUI_COLOR_CHAT_VALUE),
...
/*
 * Callback for command "/version": displays DogeChat version.
 */
...
command_init ()
{
    ...
    hook_command (
        ...
        N_("         list: list all bars\n"
           ...
           "               other condition: see /help dogechat.bar.xxx.conditions "
           "and /help eval\n"
    ...
    hook_command (
        ...
        N_("    list: list buffers (without argument, this list is displayed)\n"
           ...
           "renumber: renumber buffers (works only if option dogechat.look."
           ...
           "          '*': jump to number, using option \"dogechat.look."
           ...
           "    /buffer swap 1 3\n"
           "  swap buffer #dogechat with current buffer:\n"
           "    /buffer swap #dogechat\n"
    ...
    hook_command (
        ...
        N_("   alias: add an alias for a color\n"
           ...
           "dogechat.look.color_pairs_auto_reset)\n"
    ...
    hook_command (
        NULL, "50000|command",
        N_("launch explicit DogeChat or plugin command"),
        ...
        N_("-buffer: execute the command on this buffer\n"
           ...
           "DogeChat command, '*' for automatic plugin (it depends on the "
    ...
    hook_command (
        ...
        N_("     list: list plugins with debug levels\n"
           ...
           "   plugin: name of plugin (\"core\" for DogeChat core)\n"
           "    level: debug level for plugin (0 = disable debug)\n"
           "     dump: save memory dump in DogeChat log file (same dump is "
           "written when DogeChat crashes)\n"
    ...
    hook_command (
        ...
        N_("        -n: display result without sending it to buffer "
           ...
           "    ${plugin[dogechat_plugins].name}: name of first plugin in "
           ...
           "reference\", function \"dogechat_hdata_get\".\n"
           "\n"
           "Examples (simple strings):\n"
           "  /eval -n ${info:version}                     ==> 0.4.3\n"
           "  /eval -n ${env:HOME}                         ==> /home/user\n"
           "  /eval -n ${dogechat.look.scroll_amount}       ==> 3\n"
           "  /eval -n ${window}                           ==> 0x2549aa0\n"
           "  /eval -n ${window.buffer}                    ==> 0x2549320\n"
           "  /eval -n ${window.buffer.full_name}          ==> core.dogechat\n"
    ...
    hook_command (
        ...
        N_("   list: list all filters\n"
           ...
           "freenode.#dogechat\" or \"irc.server.freenode\")\n"
           "         - \"*\" means all buffers\n"
           ...
           "  use IRC smart filter on all buffers except those with \"#dogechat\" "
           "in name:\n"
           "    /filter add irc_smart *,!*#dogechat* irc_smart_filter *\n"
           "  filter all IRC join/part/quit messages:\n"
           "    /filter add joinquit * irc_join,irc_part,irc_quit *\n"
           "  filter nicks displayed when joining channels or with /names:\n"
           "    /filter add nicks * irc_366 *\n"
           "  filter nick \"toto\" on IRC channel #dogechat:\n"
           "    /filter add toto irc.freenode.#dogechat nick_toto *\n"
           "  filter IRC join/action messages from nick \"toto\":\n"
           "    /filter add toto * nick_toto+irc_join,nick_toto+irc_action *\n"
           "  filter lines containing \"dogechat sucks\" on IRC channel #dogechat:\n"
           "    /filter add sucks irc.freenode.#dogechat * dogechat sucks\n"
           "  filter lines that are strictly equal to \"DogeChat sucks\" on "
           "all buffers:\n"
           "    /filter add sucks2 * * (?-i)^DogeChat sucks$"),
        "list"
    ...
    hook_command (
        ...
        N_("       list: list all current keys (without argument, this list is "
           ...
           "after installing new DogeChat version\n"
           "\n"
           ...
           "  key alt-r to jump to #dogechat IRC channel:\n"
           "    /key bind meta-r /buffer #dogechat\n"
    ...
    hook_command (
        ...
        N_(" enable: enable mouse\n"
           ...
           "\n"
           "The mouse state is saved in option \"dogechat.look.mouse\".\n"
           "\n"
           "Examples:\n"
           "  enable mouse:\n"
           "    /mouse enable\n"
    ...
    hook_command (
        ...
        N_("   -core: no output on DogeChat core buffer\n"
           ...
           "    /mute -current msg * hi!\n"
           "  message to #dogechat channel:\n"
           "    /mute -buffer irc.freenode.#dogechat msg #dogechat hi!"),
    ...
    hook_command (
        ...
        N_(" -buffer: display text in this buffer (default: buffer where "
           "command is executed)\n"
           "   -core: alias of \"-buffer core.dogechat\"\n"
           ...
           "  display a message on channel #dogechat:\n"
           "    /print -buffer irc.freenode.#dogechat Message on #dogechat\n"
    ...
    hook_command (
        ...
        N_("quit DogeChat"),
        N_("[-yes] [<arguments>]"),
        N_("     -yes: required if option dogechat.look.confirm_quit is enabled\n"
    ...
    hook_command (
        ...
        N_("file: configuration file to reload (without extension \".conf\")\n"
           "\n"
           "Without argument, all files (DogeChat and plugins) are reloaded."),
        "%(config_files)|%*",
        &command_reload, NULL);
    hook_command (
        NULL, "repeat",
    ...
    hook_command (
        ...
        N_("file: configuration file to save (without extension \".conf\")\n"
           "\n"
           "Without argument, all files (DogeChat and plugins) are saved."),
        "%(config_files)|%*",
        &command_save, NULL);
    hook_command (
        NULL, "secure",
        ...
        N_("passphrase: change the passphrase (without passphrase, data is "
           ...
           "buffer.\n"
           "\n"
           "When a passphrase is used (data encrypted), it is asked by DogeChat "
           "on startup.\n"
           "It is possible to set environment variable \"DOGECHAT_PASSPHRASE\" "
           "to prevent the prompt (this same variable is used by DogeChat on "
           "/upgrade).\n"
           "\n"
           "Secured data with format ${sec.data.xxx} can be used in:\n"
           "  - command /eval\n"
           "  - command line argument \"--run-command\"\n"
           "  - options dogechat.startup.command_{before|after}_plugins\n"
    ...
    hook_command (
        ...
        N_("option: name of an option (wildcard \"*\" is allowed)\n"
           ...
           "    /set dogechat.look.highlight \"word\"\n"
    ...
    hook_command (
        ...
        N_("option: name of an option\n"
           ...
           "Examples:\n"
           "  reset one option:\n"
           "    /unset dogechat.look.item_time_format\n"
           "  reset all color options:\n"
           "    /unset -mask dogechat.color.*"),
        "%(config_options)"
        " || -mask %(config_options)",
        &command_unset, NULL);
    hook_command (
        NULL, "upgrade",
        N_("upgrade DogeChat without disconnecting from servers"),
        N_("[-yes] [<path_to_binary>|-quit]"),
        N_("          -yes: required if option dogechat.look.confirm_upgrade "
           "is enabled\n"
           "path_to_binary: path to DogeChat binary (default is current binary)\n"
           "        -dummy: do nothing (option used to prevent accidental "
           "completion with \"-quit\")\n"
           "         -quit: close *ALL* connections, save session and quit "
           "DogeChat, which makes possible a delayed restoration (see below)\n"
           "\n"
           "This command upgrades and reloads a running DogeChat session. The "
           "new DogeChat binary must have been compiled or installed with a "
           ...
           "on disk)\n"
           "  3. save DogeChat configuration (dogechat.conf)\n"
           "  4. execute new DogeChat binary and reload session.\n"
           "\n"
           "With option \"-quit\", the process is slightly different:\n"
           "  1. close *ALL* connections (irc, xfer, relay, ...)\n"
           "  2. save session into files (*.upgrade)\n"
           "  3. unload all plugins\n"
           "  4. save DogeChat configuration\n"
           "  5. quit DogeChat\n"
           "Then later you can restore session with command: dogechat --upgrade\n"
           "IMPORTANT: you must restore the session with exactly same "
           "configuration (files *.conf).\n"
           "It is possible to restore DogeChat session on another machine if you "
           "copy the content of directory \"~/.dogechat\"."),
        "%(filename)|-dummy|-quit",
        &command_upgrade, NULL);
    hook_command (
        NULL, "uptime",
        N_("show DogeChat uptime"),
        "[-o | -ol]",
    ...
    hook_command (
        NULL, "version",
        N_("show DogeChat version and compilation date"),
        "[-o | -ol]",
    ...
    hook_command (
        ...
        N_(" number: amount of time to wait (integer number)\n"
           ...
           "execution of command), then command is executed on DogeChat core "
           "buffer).\n"
           "\n"
           "Examples:\n"
...
command_exec_list (const char *command_list)
{
    ...
    if (command_list2 && command_list2[0])
    {
        ...
        if (commands)
        {
            ...
            {
                (void) input_data (dogechat_buffer, *ptr_cmd);
            }
            string_free_split_command (commands);
        }
    }
...
command_startup (int plugins_loaded)
{
    if (plugins_loaded)
    {
        ...
        command_exec_list(dogechat_startup_commands);
    }
    else
