...
int
dogeurl_search_constant (struct t_url_constant *constants, const char *name)
{
    int i;
...
long
dogeurl_get_mask_value (struct t_url_constant *constants,
                       const char *string_mask)
{
    ...
    if (items)
    {
        for (i = 0; i < num_items; i++)
        {
            ...
            if (item)
            {
                index = dogeurl_search_constant (constants, item);
                if (index >= 0)
...
int
dogeurl_search_option (const char *name)
{
    int i;
...
size_t
dogeurl_read (void *buffer, size_t size, size_t nmemb, void *stream)
{
...
size_t
dogeurl_write (void *buffer, size_t size, size_t nmemb, void *stream)
{
...
void
dogeurl_option_map_cb (void *data,
                      ...
                      const void *key, const void *value)
{
    ...
    index = dogeurl_search_option ((const char *)key);
    if (index >= 0)
    {
        switch (url_options[index].type)
        {
            ...
            case URL_TYPE_LONG:
                ...
                {
                    index_constant = dogeurl_search_constant (url_options[index].constants,
                                                             (const char *)value);
                    if (index_constant >= 0)
                    {
                        curl_easy_setopt (curl, url_options[index].option,
            ...
            case URL_TYPE_MASK:
                if (url_options[index].constants)
                {
                    long_value = dogeurl_get_mask_value (url_options[index].constants,
                                                        (const char *)value);
                    curl_easy_setopt (curl, url_options[index].option,
                                      long_value);
                }
...
void
dogeurl_set_proxy (CURL *curl, struct t_proxy *proxy)
{
    if (!proxy)
        return;
...
int
dogeurl_download (const char *url, struct t_hashtable *options)
{
    CURL *curl;
    ...
    void *url_file_opt_cb[2] = { &dogeurl_read, &dogeurl_write };
    ...
    /* set proxy (if option dogechat.network.proxy_curl is set) */
        ...
        && CONFIG_STRING(config_network_proxy_curl)[0])
    {
        ...
        if (ptr_proxy)
            dogeurl_set_proxy (curl, ptr_proxy);
    }
    ...
    hashtable_map (options, &dogeurl_option_map_cb, curl);
...
int
dogeurl_option_add_to_infolist (struct t_infolist *infolist,
