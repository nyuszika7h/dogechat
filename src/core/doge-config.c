/*
 * doge-config.c - DogeChat configuration options (file dogechat.conf)
 *
...
struct t_config_file *dogechat_config_file = NULL;
struct t_config_section *dogechat_config_section_debug = NULL;
struct t_config_section *dogechat_config_section_color = NULL;
struct t_config_section *dogechat_config_section_proxy = NULL;
struct t_config_section *dogechat_config_section_bar = NULL;
struct t_config_section *dogechat_config_section_notify = NULL;
...
/*
 * Callback for changes on option "dogechat.startup.sys_rlimit".
 */
...
/*
 * Callback for changes on options "dogechat.look.save_{config|layout}_on_exit".
 */
...
config_change_save_config_layout_on_exit ()
{
        ...
        && (CONFIG_INTEGER(config_look_save_layout_on_exit) != CONFIG_LOOK_SAVE_LAYOUT_ON_EXIT_NONE))
    {
        gui_chat_printf (NULL,
                         _("Warning: option dogechat.look.save_config_on_exit "
                           "is disabled, so the option "
                           "dogechat.look.save_layout_on_exit is ignored"));
    }
}

/*
 * Callback for changes on option "dogechat.look.save_config_on_exit".
 */
    ...
    if (gui_init_ok && !CONFIG_BOOLEAN(config_look_save_config_on_exit))
    {
        gui_chat_printf (NULL,
                         _("Warning: you should now issue /save to write "
                           "option dogechat.look.save_config_on_exit in "
                           "configuration file"));
    }
...
/*
 * Callback for changes on option "dogechat.look.save_layout_on_exit".
 */
...
/*
 * Callback for changes on option "dogechat.look.window_title".
 */
...
/*
 * Callback for changes on option "dogechat.look.word_chars_highlight".
 */
...
/*
 * Callback for changes on option "dogechat.look.word_chars_input".
 */
...
/*
 * Callback for changes on option "dogechat.look.mouse".
 */
...
/*
 * Callback for changes on option "dogechat.look.buffer_auto_renumber".
 */
...
/*
 * Callback for changes on option "dogechat.look.buffer_notify_default".
 */
...
/*
 * Callback for changes on option "dogechat.look.buffer_time_format".
 */
...
/*
 * Callback for changes on options "dogechat.look.nick_prefix" and
 * "dogechat.look.nick_suffix".
 */
...
/*
 * Callback for changes on option "dogechat.look.prefix_same_nick".
 */
...
/*
 * Callback for changes on option "dogechat.look.eat_newline_glitch".
 */
...
/*
 * Callback for changes on option "dogechat.look.emphasized_attributes".
 */
...
/*
 * Callback for changes on option "dogechat.look.highlight_regex".
 */
...
/*
 * Callback for changes on option "dogechat.look.highlight_tags".
 */
...
/*
 * Callback for changes on option "dogechat.look.hotlist_sort".
 */
...
/*
 * Callback for changes on option "dogechat.look.paste_bracketed".
 */
...
/*
 * Callback for changes on option "dogechat.look.read_marker".
 */
...
/*
 * Callback for changes on option "dogechat.look.prefix_align_min".
 */
...
/*
 * Checks option "dogechat.look.prefix_align_more".
 */
...
/*
 * Checks option "dogechat.look.prefix_buffer_align_more".
 */
...
/*
 * Checks options "dogechat.look.separator_{horizontal|vertical}".
 */
...
/*
 * Callback for changes on option "dogechat.look.tab_width".
 */
...
/*
 * Callback for changes on option "dogechat.network.gnutls_ca_file".
 */
...
/*
 * Checks option "dogechat.network.proxy_curl".
 */
...
/*
 * Callback for changes on option "dogechat.plugin.extension".
 */
...
config_day_change_timer_cb (void *data, int remaining_calls)
{
        ...
        && (new_mday != config_day_change_old_day))
    {
        ...
        (void) hook_signal_send ("day_changed",
                                 DOGECHAT_HOOK_SIGNAL_STRING, str_time);
    }
...
/*
 * Initializes some things after reading/reloading DogeChat configuration file.
 */

void
config_dogechat_init_after_read ()
{
    int i;
...
/*
 * Reloads DogeChat configuration file.
 *
 * Returns:
 *   DOGECHAT_CONFIG_READ_OK: OK
 *   DOGECHAT_CONFIG_READ_MEMORY_ERROR: not enough memory
 *   DOGECHAT_CONFIG_READ_FILE_NOT_FOUND: file not found
 */

int
config_dogechat_reload_cb (void *data, struct t_config_file *config_file)
{
    int i, rc;
    ...
    config_file_section_free_options (dogechat_config_section_notify);
...
config_dogechat_debug_get (const char *plugin_name)
{
    return config_file_search_option (dogechat_config_file,
                                      dogechat_config_section_debug,
                                      plugin_name);
}
...
void
config_dogechat_debug_set_all ()
{
    struct t_config_option *ptr_option;
    struct t_dogechat_plugin *ptr_plugin;

    /* set debug for core */
    ptr_option = config_dogechat_debug_get (PLUGIN_CORE);
    dogechat_debug_core = (ptr_option) ? CONFIG_INTEGER(ptr_option) : 0;

    /* set debug for plugins */
    for (ptr_plugin = dogechat_plugins; ptr_plugin;
         ptr_plugin = ptr_plugin->next_plugin)
    {
        ptr_option = config_dogechat_debug_get (ptr_plugin->name);
...
void
config_dogechat_debug_change_cb (void *data,
                                struct t_config_option *option)
{
    /* make C compiler happy */
    (void) data;
...
int
config_dogechat_debug_create_option_cb (void *data,
        ...
        else
        {
            if (value && value[0])
            {
                ptr_option = config_file_new_option (
                    config_file, section,
                    option_name, "integer",
                    _("debug level for plugin (\"core\" for DogeChat core)"),
                    NULL, 0, 32, "0", value, 0, NULL, NULL,
                    &config_dogechat_debug_change_cb, NULL,
                    NULL, NULL);
                rc = (ptr_option) ?
...
int
config_dogechat_debug_delete_option_cb (void *data,
                                       struct t_config_file *config_file,
                                       struct t_config_section *section,
                                       struct t_config_option *option)
{
...
int
config_dogechat_debug_set (const char *plugin_name, const char *value)
{
    return config_dogechat_debug_create_option_cb (NULL,
                                                  dogechat_config_file,
                                                  dogechat_config_section_debug,
                                                  plugin_name,
                                                  value);
}
...
void
config_dogechat_palette_change_cb (void *data,
                                  struct t_config_option *option)
{
    char *error;
    int number;
...
int
config_dogechat_palette_create_option_cb (void *data,
                                         struct t_config_file *config_file,
                                         struct t_config_section *section,
                                         const char *option_name,
                                         const char *value)
            ...
            else
            {
                if (value)
                {
                    ptr_option = config_file_new_option (
                        ...
                        &config_dogechat_palette_change_cb, NULL,
                        NULL, NULL);
                    rc = (ptr_option) ?
                        DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE : DOGECHAT_CONFIG_OPTION_SET_ERROR;
                    if (ptr_option)
...
int
config_dogechat_palette_delete_option_cb (void *data,
                                         struct t_config_file *config_file,
                                         struct t_config_section *section,
                                         struct t_config_option *option)
{
...
/*
 * Reads a proxy option in DogeChat configuration file.
 */

int
config_dogechat_proxy_read_cb (void *data, struct t_config_file *config_file,
                              struct t_config_section *section,
                              const char *option_name, const char *value)
{
    ...
    for (ptr_temp_proxy = dogechat_temp_proxies; ptr_temp_proxy;
    ...
    if (!ptr_temp_proxy)
    {
        ...
        if (ptr_temp_proxy)
        {
            ...
            ptr_temp_proxy->prev_proxy = last_dogechat_temp_proxy;
            ptr_temp_proxy->next_proxy = NULL;
            if (!dogechat_temp_proxies)
                dogechat_temp_proxies = ptr_temp_proxy;
            else
                last_dogechat_temp_proxy->next_proxy = ptr_temp_proxy;
            last_dogechat_temp_proxy = ptr_temp_proxy;
        }
    }
...
/*
 * Reads a bar option in DogeChat configuration file.
 */

int
config_dogechat_bar_read_cb (void *data, struct t_config_file *config_file,
                            struct t_config_section *section,
                            const char *option_name, const char *value)
{
...
/*
 * Reads a layout option in DogeChat configuration file.
 */

int
config_dogechat_layout_read_cb (void *data, struct t_config_file *config_file,
                               struct t_config_section *section,
                               const char *option_name, const char *value)
{
    ...
    {
        /*
         * old config file (DogeChat <= 0.4.0): no "." in name, use default
         * layout name
         */
...
/*
 * Writes layout of windows in DogeChat configuration file.
 *
 * Returns:
 *   1: OK
 *   0: write error
...
int
config_dogechat_layout_write_tree (struct t_config_file *config_file,
                                  const char *option_name,
                                  struct t_gui_layout_window *layout_window)
{
    ...
    {
        if (!config_dogechat_layout_write_tree (config_file, option_name,
...
/*
 * Writes section "layout" in DogeChat configuration file.
 */

int
config_dogechat_layout_write_cb (void *data, struct t_config_file *config_file,
...
void
config_dogechat_notify_change_cb (void *data, struct t_config_option *option)
{
    /* make C compiler happy */
    (void) data;
    (void) option;
...
int
config_dogechat_notify_create_option_cb (void *data,
                                        struct t_config_file *config_file,
                                        struct t_config_section *section,
                                        const char *option_name,
                                        const char *value)
        ...
        else
        {
            if (value && value[0])
            {
                ptr_option = config_file_new_option (
                    ...
                    &config_dogechat_notify_change_cb, NULL,
                    NULL, NULL);
                rc = (ptr_option) ?
                    DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE : DOGECHAT_CONFIG_OPTION_SET_ERROR;
            }
...
int
config_dogechat_notify_delete_option_cb (void *data,
                                        struct t_config_file *config_file,
                                        struct t_config_section *section,
                                        struct t_config_option *option)
{
...
int
config_dogechat_notify_set (struct t_gui_buffer *buffer, const char *notify)
{
    int i, value;
    ...
    return (config_dogechat_notify_create_option_cb (NULL,
                                                    dogechat_config_file,
                                                    dogechat_config_section_notify,
                                                    buffer->full_name,
                                                    (value < 0) ?
                                                    NULL : gui_buffer_notify_string[value]) != DOGECHAT_CONFIG_OPTION_SET_ERROR) ? 1 : 0;
}

/*
 * Reads a filter option in DogeChat configuration file.
 */

int
config_dogechat_filter_read_cb (void *data,
                               struct t_config_file *config_file,
                               struct t_config_section *section,
                               const char *option_name, const char *value)
{
...
/*
 * Writes section "filter" in DogeChat configuration file.
 */

int
config_dogechat_filter_write_cb (void *data, struct t_config_file *config_file,
                                const char *section_name)
{
...
/*
 * Reads a key option in DogeChat configuration file.
 */

int
config_dogechat_key_read_cb (void *data, struct t_config_file *config_file,
                            struct t_config_section *section,
                            const char *option_name, const char *value)
{
    int context;
...
/*
 * Writes section "key" in DogeChat configuration file.
 */

int
config_dogechat_key_write_cb (void *data, struct t_config_file *config_file,
                             const char *section_name)
{
...
/*
 * Creates options in DogeChat configuration.
 *
 * Returns:
 *   1: OK
 *   0: error
...
int
config_dogechat_init_options ()
{
    struct t_config_section *ptr_section;
    int i;
    char section_name[128];

    dogechat_config_file = config_file_new (NULL, DOGECHAT_CONFIG_NAME,
                                           &config_dogechat_reload_cb, NULL);
    if (!dogechat_config_file)
        return 0;

    /* debug */
    ptr_section = config_file_new_section (dogechat_config_file, "debug",
                                           1, 1,
                                           NULL, NULL, NULL, NULL,
                                           NULL, NULL,
                                           &config_dogechat_debug_create_option_cb, NULL,
                                           &config_dogechat_debug_delete_option_cb, NULL);
    if (!ptr_section)
    {
        config_file_free (dogechat_config_file);
        return 0;
    }

    dogechat_config_section_debug = ptr_section;

    /* startup */
    ptr_section = config_file_new_section (dogechat_config_file, "startup",
                                           0, 0,
    ...
    /* look */
    ptr_section = config_file_new_section (dogechat_config_file, "look",
                                           0, 0,
                                           NULL, NULL, NULL, NULL, NULL, NULL,
                                           NULL, NULL, NULL, NULL);
    if (!ptr_section)
    ...
    config_look_buffer_notify_default = config_file_new_option (
        ...
        N_("default notify level for buffers (used to tell DogeChat if buffer "
    ...
    config_look_eat_newline_glitch = config_file_new_option (
        ...
        N_("if set, the eat_newline_glitch will be set to 0; this is used to "
           ...
           "text when you copy/paste text from DogeChat to another application "
    ...
    config_look_emphasized_attributes = config_file_new_option (
        ...
        N_("attributes for emphasized text: one or more attribute chars ("
           ...
           "dogechat.color.emphasized* are used"),
    ...
    config_look_mouse_timer_delay = config_file_new_option (
        ...
        N_("delay (in milliseconds) to grab a mouse event: DogeChat will "
    ...
    config_look_paste_bracketed = config_file_new_option (
        ...
        N_("enable terminal \"bracketed paste mode\" (not supported in all "
           ...
           "with control sequences so that DogeChat can differentiate pasted "
    ...
    config_look_window_title = config_file_new_option (
        ...
        N_("title for window (terminal for Curses GUI), set on startup; "
           ...
           "(note: content is evaluated, see /help eval)"),
        NULL, 0, 0, "DogeChat ${info:version}", NULL, 0, NULL, NULL,
    ...
    /* palette */
    ptr_section = config_file_new_section (dogechat_config_file, "palette",
                                           1, 1,
                                           NULL, NULL, NULL, NULL,
                                           NULL, NULL,
                                           &config_dogechat_palette_create_option_cb, NULL,
                                           &config_dogechat_palette_delete_option_cb, NULL);
    if (!ptr_section)
    {
        config_file_free (dogechat_config_file);
        return 0;
    ...
    /* colors */
    ptr_section = config_file_new_section (dogechat_config_file, "color",
                                           0, 0,
                                           NULL, NULL, NULL, NULL, NULL, NULL,
                                           NULL, NULL, NULL, NULL);
    if (!ptr_section)
    ...
    dogechat_config_section_color = ptr_section;
    ...
    config_color_chat_nick_offline = config_file_new_option (
        ...
        N_("text color for offline nick (not in nicklist any more); this "
           "color is used only if option dogechat.look.color_nick_offline is "
           "enabled"),
    ...
    config_color_chat_nick_offline_highlight = config_file_new_option (
        ...
        N_("text color for offline nick with highlight; this color is used "
           "only if option dogechat.look.color_nick_offline is enabled"),
        NULL, -1, 0, "default", NULL, 0,
        NULL, NULL, &config_change_color, NULL, NULL, NULL);
    config_color_chat_nick_offline_highlight_bg = config_file_new_option (
        ...
        N_("background color for offline nick with highlight; this color is "
           "used only if option dogechat.look.color_nick_offline is enabled"),
    ...
    config_color_emphasized = config_file_new_option (
        ...
        N_("text color for emphasized text (for example when searching text); "
           "this option is used only if option dogechat.look.emphasized_attributes "
    ...
    config_color_emphasized_bg = config_file_new_option (
        ...
        N_("background color for emphasized text (for example when searching "
           "text); used only if option dogechat.look.emphasized_attributes is an "
    ...
    /* completion */
    ptr_section = config_file_new_section (dogechat_config_file, "completion",
                                           0, 0,
                                           NULL, NULL, NULL, NULL, NULL, NULL,
                                           NULL, NULL, NULL, NULL);
    if (!ptr_section)
    ...
    /* history */
    ptr_section = config_file_new_section (dogechat_config_file, "history",
                                           0, 0,
                                           NULL, NULL, NULL, NULL, NULL, NULL,
                                           NULL, NULL, NULL, NULL);
    if (!ptr_section)
    ...
    config_history_max_buffer_lines_minutes = config_file_new_option (
        ...
        N_("maximum number of minutes in history per buffer "
           "(0 = unlimited); examples: 1440 = one day, 10080 = one dogek, "
           "43200 = one month, 525600 = one year; use 0 ONLY if option "
           "dogechat.history.max_buffer_lines_number is NOT set to 0"),
    ...
    config_history_max_buffer_lines_number = config_file_new_option (
        ...
        N_("maximum number of lines in history per buffer "
           ...
           "dogechat.history.max_buffer_lines_minutes is NOT set to 0"),
    ...
    /* proxies */
    ptr_section = config_file_new_section (dogechat_config_file, "proxy",
                                           0, 0,
                                           &config_dogechat_proxy_read_cb, NULL,
                                           NULL, NULL, NULL, NULL, NULL, NULL,
                                           NULL, NULL);
    if (!ptr_section)
    {
    ...
    dogechat_config_section_proxy = ptr_section;

    /* network */
    ptr_section = config_file_new_section (dogechat_config_file, "network",
                                           0, 0,
                                           NULL, NULL, NULL, NULL, NULL, NULL,
                                           NULL, NULL, NULL, NULL);
    if (!ptr_section)
    ...
    /* plugin */
    ptr_section = config_file_new_section (dogechat_config_file, "plugin",
                                           0, 0,
                                           NULL, NULL, NULL, NULL, NULL, NULL,
                                           NULL, NULL, NULL, NULL);
    if (!ptr_section)
    ...
    config_plugin_path = config_file_new_option (
        ...
        N_("path for searching plugins (\"%h\" will be replaced by "
           "DogeChat home, \"~/.dogechat\" by default)"),
    ...
    /* bars */
    ptr_section = config_file_new_section (dogechat_config_file, "bar",
                                           0, 0,
                                           &config_dogechat_bar_read_cb, NULL,
                                           NULL, NULL, NULL, NULL, NULL, NULL,
                                           NULL, NULL);
    if (!ptr_section)
    {
    ...
    dogechat_config_section_bar = ptr_section;

    /* layout */
    ptr_section = config_file_new_section (dogechat_config_file, "layout",
                                           0, 0,
                                           &config_dogechat_layout_read_cb, NULL,
                                           &config_dogechat_layout_write_cb, NULL,
                                           NULL, NULL, NULL, NULL, NULL, NULL);
    if (!ptr_section)
    {
        config_file_free (dogechat_config_file);
    ...
    /* notify */
    ptr_section = config_file_new_section (dogechat_config_file, "notify",
                                           1, 1,
                                           NULL, NULL,
                                           NULL, NULL,
                                           NULL, NULL,
                                           &config_dogechat_notify_create_option_cb, NULL,
                                           &config_dogechat_notify_delete_option_cb, NULL);
    if (!ptr_section)
    {
        config_file_free (dogechat_config_file);
        return 0;
    ...
    dogechat_config_section_notify = ptr_section;

    /* filters */
    ptr_section = config_file_new_section (dogechat_config_file, "filter",
                                           0, 0,
                                           &config_dogechat_filter_read_cb, NULL,
                                           &config_dogechat_filter_write_cb, NULL,
                                           &config_dogechat_filter_write_cb, NULL,
                                           NULL, NULL, NULL, NULL);
    if (!ptr_section)
    {
    ...
    for (i = 0; i < GUI_KEY_NUM_CONTEXTS; i++)
    {
        ...
        ptr_section = config_file_new_section (dogechat_config_file, section_name,
                                               0, 0,
                                               &config_dogechat_key_read_cb, NULL,
                                               &config_dogechat_key_write_cb, NULL,
                                               &config_dogechat_key_write_cb, NULL,
                                               NULL, NULL, NULL, NULL);
        if (!ptr_section)
        {
...
/*
 * Initializes DogeChat configuration.
 *
 * Returns:
 *   1: OK
 *   0: error
...
config_dogechat_init ()
{
    ...
    rc = config_dogechat_init_options ();
...
/*
 * Reads DogeChat configuration file.
 *
 * Returns:
 *   DOGECHAT_CONFIG_READ_OK: OK
 *   DOGECHAT_CONFIG_READ_MEMORY_ERROR: not enough memory
...
int
config_dogechat_read ()
{
    int rc;

    rc = config_file_read (dogechat_config_file);
...
/*
 * Writes DogeChat configuration file.
 *
 * Returns:
 *   DOGECHAT_CONFIG_WRITE_OK: OK
 *   DOGECHAT_CONFIG_WRITE_ERROR: error
 *   DOGECHAT_CONFIG_WRITE_MEMORY_ERROR: not enough memory
 */

int
config_dogechat_write ()
{
    return config_file_write (dogechat_config_file);
}

/*
 * Frees DogeChat configuration file and variables.
 */

void
config_dogechat_free ()
{
    int i;
