/*
 * doge-upgrade-file.c - save/restore data for upgrading DogeChat
 *
                    ...
                    char *message2, char *file, int line)
{
    gui_chat_printf (NULL,
                     _("%sError upgrading DogeChat with file \"%s\":"),
...
upgrade_file_new (const char *filename, int write)
{
    ...
    {
        /* build name of file */
        length = strlen (dogechat_home) + 1 + strlen (filename) + 16 + 1;
