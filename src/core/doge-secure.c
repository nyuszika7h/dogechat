/*
 * doge-secure.c - secured data configuration options (file sec.conf)
 *
...
secure_get_passphrase_from_user (const char *error)
{
    ...
    prompt[2] = _("(press ctrl-C to exit DogeChat now)");
    prompt[3] = error;
    prompt[4] = NULL;
...
secure_init_options ()
{
    ...
    secure_config_crypt_passphrase_file = config_file_new_option (
        ...
        N_("path to a file containing the passphrase to encrypt/decrypt secured "
           ...
           "environment variable \"DOGECHAT_PASSPHRASE\" is not set (the "
           ...
           "outside DogeChat home (for example in your home); example: "
           "\"~/.dogechat-passphrase\""),
        NULL, 0, 0, "", NULL, 0,
...
secure_init ()
{
    ...
    /* try to read passphrase (if not set) from env var "DOGECHAT_PASSPHRASE" */
    if (!secure_passphrase)
    {
...
secure_buffer_display ()
{
    ...
    gui_buffer_set_title (secure_buffer,
                          _("DogeChat secured data (sec.conf) | "
