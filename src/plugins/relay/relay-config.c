                                         ...
                                         struct t_config_option *option)
{
    ...
    allowed_ips = dogechat_config_string (relay_config_network_allowed_ips);
    if (allowed_ips && allowed_ips[0])
    {
        relay_config_regex_allowed_ips = malloc (sizeof (*relay_config_regex_allowed_ips));
        if (relay_config_regex_allowed_ips)
        {
            if (dogechat_string_regcomp (relay_config_regex_allowed_ips,
                                        allowed_ips,
                                                       ...
                                                       struct t_config_option *option)
{
    ...
    allowed_origins = dogechat_config_string (relay_config_network_websocket_allowed_origins);
    if (allowed_origins && allowed_origins[0])
    {
        ...
        if (relay_config_regex_websocket_allowed_origins)
        {
            if (dogechat_string_regcomp (relay_config_regex_websocket_allowed_origins,
                                        allowed_origins,
                                        REG_EXTENDED | REG_ICASE) != 0)
            {
                free (relay_config_regex_websocket_allowed_origins);
                                     ...
                                     const char *value)
{
    ...
    tags = dogechat_string_split (value, ",", 0, 0, &num_tags);
    if (tags)
    {
                                      ...
                                      struct t_config_option *option)
{
    ...
    {
        relay_config_hashtable_irc_backlog_tags = dogechat_hashtable_new (32,
        ...
        dogechat_hashtable_remove_all (relay_config_hashtable_irc_backlog_tags);

    items = dogechat_string_split (dogechat_config_string (relay_config_irc_backlog_tags),
                                  ",", 0, 0, &num_items);
    if (items)
    {
        for (i = 0; i < num_items; i++)
        {
            dogechat_hashtable_set (relay_config_hashtable_irc_backlog_tags,
                                   items[i],
                                   NULL);
        }
...
relay_config_change_port_cb (void *data, struct t_config_option *option)
{
    ...
    ptr_server = relay_server_search (dogechat_config_option_get_pointer (option, "name"));
    if (ptr_server)
    {
        relay_server_update_port (ptr_server,
                                  *((int *)dogechat_config_option_get_pointer (option, "value")));
    }
}
                                 ...
                                 const char *value)
{
    ...
    {
        dogechat_printf (NULL,
                        _("%s%s: cannot use SSL because DogeChat was not built "
                          "with GnuTLS support"),
                        dogechat_prefix ("error"), RELAY_PLUGIN_NAME);
    ...
    if (rc != DOGECHAT_CONFIG_OPTION_SET_ERROR)
    {
        ...
        {
            dogechat_printf (NULL, _("%s%s: error: unknown protocol \"%s\""),
                            dogechat_prefix ("error"),
                            RELAY_PLUGIN_NAME, protocol);
            rc = DOGECHAT_CONFIG_OPTION_SET_ERROR;
        }

        if ((protocol_number == RELAY_PROTOCOL_DOGECHAT) && protocol_args)
        {
            dogechat_printf (NULL, _("%s%s: error: name is not allowed for "
                                    "protocol \"%s\""),
                            dogechat_prefix ("error"),
                            RELAY_PLUGIN_NAME, protocol);
            rc = DOGECHAT_CONFIG_OPTION_SET_ERROR;
    ...
    if (rc != DOGECHAT_CONFIG_OPTION_SET_ERROR)
    {
        if (dogechat_config_search_option (config_file, section, option_name))
        {
            dogechat_printf (NULL, _("%s%s: error: relay for \"%s\" already exists"),
                            dogechat_prefix ("error"),
...
relay_config_reload (void *data, struct t_config_file *config_file)
{
    ...
    dogechat_config_section_free_options (relay_config_section_port);
...
relay_config_init ()
{
    ...
    relay_config_file = dogechat_config_new (RELAY_CONFIG_NAME,
                                            &relay_config_reload, NULL);
    if (!relay_config_file)
        return 0;

    /* section look */
    ptr_section = dogechat_config_new_section (relay_config_file, "look",
                                              0, 0,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL);
    if (!ptr_section)
    {
        dogechat_config_free (relay_config_file);
        return 0;
    }

    relay_config_look_auto_open_buffer = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "auto_open_buffer", "boolean",
        N_("auto open relay buffer when a new client is connecting"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    relay_config_look_raw_messages = dogechat_config_new_option (
        relay_config_file, ptr_section,
    ...
    /* section color */
    ptr_section = dogechat_config_new_section (relay_config_file, "color",
                                              0, 0,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL);
    ...
    relay_config_color_client = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "client", "color",
        N_("text color for client description"),
        NULL, 0, 0, "cyan", NULL, 0,
        NULL, NULL, NULL, NULL, NULL, NULL);
    relay_config_color_status[RELAY_STATUS_CONNECTING] = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "status_connecting", "color",
        N_("text color for \"connecting\" status"),
        NULL, 0, 0, "yellow", NULL, 0,
        NULL, NULL, &relay_config_refresh_cb, NULL, NULL, NULL);
    relay_config_color_status[RELAY_STATUS_WAITING_AUTH] = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "status_waiting_auth", "color",
        N_("text color for \"waiting authentication\" status"),
        NULL, 0, 0, "brown", NULL, 0,
        NULL, NULL, &relay_config_refresh_cb, NULL, NULL, NULL);
    relay_config_color_status[RELAY_STATUS_CONNECTED] = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "status_active", "color",
        N_("text color for \"connected\" status"),
        NULL, 0, 0, "lightblue", NULL, 0,
        NULL, NULL, &relay_config_refresh_cb, NULL, NULL, NULL);
    relay_config_color_status[RELAY_STATUS_AUTH_FAILED] = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "status_auth_failed", "color",
        N_("text color for \"authentication failed\" status"),
        NULL, 0, 0, "lightred", NULL, 0,
        NULL, NULL, &relay_config_refresh_cb, NULL, NULL, NULL);
    relay_config_color_status[RELAY_STATUS_DISCONNECTED] = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "status_disconnected", "color",
        N_("text color for \"disconnected\" status"),
        NULL, 0, 0, "lightred", NULL, 0,
        NULL, NULL, &relay_config_refresh_cb, NULL, NULL, NULL);
    relay_config_color_text = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "text", "color",
        N_("text color in relay buffer"),
        NULL, 0, 0, "default", NULL, 0,
        NULL, NULL, &relay_config_refresh_cb, NULL, NULL, NULL);
    relay_config_color_text_bg = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "text_bg", "color",
        N_("background color in relay buffer"),
        NULL, 0, 0, "default", NULL, 0,
        NULL, NULL, &relay_config_refresh_cb, NULL, NULL, NULL);
    relay_config_color_text_selected = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "text_selected", "color",
    ...
    /* section network */
    ptr_section = dogechat_config_new_section (relay_config_file, "network",
                                              0, 0,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL);
    ...
    relay_config_network_allowed_ips = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "allowed_ips", "string",
    ...
    relay_config_network_bind_address = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "bind_address", "string",
    ...
    relay_config_network_clients_purge_delay = dogechat_config_new_option (
        relay_config_file, ptr_section,
        ...
        NULL, NULL, NULL, NULL, NULL, NULL);
    relay_config_network_compression_level = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "compression_level", "integer",
        N_("compression level for packets sent to client with DogeChat protocol "
           "(0 = disable compression, 1 = low compression ... 9 = best "
           "compression)"),
        NULL, 0, 9, "6", NULL, 0,
        NULL, NULL, NULL, NULL, NULL, NULL);
    relay_config_network_ipv6 = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "ipv6", "boolean",
    ...
    relay_config_network_max_clients = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "max_clients", "integer",
        N_("maximum number of clients connecting to a port"),
        NULL, 1, 1024, "5", NULL, 0,
        NULL, NULL, NULL, NULL, NULL, NULL);
    relay_config_network_password = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "password", "string",
    ...
    relay_config_network_ssl_cert_key = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "ssl_cert_key", "string",
    ...
    relay_config_network_ssl_priorities = dogechat_config_new_option (
        relay_config_file, ptr_section,
    ...
    relay_config_network_websocket_allowed_origins = dogechat_config_new_option (
        relay_config_file, ptr_section,
    ...
    /* section irc */
    ptr_section = dogechat_config_new_section (relay_config_file, "irc",
                                              0, 0,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL);
    ...
    relay_config_irc_backlog_max_minutes = dogechat_config_new_option (
        relay_config_file, ptr_section,
        ...
        NULL, NULL, NULL, NULL, NULL, NULL);
    relay_config_irc_backlog_max_number = dogechat_config_new_option (
        relay_config_file, ptr_section,
        ...
        NULL, NULL, NULL, NULL, NULL, NULL);
    relay_config_irc_backlog_since_last_disconnect = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "backlog_since_last_disconnect", "boolean",
        N_("display backlog starting from last client disconnect"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    relay_config_irc_backlog_since_last_message = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "backlog_since_last_message", "boolean",
        N_("display backlog starting from your last message"),
        NULL, 0, 0, "off", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    relay_config_irc_backlog_tags = dogechat_config_new_option (
        relay_config_file, ptr_section,
        "backlog_tags", "string",
    ...
    relay_config_irc_backlog_time_format = dogechat_config_new_option (
        relay_config_file, ptr_section,
    ...
    /* section port */
    ptr_section = dogechat_config_new_section (relay_config_file, "port",
                                              1, 1,
                                              NULL, NULL,
                                              NULL, NULL,
                                              NULL, NULL,
...
relay_config_read ()
{
    ...
    rc = dogechat_config_read (relay_config_file);
...
relay_config_write ()
{
    return dogechat_config_write (relay_config_file);
}
...
relay_config_free ()
{
    ...
    if (relay_config_hashtable_irc_backlog_tags)
    {
        dogechat_hashtable_free (relay_config_hashtable_irc_backlog_tags);
