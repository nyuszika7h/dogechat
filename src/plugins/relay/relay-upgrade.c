/*
 * relay-upgrade.c - save/restore relay plugin data when upgrading DogeChat
 *
...
relay_upgrade_save ()
{
    ...
    upgrade_file = dogechat_upgrade_new (RELAY_UPGRADE_FILENAME, 1);
    if (!upgrade_file)
        return 0;
...
relay_upgrade_set_buffer_callbacks ()
{
    ...
    if (infolist)
    {
        ...
        {
            if (dogechat_infolist_pointer (infolist, "plugin") == dogechat_relay_plugin)
            {
                ptr_buffer = dogechat_infolist_pointer (infolist, "pointer");
                dogechat_buffer_set_pointer (ptr_buffer, "close_callback", &relay_buffer_close_cb);
                dogechat_buffer_set_pointer (ptr_buffer, "input_callback", &relay_buffer_input_cb);
    ...
    while (dogechat_infolist_next (infolist))
    {
        switch (object_id)
        {
            ...
            case RELAY_UPGRADE_TYPE_RAW_MESSAGE:
                relay_raw_message_add_to_list (dogechat_infolist_time (infolist, "date"),
...
relay_upgrade_load ()
{
    ...
    upgrade_file = dogechat_upgrade_new (RELAY_UPGRADE_FILENAME, 0);
    if (!upgrade_file)
        return 0;
    rc = dogechat_upgrade_read (upgrade_file, &relay_upgrade_read_cb, NULL);
    dogechat_upgrade_close (upgrade_file);
