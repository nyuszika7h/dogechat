...
relay_raw_message_print (struct t_relay_raw_message *raw_message)
{
    ...
    {
        dogechat_printf_date_tags (relay_raw_buffer,
...
relay_raw_open (int switch_to_buffer)
{
    ...
    if (!relay_raw_buffer)
    {
        relay_raw_buffer = dogechat_buffer_search (RELAY_PLUGIN_NAME,
                                                  RELAY_RAW_BUFFER_NAME);
        if (!relay_raw_buffer)
        {
            relay_raw_buffer = dogechat_buffer_new (RELAY_RAW_BUFFER_NAME,
            ...
            dogechat_buffer_set (relay_raw_buffer,
                                "title", _("Relay raw messages"));

            if (!dogechat_buffer_get_integer (relay_raw_buffer, "short_name_is_set"))
            {
                dogechat_buffer_set (relay_raw_buffer, "short_name",
                                    RELAY_RAW_BUFFER_NAME);
            }
            dogechat_buffer_set (relay_raw_buffer, "localvar_set_type", "debug");
            dogechat_buffer_set (relay_raw_buffer, "localvar_set_server", RELAY_RAW_BUFFER_NAME);
            dogechat_buffer_set (relay_raw_buffer, "localvar_set_channel", RELAY_RAW_BUFFER_NAME);
            dogechat_buffer_set (relay_raw_buffer, "localvar_set_no_log", "1");

            /* disable all highlights on this buffer */
            dogechat_buffer_set (relay_raw_buffer, "highlight_words", "-");
    ...
    if (relay_raw_buffer && switch_to_buffer)
        dogechat_buffer_set (relay_raw_buffer, "display", "1");
}
...
relay_raw_message_remove_old ()
{
    ...
    max_messages = dogechat_config_integer (relay_config_look_raw_messages);
                       ...
                       const char *data, int data_size)
{
    ...
    {
        /* binary message */
        buf = dogechat_string_hex_dump (data, data_size, 16, "  > ", NULL);
        snprintf (prefix, sizeof (prefix), " ");
    }
    else
    {
        /* text message */
        buf = dogechat_iconv_to_internal (NULL, data);
        buf2 = dogechat_string_replace (buf, "\r", "");
        if (buf2)
        {
            free (buf);
            buf = buf2;
    ...
    if (new_raw_message)
    {
        ...
        if (dogechat_config_integer (relay_config_look_raw_messages) == 0)
                 ...
                 const char *data, int data_size)
{
    ...
    if (!relay_raw_buffer && (dogechat_relay_plugin->debug >= 1))
        relay_raw_open (0);
