/*
 * dogechat-aspell.c - aspell plugin for DogeChat: color for misspelled words
 *
...
#include "dogechat-aspell-bar-item.h"
#include "dogechat-aspell-command.h"
#include "dogechat-aspell-completion.h"
#include "dogechat-aspell-config.h"
#include "dogechat-aspell-info.h"
#include "dogechat-aspell-speller.h"


DOGECHAT_PLUGIN_NAME(ASPELL_PLUGIN_NAME);
DOGECHAT_PLUGIN_DESCRIPTION(N_("Spell checker for input (with Aspell)"));
...
DOGECHAT_PLUGIN_PRIORITY(9000);

struct t_dogechat_plugin *dogechat_aspell_plugin = NULL;
...
char *
dogechat_aspell_build_option_name (struct t_gui_buffer *buffer)
{
...
const char *
dogechat_aspell_get_dict_with_buffer_name (const char *name)
{
    ...
    if (option_name)
    {
        ...
        {
            ptr_option = dogechat_aspell_config_get_dict (option_name);
            if (ptr_option)
            {
                free (option_name);
    ...
    if (dogechat_config_string (dogechat_aspell_config_check_default_dict)
        && dogechat_config_string (dogechat_aspell_config_check_default_dict)[0])
    {
        return dogechat_config_string (dogechat_aspell_config_check_default_dict);
    }
...
const char *
dogechat_aspell_get_dict (struct t_gui_buffer *buffer)
{
    char *name;
    const char *dict;
    ...
    dict = dogechat_aspell_get_dict_with_buffer_name (name);
...
int
dogechat_aspell_command_authorized (const char *command)
{
    ...
    {
        if ((dogechat_aspell_length_commands_to_check[i] == length_command)
            && (dogechat_strcasecmp (command,
                                    dogechat_aspell_commands_to_check[i]) == 0))
        {
...
int
dogechat_aspell_string_is_url (const char *word)
{
    int i;

    for (i = 0; aspell_url_prefix[i]; i++)
    {
        if (dogechat_strncasecmp (word, aspell_url_prefix[i],
...
int
dogechat_aspell_string_is_nick (struct t_gui_buffer *buffer, const char *word)
{
    ...
    nick_completer = dogechat_config_string (
        dogechat_config_get ("dogechat.completion.nick_completer"));
    ...
    rc = (dogechat_nicklist_search_nick (buffer, NULL, word)) ? 1 : 0;

    if (!rc)
    {
        /* for "private" buffers, check if word is self or remote nick */
        buffer_type = dogechat_buffer_get_string (buffer, "localvar_type");
        if (buffer_type && (strcmp (buffer_type, "private") == 0))
        {
            /* check self nick */
            buffer_nick = dogechat_buffer_get_string (buffer, "localvar_nick");
            if (buffer_nick && (dogechat_strcasecmp (buffer_nick, word) == 0))
            {
                rc = 1;
            }
            else
            {
                /* check remote nick */
                buffer_channel = dogechat_buffer_get_string (buffer,
                                                            "localvar_channel");
                if (buffer_channel
                    && (dogechat_strcasecmp (buffer_channel, word) == 0))
                {
                    rc = 1;
                }
            }
...
int
dogechat_aspell_string_is_simili_number (const char *word)
{
    int code_point;
    ...
    while (word && word[0])
    {
        code_point = dogechat_utf8_char_int (word);
        if (!iswpunct (code_point) && !iswdigit (code_point))
            return 0;
        word = dogechat_utf8_next_char (word);
    }
...
int
dogechat_aspell_check_word (struct t_aspell_speller_buffer *speller_buffer,
                           const char *word)
{
    int i;

    /* word too small? then do not check word */
    if ((dogechat_config_integer (dogechat_aspell_config_check_word_min_length) > 0)
        && ((int)strlen (word) < dogechat_config_integer (dogechat_aspell_config_check_word_min_length)))
        return 1;

    /* word is a number? then do not check word */
    if (dogechat_aspell_string_is_simili_number (word))
        return 1;
...
char *
dogechat_aspell_get_suggestions (struct t_aspell_speller_buffer *speller_buffer,
                                const char *word)
{
    ...
    max_suggestions = dogechat_config_integer (dogechat_aspell_config_check_suggestions);
...
char *
dogechat_aspell_modifier_cb (void *data, const char *modifier,
                            const char *modifier_data, const char *string)
{
    ...
    if (dogechat_buffer_get_integer (buffer, "text_search")
        && !dogechat_config_boolean (dogechat_aspell_config_check_during_search))
        return NULL;
     ...
     */
    input_pos = dogechat_buffer_get_integer (buffer, "input_pos");
    if (ptr_speller_buffer->modifier_string
        && (strcmp (string, ptr_speller_buffer->modifier_string) == 0)
        && ((dogechat_config_integer (dogechat_aspell_config_check_suggestions) < 0)
    ...
    color_normal = dogechat_color ("bar_fg");
    length_color_normal = strlen (color_normal);
    color_error = dogechat_color (dogechat_config_string (dogechat_aspell_config_color_misspelled));
    ...
    if (result)
    {
        ...
        if (!dogechat_string_input_for_buffer (ptr_string))
        {
            char_size = dogechat_utf8_char_size (ptr_string);
            ptr_string += char_size;
            pos_space = ptr_string;
            while (pos_space && pos_space[0] && (pos_space[0] != ' '))
            {
                pos_space = (char *)dogechat_utf8_next_char (pos_space);
            }
            ...
            if (!dogechat_aspell_command_authorized (ptr_string))
            {
                free (result);
                return NULL;
            }
        ...
        while (ptr_string[0])
        {
            ...
            code_point = dogechat_utf8_char_int (ptr_string);
            ...
            ptr_end = (char *)dogechat_utf8_next_char (ptr_string);
            code_point = dogechat_utf8_char_int (ptr_end);
                   ...
                   || (code_point == '-'))
            {
                ...
                ptr_end = (char *)dogechat_utf8_next_char (ptr_end);
                if (!ptr_end[0])
                    break;
                code_point = dogechat_utf8_char_int (ptr_end);
            }
            ptr_end = (char *)dogechat_utf8_next_char (ptr_end_valid);
            word_end_pos = word_end_pos_valid;
            word_ok = 0;
            if (dogechat_aspell_string_is_url (ptr_string)
                || dogechat_aspell_string_is_nick (buffer, ptr_string_orig))
            {
                /*
            ...
            {
                if ((save_end != '\0')
                    || (dogechat_config_integer (dogechat_aspell_config_check_real_time)))
                {
                    word_ok = dogechat_aspell_check_word (ptr_speller_buffer,
                                                         ptr_string);
    ...
    if (word_for_suggestions)
    {
        suggestions = dogechat_aspell_get_suggestions (ptr_speller_buffer,
        ...
        if (suggestions)
        {
            ...
            if (word_and_suggestions)
            {
                ...
                dogechat_buffer_set (buffer, "localvar_set_aspell_suggest",
                                    word_and_suggestions);
                free (word_and_suggestions);
            }
            else
            {
                dogechat_buffer_set (buffer, "localvar_del_aspell_suggest", "");
            }
            free (suggestions);
        }
        else
            ...
            && (strcmp (old_suggestions, ptr_suggestions) != 0)))
    {
        ...
        (void) dogechat_hook_signal_send ("aspell_suggest",
...
int
dogechat_aspell_buffer_switch_cb (void *data, const char *signal,
                                 const char *type_data, void *signal_data)
{
    ...
    dogechat_bar_item_update ("aspell_dict");
    dogechat_bar_item_update ("aspell_suggest");
...
int
dogechat_aspell_window_switch_cb (void *data, const char *signal,
                                 const char *type_data, void *signal_data)
{
    /* make C compiler happy */
    (void) data;
...
/*
 * Removes struct for buffer in hashtable "dogechat_aspell_speller_buffer" on
 * signal "buffer_closed".
 */

int
dogechat_aspell_buffer_closed_cb (void *data, const char *signal,
                                 const char *type_data, void *signal_data)
{
    /* make C compiler happy */
    (void) data;
    ...
    dogechat_hashtable_remove (dogechat_aspell_speller_buffer, signal_data);
...
int
dogechat_aspell_debug_libs_cb (void *data, const char *signal,
                              const char *type_data, void *signal_data)
{
    /* make C compiler happy */
    (void) data;
    ...
    dogechat_printf (NULL, "  %s: enchant %s",
                    ASPELL_PLUGIN_NAME, enchant_get_version ());
#else
    dogechat_printf (NULL, "  %s: enchant (?)", ASPELL_PLUGIN_NAME);
#endif /* HAVE_ENCHANT_GET_VERSION */
#else
#ifdef HAVE_ASPELL_VERSION_STRING
    dogechat_printf (NULL, "  %s: aspell %s",
                    ASPELL_PLUGIN_NAME, aspell_version_string ());
#else
    dogechat_printf (NULL, "  %s: aspell (?)", ASPELL_PLUGIN_NAME);
...
dogechat_plugin_init (struct t_dogechat_plugin *plugin, int argc, char *argv[])
{
    ...
    if (!dogechat_aspell_speller_init ())
        return DOGECHAT_RC_ERROR;

    if (!dogechat_aspell_config_init ())
        return DOGECHAT_RC_ERROR;
    ...
    dogechat_aspell_completion_init ();
     ...
     */
    dogechat_hook_modifier ("500|input_text_display",
                           &dogechat_aspell_modifier_cb, NULL);
    ...
    dogechat_hook_signal ("buffer_switch",
                         &dogechat_aspell_buffer_switch_cb, NULL);
    dogechat_hook_signal ("window_switch",
                         &dogechat_aspell_window_switch_cb, NULL);
    dogechat_hook_signal ("buffer_closed",
                         &dogechat_aspell_buffer_closed_cb, NULL);
    dogechat_hook_signal ("debug_libs",
                         &dogechat_aspell_debug_libs_cb, NULL);
