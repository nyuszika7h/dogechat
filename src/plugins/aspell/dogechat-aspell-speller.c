/*
 * dogechat-aspell-speller.c - speller management for aspell plugin
 *
 ...
 */
struct t_hashtable *dogechat_aspell_spellers = NULL;

/*
 * spellers by buffer (key is buffer pointer, value is pointer on
 * struct t_aspell_speller_buffer)
 */
struct t_hashtable *dogechat_aspell_speller_buffer = NULL;
...
int
dogechat_aspell_speller_dict_supported (const char *lang)
{
#ifdef USE_ENCHANT
...
void
dogechat_aspell_speller_check_dictionaries (const char *dict_list)
{
    char **argv;
    int argc, i;
    ...
    if (dict_list)
    {
        argv = dogechat_string_split (dict_list, ",", 0, 0, &argc);
        if (argv)
        {
            for (i = 0; i < argc; i++)
            {
                if (!dogechat_aspell_speller_dict_supported (argv[i]))
                {
...
AspellSpeller *
#endif /* USE_ENCHANT */
dogechat_aspell_speller_new (const char *lang)
{
#ifdef USE_ENCHANT
    ...
    if (dogechat_aspell_plugin->debug)
    {
        dogechat_printf (NULL,
    ...
    /* apply all options */
    infolist = dogechat_infolist_get ("option", NULL, "aspell.option.*");
    if (infolist)
    {
    ...
    dogechat_hashtable_set (dogechat_aspell_spellers, lang, new_speller);
...
void
dogechat_aspell_speller_add_dicts_to_hash (struct t_hashtable *hashtable,
                                          const char *dict)
{
    char **dicts;
    int num_dicts, i;
    ...
    dicts = dogechat_string_split (dict, ",", 0, 0, &num_dicts);
    if (dicts)
    {
        for (i = 0; i < num_dicts; i++)
        {
            dogechat_hashtable_set (hashtable, dicts[i], NULL);
        }
        dogechat_string_free_split (dicts);
    }
}
...
void
dogechat_aspell_speller_remove_unused_cb (void *data,
                                         ...
                                         const void *key, const void *value)
{
    ...
    if (!dogechat_hashtable_has_key (used_spellers, key))
        dogechat_hashtable_remove (hashtable, key);
}

/*
 * Removes unused spellers from hashtable "dogechat_aspell_spellers".
 */
...
dogechat_aspell_speller_remove_unused ()
{
    ...
    used_spellers = dogechat_hashtable_new (32,
    ...
    dogechat_aspell_speller_add_dicts_to_hash (used_spellers,
                                              dogechat_config_string (dogechat_aspell_config_check_default_dict));
     ...
     * "used_spellers"
     */
    dogechat_hashtable_map (dogechat_aspell_spellers,
                           &dogechat_aspell_speller_remove_unused_cb,
                           used_spellers);

    dogechat_hashtable_free (used_spellers);
}

/*
 * Callback called when a key is removed in hashtable "dogechat_aspell_spellers".
 */

void
dogechat_aspell_speller_free_value_cb (struct t_hashtable *hashtable,
 ...
 * "dogechat_aspell_buffer_spellers".
 */

struct t_aspell_speller_buffer *
dogechat_aspell_speller_buffer_new (struct t_gui_buffer *buffer)
{
    ...
    dogechat_hashtable_remove (dogechat_aspell_speller_buffer, buffer);
    ...
    buffer_dicts = dogechat_aspell_get_dict (buffer);
    if (buffer_dicts)
    {
        dicts = dogechat_string_split (buffer_dicts, ",", 0, 0, &num_dicts);
        if (dicts && (num_dicts > 0))
        {
            ...
            if (new_speller_buffer->spellers)
            {
                for (i = 0; i < num_dicts; i++)
                {
                    ptr_speller = dogechat_hashtable_get (dogechat_aspell_spellers,
                                                         dicts[i]);
                    if (!ptr_speller)
                        ptr_speller = dogechat_aspell_speller_new (dicts[i]);
    ...
    dogechat_hashtable_set (dogechat_aspell_speller_buffer,
                           buffer,
                           new_speller_buffer);
 ...
 * "dogechat_aspell_speller_buffer".
 */

void
dogechat_aspell_speller_buffer_free_value_cb (struct t_hashtable *hashtable,
                                             const void *key, void *value)
{
...
int
dogechat_aspell_speller_init ()
{
    dogechat_aspell_spellers = dogechat_hashtable_new (32,
                                                     DOGECHAT_HASHTABLE_STRING,
                                                     DOGECHAT_HASHTABLE_POINTER,
                                                     NULL,
                                                     NULL);
    if (!dogechat_aspell_spellers)
        return 0;
    dogechat_hashtable_set_pointer (dogechat_aspell_spellers,
                                   "callback_free_value",
                                   &dogechat_aspell_speller_free_value_cb);

    dogechat_aspell_speller_buffer = dogechat_hashtable_new (32,
                                                           DOGECHAT_HASHTABLE_POINTER,
                                                           DOGECHAT_HASHTABLE_POINTER,
                                                           NULL,
                                                           NULL);
    if (!dogechat_aspell_speller_buffer)
    {
        dogechat_hashtable_free (dogechat_aspell_spellers);
        return 0;
    }
    dogechat_hashtable_set_pointer (dogechat_aspell_speller_buffer,
                                   "callback_free_value",
                                   &dogechat_aspell_speller_buffer_free_value_cb);
...
{
    dogechat_hashtable_free (dogechat_aspell_spellers);
    dogechat_hashtable_free (dogechat_aspell_speller_buffer);
}
