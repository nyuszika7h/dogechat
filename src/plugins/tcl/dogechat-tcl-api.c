/*
 * dogechat-tcl-api.c - tcl API functions
 *
...
#define API_DEF_FUNC(__name)                                            \
    Tcl_CreateObjCommand (interp, "dogechat::" #__name,                  \
                          dogechat_tcl_api_##__name,                     \
                          (ClientData) NULL,                            \
                          (Tcl_CmdDeleteProc*)NULL);
#define API_FUNC(__name)                                                \
    static int                                                          \
    dogechat_tcl_api_##__name (ClientData clientData,                    \
                              Tcl_Interp *interp,                       \
                              int objc,                                 \
                              Tcl_Obj *CONST objv[])
#define API_INIT_FUNC(__init, __name, __ret)                            \
    ...
    {                                                                   \
        DOGECHAT_SCRIPT_MSG_NOT_INIT(TCL_CURRENT_SCRIPT_NAME,            \
                                    tcl_function_name);                 \
        __ret;                                                          \
    }
#define API_WRONG_ARGS(__ret)                                           \
    {                                                                   \
        DOGECHAT_SCRIPT_MSG_WRONG_ARGS(TCL_CURRENT_SCRIPT_NAME,          \
                                      tcl_function_name);               \
        __ret;                                                          \
    }
...
#define API_STR2PTR(__string)                                           \
    plugin_script_str2ptr (dogechat_tcl_plugin,                          \
...
API_FUNC(register)
{
    ...
    if (plugin_script_search (dogechat_tcl_plugin, tcl_scripts, name))
    {
    ...
    /* register script */
    tcl_current_script = plugin_script_add (dogechat_tcl_plugin,
...
API_FUNC(plugin_get_name)
{
    ...
    result = dogechat_plugin_get_name (API_STR2PTR(plugin));
...
API_FUNC(iconv_to_internal)
{
    ...
    result = dogechat_iconv_to_internal (charset, string);
...
API_FUNC(iconv_from_internal)
{
    ...
    result = dogechat_iconv_from_internal (charset, string);
...
API_FUNC(gettext)
{
    ...
    result = dogechat_gettext (Tcl_GetStringFromObj (objv[1], &i)); /* string */
...
API_FUNC(ngettext)
{
    ...
    result = dogechat_ngettext (single, plural, count);
...
API_FUNC(strlen_screen)
{
    ...
    result = dogechat_strlen_screen (string);
...
API_FUNC(string_match)
{
    ...
    result = dogechat_string_match (string, mask, case_sensitive);
...
API_FUNC(string_has_highlight)
{
    ...
    result = dogechat_string_has_highlight (string, highlight_words);
...
API_FUNC(string_has_highlight_regex)
{
    ...
    result = dogechat_string_has_highlight_regex (string, regex);
...
API_FUNC(string_mask_to_regex)
{
    ...
    result = dogechat_string_mask_to_regex (mask);
...
API_FUNC(string_remove_color)
{
    ...
    result = dogechat_string_remove_color (string, replacement);
...
API_FUNC(string_is_command_char)
{
    ...
    result = dogechat_string_is_command_char (Tcl_GetStringFromObj (objv[1], &i)); /* string */
...
API_FUNC(string_input_for_buffer)
{
    ...
    result = dogechat_string_input_for_buffer (Tcl_GetStringFromObj (objv[1], &i));
...
API_FUNC(string_eval_expression)
{
    ...
    pointers = dogechat_tcl_dict_to_hashtable (interp, objv[2],
                                              DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                              DOGECHAT_HASHTABLE_STRING,
                                              DOGECHAT_HASHTABLE_POINTER);
    extra_vars = dogechat_tcl_dict_to_hashtable (interp, objv[3],
                                                DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                DOGECHAT_HASHTABLE_STRING,
                                                DOGECHAT_HASHTABLE_STRING);
    options = dogechat_tcl_dict_to_hashtable (interp, objv[4],
                                             DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                             DOGECHAT_HASHTABLE_STRING,
                                             DOGECHAT_HASHTABLE_STRING);

    result = dogechat_string_eval_expression (expr, pointers, extra_vars,
                                             options);
...
API_FUNC(string_eval_path_home)
{
    ...
    pointers = dogechat_tcl_dict_to_hashtable (
        interp, objv[2],
        DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
        DOGECHAT_HASHTABLE_STRING,
        DOGECHAT_HASHTABLE_POINTER);
    extra_vars = dogechat_tcl_dict_to_hashtable (
        interp, objv[3],
        DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
        DOGECHAT_HASHTABLE_STRING,
        DOGECHAT_HASHTABLE_STRING);
    options = dogechat_tcl_dict_to_hashtable (
        interp, objv[4],
        DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
        DOGECHAT_HASHTABLE_STRING,
        DOGECHAT_HASHTABLE_STRING);

    result = dogechat_string_eval_path_home (path, pointers, extra_vars,
                                            options);
...
API_FUNC(mkdir_home)
{
    ...
    if (dogechat_mkdir_home (Tcl_GetStringFromObj (objv[1], &i), /* directory */
                            mode))
        API_RETURN_OK;
...
API_FUNC(mkdir)
{
    ...
    if (dogechat_mkdir (Tcl_GetStringFromObj (objv[1], &i), /* directory */
                       mode))
        API_RETURN_OK;
...
API_FUNC(mkdir_parents)
{
    ...
    if (dogechat_mkdir_parents (Tcl_GetStringFromObj (objv[1], &i), /* directory */
                               mode))
        API_RETURN_OK;
...
{
    Tcl_Obj *objp;
    char *result, *dogelist, *data, *where, *user_data;
    int i;
...
API_FUNC(list_search)
{
    ...
    result = API_PTR2STR(dogechat_list_search (API_STR2PTR(dogelist),
                                              data));
...
API_FUNC(list_search_pos)
{
    ...
    pos = dogechat_list_search_pos (API_STR2PTR(dogelist), data);
...
API_FUNC(list_casesearch)
{
    ...
    result = API_PTR2STR(dogechat_list_casesearch (API_STR2PTR(dogelist),
                                                  data));
...
API_FUNC(list_casesearch_pos)
{
    ...
    pos = dogechat_list_casesearch_pos (API_STR2PTR(dogelist), data);
...
API_FUNC(list_get)
{
    ...
    result = API_PTR2STR(dogechat_list_get (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)), /* dogelist */
                                           position));
...
API_FUNC(list_set)
{
    ...
    dogechat_list_set (API_STR2PTR(item), new_value);
...
API_FUNC(list_next)
{
    ...
    result = API_PTR2STR(dogechat_list_next (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)))); /* item */
...
API_FUNC(list_prev)
{
    ...
    result = API_PTR2STR(dogechat_list_prev (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)))); /* item */
...
API_FUNC(list_string)
{
    ...
    result = dogechat_list_string (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* item */
...
API_FUNC(list_size)
{
    ...
    size = dogechat_list_size (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* dogelist */
...
API_FUNC(list_remove)
{
    ...
    dogechat_list_remove (API_STR2PTR(dogelist),
                         API_STR2PTR(item));
...
API_FUNC(list_remove_all)
{
    ...
    dogechat_list_remove_all (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* dogelist */
...
API_FUNC(list_free)
{
    ...
    dogechat_list_free (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* dogelist */

    API_RETURN_OK;
}

int
dogechat_tcl_api_config_reload_cb (void *data,
                                  struct t_config_file *config_file)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        rc = (int *) dogechat_tcl_exec (script_callback->script,
...
API_FUNC(config_new)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new (dogechat_tcl_plugin,
                                                       tcl_current_script,
                                                       name,
                                                       &dogechat_tcl_api_config_reload_cb,
                                                       function,
                                                       data));
...
int
dogechat_tcl_api_config_section_read_cb (void *data,
                                        struct t_config_file *config_file,
...
int
dogechat_tcl_api_config_section_write_cb (void *data,
                                          struct t_config_file *config_file,
...
int
dogechat_tcl_api_config_section_write_default_cb (void *data,
                                                  struct t_config_file *config_file,
                                                  const char *section_name)
{
    struct t_plugin_script_cb *script_callback;
...
int
dogechat_tcl_api_config_section_create_option_cb (void *data,
                                                  struct t_config_file *config_file,
                                                  struct t_config_section *section,
...
int
dogechat_tcl_api_config_section_delete_option_cb (void *data,
                                                 struct t_config_file *config_file,
                                                 struct t_config_section *section,
...
API_FUNC(config_new_section)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new_section (dogechat_tcl_plugin,
                                                               tcl_current_script,
                                                               API_STR2PTR(cfg_file),
                                                               name,
                                                               can_add, /* user_can_add_options */
                                                               can_delete, /* user_can_delete_options */
                                                               &dogechat_tcl_api_config_section_read_cb,
                                                               function_read,
                                                               data_read,
                                                               &dogechat_tcl_api_config_section_write_cb,
                                                               function_write,
                                                               data_write,
                                                               &dogechat_tcl_api_config_section_write_default_cb,
                                                               function_write_default,
                                                               data_write_default,
                                                               &dogechat_tcl_api_config_section_create_option_cb,
                                                               function_create_option,
                                                               data_create_option,
                                                               &dogechat_tcl_api_config_section_delete_option_cb,
...
API_FUNC(config_search_section)
{
    ...
    result = API_PTR2STR(dogechat_config_search_section (API_STR2PTR(config_file),
                                                        section_name));
...
int
dogechat_tcl_api_config_option_check_value_cb (void *data,
                                               struct t_config_option *option,
                                               const char *value)
{
    struct t_plugin_script_cb *script_callback;
...
void
dogechat_tcl_api_config_option_change_cb (void *data,
                                          struct t_config_option *option)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
void
dogechat_tcl_api_config_option_delete_cb (void *data,
                                          struct t_config_option *option)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
API_FUNC(config_new_option)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new_option (dogechat_tcl_plugin,
                                                              tcl_current_script,
                                                              ...
                                                              value,
                                                              null_value_allowed,
                                                              &dogechat_tcl_api_config_option_check_value_cb,
                                                              function_check_value,
                                                              data_check_value,
                                                              &dogechat_tcl_api_config_option_change_cb,
                                                              function_change,
                                                              data_change,
                                                              &dogechat_tcl_api_config_option_delete_cb,
                                                              function_delete,
                                                              data_delete));
...
API_FUNC(config_search_option)
{
    ...
    result = API_PTR2STR(dogechat_config_search_option (API_STR2PTR(config_file),
                                                       API_STR2PTR(section),
                                                       option_name));
...
API_FUNC(config_string_to_boolean)
{
    ...
    result = dogechat_config_string_to_boolean (Tcl_GetStringFromObj (objv[1], &i)); /* text */
...
API_FUNC(config_option_reset)
{
    ...
    rc = dogechat_config_option_reset (API_STR2PTR(option),
                                      run_callback);
...
API_FUNC(config_option_set)
{
    ...
    rc = dogechat_config_option_set (API_STR2PTR(option),
                                    new_value,
                                    run_callback);
...
API_FUNC(config_option_set_null)
{
    ...
    rc = dogechat_config_option_set_null (API_STR2PTR(option),
                                         run_callback);
...
API_FUNC(config_option_unset)
{
    ...
    rc = dogechat_config_option_unset (API_STR2PTR(option));
...
API_FUNC(config_option_rename)
{
    ...
    dogechat_config_option_rename (API_STR2PTR(option),
                                  new_name);
...
API_FUNC(config_option_is_null)
{
    ...
    result = dogechat_config_option_is_null (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* option */
...
API_FUNC(config_option_default_is_null)
{
    ...
    result = dogechat_config_option_default_is_null (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* option */
...
API_FUNC(config_boolean)
{
    ...
    result = dogechat_config_boolean (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* option */
...
API_FUNC(config_boolean_default)
{
    ...
    result = dogechat_config_boolean_default (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* option */
...
API_FUNC(config_integer)
{
    ...
    result = dogechat_config_integer (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* option */
...
API_FUNC(config_integer_default)
{
    ...
    result = dogechat_config_integer_default (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* option */
...
API_FUNC(config_string)
{
    ...
    result = dogechat_config_string (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* option */
...
API_FUNC(config_string_default)
{
    ...
    result = dogechat_config_string_default (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* option */
...
API_FUNC(config_color)
{
    ...
    result = dogechat_config_color (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* option */
...
API_FUNC(config_color_default)
{
    ...
    result = dogechat_config_color_default (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* option */
...
API_FUNC(config_write_option)
{
    ...
    dogechat_config_write_option (API_STR2PTR(config_file),
                                 API_STR2PTR(option));
...
API_FUNC(config_write_line)
{
    ...
    dogechat_config_write_line (API_STR2PTR(config_file), option_name,
                               "%s", value);
...
API_FUNC(config_write)
{
    ...
    rc = dogechat_config_write (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* config_file */
...
API_FUNC(config_read)
{
    ...
    rc = dogechat_config_read (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* config_file */
...
API_FUNC(config_reload)
{
    ...
    rc = dogechat_config_reload (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* config_file */
...
API_FUNC(config_option_free)
{
    ...
    plugin_script_api_config_option_free (dogechat_tcl_plugin,
                                          tcl_current_script,
...
API_FUNC(config_section_free_options)
{
    ...
    plugin_script_api_config_section_free_options (dogechat_tcl_plugin,
                                                   tcl_current_script,
...
API_FUNC(config_section_free)
{
    ...
    plugin_script_api_config_section_free (dogechat_tcl_plugin,
                                           tcl_current_script,
                                           API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* section */
...
API_FUNC(config_free)
{
    ...
    plugin_script_api_config_free (dogechat_tcl_plugin,
                                   tcl_current_script,
...
API_FUNC(config_get)
{
    ...
    result = API_PTR2STR(dogechat_config_get (Tcl_GetStringFromObj (objv[1], &i)));
...
API_FUNC(config_get_plugin)
{
    ...
    result = plugin_script_api_config_get_plugin (dogechat_tcl_plugin,
                                                  tcl_current_script,
...
API_FUNC(config_is_set_plugin)
{
    ...
    rc = plugin_script_api_config_is_set_plugin (dogechat_tcl_plugin,
                                                 tcl_current_script,
                                                 option);
...
API_FUNC(config_set_plugin)
{
    ...
    rc = plugin_script_api_config_set_plugin (dogechat_tcl_plugin,
                                              tcl_current_script,
                                              option,
                                              value);
...
API_FUNC(config_set_desc_plugin)
{
    ...
    plugin_script_api_config_set_desc_plugin (dogechat_tcl_plugin,
                                              tcl_current_script,
                                              option,
                                              description);
...
API_FUNC(config_unset_plugin)
{
    ...
    rc = plugin_script_api_config_unset_plugin (dogechat_tcl_plugin,
                                                tcl_current_script,
                                                option);
...
API_FUNC(key_bind)
{
    ...
    hashtable = dogechat_tcl_dict_to_hashtable (interp, objv[2],
                                               DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                               DOGECHAT_HASHTABLE_STRING,
                                               DOGECHAT_HASHTABLE_STRING);

    num_keys = dogechat_key_bind (context, hashtable);
...
API_FUNC(key_unbind)
{
    ...
    num_keys = dogechat_key_unbind (context, key);
...
API_FUNC(prefix)
{
    ...
    result = dogechat_prefix (Tcl_GetStringFromObj (objv[1], &i)); /* prefix */
...
API_FUNC(color)
{
    ...
    result = dogechat_color (Tcl_GetStringFromObj (objv[1], &i)); /* color */
...
API_FUNC(print)
{
    ...
    plugin_script_api_printf (dogechat_tcl_plugin,
                              tcl_current_script,
                              API_STR2PTR(buffer),
                              "%s", message);
...
API_FUNC(print_date_tags)
{
    ...
    plugin_script_api_printf_date_tags (dogechat_tcl_plugin,
                                        tcl_current_script,
                                        API_STR2PTR(buffer),
                                        tdate,
                                        tags,
...
API_FUNC(print_y)
{
    ...
    plugin_script_api_printf_y (dogechat_tcl_plugin,
                                tcl_current_script,
                                API_STR2PTR(buffer),
                                y,
                                "%s", message);
...
API_FUNC(log_print)
{
    ...
    plugin_script_api_log_printf (dogechat_tcl_plugin,
                                  tcl_current_script,
...
int
dogechat_tcl_api_hook_command_cb (void *data, struct t_gui_buffer *buffer,
...
API_FUNC(hook_command)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_command (dogechat_tcl_plugin,
                                                         tcl_current_script,
                                                         command,
                                                         description,
                                                         args,
                                                         args_description,
                                                         completion,
                                                         &dogechat_tcl_api_hook_command_cb,
                                                         function,
                                                         data));
...
int
dogechat_tcl_api_hook_command_run_cb (void *data, struct t_gui_buffer *buffer,
                                     const char *command)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
...
API_FUNC(hook_command_run)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_command_run (dogechat_tcl_plugin,
                                                             tcl_current_script,
                                                             command,
                                                             &dogechat_tcl_api_hook_command_run_cb,
                                                             function,
                                                             data));
...
int
dogechat_tcl_api_hook_timer_cb (void *data, int remaining_calls)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
API_FUNC(hook_timer)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_timer (dogechat_tcl_plugin,
                                                       tcl_current_script,
                                                       interval, /* interval */
                                                       align_second, /* align_second */
                                                       max_calls, /* max_calls */
                                                       &dogechat_tcl_api_hook_timer_cb,
...
int
dogechat_tcl_api_hook_fd_cb (void *data, int fd)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
API_FUNC(hook_fd)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_fd (dogechat_tcl_plugin,
                                                    tcl_current_script,
                                                    fd, /* fd */
                                                    read, /* read */
                                                    write, /* write */
                                                    exception, /* exception */
                                                    &dogechat_tcl_api_hook_fd_cb,
...
int
dogechat_tcl_api_hook_process_cb (void *data,
...
API_FUNC(hook_process)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_process (dogechat_tcl_plugin,
                                                         tcl_current_script,
                                                         command,
                                                         timeout,
                                                         &dogechat_tcl_api_hook_process_cb,
                                                         function,
                                                         data));
...
API_FUNC(hook_process_hashtable)
{
    ...
    command = Tcl_GetStringFromObj (objv[1], &i);
    options = dogechat_tcl_dict_to_hashtable (interp, objv[2],
                                             DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                             DOGECHAT_HASHTABLE_STRING,
                                             DOGECHAT_HASHTABLE_STRING);
    ...
    result = API_PTR2STR(plugin_script_api_hook_process_hashtable (dogechat_tcl_plugin,
                                                                   tcl_current_script,
                                                                   command,
                                                                   options,
                                                                   timeout,
...
int
dogechat_tcl_api_hook_connect_cb (void *data, int status, int gnutls_rc,
...
API_FUNC(hook_connect)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_connect (dogechat_tcl_plugin,
                                                         tcl_current_script,
                                                         proxy,
                                                         address,
                                                         port,
                                                         ...
                                                         local_hostname,
                                                         &dogechat_tcl_api_hook_connect_cb,
                                                         function,
                                                         data));
...
int
dogechat_tcl_api_hook_print_cb (void *data, struct t_gui_buffer *buffer,
                               time_t date,
...
API_FUNC(hook_print)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_print (dogechat_tcl_plugin,
                                                       tcl_current_script,
                                                       API_STR2PTR(buffer),
                                                       tags,
                                                       message,
                                                       strip_colors, /* strip_colors */
                                                       &dogechat_tcl_api_hook_print_cb,
                                                       function,
                                                       data));
...
int
dogechat_tcl_api_hook_signal_cb (void *data, const char *signal, const char *type_data,
                                 void *signal_data)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
...
API_FUNC(hook_signal)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_signal (dogechat_tcl_plugin,
                                                        tcl_current_script,
                                                        signal,
                                                        &dogechat_tcl_api_hook_signal_cb,
                                                        function,
                                                        data));
...
API_FUNC(hook_signal_send)
{
    ...
    if (strcmp (type_data, DOGECHAT_HOOK_SIGNAL_STRING) == 0)
    {
        rc = dogechat_hook_signal_send (signal,
                                       type_data,
...
int
dogechat_tcl_api_hook_hsignal_cb (void *data, const char *signal,
                                 struct t_hashtable *hashtable)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
...
API_FUNC(hook_hsignal)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_hsignal (dogechat_tcl_plugin,
                                                         tcl_current_script,
                                                         signal,
                                                         &dogechat_tcl_api_hook_hsignal_cb,
                                                         function,
                                                         data));
...
API_FUNC(hook_hsignal_send)
{
    ...
    rc = dogechat_hook_hsignal_send (signal, hashtable);
...
int
dogechat_tcl_api_hook_config_cb (void *data, const char *option, const char *value)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
    char empty_arg[1] = { '\0' };
...
API_FUNC(hook_config)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_config (dogechat_tcl_plugin,
                                                        tcl_current_script,
                                                        option,
                                                        &dogechat_tcl_api_hook_config_cb,
                                                        function,
                                                        data));
...
int
dogechat_tcl_api_hook_completion_cb (void *data, const char *completion_item,
...
API_FUNC(hook_completion)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_completion (dogechat_tcl_plugin,
                                                            tcl_current_script,
                                                            completion,
                                                            description,
                                                            &dogechat_tcl_api_hook_completion_cb,
                                                            function,
                                                            data));
...
API_FUNC(hook_completion_get_string)
{
    ...
    result = dogechat_hook_completion_get_string (API_STR2PTR(completion),
                                                 property);
...
API_FUNC(hook_completion_list_add)
{
    ...
    dogechat_hook_completion_list_add (API_STR2PTR(completion),
                                      word,
...
char *
dogechat_tcl_api_hook_modifier_cb (void *data, const char *modifier,
                                   const char *modifier_data, const char *string)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (char *)dogechat_tcl_exec (script_callback->script,
...
API_FUNC(hook_modifier)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_modifier (dogechat_tcl_plugin,
                                                          tcl_current_script,
                                                          modifier,
                                                          &dogechat_tcl_api_hook_modifier_cb,
                                                          function,
                                                          data));
...
API_FUNC(hook_modifier_exec)
{
    ...
    result = dogechat_hook_modifier_exec (modifier, modifier_data, string);

    API_RETURN_STRING_FREE(result);
}

const char *
dogechat_tcl_api_hook_info_cb (void *data, const char *info_name,
                               const char *arguments)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (const char *)dogechat_tcl_exec (script_callback->script,
                                               DOGECHAT_SCRIPT_EXEC_STRING,
                                               script_callback->function,
                                               "sss", func_argv);
    }
...
API_FUNC(hook_info)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_info (dogechat_tcl_plugin,
                                                      tcl_current_script,
                                                      info_name,
                                                      description,
                                                      args_description,
                                                      &dogechat_tcl_api_hook_info_cb,
                                                      function,
                                                      data));
...
struct t_hashtable *
dogechat_tcl_api_hook_info_hashtable_cb (void *data, const char *info_name,
                                        struct t_hashtable *hashtable)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (struct t_hashtable *)dogechat_tcl_exec (script_callback->script,
...
API_FUNC(hook_info_hashtable)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_info_hashtable (dogechat_tcl_plugin,
                                                                tcl_current_script,
                                                                info_name,
                                                                description,
                                                                args_description,
                                                                output_description,
                                                                &dogechat_tcl_api_hook_info_hashtable_cb,
                                                                function,
                                                                data));
...
struct t_infolist *
dogechat_tcl_api_hook_infolist_cb (void *data, const char *infolist_name,
                                   void *pointer, const char *arguments)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        result = (struct t_infolist *)dogechat_tcl_exec (script_callback->script,
                                                        DOGECHAT_SCRIPT_EXEC_STRING,
                                                        script_callback->function,
                                                        "ssss", func_argv);
...
API_FUNC(hook_infolist)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_infolist (dogechat_tcl_plugin,
                                                          tcl_current_script,
                                                          infolist_name,
                                                          description,
                                                          pointer_description,
                                                          args_description,
                                                          &dogechat_tcl_api_hook_infolist_cb,
                                                          function,
                                                          data));
...
struct t_hashtable *
dogechat_tcl_api_hook_focus_cb (void *data,
...
API_FUNC(hook_focus)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_focus (dogechat_tcl_plugin,
                                                       tcl_current_script,
                                                       area,
                                                       &dogechat_tcl_api_hook_focus_cb,
                                                       function,
                                                       data));
...
API_FUNC(hook_set)
{
    ...
    dogechat_hook_set (API_STR2PTR(hook), property, value);
...
API_FUNC(unhook)
{
    ...
    plugin_script_api_unhook (dogechat_tcl_plugin,
                              tcl_current_script,
...
API_FUNC(unhook_all)
{
    ...
    plugin_script_api_unhook_all (dogechat_tcl_plugin, tcl_current_script);

    API_RETURN_OK;
}

int
dogechat_tcl_api_buffer_input_data_cb (void *data, struct t_gui_buffer *buffer,
...
int
dogechat_tcl_api_buffer_close_cb (void *data, struct t_gui_buffer *buffer)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
    char empty_arg[1] = { '\0' };
...
API_FUNC(buffer_new)
{
    ...
    result = API_PTR2STR(plugin_script_api_buffer_new (dogechat_tcl_plugin,
                                                       tcl_current_script,
                                                       name,
...
API_FUNC(buffer_search)
{
    ...
    result = API_PTR2STR(dogechat_buffer_search (plugin, name));
...
API_FUNC(buffer_clear)
{
    ...
    dogechat_buffer_clear (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* buffer */
...
API_FUNC(buffer_close)
{
    ...
    plugin_script_api_buffer_close (dogechat_tcl_plugin,
                                    tcl_current_script,
...
API_FUNC(buffer_merge)
{
    ...
    dogechat_buffer_merge (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)), /* buffer */
...
API_FUNC(buffer_unmerge)
{
    ...
    dogechat_buffer_unmerge (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)),
                            number);
...
API_FUNC(buffer_get_integer)
{
    ...
    result = dogechat_buffer_get_integer (API_STR2PTR(buffer), property);
...
API_FUNC(buffer_get_string)
{
    ...
    result = dogechat_buffer_get_string (API_STR2PTR(buffer), property);
...
API_FUNC(buffer_get_pointer)
{
    ...
    result = API_PTR2STR(dogechat_buffer_get_pointer (API_STR2PTR(buffer),
                                                     property));
...
API_FUNC(buffer_set)
{
    ...
    dogechat_buffer_set (API_STR2PTR(buffer), property, value);
...
API_FUNC(buffer_string_replace_local_var)
{
    ...
    result = dogechat_buffer_string_replace_local_var (API_STR2PTR(buffer), string);
...
API_FUNC(buffer_match_list)
{
    ...
    result = dogechat_buffer_match_list (API_STR2PTR(buffer), string);
...
API_FUNC(window_search_with_buffer)
{
    ...
    result = API_PTR2STR(dogechat_window_search_with_buffer (API_STR2PTR(buffer)));
...
API_FUNC(window_get_integer)
{
    ...
    result = dogechat_window_get_integer (API_STR2PTR(window), property);
...
API_FUNC(window_get_string)
{
    ...
    result = dogechat_window_get_string (API_STR2PTR(window), property);
...
API_FUNC(window_get_pointer)
{
    ...
    result = API_PTR2STR(dogechat_window_get_pointer (API_STR2PTR(window),
                                                     property));
...
API_FUNC(window_set_title)
{
    ...
    dogechat_window_set_title (title);
...
API_FUNC(nicklist_add_group)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_add_group (API_STR2PTR(buffer),
...
API_FUNC(nicklist_search_group)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_search_group (API_STR2PTR(buffer),
...
API_FUNC(nicklist_add_nick)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_add_nick (API_STR2PTR(buffer),
                                                    API_STR2PTR(group),
                                                    name,
                                                    color,
                                                    prefix,
...
API_FUNC(nicklist_search_nick)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_search_nick (API_STR2PTR(buffer),
                                                       API_STR2PTR(from_group),
                                                       name));
...
API_FUNC(nicklist_remove_group)
{
    ...
    dogechat_nicklist_remove_group (API_STR2PTR(buffer),
                                   API_STR2PTR(group));
...
API_FUNC(nicklist_remove_nick)
{
    ...
    dogechat_nicklist_remove_nick (API_STR2PTR(buffer),
                                  API_STR2PTR(nick));
...
API_FUNC(nicklist_remove_all)
{
    ...
    dogechat_nicklist_remove_all (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* buffer */
...
API_FUNC(nicklist_group_get_integer)
{
    ...
    result = dogechat_nicklist_group_get_integer (API_STR2PTR(buffer),
                                                 API_STR2PTR(group),
                                                 property);
...
API_FUNC(nicklist_group_get_string)
{
    ...
    result = dogechat_nicklist_group_get_string (API_STR2PTR(buffer),
                                                API_STR2PTR(group),
                                                property);
...
API_FUNC(nicklist_group_get_pointer)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_group_get_pointer (API_STR2PTR(buffer),
                                                             API_STR2PTR(group),
                                                             property));
...
API_FUNC(nicklist_group_set)
{
    ...
    dogechat_nicklist_group_set (API_STR2PTR(buffer),
                                API_STR2PTR(group),
                                property,
                                value);
...
API_FUNC(nicklist_nick_get_integer)
{
    ...
    result = dogechat_nicklist_nick_get_integer (API_STR2PTR(buffer),
                                                API_STR2PTR(nick),
                                                property);
...
API_FUNC(nicklist_nick_get_string)
{
    ...
    result = dogechat_nicklist_nick_get_string (API_STR2PTR(buffer),
                                               API_STR2PTR(nick),
                                               property);
...
API_FUNC(nicklist_nick_get_pointer)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_nick_get_pointer (API_STR2PTR(buffer),
                                                            API_STR2PTR(nick),
                                                            property));
...
API_FUNC(nicklist_nick_set)
{
    ...
    dogechat_nicklist_nick_set (API_STR2PTR(buffer),
                               API_STR2PTR(nick),
                               property,
                               value);
...
API_FUNC(bar_item_search)
{
    ...
    result = API_PTR2STR(dogechat_bar_item_search (Tcl_GetStringFromObj (objv[1], &i))); /* name */

    API_RETURN_STRING_FREE(result);
}

char *
dogechat_tcl_api_bar_item_build_cb (void *data, struct t_gui_bar_item *item,
                                   ...
                                   struct t_hashtable *extra_info)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        if (strncmp (script_callback->function, "(extra)", 7) == 0)
        {
            ...
            ret = (char *)dogechat_tcl_exec (script_callback->script,
                                            DOGECHAT_SCRIPT_EXEC_STRING,
...
API_FUNC(bar_item_new)
{
    ...
    result = API_PTR2STR(plugin_script_api_bar_item_new (dogechat_tcl_plugin,
                                                         tcl_current_script,
                                                         name,
                                                         &dogechat_tcl_api_bar_item_build_cb,
                                                         function,
                                                         data));
...
API_FUNC(bar_item_update)
{
    ...
    dogechat_bar_item_update (Tcl_GetStringFromObj (objv[1], &i)); /* name */
...
API_FUNC(bar_item_remove)
{
    ...
    plugin_script_api_bar_item_remove (dogechat_tcl_plugin,
                                       tcl_current_script,
...
API_FUNC(bar_search)
{
    ...
    result = API_PTR2STR(dogechat_bar_search (Tcl_GetStringFromObj (objv[1], &i))); /* name */
...
API_FUNC(bar_new)
{
    ...
    result = API_PTR2STR(dogechat_bar_new (name,
                                          hidden,
                                          priority,
                                          type,
                                          conditions,
...
API_FUNC(bar_set)
{
    ...
    rc = dogechat_bar_set (API_STR2PTR(bar), property, value);
...
API_FUNC(bar_update)
{
    ...
    dogechat_bar_update (Tcl_GetStringFromObj (objv[1], &i)); /* name */
...
API_FUNC(bar_remove)
{
    ...
    dogechat_bar_remove (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* bar */
...
API_FUNC(command)
{
    ...
    rc = plugin_script_api_command (dogechat_tcl_plugin,
                                    tcl_current_script,
                                    API_STR2PTR(buffer),
                                    command);
...
API_FUNC(info_get)
{
    ...
    result = dogechat_info_get (Tcl_GetStringFromObj (objv[1], &i),
...
API_FUNC(info_get_hashtable)
{
    ...
    result_hashtable = dogechat_info_get_hashtable (Tcl_GetStringFromObj (objv[1], &i),
                                                   hashtable);
    result_dict = dogechat_tcl_hashtable_to_dict (interp, result_hashtable);
...
API_FUNC(infolist_new_item)
{
    ...
    result = API_PTR2STR(dogechat_infolist_new_item (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)))); /* infolist */
...
API_FUNC(infolist_new_var_integer)
{
    ...
    result = API_PTR2STR(dogechat_infolist_new_var_integer (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)), /* item */
...
API_FUNC(infolist_new_var_string)
{
    ...
    result = API_PTR2STR(dogechat_infolist_new_var_string (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)), /* item */
                                                          Tcl_GetStringFromObj (objv[2], &i), /* name */
...
API_FUNC(infolist_new_var_pointer)
{
    ...
    result = API_PTR2STR(dogechat_infolist_new_var_pointer (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)), /* item */
                                                           Tcl_GetStringFromObj (objv[2], &i), /* name */
...
API_FUNC(infolist_new_var_time)
{
    ...
    result = API_PTR2STR(dogechat_infolist_new_var_time (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)), /* item */
                                                        Tcl_GetStringFromObj (objv[2], &i), /* name */
                                                        value));
...
API_FUNC(infolist_search_var)
{
    ...
    result = API_PTR2STR(dogechat_infolist_search_var (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i)), /* infolist */
                                                      Tcl_GetStringFromObj (objv[2], &i))); /* name */
...
API_FUNC(infolist_get)
{
    ...
    result = API_PTR2STR(dogechat_infolist_get (name,
                                               API_STR2PTR(pointer),
                                               arguments));
...
API_FUNC(infolist_next)
{
    ...
    result = dogechat_infolist_next (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* infolist */
...
API_FUNC(infolist_prev)
{
    ...
    result = dogechat_infolist_prev (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* infolist */
...
API_FUNC(infolist_reset_item_cursor)
{
    ...
    dogechat_infolist_reset_item_cursor (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* infolist */
...
API_FUNC(infolist_fields)
{
    ...
    result = dogechat_infolist_fields (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* infolist */
...
API_FUNC(infolist_integer)
{
    ...
    result = dogechat_infolist_integer (API_STR2PTR(infolist), variable);
...
API_FUNC(infolist_string)
{
    ...
    result = dogechat_infolist_string (API_STR2PTR(infolist), variable);
...
API_FUNC(infolist_pointer)
{
    ...
    result = API_PTR2STR(dogechat_infolist_pointer (API_STR2PTR(infolist), variable));
...
API_FUNC(infolist_time)
{
    ...
    timebuffer[0] = '\0';
    time = dogechat_infolist_time (API_STR2PTR(infolist), variable);
...
API_FUNC(infolist_free)
{
    ...
    dogechat_infolist_free (API_STR2PTR(Tcl_GetStringFromObj (objv[1], &i))); /* infolist */
...
API_FUNC(hdata_get)
{
    ...
    result = API_PTR2STR(dogechat_hdata_get (name));
...
API_FUNC(hdata_get_var_offset)
{
    ...
    result = dogechat_hdata_get_var_offset (API_STR2PTR(hdata), name);
...
API_FUNC(hdata_get_var_type_string)
{
    ...
    result = dogechat_hdata_get_var_type_string (API_STR2PTR(hdata), name);
...
API_FUNC(hdata_get_var_array_size)
{
    ...
    result = dogechat_hdata_get_var_array_size (API_STR2PTR(hdata),
                                               API_STR2PTR(pointer),
                                               name);
...
API_FUNC(hdata_get_var_array_size_string)
{
    ...
    result = dogechat_hdata_get_var_array_size_string (API_STR2PTR(hdata),
                                                      API_STR2PTR(pointer),
                                                      name);
...
API_FUNC(hdata_get_var_hdata)
{
    ...
    result = dogechat_hdata_get_var_hdata (API_STR2PTR(hdata), name);
...
API_FUNC(hdata_get_list)
{
    ...
    result = API_PTR2STR(dogechat_hdata_get_list (API_STR2PTR(hdata),
                                                 name));
...
API_FUNC(hdata_check_pointer)
{
    ...
    result = dogechat_hdata_check_pointer (API_STR2PTR(hdata),
                                          API_STR2PTR(list),
                                          API_STR2PTR(pointer));
...
API_FUNC(hdata_move)
{
    ...
    result = API_PTR2STR(dogechat_hdata_move (API_STR2PTR(hdata),
                                             API_STR2PTR(pointer),
                                             count));
...
API_FUNC(hdata_search)
{
    ...
    result = API_PTR2STR(dogechat_hdata_search (API_STR2PTR(hdata),
                                               API_STR2PTR(pointer),
                                               search,
                                               move));
...
API_FUNC(hdata_char)
{
    ...
    result = (int)dogechat_hdata_char (API_STR2PTR(hdata),
                                      API_STR2PTR(pointer),
                                      name);
...
API_FUNC(hdata_integer)
{
    ...
    result = dogechat_hdata_integer (API_STR2PTR(hdata),
                                    API_STR2PTR(pointer),
                                    name);
...
API_FUNC(hdata_long)
{
    ...
    result = dogechat_hdata_long (API_STR2PTR(hdata),
                                 API_STR2PTR(pointer),
                                 name);
...
API_FUNC(hdata_string)
{
    ...
    result = dogechat_hdata_string (API_STR2PTR(hdata),
                                   API_STR2PTR(pointer),
                                   name);
...
API_FUNC(hdata_pointer)
{
    ...
    result = API_PTR2STR(dogechat_hdata_pointer (API_STR2PTR(hdata),
                                                API_STR2PTR(pointer),
                                                name));
...
API_FUNC(hdata_time)
{
    ...
    time = dogechat_hdata_time (API_STR2PTR(hdata),
                               API_STR2PTR(pointer),
                               name);
...
API_FUNC(hdata_hashtable)
{
    ...
    result_dict = dogechat_tcl_hashtable_to_dict (
        interp,
        dogechat_hdata_hashtable (API_STR2PTR(hdata),
                                 API_STR2PTR(pointer),
                                 name));
...
API_FUNC(hdata_update)
{
    ...
    hdata = Tcl_GetStringFromObj (objv[1], &i);
    pointer = Tcl_GetStringFromObj (objv[2], &i);
    hashtable = dogechat_tcl_dict_to_hashtable (interp, objv[3],
                                               DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                               DOGECHAT_HASHTABLE_STRING,
                                               DOGECHAT_HASHTABLE_STRING);

    value = dogechat_hdata_update (API_STR2PTR(hdata),
                                  API_STR2PTR(pointer),
                                  hashtable);
...
API_FUNC(hdata_get_string)
{
    ...
    result = dogechat_hdata_get_string (API_STR2PTR(hdata), property);
...
API_FUNC(upgrade_new)
{
    ...
    result = API_PTR2STR(dogechat_upgrade_new (filename, write));
...
API_FUNC(upgrade_write_object)
{
    ...
    rc = dogechat_upgrade_write_object (API_STR2PTR(upgrade_file),
                                       object_id,
...
int
dogechat_tcl_api_upgrade_read_cb (void *data,
...
API_FUNC(upgrade_read)
{
    ...
    rc = plugin_script_api_upgrade_read (dogechat_tcl_plugin,
                                         tcl_current_script,
                                         API_STR2PTR(upgrade_file),
                                         &dogechat_tcl_api_upgrade_read_cb,
                                         function,
                                         data);
...
API_FUNC(upgrade_close)
{
    ...
    dogechat_upgrade_close (API_STR2PTR(upgrade_file));
...
void dogechat_tcl_api_init (Tcl_Interp *interp)
{
    int i;
    Tcl_Obj *objp;
    ...
    Tcl_Eval (interp,"namespace eval dogechat {}");
    ...
    objp = Tcl_NewIntObj (DOGECHAT_RC_OK);
    Tcl_IncrRefCount (objp);

    Tcl_SetVar (interp, "dogechat::DOGECHAT_RC_OK", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_RC_OK_EAT);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_RC_OK_EAT", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_RC_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_RC_ERROR", Tcl_GetStringFromObj (objp, &i), 0);

    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_READ_OK);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_READ_OK", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_READ_MEMORY_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_READ_MEMORY_ERROR", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_READ_FILE_NOT_FOUND);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_READ_FILE_NOT_FOUND", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_WRITE_OK);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_WRITE_OK", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_WRITE_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_WRITE_ERROR", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_WRITE_MEMORY_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_WRITE_MEMORY_ERROR", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_OPTION_SET_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_OPTION_SET_ERROR", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_CONFIG_OPTION_UNSET_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_CONFIG_OPTION_UNSET_ERROR", Tcl_GetStringFromObj (objp, &i), 0);

    Tcl_SetStringObj (objp, DOGECHAT_LIST_POS_SORT, -1);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_LIST_POS_SORT", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetStringObj (objp, DOGECHAT_LIST_POS_BEGINNING, -1);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_LIST_POS_BEGINNING", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetStringObj (objp, DOGECHAT_LIST_POS_END, -1);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_LIST_POS_END", Tcl_GetStringFromObj (objp, &i), 0);

    Tcl_SetStringObj (objp, DOGECHAT_HOTLIST_LOW, -1);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOTLIST_LOW", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetStringObj (objp, DOGECHAT_HOTLIST_MESSAGE, -1);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOTLIST_MESSAGE", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetStringObj (objp, DOGECHAT_HOTLIST_PRIVATE, -1);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOTLIST_PRIVATE", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetStringObj (objp, DOGECHAT_HOTLIST_HIGHLIGHT, -1);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOTLIST_HIGHLIGHT", Tcl_GetStringFromObj (objp, &i), 0);

    Tcl_SetIntObj (objp, DOGECHAT_HOOK_PROCESS_RUNNING);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_PROCESS_RUNNING", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_PROCESS_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_PROCESS_ERROR", Tcl_GetStringFromObj (objp, &i), 0);

    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_OK);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_OK", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_CONNECTION_REFUSED);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_CONNECTION_REFUSED", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_PROXY_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_PROXY_ERROR", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_GNUTLS_INIT_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_GNUTLS_INIT_ERROR", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_MEMORY_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_MEMORY_ERROR", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_TIMEOUT);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_TIMEOUT", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetIntObj (objp, DOGECHAT_HOOK_CONNECT_SOCKET_ERROR);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_CONNECT_SOCKET_ERROR", Tcl_GetStringFromObj (objp, &i), 0);

    Tcl_SetStringObj (objp, DOGECHAT_HOOK_SIGNAL_STRING, -1);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_SIGNAL_STRING", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetStringObj (objp, DOGECHAT_HOOK_SIGNAL_INT, -1);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_SIGNAL_INT", Tcl_GetStringFromObj (objp, &i), 0);
    Tcl_SetStringObj (objp, DOGECHAT_HOOK_SIGNAL_POINTER, -1);
    Tcl_SetVar (interp, "dogechat::DOGECHAT_HOOK_SIGNAL_POINTER", Tcl_GetStringFromObj (objp, &i), 0);
