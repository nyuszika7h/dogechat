/*
 * dogechat-ruby-api.c - ruby API functions
 *
...
#define API_DEF_FUNC(__name, __argc)                                    \
    rb_define_module_function (ruby_mDogechat, #__name,                  \
                               &dogechat_ruby_api_##__name, __argc);
#define API_INIT_FUNC(__init, __name, __ret)                            \
    ...
    {                                                                   \
        DOGECHAT_SCRIPT_MSG_NOT_INIT(RUBY_CURRENT_SCRIPT_NAME,           \
                                    ruby_function_name);                \
        __ret;                                                          \
    }
#define API_WRONG_ARGS(__ret)                                           \
    {                                                                   \
        DOGECHAT_SCRIPT_MSG_WRONG_ARGS(RUBY_CURRENT_SCRIPT_NAME,         \
                                      ruby_function_name);              \
        __ret;                                                          \
    }
...
#define API_STR2PTR(__string)                                           \
    plugin_script_str2ptr (dogechat_ruby_plugin,                         \
...
static VALUE
dogechat_ruby_api_register (VALUE class, VALUE name, VALUE author,
                           ...
                           VALUE shutdown_func, VALUE charset)
{
    ...
    if (plugin_script_search (dogechat_ruby_plugin, ruby_scripts, c_name))
    {
        ...
        dogechat_printf (NULL,
                        ...
                        dogechat_prefix ("error"), RUBY_PLUGIN_NAME, c_name);
        API_RETURN_ERROR;
    }

    /* register script */
    ruby_current_script = plugin_script_add (dogechat_ruby_plugin,
...
static VALUE
dogechat_ruby_api_plugin_get_name (VALUE class, VALUE plugin)
{
    char *c_plugin;
    const char *result;
    ...
    result = dogechat_plugin_get_name (API_STR2PTR(c_plugin));

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_charset_set (VALUE class, VALUE charset)
{
    char *c_charset;
...
static VALUE
dogechat_ruby_api_iconv_to_internal (VALUE class, VALUE charset, VALUE string)
{
    ...
    result = dogechat_iconv_to_internal (c_charset, c_string);

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_iconv_from_internal (VALUE class, VALUE charset, VALUE string)
{
    char *c_charset, *c_string, *result;
    VALUE return_value;
    ...
    result = dogechat_iconv_from_internal (c_charset, c_string);

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_gettext (VALUE class, VALUE string)
{
    char *c_string;
    const char *result;
    ...
    result = dogechat_gettext (c_string);

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_ngettext (VALUE class, VALUE single, VALUE plural,
                           VALUE count)
{
    ...
    result = dogechat_ngettext (c_single, c_plural, c_count);

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_strlen_screen (VALUE class, VALUE string)
{
    char *c_string;
    int value;
    ...
    value = dogechat_strlen_screen (c_string);

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_string_match (VALUE class, VALUE string, VALUE mask,
                               VALUE case_sensitive)
{
    ...
    value = dogechat_string_match (c_string, c_mask, c_case_sensitive);

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_string_has_highlight (VALUE class, VALUE string,
                                       VALUE highlight_words)
{
    ...
    value = dogechat_string_has_highlight (c_string, c_highlight_words);

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_string_has_highlight_regex (VALUE class, VALUE string,
                                             VALUE regex)
{
    ...
    value = dogechat_string_has_highlight_regex (c_string, c_regex);

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_string_mask_to_regex (VALUE class, VALUE mask)
{
    char *c_mask, *result;
    VALUE return_value;
    ...
    result = dogechat_string_mask_to_regex (c_mask);

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_string_remove_color (VALUE class, VALUE string,
                                      VALUE replacement)
{
    ...
    result = dogechat_string_remove_color (c_string, c_replacement);

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_string_is_command_char (VALUE class, VALUE string)
{
    char *c_string;
    int value;
    ...
    value = dogechat_string_is_command_char (c_string);

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_string_input_for_buffer (VALUE class, VALUE string)
{
    char *c_string;
    const char *result;
    ...
    result = dogechat_string_input_for_buffer (c_string);

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_string_eval_expression (VALUE class, VALUE expr,
                                         ...
                                         VALUE options)
{
    ...
    c_pointers = dogechat_ruby_hash_to_hashtable (pointers,
                                                 DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                 DOGECHAT_HASHTABLE_STRING,
                                                 DOGECHAT_HASHTABLE_POINTER);
    c_extra_vars = dogechat_ruby_hash_to_hashtable (extra_vars,
                                                   DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                   DOGECHAT_HASHTABLE_STRING,
                                                   DOGECHAT_HASHTABLE_STRING);
    c_options = dogechat_ruby_hash_to_hashtable (options,
                                                DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                DOGECHAT_HASHTABLE_STRING,
                                                DOGECHAT_HASHTABLE_STRING);

    result = dogechat_string_eval_expression (c_expr, c_pointers, c_extra_vars,
                                             c_options);
...
static VALUE
dogechat_ruby_api_string_eval_path_home (VALUE class, VALUE path,
                                        VALUE pointers, VALUE extra_vars,
                                        VALUE options)
{
    char *c_path, *result;
    ...
    c_pointers = dogechat_ruby_hash_to_hashtable (
        pointers,
        DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
        DOGECHAT_HASHTABLE_STRING,
        DOGECHAT_HASHTABLE_POINTER);
    c_extra_vars = dogechat_ruby_hash_to_hashtable (
        extra_vars,
        DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
        DOGECHAT_HASHTABLE_STRING,
        DOGECHAT_HASHTABLE_STRING);
    c_options = dogechat_ruby_hash_to_hashtable (
        options,
        DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
        DOGECHAT_HASHTABLE_STRING,
        DOGECHAT_HASHTABLE_STRING);

    result = dogechat_string_eval_path_home (c_path, c_pointers, c_extra_vars,
                                            c_options);
...
static VALUE
dogechat_ruby_api_mkdir_home (VALUE class, VALUE directory, VALUE mode)
{
    char *c_directory;
    int c_mode;
    ...
    if (dogechat_mkdir_home (c_directory, c_mode))
        API_RETURN_OK;
...
static VALUE
dogechat_ruby_api_mkdir (VALUE class, VALUE directory, VALUE mode)
{
    char *c_directory;
    int c_mode;
    ...
    if (dogechat_mkdir (c_directory, c_mode))
        API_RETURN_OK;
...
static VALUE
dogechat_ruby_api_mkdir_parents (VALUE class, VALUE directory, VALUE mode)
{
    char *c_directory;
    int c_mode;
    ...
    if (dogechat_mkdir_parents (c_directory, c_mode))
        API_RETURN_OK;
...
static VALUE
dogechat_ruby_api_list_new (VALUE class)
{
    char *result;
...
static VALUE
dogechat_ruby_api_list_add (VALUE class, VALUE dogelist, VALUE data, VALUE where,
                           VALUE user_data)
{
    char *c_dogelist, *c_data, *c_where, *c_user_data, *result;

    API_INIT_FUNC(1, "list_add", API_RETURN_EMPTY);
    if (NIL_P (dogelist) || NIL_P (data) || NIL_P (where) || NIL_P (user_data))
        API_WRONG_ARGS(API_RETURN_EMPTY);

    Check_Type (dogelist, T_STRING);
    Check_Type (data, T_STRING);
    Check_Type (where, T_STRING);
    Check_Type (user_data, T_STRING);

    c_dogelist = StringValuePtr (dogelist);
    c_data = StringValuePtr (data);
    c_where = StringValuePtr (where);
    c_user_data = StringValuePtr (user_data);

    result = API_PTR2STR(dogechat_list_add (API_STR2PTR(c_dogelist),
                                           c_data,
                                           c_where,
...
static VALUE
dogechat_ruby_api_list_search (VALUE class, VALUE dogelist, VALUE data)
{
    char *c_dogelist, *c_data, *result;

    API_INIT_FUNC(1, "list_search", API_RETURN_EMPTY);
    if (NIL_P (dogelist) || NIL_P (data))
        API_WRONG_ARGS(API_RETURN_EMPTY);
    ...
    result = API_PTR2STR(dogechat_list_search (API_STR2PTR(c_dogelist),
                                              c_data));
...
static VALUE
dogechat_ruby_api_list_search_pos (VALUE class, VALUE dogelist, VALUE data)
{
    char *c_dogelist, *c_data;
    int pos;
    ...
    pos = dogechat_list_search_pos (API_STR2PTR(c_dogelist), c_data);

    API_RETURN_INT(pos);
}

static VALUE
dogechat_ruby_api_list_casesearch (VALUE class, VALUE dogelist, VALUE data)
{
    char *c_dogelist, *c_data, *result;
    ...
    result = API_PTR2STR(dogechat_list_casesearch (API_STR2PTR(c_dogelist),
                                                  c_data));
...
static VALUE
dogechat_ruby_api_list_casesearch_pos (VALUE class, VALUE dogelist, VALUE data)
{
    char *c_dogelist, *c_data;
    int pos;
    ...
    pos = dogechat_list_casesearch_pos (API_STR2PTR(c_dogelist), c_data);

    API_RETURN_INT(pos);
}

static VALUE
dogechat_ruby_api_list_get (VALUE class, VALUE dogelist, VALUE position)
{
    char *c_dogelist, *result;
    int c_position;

    API_INIT_FUNC(1, "list_get", API_RETURN_EMPTY);
    if (NIL_P (dogelist) || NIL_P (position))
        API_WRONG_ARGS(API_RETURN_EMPTY);
    ...
    result = API_PTR2STR(dogechat_list_get (API_STR2PTR(c_dogelist),
                                           c_position));
...
static VALUE
dogechat_ruby_api_list_set (VALUE class, VALUE item, VALUE new_value)
{
    ...
    dogechat_list_set (API_STR2PTR(c_item),
                      c_new_value);
...
static VALUE
dogechat_ruby_api_list_next (VALUE class, VALUE item)
{
    char *c_item, *result;
    ...
    result = API_PTR2STR(dogechat_list_next (API_STR2PTR(c_item)));

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_list_prev (VALUE class, VALUE item)
{
    char *c_item, *result;
    ...
    result = API_PTR2STR(dogechat_list_prev (API_STR2PTR(c_item)));

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_list_string (VALUE class, VALUE item)
{
    char *c_item;
    const char *result;
    ...
    result = dogechat_list_string (API_STR2PTR(c_item));

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_list_size (VALUE class, VALUE dogelist)
{
    char *c_dogelist;
    int size;
    ...
    size = dogechat_list_size (API_STR2PTR(c_dogelist));

    API_RETURN_INT(size);
}

static VALUE
dogechat_ruby_api_list_remove (VALUE class, VALUE dogelist, VALUE item)
{
    char *c_dogelist, *c_item;

    API_INIT_FUNC(1, "list_remove", API_RETURN_ERROR);
    if (NIL_P (dogelist) || NIL_P (item))
        API_WRONG_ARGS(API_RETURN_ERROR);
    ...
    dogechat_list_remove (API_STR2PTR(c_dogelist),
                         API_STR2PTR(c_item));
...
static VALUE
dogechat_ruby_api_list_remove_all (VALUE class, VALUE dogelist)
{
    char *c_dogelist;
    ...
    dogechat_list_remove_all (API_STR2PTR(c_dogelist));

    API_RETURN_OK;
}

static VALUE
dogechat_ruby_api_list_free (VALUE class, VALUE dogelist)
{
    char *c_dogelist;
    ...
    dogechat_list_free (API_STR2PTR(c_dogelist));

    API_RETURN_OK;
}

int
dogechat_ruby_api_config_reload_cb (void *data,
                                   struct t_config_file *config_file)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        rc = (int *) dogechat_ruby_exec (script_callback->script,
...
static VALUE
dogechat_ruby_api_config_new (VALUE class, VALUE name, VALUE function,
                             VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new (dogechat_ruby_plugin,
                                                       ruby_current_script,
                                                       c_name,
                                                       &dogechat_ruby_api_config_reload_cb,
                                                       c_function,
                                                       c_data));
...
int
dogechat_ruby_api_config_read_cb (void *data,
                                 struct t_config_file *config_file,
...
int
dogechat_ruby_api_config_section_write_cb (void *data,
                                          struct t_config_file *config_file,
...
int
dogechat_ruby_api_config_section_write_default_cb (void *data,
                                                  struct t_config_file *config_file,
                                                  const char *section_name)
{
    struct t_plugin_script_cb *script_callback;
...
int
dogechat_ruby_api_config_section_create_option_cb (void *data,
                                                  struct t_config_file *config_file,
                                                  struct t_config_section *section,
...
int
dogechat_ruby_api_config_section_delete_option_cb (void *data,
                                                  struct t_config_file *config_file,
                                                  struct t_config_section *section,
...
static VALUE
dogechat_ruby_api_config_new_section (VALUE class, VALUE config_file,
                                     ...
                                     VALUE data_delete_option)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new_section (dogechat_ruby_plugin,
                                                               ruby_current_script,
                                                               API_STR2PTR(c_config_file),
                                                               c_name,
                                                               c_user_can_add_options,
                                                               c_user_can_delete_options,
                                                               &dogechat_ruby_api_config_read_cb,
                                                               c_function_read,
                                                               c_data_read,
                                                               &dogechat_ruby_api_config_section_write_cb,
                                                               c_function_write,
                                                               c_data_write,
                                                               &dogechat_ruby_api_config_section_write_default_cb,
                                                               c_function_write_default,
                                                               c_data_write_default,
                                                               &dogechat_ruby_api_config_section_create_option_cb,
                                                               c_function_create_option,
                                                               c_data_create_option,
                                                               &dogechat_ruby_api_config_section_delete_option_cb,
...
static VALUE
dogechat_ruby_api_config_search_section (VALUE class, VALUE config_file,
                                        VALUE section_name)
{
    ...
    result = API_PTR2STR(dogechat_config_search_section (API_STR2PTR(c_config_file),
                                                        c_section_name));
...
int
dogechat_ruby_api_config_option_check_value_cb (void *data,
                                               struct t_config_option *option,
                                               const char *value)
{
    struct t_plugin_script_cb *script_callback;
...
void
dogechat_ruby_api_config_option_change_cb (void *data,
                                          struct t_config_option *option)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
void
dogechat_ruby_api_config_option_delete_cb (void *data,
                                          struct t_config_option *option)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
static VALUE
dogechat_ruby_api_config_new_option (VALUE class, VALUE config_file,
                                    ...
                                    VALUE callbacks)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new_option (dogechat_ruby_plugin,
                                                              ruby_current_script,
                                                              API_STR2PTR(c_config_file),
                                                              ...
                                                              &dogechat_ruby_api_config_option_check_value_cb,
                                                              c_function_check_value,
                                                              c_data_check_value,
                                                              &dogechat_ruby_api_config_option_change_cb,
                                                              c_function_change,
                                                              c_data_change,
                                                              &dogechat_ruby_api_config_option_delete_cb,
                                                              c_function_delete,
                                                              c_data_delete));
...
static VALUE
dogechat_ruby_api_config_search_option (VALUE class, VALUE config_file,
                                       VALUE section, VALUE option_name)
{
    ...
    result = API_PTR2STR(dogechat_config_search_option (API_STR2PTR(c_config_file),
                                                       API_STR2PTR(c_section),
                                                       c_option_name));
...
static VALUE
dogechat_ruby_api_config_string_to_boolean (VALUE class, VALUE text)
{
    char *c_text;
    int value;
    ...
    value = dogechat_config_string_to_boolean (c_text);

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_config_option_reset (VALUE class, VALUE option,
                                      VALUE run_callback)
{
    char *c_option;
    ...
    rc = dogechat_config_option_reset (API_STR2PTR(c_option),
                                      c_run_callback);
...
static VALUE
dogechat_ruby_api_config_option_set (VALUE class, VALUE option, VALUE new_value,
                                    VALUE run_callback)
{
    ...
    rc = dogechat_config_option_set (API_STR2PTR(c_option),
                                    c_new_value,
                                    c_run_callback);
...
static VALUE
dogechat_ruby_api_config_option_set_null (VALUE class, VALUE option,
                                         VALUE run_callback)
{
    char *c_option;
    int c_run_callback, rc;
    ...
    rc = dogechat_config_option_set_null (API_STR2PTR(c_option),
                                         c_run_callback);
...
static VALUE
dogechat_ruby_api_config_option_unset (VALUE class, VALUE option)
{
    char *c_option;
    int rc;
    ...
    rc = dogechat_config_option_unset (API_STR2PTR(c_option));

    API_RETURN_INT(rc);
}

static VALUE
dogechat_ruby_api_config_option_rename (VALUE class, VALUE option,
                                       VALUE new_name)
{
    ...
    dogechat_config_option_rename (API_STR2PTR(c_option),
                                  c_new_name);
...
static VALUE
dogechat_ruby_api_config_option_is_null (VALUE class, VALUE option)
{
    char *c_option;
    int value;
    ...
    value = dogechat_config_option_is_null (API_STR2PTR(c_option));

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_config_option_default_is_null (VALUE class, VALUE option)
{
    char *c_option;
    int value;
    ...
    value = dogechat_config_option_default_is_null (API_STR2PTR(c_option));

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_config_boolean (VALUE class, VALUE option)
{
    char *c_option;
    int value;
    ...
    value = dogechat_config_boolean (API_STR2PTR(c_option));

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_config_boolean_default (VALUE class, VALUE option)
{
    char *c_option;
    int value;
    ...
    value = dogechat_config_boolean_default (API_STR2PTR(c_option));

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_config_integer (VALUE class, VALUE option)
{
    char *c_option;
    int value;
    ...
    value = dogechat_config_integer (API_STR2PTR(c_option));

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_config_integer_default (VALUE class, VALUE option)
{
    char *c_option;
    int value;
    ...
    value = dogechat_config_integer_default (API_STR2PTR(c_option));

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_config_string (VALUE class, VALUE option)
{
    char *c_option;
    const char *result;
    ...
    result = dogechat_config_string (API_STR2PTR(c_option));

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_config_string_default (VALUE class, VALUE option)
{
    char *c_option;
    const char *result;
    ...
    result = dogechat_config_string_default (API_STR2PTR(c_option));

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_config_color (VALUE class, VALUE option)
{
    char *c_option;
    const char *result;
    ...
    result = dogechat_config_color (API_STR2PTR(c_option));

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_config_color_default (VALUE class, VALUE option)
{
    char *c_option;
    const char *result;
    ...
    result = dogechat_config_color_default (API_STR2PTR(c_option));

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_config_write_option (VALUE class, VALUE config_file,
                                      VALUE option)
{
    ...
    dogechat_config_write_option (API_STR2PTR(c_config_file),
...
static VALUE
dogechat_ruby_api_config_write_line (VALUE class, VALUE config_file,
                                    VALUE option_name, VALUE value)
{
    ...
    dogechat_config_write_line (API_STR2PTR(c_config_file),
                               c_option_name,
                               "%s",
                               c_value);
...
static VALUE
dogechat_ruby_api_config_write (VALUE class, VALUE config_file)
{
    char *c_config_file;
    int rc;
    ...
    rc = dogechat_config_write (API_STR2PTR(c_config_file));

    API_RETURN_INT(rc);
}

static VALUE
dogechat_ruby_api_config_read (VALUE class, VALUE config_file)
{
    char *c_config_file;
    int rc;
    ...
    rc = dogechat_config_read (API_STR2PTR(c_config_file));

    API_RETURN_INT(rc);
}

static VALUE
dogechat_ruby_api_config_reload (VALUE class, VALUE config_file)
{
    char *c_config_file;
    int rc;
    ...
    rc = dogechat_config_reload (API_STR2PTR(c_config_file));

    API_RETURN_INT(rc);
}

static VALUE
dogechat_ruby_api_config_option_free (VALUE class, VALUE option)
{
    char *c_option;
    ...
    plugin_script_api_config_option_free (dogechat_ruby_plugin,
                                          ruby_current_script,
                                          API_STR2PTR(c_option));
...
static VALUE
dogechat_ruby_api_config_section_free_options (VALUE class, VALUE section)
{
    char *c_section;
    ...
    plugin_script_api_config_section_free_options (dogechat_ruby_plugin,
                                                   ruby_current_script,
                                                   API_STR2PTR(c_section));
...
static VALUE
dogechat_ruby_api_config_section_free (VALUE class, VALUE section)
{
    char *c_section;
    ...
    plugin_script_api_config_section_free (dogechat_ruby_plugin,
                                           ruby_current_script,
                                           API_STR2PTR(c_section));
...
static VALUE
dogechat_ruby_api_config_free (VALUE class, VALUE config_file)
{
    char *c_config_file;
    ...
    plugin_script_api_config_free (dogechat_ruby_plugin,
                                   ruby_current_script,
                                   API_STR2PTR(c_config_file));
...
static VALUE
dogechat_ruby_api_config_get (VALUE class, VALUE option)
{
    ...
    result = API_PTR2STR(dogechat_config_get (c_option));

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_config_get_plugin (VALUE class, VALUE option)
{
    char *c_option;
    const char *result;
    ...
    result = plugin_script_api_config_get_plugin (dogechat_ruby_plugin,
                                                  ruby_current_script,
                                                  c_option);
...
static VALUE
dogechat_ruby_api_config_is_set_plugin (VALUE class, VALUE option)
{
    char *c_option;
    int rc;
    ...
    rc = plugin_script_api_config_is_set_plugin (dogechat_ruby_plugin,
                                                 ruby_current_script,
                                                 c_option);
...
static VALUE
dogechat_ruby_api_config_set_plugin (VALUE class, VALUE option, VALUE value)
{
    ...
    rc = plugin_script_api_config_set_plugin (dogechat_ruby_plugin,
                                              ruby_current_script,
                                              c_option,
                                              c_value);
...
static VALUE
dogechat_ruby_api_config_set_desc_plugin (VALUE class, VALUE option,
                                         VALUE description)
{
    ...
    plugin_script_api_config_set_desc_plugin (dogechat_ruby_plugin,
                                              ruby_current_script,
                                              c_option,
                                              c_description);
...
static VALUE
dogechat_ruby_api_config_unset_plugin (VALUE class, VALUE option)
{
    char *c_option;
    int rc;
    ...
    rc = plugin_script_api_config_unset_plugin (dogechat_ruby_plugin,
                                                ruby_current_script,
                                                c_option);
...
static VALUE
dogechat_ruby_api_key_bind (VALUE class, VALUE context, VALUE keys)
{
    char *c_context;
    ...
    c_keys = dogechat_ruby_hash_to_hashtable (keys,
                                             DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                             DOGECHAT_HASHTABLE_STRING,
                                             DOGECHAT_HASHTABLE_STRING);

    num_keys = dogechat_key_bind (c_context, c_keys);
...
static VALUE
dogechat_ruby_api_key_unbind (VALUE class, VALUE context, VALUE key)
{
    ...
    num_keys = dogechat_key_unbind (c_context, c_key);

    API_RETURN_INT(num_keys);
}

static VALUE
dogechat_ruby_api_prefix (VALUE class, VALUE prefix)
{
    char *c_prefix;
    const char *result;
    ...
    result = dogechat_prefix (c_prefix);

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_color (VALUE class, VALUE color)
{
    char *c_color;
    const char *result;
    ...
    result = dogechat_color (c_color);

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_print (VALUE class, VALUE buffer, VALUE message)
{
    ...
    plugin_script_api_printf (dogechat_ruby_plugin,
                              ruby_current_script,
...
static VALUE
dogechat_ruby_api_print_date_tags (VALUE class, VALUE buffer, VALUE date,
                                  VALUE tags, VALUE message)
{
    ...
    plugin_script_api_printf_date_tags (dogechat_ruby_plugin,
                                        ruby_current_script,
                                        API_STR2PTR(c_buffer),
                                        c_date,
                                        c_tags,
...
static VALUE
dogechat_ruby_api_print_y (VALUE class, VALUE buffer, VALUE y, VALUE message)
{
    char *c_buffer, *c_message;
    int c_y;
    ...
    plugin_script_api_printf_y (dogechat_ruby_plugin,
                                ruby_current_script,
                                API_STR2PTR(c_buffer),
                                c_y,
                                "%s", c_message);
...
static VALUE
dogechat_ruby_api_log_print (VALUE class, VALUE message)
{
    char *c_message;
    ...
    plugin_script_api_log_printf (dogechat_ruby_plugin,
                                  ruby_current_script,
                                  "%s", c_message);
...
int
dogechat_ruby_api_hook_command_cb (void *data, struct t_gui_buffer *buffer,
...
static VALUE
dogechat_ruby_api_hook_command (VALUE class, VALUE command, VALUE description,
                               ...
                               VALUE completion, VALUE function, VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_command (dogechat_ruby_plugin,
                                                         ruby_current_script,
                                                         c_command,
                                                         c_description,
                                                         c_args,
                                                         c_args_description,
                                                         c_completion,
                                                         &dogechat_ruby_api_hook_command_cb,
                                                         c_function,
                                                         c_data));
...
int
dogechat_ruby_api_hook_command_run_cb (void *data, struct t_gui_buffer *buffer,
                                      const char *command)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
...
static VALUE
dogechat_ruby_api_hook_command_run (VALUE class, VALUE command, VALUE function,
                                   VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_command_run (dogechat_ruby_plugin,
                                                             ruby_current_script,
                                                             c_command,
                                                             &dogechat_ruby_api_hook_command_run_cb,
                                                             c_function,
                                                             c_data));
...
int
dogechat_ruby_api_hook_timer_cb (void *data, int remaining_calls)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
static VALUE
dogechat_ruby_api_hook_timer (VALUE class, VALUE interval, VALUE align_second,
                             VALUE max_calls, VALUE function, VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_timer (dogechat_ruby_plugin,
                                                       ruby_current_script,
                                                       c_interval,
                                                       c_align_second,
                                                       c_max_calls,
                                                       &dogechat_ruby_api_hook_timer_cb,
                                                       c_function,
                                                       c_data));
...
int
dogechat_ruby_api_hook_fd_cb (void *data, int fd)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
static VALUE
dogechat_ruby_api_hook_fd (VALUE class, VALUE fd, VALUE read, VALUE write,
                          VALUE exception, VALUE function, VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_fd (dogechat_ruby_plugin,
                                                    ruby_current_script,
                                                    c_fd,
                                                    c_read,
                                                    c_write,
                                                    c_exception,
                                                    &dogechat_ruby_api_hook_fd_cb,
                                                    c_function,
                                                    c_data));
...
int
dogechat_ruby_api_hook_process_cb (void *data,
...
static VALUE
dogechat_ruby_api_hook_process (VALUE class, VALUE command, VALUE timeout,
                               VALUE function, VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_process (dogechat_ruby_plugin,
                                                         ruby_current_script,
                                                         c_command,
                                                         c_timeout,
                                                         &dogechat_ruby_api_hook_process_cb,
                                                         c_function,
                                                         c_data));
...
static VALUE
dogechat_ruby_api_hook_process_hashtable (VALUE class, VALUE command,
                                         ...
                                         VALUE function, VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_process_hashtable (dogechat_ruby_plugin,
                                                                   ruby_current_script,
                                                                   c_command,
                                                                   c_options,
                                                                   c_timeout,
...
int
dogechat_ruby_api_hook_connect_cb (void *data, int status, int gnutls_rc,
...
static VALUE
dogechat_ruby_api_hook_connect (VALUE class, VALUE proxy, VALUE address,
                               ...
                               VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_connect (dogechat_ruby_plugin,
                                                         ruby_current_script,
                                                         c_proxy,
                                                         c_address,
                                                         c_port,
                                                         ...
                                                         c_local_hostname,
                                                         &dogechat_ruby_api_hook_connect_cb,
                                                         c_function,
                                                         c_data));
...
int
dogechat_ruby_api_hook_print_cb (void *data, struct t_gui_buffer *buffer,
                                time_t date,
...
static VALUE
dogechat_ruby_api_hook_print (VALUE class, VALUE buffer, VALUE tags,
                             ...
                             VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_print (dogechat_ruby_plugin,
                                                       ruby_current_script,
                                                       API_STR2PTR(c_buffer),
                                                       c_tags,
                                                       c_message,
                                                       c_strip_colors,
                                                       &dogechat_ruby_api_hook_print_cb,
                                                       c_function,
                                                       c_data));
...
int
dogechat_ruby_api_hook_signal_cb (void *data, const char *signal, const char *type_data,
                                 void *signal_data)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
...
static VALUE
dogechat_ruby_api_hook_signal (VALUE class, VALUE signal, VALUE function,
                              VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_signal (dogechat_ruby_plugin,
                                                        ruby_current_script,
                                                        c_signal,
                                                        &dogechat_ruby_api_hook_signal_cb,
                                                        c_function,
                                                        c_data));
...
static VALUE
dogechat_ruby_api_hook_signal_send (VALUE class, VALUE signal, VALUE type_data,
                                   VALUE signal_data)
{
    ...
    if (strcmp (c_type_data, DOGECHAT_HOOK_SIGNAL_STRING) == 0)
    {
        Check_Type (signal_data, T_STRING);
        c_signal_data = StringValuePtr (signal_data);
        rc = dogechat_hook_signal_send (c_signal, c_type_data, c_signal_data);
        API_RETURN_INT(rc);
    }
    else if (strcmp (c_type_data, DOGECHAT_HOOK_SIGNAL_INT) == 0)
    {
        Check_Type (signal_data, T_FIXNUM);
        number = FIX2INT (signal_data);
        rc = dogechat_hook_signal_send (c_signal, c_type_data, &number);
        API_RETURN_INT(rc);
    }
    else if (strcmp (c_type_data, DOGECHAT_HOOK_SIGNAL_POINTER) == 0)
    {
        Check_Type (signal_data, T_STRING);
        c_signal_data = StringValuePtr (signal_data);
        rc = dogechat_hook_signal_send (c_signal, c_type_data,
...
int
dogechat_ruby_api_hook_hsignal_cb (void *data, const char *signal,
...
static VALUE
dogechat_ruby_api_hook_hsignal (VALUE class, VALUE signal, VALUE function,
                               VALUE data)
{
    char *c_signal, *c_function, *c_data, *result;
    VALUE return_value;
    ...
    result = API_PTR2STR(plugin_script_api_hook_hsignal (dogechat_ruby_plugin,
                                                         ruby_current_script,
                                                         c_signal,
                                                         &dogechat_ruby_api_hook_hsignal_cb,
                                                         c_function,
                                                         c_data));
...
static VALUE
dogechat_ruby_api_hook_hsignal_send (VALUE class, VALUE signal, VALUE hashtable)
{
    char *c_signal;
    ...
    c_signal = StringValuePtr (signal);
    c_hashtable = dogechat_ruby_hash_to_hashtable (hashtable,
                                                  DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                  DOGECHAT_HASHTABLE_STRING,
                                                  DOGECHAT_HASHTABLE_STRING);

    rc = dogechat_hook_hsignal_send (c_signal, c_hashtable);
...
int
dogechat_ruby_api_hook_config_cb (void *data, const char *option, const char *value)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
    char empty_arg[1] = { '\0' };
...
static VALUE
dogechat_ruby_api_hook_config (VALUE class, VALUE option, VALUE function,
                              VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_config (dogechat_ruby_plugin,
                                                        ruby_current_script,
                                                        c_option,
                                                        &dogechat_ruby_api_hook_config_cb,
                                                        c_function,
                                                        c_data));
...
int
dogechat_ruby_api_hook_completion_cb (void *data, const char *completion_item,
...
static VALUE
dogechat_ruby_api_hook_completion (VALUE class, VALUE completion,
                                  ...
                                  VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_completion (dogechat_ruby_plugin,
                                                            ruby_current_script,
                                                            c_completion,
                                                            c_description,
                                                            &dogechat_ruby_api_hook_completion_cb,
                                                            c_function,
                                                            c_data));
...
static VALUE
dogechat_ruby_api_hook_completion_get_string (VALUE class, VALUE completion,
                                             VALUE property)
{
    ...
    result = dogechat_hook_completion_get_string (API_STR2PTR(c_completion),
                                                 c_property);
...
static VALUE
dogechat_ruby_api_hook_completion_list_add (VALUE class, VALUE completion,
                                           ...
                                           VALUE where)
{
    ...
    dogechat_hook_completion_list_add (API_STR2PTR(c_completion),
                                      c_word,
                                      c_nick_completion,
                                      c_where);
...
char *
dogechat_ruby_api_hook_modifier_cb (void *data, const char *modifier,
                                   const char *modifier_data,  const char *string)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (char *)dogechat_ruby_exec (script_callback->script,
...
static VALUE
dogechat_ruby_api_hook_modifier (VALUE class, VALUE modifier, VALUE function,
                                VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_modifier (dogechat_ruby_plugin,
                                                          ruby_current_script,
                                                          c_modifier,
                                                          &dogechat_ruby_api_hook_modifier_cb,
                                                          c_function,
                                                          c_data));
...
static VALUE
dogechat_ruby_api_hook_modifier_exec (VALUE class, VALUE modifier,
                                     VALUE modifier_data, VALUE string)
{
    ...
    result = dogechat_hook_modifier_exec (c_modifier, c_modifier_data, c_string);

    API_RETURN_STRING_FREE(result);
}

const char *
dogechat_ruby_api_hook_info_cb (void *data, const char *info_name,
                               const char *arguments)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (const char *)dogechat_ruby_exec (script_callback->script,
                                                DOGECHAT_SCRIPT_EXEC_STRING,
                                                script_callback->function,
                                                "sss", func_argv);
    }
...
static VALUE
dogechat_ruby_api_hook_info (VALUE class, VALUE info_name, VALUE description,
                            VALUE args_description, VALUE function, VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_info (dogechat_ruby_plugin,
                                                      ruby_current_script,
                                                      c_info_name,
                                                      c_description,
                                                      c_args_description,
                                                      &dogechat_ruby_api_hook_info_cb,
                                                      c_function,
                                                      c_data));
...
struct t_hashtable *
dogechat_ruby_api_hook_info_hashtable_cb (void *data, const char *info_name,
                                         struct t_hashtable *hashtable)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (struct t_hashtable *)dogechat_ruby_exec (script_callback->script,
...
static VALUE
dogechat_ruby_api_hook_info_hashtable (VALUE class, VALUE info_name,
                                      VALUE description,
                                      ...
                                      VALUE function, VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_info_hashtable (dogechat_ruby_plugin,
                                                                ruby_current_script,
                                                                c_info_name,
                                                                c_description,
                                                                c_args_description,
                                                                c_output_description,
                                                                &dogechat_ruby_api_hook_info_hashtable_cb,
                                                                c_function,
                                                                c_data));
...
struct t_infolist *
dogechat_ruby_api_hook_infolist_cb (void *data, const char *infolist_name,
                                   void *pointer, const char *arguments)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        result = (struct t_infolist *)dogechat_ruby_exec (script_callback->script,
                                                         DOGECHAT_SCRIPT_EXEC_STRING,
                                                         script_callback->function,
                                                         "ssss", func_argv);
...
static VALUE
dogechat_ruby_api_hook_infolist (VALUE class, VALUE infolist_name,
                                ...
                                VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_infolist (dogechat_ruby_plugin,
                                                          ruby_current_script,
                                                          c_infolist_name,
                                                          c_description,
                                                          c_pointer_description,
                                                          c_args_description,
                                                          &dogechat_ruby_api_hook_infolist_cb,
                                                          c_function,
                                                          c_data));
...
struct t_hashtable *
dogechat_ruby_api_hook_focus_cb (void *data, struct t_hashtable *info)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
    char empty_arg[1] = { '\0' };
...
static VALUE
dogechat_ruby_api_hook_focus (VALUE class, VALUE area, VALUE function,
                             VALUE data)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_focus (dogechat_ruby_plugin,
                                                       ruby_current_script,
                                                       c_area,
                                                       &dogechat_ruby_api_hook_focus_cb,
                                                       c_function,
                                                       c_data));
...
static VALUE
dogechat_ruby_api_hook_set (VALUE class, VALUE hook, VALUE property,
                           VALUE value)
{
    ...
    dogechat_hook_set (API_STR2PTR(c_hook),
                      c_property,
                      c_value);
...
static VALUE
dogechat_ruby_api_unhook (VALUE class, VALUE hook)
{
    char *c_hook;
    ...
    plugin_script_api_unhook (dogechat_ruby_plugin,
                              ruby_current_script,
                              API_STR2PTR(c_hook));
...
static VALUE
dogechat_ruby_api_unhook_all (VALUE class)
{
    API_INIT_FUNC(1, "unhook_all", API_RETURN_ERROR);

    plugin_script_api_unhook_all (dogechat_ruby_plugin, ruby_current_script);

    API_RETURN_OK;
}

int
dogechat_ruby_api_buffer_input_data_cb (void *data, struct t_gui_buffer *buffer,
...
int
dogechat_ruby_api_buffer_close_cb (void *data, struct t_gui_buffer *buffer)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
    char empty_arg[1] = { '\0' };
...
static VALUE
dogechat_ruby_api_buffer_new (VALUE class, VALUE name, VALUE function_input,
                             ...
                             VALUE data_close)
{
    ...
    result = API_PTR2STR(plugin_script_api_buffer_new (dogechat_ruby_plugin,
                                                       ruby_current_script,
                                                       c_name,
...
static VALUE
dogechat_ruby_api_buffer_search (VALUE class, VALUE plugin, VALUE name)
{
    ...
    result = API_PTR2STR(dogechat_buffer_search (c_plugin, c_name));

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_buffer_search_main (VALUE class)
{
    char *result;
    VALUE return_value;
...
static VALUE
dogechat_ruby_api_current_buffer (VALUE class)
{
    char *result;
    VALUE return_value;
...
static VALUE
dogechat_ruby_api_buffer_clear (VALUE class, VALUE buffer)
{
    char *c_buffer;
    ...
    dogechat_buffer_clear (API_STR2PTR(c_buffer));

    API_RETURN_OK;
}

static VALUE
dogechat_ruby_api_buffer_close (VALUE class, VALUE buffer)
{
    char *c_buffer;
    ...
    plugin_script_api_buffer_close (dogechat_ruby_plugin,
                                    ruby_current_script,
                                    API_STR2PTR(c_buffer));
...
static VALUE
dogechat_ruby_api_buffer_merge (VALUE class, VALUE buffer, VALUE target_buffer)
{
    ...
    dogechat_buffer_merge (API_STR2PTR(c_buffer),
...
static VALUE
dogechat_ruby_api_buffer_unmerge (VALUE class, VALUE buffer, VALUE number)
{
    char *c_buffer;
    int c_number;
    ...
    dogechat_buffer_unmerge (API_STR2PTR(c_buffer), c_number);

    API_RETURN_OK;
}

static VALUE
dogechat_ruby_api_buffer_get_integer (VALUE class, VALUE buffer, VALUE property)
{
    ...
    value = dogechat_buffer_get_integer (API_STR2PTR(c_buffer),
                                        c_property);
...
static VALUE
dogechat_ruby_api_buffer_get_string (VALUE class, VALUE buffer, VALUE property)
{
    char *c_buffer, *c_property;
    const char *result;
    ...
    result = dogechat_buffer_get_string (API_STR2PTR(c_buffer),
                                        c_property);
...
static VALUE
dogechat_ruby_api_buffer_get_pointer (VALUE class, VALUE buffer, VALUE property)
{
    ...
    result = API_PTR2STR(dogechat_buffer_get_pointer (API_STR2PTR(c_buffer),
                                                     c_property));
...
static VALUE
dogechat_ruby_api_buffer_set (VALUE class, VALUE buffer, VALUE property,
                             VALUE value)
{
    ...
    dogechat_buffer_set (API_STR2PTR(c_buffer),
                        c_property,
                        c_value);
...
static VALUE
dogechat_ruby_api_buffer_string_replace_local_var (VALUE class, VALUE buffer, VALUE string)
{
    ...
    result = dogechat_buffer_string_replace_local_var (API_STR2PTR(c_buffer), c_string);

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_buffer_match_list (VALUE class, VALUE buffer, VALUE string)
{
    ...
    value = dogechat_buffer_match_list (API_STR2PTR(c_buffer),
                                       c_string);
...
static VALUE
dogechat_ruby_api_current_window (VALUE class)
{
    char *result;
    VALUE return_value;
...
static VALUE
dogechat_ruby_api_window_search_with_buffer (VALUE class, VALUE buffer)
{
    ...
    result = API_PTR2STR(dogechat_window_search_with_buffer (API_STR2PTR(c_buffer)));

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_window_get_integer (VALUE class, VALUE window, VALUE property)
{
    ...
    value = dogechat_window_get_integer (API_STR2PTR(c_window),
                                        c_property);
...
static VALUE
dogechat_ruby_api_window_get_string (VALUE class, VALUE window, VALUE property)
{
    char *c_window, *c_property;
    const char *result;
    ...
    result = dogechat_window_get_string (API_STR2PTR(c_window),
                                        c_property);
...
static VALUE
dogechat_ruby_api_window_get_pointer (VALUE class, VALUE window, VALUE property)
{
    ...
    result = API_PTR2STR(dogechat_window_get_pointer (API_STR2PTR(c_window),
                                                     c_property));
...
static VALUE
dogechat_ruby_api_window_set_title (VALUE class, VALUE title)
{
    char *c_title;
    ...
    dogechat_window_set_title (c_title);

    API_RETURN_OK;
}

static VALUE
dogechat_ruby_api_nicklist_add_group (VALUE class, VALUE buffer,
                                     ...
                                     VALUE color, VALUE visible)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_add_group (API_STR2PTR(c_buffer),
...
static VALUE
dogechat_ruby_api_nicklist_search_group (VALUE class, VALUE buffer,
                                        VALUE from_group, VALUE name)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_search_group (API_STR2PTR(c_buffer),
...
static VALUE
dogechat_ruby_api_nicklist_add_nick (VALUE class, VALUE buffer, VALUE group,
                                    ...
                                    VALUE prefix_color, VALUE visible)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_add_nick (API_STR2PTR(c_buffer),
                                                    API_STR2PTR(c_group),
                                                    c_name,
                                                    c_color,
                                                    c_prefix,
...
static VALUE
dogechat_ruby_api_nicklist_search_nick (VALUE class, VALUE buffer,
                                       VALUE from_group, VALUE name)
{
    char *c_buffer, *c_from_group, *c_name, *result;
    VALUE return_value;
    ...
    result = API_PTR2STR(dogechat_nicklist_search_nick (API_STR2PTR(c_buffer),
                                                       API_STR2PTR(c_from_group),
                                                       c_name));
...
static VALUE
dogechat_ruby_api_nicklist_remove_group (VALUE class, VALUE buffer, VALUE group)
{
    ...
    dogechat_nicklist_remove_group (API_STR2PTR(c_buffer),
                                   API_STR2PTR(c_group));
...
static VALUE
dogechat_ruby_api_nicklist_remove_nick (VALUE class, VALUE buffer, VALUE nick)
{
    ...
    dogechat_nicklist_remove_nick (API_STR2PTR(c_buffer),
                                  API_STR2PTR(c_nick));
...
static VALUE
dogechat_ruby_api_nicklist_remove_all (VALUE class, VALUE buffer)
{
    char *c_buffer;
    ...
    dogechat_nicklist_remove_all (API_STR2PTR(c_buffer));

    API_RETURN_OK;
}

static VALUE
dogechat_ruby_api_nicklist_group_get_integer (VALUE class, VALUE buffer,
                                             VALUE group, VALUE property)
{
    ...
    value = dogechat_nicklist_group_get_integer (API_STR2PTR(c_buffer),
                                                API_STR2PTR(c_group),
                                                c_property);
...
static VALUE
dogechat_ruby_api_nicklist_group_get_string (VALUE class, VALUE buffer,
                                            VALUE group, VALUE property)
{
    char *c_buffer, *c_group, *c_property;
    const char *result;
    ...
    result = dogechat_nicklist_group_get_string (API_STR2PTR(c_buffer),
                                                API_STR2PTR(c_group),
                                                c_property);
...
static VALUE
dogechat_ruby_api_nicklist_group_get_pointer (VALUE class, VALUE buffer,
                                             VALUE group, VALUE property)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_group_get_pointer (API_STR2PTR(c_buffer),
                                                             API_STR2PTR(c_group),
                                                             c_property));
...
static VALUE
dogechat_ruby_api_nicklist_group_set (VALUE class, VALUE buffer, VALUE group,
                                     VALUE property, VALUE value)
{
    ...
    dogechat_nicklist_group_set (API_STR2PTR(c_buffer),
                                API_STR2PTR(c_group),
                                c_property,
                                c_value);
...
static VALUE
dogechat_ruby_api_nicklist_nick_get_integer (VALUE class, VALUE buffer,
                                            VALUE nick, VALUE property)
{
    ...
    value = dogechat_nicklist_nick_get_integer (API_STR2PTR(c_buffer),
                                               API_STR2PTR(c_nick),
                                               c_property);
...
static VALUE
dogechat_ruby_api_nicklist_nick_get_string (VALUE class, VALUE buffer,
                                           VALUE nick, VALUE property)
{
    char *c_buffer, *c_nick, *c_property;
    const char *result;
    ...
    result = dogechat_nicklist_nick_get_string (API_STR2PTR(c_buffer),
                                               API_STR2PTR(c_nick),
                                               c_property);
...
static VALUE
dogechat_ruby_api_nicklist_nick_get_pointer (VALUE class, VALUE buffer,
                                            VALUE nick, VALUE property)
{
    ...
    result = API_PTR2STR(dogechat_nicklist_nick_get_pointer (API_STR2PTR(c_buffer),
                                                            API_STR2PTR(c_nick),
                                                            c_property));
...
static VALUE
dogechat_ruby_api_nicklist_nick_set (VALUE class, VALUE buffer, VALUE nick,
                                    VALUE property, VALUE value)
{
    ...
    dogechat_nicklist_nick_set (API_STR2PTR(c_buffer),
                               API_STR2PTR(c_nick),
                               c_property,
                               c_value);
...
static VALUE
dogechat_ruby_api_bar_item_search (VALUE class, VALUE name)
{
    char *c_name, *result;
    VALUE return_value;
    ...
    result = API_PTR2STR(dogechat_bar_item_search (c_name));

    API_RETURN_STRING_FREE(result);
}

char *
dogechat_ruby_api_bar_item_build_cb (void *data, struct t_gui_bar_item *item,
                                    ...
                                    struct t_hashtable *extra_info)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        if (strncmp (script_callback->function, "(extra)", 7) == 0)
        {
            ...
            ret = (char *)dogechat_ruby_exec (script_callback->script,
                                             DOGECHAT_SCRIPT_EXEC_STRING,
...
static VALUE
dogechat_ruby_api_bar_item_new (VALUE class, VALUE name, VALUE function,
                               VALUE data)
{
    char *c_name, *c_function, *c_data, *result;
    VALUE return_value;
    ...
    result = API_PTR2STR(plugin_script_api_bar_item_new (dogechat_ruby_plugin,
                                                         ruby_current_script,
                                                         c_name,
                                                         &dogechat_ruby_api_bar_item_build_cb,
                                                         c_function,
                                                         c_data));
...
static VALUE
dogechat_ruby_api_bar_item_update (VALUE class, VALUE name)
{
    char *c_name;
    ...
    dogechat_bar_item_update (c_name);

    API_RETURN_OK;
}

static VALUE
dogechat_ruby_api_bar_item_remove (VALUE class, VALUE item)
{
    char *c_item;
    ...
    plugin_script_api_bar_item_remove (dogechat_ruby_plugin,
                                       ruby_current_script,
                                       API_STR2PTR(c_item));
...
static VALUE
dogechat_ruby_api_bar_search (VALUE class, VALUE name)
{
    char *c_name, *result;
    VALUE return_value;
    ...
    result = API_PTR2STR(dogechat_bar_search (c_name));

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_bar_new (VALUE class, VALUE name, VALUE hidden,
                          ...
                          VALUE color_bg, VALUE separator, VALUE items)
{
    ...
    result = API_PTR2STR(dogechat_bar_new (c_name,
                                          c_hidden,
                                          c_priority,
                                          c_type,
                                          c_conditions,
...
static VALUE
dogechat_ruby_api_bar_set (VALUE class, VALUE bar, VALUE property, VALUE value)
{
    ...
    rc = dogechat_bar_set (API_STR2PTR(c_bar), c_property, c_value);

    API_RETURN_INT(rc);
}

static VALUE
dogechat_ruby_api_bar_update (VALUE class, VALUE name)
{
    char *c_name;
    ...
    dogechat_bar_update (c_name);

    API_RETURN_OK;
}

static VALUE
dogechat_ruby_api_bar_remove (VALUE class, VALUE bar)
{
    char *c_bar;
    ...
    dogechat_bar_remove (API_STR2PTR(c_bar));

    API_RETURN_OK;
}

static VALUE
dogechat_ruby_api_command (VALUE class, VALUE buffer, VALUE command)
{
    ...
    rc = plugin_script_api_command (dogechat_ruby_plugin,
                                    ruby_current_script,
                                    API_STR2PTR(c_buffer),
                                    c_command);
...
static VALUE
dogechat_ruby_api_info_get (VALUE class, VALUE info_name, VALUE arguments)
{
    ...
    result = dogechat_info_get (c_info_name, c_arguments);

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_info_get_hashtable (VALUE class, VALUE info_name,
                                     VALUE hash)
{
    char *c_info_name;
    ...
    c_info_name = StringValuePtr (info_name);
    c_hashtable = dogechat_ruby_hash_to_hashtable (hash,
                                                  DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                  DOGECHAT_HASHTABLE_STRING,
                                                  DOGECHAT_HASHTABLE_STRING);

    result_hashtable = dogechat_info_get_hashtable (c_info_name, c_hashtable);
    result_hash = dogechat_ruby_hashtable_to_hash (result_hashtable);
...
static VALUE
dogechat_ruby_api_infolist_new (VALUE class)
{
    char *result;
    VALUE return_value;
...
static VALUE
dogechat_ruby_api_infolist_new_item (VALUE class, VALUE infolist)
{
    ...
    result = API_PTR2STR(dogechat_infolist_new_item (API_STR2PTR(c_infolist)));

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_infolist_new_var_integer (VALUE class, VALUE item,
                                           VALUE name, VALUE value)
{
    ...
    result = API_PTR2STR(dogechat_infolist_new_var_integer (API_STR2PTR(c_item),
                                                           c_name,
                                                           c_value));
...
static VALUE
dogechat_ruby_api_infolist_new_var_string (VALUE class, VALUE item,
                                          VALUE name, VALUE value)
{
    ...
    result = API_PTR2STR(dogechat_infolist_new_var_string (API_STR2PTR(c_item),
                                                          c_name,
                                                          c_value));
...
static VALUE
dogechat_ruby_api_infolist_new_var_pointer (VALUE class, VALUE item,
                                           VALUE name, VALUE value)
{
    char *c_item, *c_name, *c_value, *result;
    VALUE return_value;
    ...
    result = API_PTR2STR(dogechat_infolist_new_var_pointer (API_STR2PTR(c_item),
                                                           c_name,
                                                           API_STR2PTR(c_value)));
...
static VALUE
dogechat_ruby_api_infolist_new_var_time (VALUE class, VALUE item,
                                        VALUE name, VALUE value)
{
    char *c_item, *c_name, *result;
    int c_value;
    ...
    result = API_PTR2STR(dogechat_infolist_new_var_time (API_STR2PTR(c_item),
                                                        c_name,
                                                        c_value));
...
static VALUE
dogechat_ruby_api_infolist_search_var (VALUE class, VALUE infolist, VALUE name)
{
    ...
    result = API_PTR2STR(dogechat_infolist_search_var (API_STR2PTR(c_infolist),
                                                      c_name));
...
static VALUE
dogechat_ruby_api_infolist_get (VALUE class, VALUE name, VALUE pointer,
                               VALUE arguments)
{
    ...
    result = API_PTR2STR(dogechat_infolist_get (c_name,
...
static VALUE
dogechat_ruby_api_infolist_next (VALUE class, VALUE infolist)
{
    char *c_infolist;
    int value;
    ...
    value = dogechat_infolist_next (API_STR2PTR(c_infolist));

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_infolist_prev (VALUE class, VALUE infolist)
{
    char *c_infolist;
    int value;
    ...
    value = dogechat_infolist_prev (API_STR2PTR(c_infolist));

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_infolist_reset_item_cursor (VALUE class, VALUE infolist)
{
    char *c_infolist;
    ...
    dogechat_infolist_reset_item_cursor (API_STR2PTR(c_infolist));

    API_RETURN_OK;
}

static VALUE
dogechat_ruby_api_infolist_fields (VALUE class, VALUE infolist)
{
    char *c_infolist;
    const char *result;
    ...
    result = dogechat_infolist_fields (API_STR2PTR(c_infolist));

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_infolist_integer (VALUE class, VALUE infolist, VALUE variable)
{
    ...
    value = dogechat_infolist_integer (API_STR2PTR(c_infolist), c_variable);

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_infolist_string (VALUE class, VALUE infolist, VALUE variable)
{
    char *c_infolist, *c_variable;
    const char *result;
    ...
    result = dogechat_infolist_string (API_STR2PTR(c_infolist), c_variable);

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_infolist_pointer (VALUE class, VALUE infolist, VALUE variable)
{
    ...
    result = API_PTR2STR(dogechat_infolist_pointer (API_STR2PTR(c_infolist), c_variable));

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_infolist_time (VALUE class, VALUE infolist, VALUE variable)
{
    ...
    timebuffer[0] = '\0';
    time = dogechat_infolist_time (API_STR2PTR(c_infolist), c_variable);
...
static VALUE
dogechat_ruby_api_infolist_free (VALUE class, VALUE infolist)
{
    char *c_infolist;
    ...
    dogechat_infolist_free (API_STR2PTR(c_infolist));

    API_RETURN_OK;
}

static VALUE
dogechat_ruby_api_hdata_get (VALUE class, VALUE name)
{
    char *c_name, *result;
    VALUE return_value;
    ...
    result = API_PTR2STR(dogechat_hdata_get (c_name));

    API_RETURN_STRING_FREE(result);
}

static VALUE
dogechat_ruby_api_hdata_get_var_offset (VALUE class, VALUE hdata, VALUE name)
{
    char *c_hdata, *c_name;
    int value;
    ...
    value = dogechat_hdata_get_var_offset (API_STR2PTR(c_hdata), c_name);

    API_RETURN_INT(value);
}

static VALUE
dogechat_ruby_api_hdata_get_var_type_string (VALUE class, VALUE hdata,
                                            VALUE name)
{
    char *c_hdata, *c_name;
    const char *result;
    ...
    result = dogechat_hdata_get_var_type_string (API_STR2PTR(c_hdata), c_name);

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_hdata_get_var_array_size (VALUE class, VALUE hdata, VALUE pointer,
                                           VALUE name)
{
    ...
    value = dogechat_hdata_get_var_array_size (API_STR2PTR(c_hdata),
                                              API_STR2PTR(c_pointer),
                                              c_name);
...
static VALUE
dogechat_ruby_api_hdata_get_var_array_size_string (VALUE class, VALUE hdata,
                                                  VALUE pointer, VALUE name)
{
    ...
    result = dogechat_hdata_get_var_array_size_string (API_STR2PTR(c_hdata),
                                                      API_STR2PTR(c_pointer),
                                                      c_name);
...
static VALUE
dogechat_ruby_api_hdata_get_var_hdata (VALUE class, VALUE hdata, VALUE name)
{
    char *c_hdata, *c_name;
    const char *result;
    ...
    result = dogechat_hdata_get_var_hdata (API_STR2PTR(c_hdata), c_name);

    API_RETURN_STRING(result);
}

static VALUE
dogechat_ruby_api_hdata_get_list (VALUE class, VALUE hdata, VALUE name)
{
    ...
    result = API_PTR2STR(dogechat_hdata_get_list (API_STR2PTR(c_hdata),
                                                 c_name));
...
static VALUE
dogechat_ruby_api_hdata_check_pointer (VALUE class, VALUE hdata, VALUE list,
                                      VALUE pointer)
{
    ...
    value = dogechat_hdata_check_pointer (API_STR2PTR(c_hdata),
                                         API_STR2PTR(c_list),
                                         API_STR2PTR(c_pointer));
...
static VALUE
dogechat_ruby_api_hdata_move (VALUE class, VALUE hdata, VALUE pointer,
                             VALUE count)
{
    ...
    result = API_PTR2STR(dogechat_hdata_move (API_STR2PTR(c_hdata),
                                             API_STR2PTR(c_pointer),
                                             c_count));
...
static VALUE
dogechat_ruby_api_hdata_search (VALUE class, VALUE hdata, VALUE pointer,
                               VALUE search, VALUE move)
{
    ...
    result = API_PTR2STR(dogechat_hdata_search (API_STR2PTR(c_hdata),
                                               API_STR2PTR(c_pointer),
                                               c_search,
                                               c_move));
...
static VALUE
dogechat_ruby_api_hdata_char (VALUE class, VALUE hdata, VALUE pointer,
                             VALUE name)
{
    char *c_hdata, *c_pointer, *c_name;
    int value;
    ...
    value = (int)dogechat_hdata_char (API_STR2PTR(c_hdata),
                                     API_STR2PTR(c_pointer),
                                     c_name);
...
static VALUE
dogechat_ruby_api_hdata_integer (VALUE class, VALUE hdata, VALUE pointer,
                                VALUE name)
{
    char *c_hdata, *c_pointer, *c_name;
    int value;
    ...
    value = dogechat_hdata_integer (API_STR2PTR(c_hdata),
                                   API_STR2PTR(c_pointer),
                                   c_name);
...
static VALUE
dogechat_ruby_api_hdata_long (VALUE class, VALUE hdata, VALUE pointer,
                             VALUE name)
{
    char *c_hdata, *c_pointer, *c_name;
    long value;
    ...
    value = dogechat_hdata_long (API_STR2PTR(c_hdata),
                                API_STR2PTR(c_pointer),
                                c_name);
...
static VALUE
dogechat_ruby_api_hdata_string (VALUE class, VALUE hdata, VALUE pointer,
                               VALUE name)
{
    char *c_hdata, *c_pointer, *c_name;
    const char *result;
    ...
    result = dogechat_hdata_string (API_STR2PTR(c_hdata),
                                   API_STR2PTR(c_pointer),
                                   c_name);
...
static VALUE
dogechat_ruby_api_hdata_pointer (VALUE class, VALUE hdata, VALUE pointer,
                                VALUE name)
{
    ...
    result = API_PTR2STR(dogechat_hdata_pointer (API_STR2PTR(c_hdata),
                                                API_STR2PTR(c_pointer),
                                                c_name));
...
static VALUE
dogechat_ruby_api_hdata_time (VALUE class, VALUE hdata, VALUE pointer,
                             VALUE name)
{
    char *c_hdata, *c_pointer, *c_name;
    time_t time;
    ...
    time = dogechat_hdata_time (API_STR2PTR(c_hdata),
                               API_STR2PTR(c_pointer),
                               c_name);
...
static VALUE
dogechat_ruby_api_hdata_hashtable (VALUE class, VALUE hdata, VALUE pointer,
                                  VALUE name)
{
    char *c_hdata, *c_pointer, *c_name;
    VALUE result_hash;
    ...
    result_hash = dogechat_ruby_hashtable_to_hash (
        dogechat_hdata_hashtable (API_STR2PTR(c_hdata),
                                 API_STR2PTR(c_pointer),
                                 c_name));
...
static VALUE
dogechat_ruby_api_hdata_update (VALUE class, VALUE hdata, VALUE pointer,
                               VALUE hashtable)
{
    ...
    value = dogechat_hdata_update (API_STR2PTR(c_hdata),
                                  API_STR2PTR(c_pointer),
                                  c_hashtable);
...
static VALUE
dogechat_ruby_api_hdata_get_string (VALUE class, VALUE hdata, VALUE property)
{
    ...
    result = dogechat_hdata_get_var_type_string (API_STR2PTR(c_hdata),
                                                c_property);
...
static VALUE
dogechat_ruby_api_upgrade_new (VALUE class, VALUE filename, VALUE write)
{
    ...
    result = API_PTR2STR(dogechat_upgrade_new (c_filename,
                                              c_write));
...
static VALUE
dogechat_ruby_api_upgrade_write_object (VALUE class, VALUE upgrade_file,
                                       VALUE object_id, VALUE infolist)
{
    ...
    rc = dogechat_upgrade_write_object (API_STR2PTR(c_upgrade_file),
                                       c_object_id,
...
int
dogechat_ruby_api_upgrade_read_cb (void *data,
...
static VALUE
dogechat_ruby_api_upgrade_read (VALUE class, VALUE upgrade_file,
                               VALUE function, VALUE data)
{
    ...
    rc = plugin_script_api_upgrade_read (dogechat_ruby_plugin,
                                         ruby_current_script,
                                         API_STR2PTR(c_upgrade_file),
                                         &dogechat_ruby_api_upgrade_read_cb,
                                         c_function,
                                         c_data);
...
static VALUE
dogechat_ruby_api_upgrade_close (VALUE class, VALUE upgrade_file)
{
    char *c_upgrade_file;
    ...
    dogechat_upgrade_close (API_STR2PTR(c_upgrade_file));
...
void
dogechat_ruby_api_init (VALUE ruby_mDogechat)
{
    rb_define_const(ruby_mDogechat, "DOGECHAT_RC_OK", INT2NUM(DOGECHAT_RC_OK));
    rb_define_const(ruby_mDogechat, "DOGECHAT_RC_OK_EAT", INT2NUM(DOGECHAT_RC_OK_EAT));
    rb_define_const(ruby_mDogechat, "DOGECHAT_RC_ERROR", INT2NUM(DOGECHAT_RC_ERROR));

    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_READ_OK", INT2NUM(DOGECHAT_CONFIG_READ_OK));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_READ_MEMORY_ERROR", INT2NUM(DOGECHAT_CONFIG_READ_MEMORY_ERROR));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_READ_FILE_NOT_FOUND", INT2NUM(DOGECHAT_CONFIG_READ_FILE_NOT_FOUND));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_WRITE_OK", INT2NUM(DOGECHAT_CONFIG_WRITE_OK));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_WRITE_ERROR", INT2NUM(DOGECHAT_CONFIG_WRITE_ERROR));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_WRITE_MEMORY_ERROR", INT2NUM(DOGECHAT_CONFIG_WRITE_MEMORY_ERROR));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED", INT2NUM(DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE", INT2NUM(DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_OPTION_SET_ERROR", INT2NUM(DOGECHAT_CONFIG_OPTION_SET_ERROR));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND", INT2NUM(DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET", INT2NUM(DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET", INT2NUM(DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED", INT2NUM(DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED));
    rb_define_const(ruby_mDogechat, "DOGECHAT_CONFIG_OPTION_UNSET_ERROR", INT2NUM(DOGECHAT_CONFIG_OPTION_UNSET_ERROR));

    rb_define_const(ruby_mDogechat, "DOGECHAT_LIST_POS_SORT", rb_str_new2(DOGECHAT_LIST_POS_SORT));
    rb_define_const(ruby_mDogechat, "DOGECHAT_LIST_POS_BEGINNING", rb_str_new2(DOGECHAT_LIST_POS_BEGINNING));
    rb_define_const(ruby_mDogechat, "DOGECHAT_LIST_POS_END", rb_str_new2(DOGECHAT_LIST_POS_END));

    rb_define_const(ruby_mDogechat, "DOGECHAT_HOTLIST_LOW", rb_str_new2(DOGECHAT_HOTLIST_LOW));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOTLIST_MESSAGE", rb_str_new2(DOGECHAT_HOTLIST_MESSAGE));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOTLIST_PRIVATE", rb_str_new2(DOGECHAT_HOTLIST_PRIVATE));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOTLIST_HIGHLIGHT", rb_str_new2(DOGECHAT_HOTLIST_HIGHLIGHT));

    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_PROCESS_RUNNING", INT2NUM(DOGECHAT_HOOK_PROCESS_RUNNING));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_PROCESS_ERROR", INT2NUM(DOGECHAT_HOOK_PROCESS_ERROR));

    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_OK", INT2NUM(DOGECHAT_HOOK_CONNECT_OK));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND", INT2NUM(DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND", INT2NUM(DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_CONNECTION_REFUSED", INT2NUM(DOGECHAT_HOOK_CONNECT_CONNECTION_REFUSED));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_PROXY_ERROR", INT2NUM(DOGECHAT_HOOK_CONNECT_PROXY_ERROR));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR", INT2NUM(DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_GNUTLS_INIT_ERROR", INT2NUM(DOGECHAT_HOOK_CONNECT_GNUTLS_INIT_ERROR));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR", INT2NUM(DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_MEMORY_ERROR", INT2NUM(DOGECHAT_HOOK_CONNECT_MEMORY_ERROR));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_TIMEOUT", INT2NUM(DOGECHAT_HOOK_CONNECT_TIMEOUT));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_CONNECT_SOCKET_ERROR", INT2NUM(DOGECHAT_HOOK_CONNECT_SOCKET_ERROR));

    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_SIGNAL_STRING", rb_str_new2(DOGECHAT_HOOK_SIGNAL_STRING));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_SIGNAL_INT", rb_str_new2(DOGECHAT_HOOK_SIGNAL_INT));
    rb_define_const(ruby_mDogechat, "DOGECHAT_HOOK_SIGNAL_POINTER", rb_str_new2(DOGECHAT_HOOK_SIGNAL_POINTER));
