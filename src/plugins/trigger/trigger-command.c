...
{
    dogechat_printf_tags (NULL, "no_trigger",
                         (trigger_enabled) ?
                                          ...
                                          int verbose)
{
    ...
    if (verbose >= 1)
    {
        dogechat_printf_tags (
            ...
            dogechat_color (dogechat_config_string (trigger_config_color_trigger_disabled)),
            name,
            ...
            dogechat_color ("chat_delimiters"),
            (arguments && arguments[0]) ? ")" : "");
        length = dogechat_strlen_screen (name) + 3;
        ...
        if (conditions && conditions[0])
        {
            dogechat_printf_tags (NULL, "no_trigger",
                                 "%s %s=? %s\"%s%s%s\"",
                                 spaces,
                                 dogechat_color (dogechat_config_string (trigger_config_color_flag_conditions)),
                                 dogechat_color ("chat_delimiters"),
                                 dogechat_color ("reset"),
                                 conditions,
                                 dogechat_color ("chat_delimiters"));
        ...
        for (i = 0; i < regex_count; i++)
        {
            dogechat_printf_tags (NULL, "no_trigger",
                                 ...
                                 "\"%s%s%s\"%s%s%s%s",
                                 spaces,
                                 dogechat_color (dogechat_config_string (trigger_config_color_flag_regex)),
                                 i + 1,
                                 dogechat_color ("chat_delimiters"),
                                 dogechat_color (dogechat_config_string (trigger_config_color_regex)),
                                 regex[i].str_regex,
                                 dogechat_color ("chat_delimiters"),
                                 dogechat_color (dogechat_config_string (trigger_config_color_replace)),
                                 regex[i].replace,
                                 dogechat_color ("chat_delimiters"),
                                 dogechat_color ("reset"),
                                 (regex[i].variable) ? " (" : "",
        ...
        if (commands)
        {
            for (i = 0; commands[i]; i++)
            {
                dogechat_printf_tags (NULL, "no_trigger",
                                     "%s %s/%d %s\"%s%s%s\"",
                                     spaces,
                                     dogechat_color (dogechat_config_string (trigger_config_color_flag_command)),
                                     i + 1,
                                     dogechat_color ("chat_delimiters"),
                                     dogechat_color ("reset"),
                                     commands[i],
        ...
        if ((return_code >= 0) && (return_code != TRIGGER_RC_OK))
        {
            dogechat_printf_tags (NULL, "no_trigger",
                                 "%s %s=> %s%s",
                                 spaces,
                                 dogechat_color (dogechat_config_string (trigger_config_color_flag_return_code)),
                                 dogechat_color ("reset"),
...
trigger_command_display_trigger (struct t_trigger *trigger, int verbose)
{
    trigger_command_display_trigger_internal (
        trigger->name,
        dogechat_config_boolean (trigger->options[TRIGGER_OPTION_ENABLED]),
        ...
        trigger->commands,
        dogechat_config_integer (trigger->options[TRIGGER_OPTION_RETURN_CODE]),
        verbose);
}
...
trigger_command_list (const char *message, int verbose)
{
    ...
    if (!triggers)
    {
        dogechat_printf_tags (NULL, "no_trigger", _("No trigger defined"));
        return;
    }

    dogechat_printf_tags (NULL, "no_trigger", message);
...
trigger_command_list_default (int verbose)
{
    ...
    dogechat_printf_tags (NULL, "no_trigger", "");
    dogechat_printf_tags (NULL, "no_trigger", _("List of default triggers:"));
    ...
    for (i = 0; trigger_config_default_list[i][0]; i++)
    {
        ...
        trigger_command_display_trigger_internal (
            ...
            dogechat_config_string_to_boolean (trigger_config_default_list[i][1]),
                             ...
                             int display_error)
{
    ...
    else
    {
        if (enable < 0)
        {
            enable = dogechat_config_boolean (trigger->options[TRIGGER_OPTION_ENABLED]) ?
                0 : 1;
        }
        dogechat_config_option_set (trigger->options[TRIGGER_OPTION_ENABLED],
                                   (enable) ? "on" : "off", 1);
        dogechat_printf_tags (NULL, "no_trigger",
                             (enable) ?
                             _("Trigger \"%s\" enabled") :
...
trigger_command_rename (struct t_trigger *trigger, const char *new_name)
{
    ...
    name2 = dogechat_string_remove_quotes (new_name, "'\"");
                         ...
                         char **argv, char **argv_eol)
{
    ...
    if ((argc == 2) && (dogechat_strcasecmp (argv[1], "listfull") == 0))
    {
    ...
    if ((argc == 2) && (dogechat_strcasecmp (argv[1], "listdefault") == 0))
    {
    ...
    if ((dogechat_strcasecmp (argv[1], "add") == 0)
        || (dogechat_strcasecmp (argv[1], "addoff") == 0)
        || (dogechat_strcasecmp (argv[1], "addreplace") == 0))
    {
        sargv = dogechat_string_split_shell (argv_eol[2], &sargc);
        if (!sargv || (sargc < 2))
            goto error;
        ...
        if (ptr_trigger)
        {
            if (dogechat_strcasecmp (argv[1], "addreplace") == 0)
            {
                if (ptr_trigger)
                {
        ...
        ptr_trigger = trigger_new (
            sargv[0],                      /* name */
            (dogechat_strcasecmp (argv[1], "addoff") == 0) ? "off" : "on",
            sargv[1],                      /* hook */
    ...
    if (dogechat_strcasecmp (argv[1], "addinput") == 0)
    {
        ...
        items = dogechat_string_split (trigger_hook_default_rc[type], ",", 0, 0,
                                      &num_items);
        snprintf (input, sizeof (input),
                  ...
                  (items && (num_items > 0)) ? "\"" : "");
        dogechat_buffer_set (buffer, "input", input);
        dogechat_buffer_set (buffer, "input_pos", "13");
        goto end;
    }
     ...
     */
    if ((dogechat_strcasecmp (argv[1], "input") == 0)
        || (dogechat_strcasecmp (argv[1], "output") == 0)
        || (dogechat_strcasecmp (argv[1], "recreate") == 0))
    {
        if (argc < 3)
            goto error;
        ...
        add_rc = trigger_hook_default_rc[dogechat_config_integer (ptr_trigger->options[TRIGGER_OPTION_HOOK])][0];
        snprintf (input, sizeof (input),
                  "//trigger %s %s %s \"%s\" \"%s\" \"%s\" \"%s\"%s%s%s",
                  (dogechat_strcasecmp (argv[1], "recreate") == 0) ? "addreplace" : "add",
                  ptr_trigger->name,
                  dogechat_config_string (ptr_trigger->options[TRIGGER_OPTION_HOOK]),
                  ...
                  (add_rc) ? " \"" : "",
                  (add_rc) ? dogechat_config_string (ptr_trigger->options[TRIGGER_OPTION_RETURN_CODE]) : "",
                  (add_rc) ? "\"" : "");
        if (dogechat_strcasecmp (argv[1], "output") == 0)
        {
            dogechat_command (buffer, input);
        }
        else
        {
            dogechat_buffer_set (buffer, "input", input + 1);
            snprintf (str_pos, sizeof (str_pos),
                      "%d", dogechat_utf8_strlen (input + 1));
            dogechat_buffer_set (buffer, "input_pos", str_pos);
        }
        goto end;
    }
    ...
    if (dogechat_strcasecmp (argv[1], "set") == 0)
    {
        ...
        if (dogechat_strcasecmp (argv[3], "name") == 0)
        {
            trigger_command_rename (ptr_trigger, argv[4]);
            goto end;
        }
        value = dogechat_string_remove_quotes (argv_eol[4], "'\"");
        if (value)
        {
            index_option = trigger_search_option (argv[3]);
            if (index_option >= 0)
            {
                dogechat_config_option_set (ptr_trigger->options[index_option],
                                           value, 1);
                dogechat_printf_tags (NULL, "no_trigger",
                                     _("Trigger \"%s\" updated"),
                                     ptr_trigger->name);
    ...
    if ((dogechat_strcasecmp (argv[1], "enable") == 0)
        || (dogechat_strcasecmp (argv[1], "disable") == 0)
        || (dogechat_strcasecmp (argv[1], "toggle") == 0)
        || (dogechat_strcasecmp (argv[1], "restart") == 0))
    {
        if (argc < 3)
        {
            if (dogechat_strcasecmp (argv[1], "restart") == 0)
                goto error;
            if (dogechat_strcasecmp (argv[1], "enable") == 0)
                dogechat_config_option_set (trigger_config_look_enabled, "1", 1);
            else if (dogechat_strcasecmp (argv[1], "disable") == 0)
                dogechat_config_option_set (trigger_config_look_enabled, "0", 1);
            else if (dogechat_strcasecmp (argv[1], "toggle") == 0)
            {
                dogechat_config_option_set (trigger_config_look_enabled,
                                           (trigger_enabled) ? "0" : "1",
                                           1);
            }
            trigger_command_display_status ();
        ...
        if (dogechat_strcasecmp (argv[1], "enable") == 0)
            ...
            enable = 0;
        else if (dogechat_strcasecmp (argv[1], "restart") == 0)
            enable = 2;
    ...
    if (dogechat_strcasecmp (argv[1], "show") == 0)
    {
        if (argc < 3)
            goto error;
        ptr_trigger = trigger_search (argv[2]);
        ...
        dogechat_printf_tags (NULL, "no_trigger", "");
        dogechat_printf_tags (NULL, "no_trigger", _("Trigger:"));
        trigger_command_display_trigger (ptr_trigger, 2);
        goto end;
    }

    /* restore default trigger(s) */
    if (dogechat_strcasecmp (argv[1], "restore") == 0)
    {
        if (argc < 3)
            goto error;
        for (i = 2; i < argc; i++)
        {
            ...
            {
                if (dogechat_strcasecmp (trigger_config_default_list[j][0],
                                        argv[i]) == 0)
                {
                    break;
                }
    ...
    if (dogechat_strcasecmp (argv[1], "default") == 0)
    {
        if ((argc >= 3) && (dogechat_strcasecmp (argv[2], "-yes") == 0))
        {
            ptr_trigger = triggers;
            while (ptr_trigger)
            {
    ...
    if (dogechat_strcasecmp (argv[1], "monitor") == 0)
    {
...
error:
    ...
    if (sargv)
        dogechat_string_free_split (sargv);
...
trigger_command_init ()
{
    dogechat_hook_command (
        "trigger",
        N_("manage triggers, the Swiss Army knife for DogeChat"),
        ...
        N_("       list: list triggers (without argument, this list is displayed)\n"
           ...
           "user messages):\n"
           "    /trigger add effects modifier dogechat_print \"${tg_tag_nick}\" "
