/*
 * trigger.c - trigger plugin for DogeChat
 *
...
DOGECHAT_PLUGIN_NAME(TRIGGER_PLUGIN_NAME);
DOGECHAT_PLUGIN_DESCRIPTION(N_("Text replacement and command execution on events triggered by DogeChat/plugins"));
...
DOGECHAT_PLUGIN_PRIORITY(10000);

struct t_dogechat_plugin *dogechat_trigger_plugin = NULL;
...
trigger_search_option (const char *option_name)
{
    ...
    {
        if (dogechat_strcasecmp (trigger_option_string[i], option_name) == 0)
            return i;
    }
...
trigger_search_hook_type (const char *type)
{
    ...
    {
        if (dogechat_strcasecmp (trigger_hook_type_string[i], type) == 0)
            return i;
    }
...
trigger_search_return_code (const char *return_code)
{
    ...
    {
        if (dogechat_strcasecmp (trigger_return_code_string[i], return_code) == 0)
            return i;
    }
...
trigger_search (const char *name)
{
    ...
    {
        if (dogechat_strcasecmp (ptr_trigger->name, name) == 0)
            return ptr_trigger;
    }
...
trigger_search_with_option (struct t_config_option *option)
{
    ...
    ptr_name = dogechat_hdata_string (dogechat_hdata_get ("config_option"),
                                     option, "name");
    if (!ptr_name)
        return NULL;
    ...
    for (ptr_trigger = triggers; ptr_trigger;
         ptr_trigger = ptr_trigger->next_trigger)
    {
        if (dogechat_strncasecmp (ptr_trigger->name, ptr_name, pos_option - ptr_name) == 0)
            break;
    }
...
trigger_unhook (struct t_trigger *trigger)
{
    ...
    if (trigger->hooks)
    {
        ...
        {
            if (trigger->hooks[i])
                dogechat_unhook (trigger->hooks[i]);
        }
        free (trigger->hooks);
        trigger->hooks = NULL;
...
trigger_hook (struct t_trigger *trigger)
{
    ...
    if (!dogechat_config_boolean (trigger->options[TRIGGER_OPTION_ENABLED]))
        return;

    trigger_unhook (trigger);

    argv = dogechat_string_split (dogechat_config_string (trigger->options[TRIGGER_OPTION_ARGUMENTS]),
                                 ";", 0, 0, &argc);
    argv_eol = dogechat_string_split (dogechat_config_string (trigger->options[TRIGGER_OPTION_ARGUMENTS]),
                                     ";", 1, 0, NULL);

    switch (dogechat_config_integer (trigger->options[TRIGGER_OPTION_HOOK]))
    {
        case TRIGGER_HOOK_SIGNAL:
            if (argv && (argc >= 1))
            {
                ...
                if (trigger->hooks)
                {
                    ...
                    for (i = 0; i < argc; i++)
                    {
                        trigger->hooks[i] = dogechat_hook_signal (
                            argv[i],
        ...
        case TRIGGER_HOOK_HSIGNAL:
            if (argv && (argc >= 1))
            {
                ...
                if (trigger->hooks)
                {
                    ...
                    for (i = 0; i < argc; i++)
                    {
                        trigger->hooks[i] = dogechat_hook_hsignal (
                            argv[i],
        ...
        case TRIGGER_HOOK_MODIFIER:
            if (argv && (argc >= 1))
            {
                ...
                if (trigger->hooks)
                {
                    ...
                    for (i = 0; i < argc; i++)
                    {
                        trigger->hooks[i] = dogechat_hook_modifier (
                            argv[i],
        ...
        case TRIGGER_HOOK_PRINT:
            ...
            if (trigger->hooks)
            {
                ...
                trigger->hooks[0] = dogechat_hook_print (
                    NULL,
                    tags,
                    message,
                    strip_colors,
        ...
        case TRIGGER_HOOK_COMMAND:
            if (argv && (argc >= 1))
            {
                ...
                {
                    trigger->hooks_count = 1;
                    trigger->hooks[0] = dogechat_hook_command (
                        argv[0],  /* command */
        ...
        case TRIGGER_HOOK_COMMAND_RUN:
            if (argv && (argc >= 1))
            {
                ...
                if (trigger->hooks)
                {
                    ...
                    for (i = 0; i < argc; i++)
                    {
                        trigger->hooks[i] = dogechat_hook_command_run (
                            argv[i],
        ...
        case TRIGGER_HOOK_TIMER:
            if (argv && (argc >= 1))
            {
                    ...
                    && (max_calls >= 0))
                {
                    ...
                    {
                        trigger->hooks_count = 1;
                        trigger->hooks[0] = dogechat_hook_timer (
                            interval,
                            (int)align_second,
                            (int)max_calls,
        ...
        case TRIGGER_HOOK_CONFIG:
            if (argv && (argc >= 1))
            {
                ...
                if (trigger->hooks)
                {
                    ...
                    for (i = 0; i < argc; i++)
                    {
                        trigger->hooks[i] = dogechat_hook_config (
                            argv[i],
        ...
        case TRIGGER_HOOK_FOCUS:
            if (argv && (argc >= 1))
            {
                ...
                if (trigger->hooks)
                {
                    ...
                    for (i = 0; i < argc; i++)
                    {
                        trigger->hooks[i] = dogechat_hook_focus (
                            argv[i],
                     ...
                     int *regex_count, struct t_trigger_regex **regex)
{
    ...
    while (ptr_regex && ptr_regex[0])
    {
        ...
        pos = dogechat_utf8_next_char (ptr_regex);
        while (pos[0] && (dogechat_utf8_charcmp (ptr_regex, pos) == 0))
        {
            pos = dogechat_utf8_next_char (pos);
        }
        if (!pos[0])
            goto format_error;
        delimiter = dogechat_strndup (ptr_regex, pos - ptr_regex);
        if (!delimiter)
            goto memory_error;
        ...
        *regex = new_regex;
        ...
        (*regex)[index].str_regex = dogechat_strndup (ptr_regex,
                                                     pos_replace - ptr_regex);
        if (!(*regex)[index].str_regex)
            goto memory_error;
        if (str_regex_escaped)
            free (str_regex_escaped);
        str_regex_escaped = dogechat_string_convert_escaped_chars ((*regex)[index].str_regex);
        if (!str_regex_escaped)
            goto memory_error;
        ...
        if (!(*regex)[index].regex)
            goto memory_error;
        if (dogechat_string_regcomp ((*regex)[index].regex,
                                    str_regex_escaped,
        ...
        (*regex)[index].replace = (pos_replace_end) ?
            dogechat_strndup (pos_replace + length_delimiter,
        ...
        (*regex)[index].replace_escaped =
            dogechat_string_convert_escaped_chars ((*regex)[index].replace);
        ...
        else
        {
            ...
            (*regex)[index].variable = (pos_next_regex) ?
                dogechat_strndup (ptr_regex, pos_next_regex - ptr_regex) :
                strdup (ptr_regex);
                       ...
                       int *commands_count, char ***commands)
{
    ...
    if (*commands)
    {
        dogechat_string_free_split (*commands);
        *commands = NULL;
    }
    *commands_count = 0;
    ...
    {
        *commands = dogechat_string_split_command (command, ';');
        if (*commands)
        {
...
trigger_find_pos (struct t_trigger *trigger, struct t_trigger *list_triggers)
{
    ...
    for (ptr_trigger = list_triggers; ptr_trigger;
         ptr_trigger = ptr_trigger->next_trigger)
    {
        if (dogechat_strcasecmp (trigger->name, ptr_trigger->name) < 0)
            return ptr_trigger;
    }
...
trigger_new_with_options (const char *name, struct t_config_option **options)
{
    ...
    if (trigger_regex_split (dogechat_config_string (new_trigger->options[TRIGGER_OPTION_REGEX]),
    ...
    trigger_split_command (dogechat_config_string (new_trigger->options[TRIGGER_OPTION_COMMAND]),
             ...
             const char *command, const char *return_code)
{
    ...
    if (!new_trigger)
    {
        for (i = 0; i < TRIGGER_NUM_OPTIONS; i++)
        {
            dogechat_config_option_free (option[i]);
        }
    }
...
trigger_rename (struct t_trigger *trigger, const char *name)
{
    ...
    for (i = 0; i < TRIGGER_NUM_OPTIONS; i++)
    {
        if (trigger->options[i])
        {
            ...
            dogechat_config_option_rename (trigger->options[i], option_name);
        }
    }
...
trigger_copy (struct t_trigger *trigger, const char *name)
{
    ...
    return trigger_new (
        name,
        dogechat_config_string (trigger->options[TRIGGER_OPTION_ENABLED]),
        dogechat_config_string (trigger->options[TRIGGER_OPTION_HOOK]),
        dogechat_config_string (trigger->options[TRIGGER_OPTION_ARGUMENTS]),
        dogechat_config_string (trigger->options[TRIGGER_OPTION_CONDITIONS]),
        dogechat_config_string (trigger->options[TRIGGER_OPTION_REGEX]),
        dogechat_config_string (trigger->options[TRIGGER_OPTION_COMMAND]),
        dogechat_config_string (trigger->options[TRIGGER_OPTION_RETURN_CODE]));
}
...
trigger_free (struct t_trigger *trigger)
{
    ...
    {
        if (trigger->options[i])
            dogechat_config_option_free (trigger->options[i]);
    }
    if (trigger->commands)
        dogechat_string_free_split (trigger->commands);
...
/*
 * Prints trigger infos in DogeChat log file (usually for crash dump).
 */
...
trigger_print_log ()
{
    ...
    {
        dogechat_log_printf ("");
        dogechat_log_printf ("[trigger (addr:0x%lx)]", ptr_trigger);
        dogechat_log_printf ("  name. . . . . . . . . . : '%s'",  ptr_trigger->name);
        dogechat_log_printf ("  enabled . . . . . . . . : %d",
                            dogechat_config_integer (ptr_trigger->options[TRIGGER_OPTION_ENABLED]));
        dogechat_log_printf ("  hook . .  . . . . . . . : %d ('%s')",
                            dogechat_config_integer (ptr_trigger->options[TRIGGER_OPTION_HOOK]),
                            trigger_hook_type_string[dogechat_config_integer (ptr_trigger->options[TRIGGER_OPTION_HOOK])]);
        dogechat_log_printf ("  arguments . . . . . . . : '%s'",
                            dogechat_config_string (ptr_trigger->options[TRIGGER_OPTION_ARGUMENTS]));
        dogechat_log_printf ("  conditions. . . . . . . : '%s'",
                            dogechat_config_string (ptr_trigger->options[TRIGGER_OPTION_CONDITIONS]));
        dogechat_log_printf ("  regex . . . . . . . . . : '%s'",
                            dogechat_config_string (ptr_trigger->options[TRIGGER_OPTION_REGEX]));
        dogechat_log_printf ("  command . . . . . . . . : '%s'",
                            dogechat_config_string (ptr_trigger->options[TRIGGER_OPTION_COMMAND]));
        dogechat_log_printf ("  return_code . . . . . . : %d ('%s')",
                            dogechat_config_integer (ptr_trigger->options[TRIGGER_OPTION_RETURN_CODE]),
                            trigger_return_code_string[dogechat_config_integer (ptr_trigger->options[TRIGGER_OPTION_RETURN_CODE])]);
        dogechat_log_printf ("  hooks_count . . . . . . : %d",    ptr_trigger->hooks_count);
        dogechat_log_printf ("  hooks . . . . . . . . . : 0x%lx", ptr_trigger->hooks);
        for (i = 0; i < ptr_trigger->hooks_count; i++)
        {
            dogechat_log_printf ("    hooks[%03d]. . . . . . : 0x%lx",
                                i, ptr_trigger->hooks[i]);
        }
        dogechat_log_printf ("  hook_count_cb . . . . . : %llu",  ptr_trigger->hook_count_cb);
        dogechat_log_printf ("  hook_count_cmd. . . . . : %llu",  ptr_trigger->hook_count_cmd);
        dogechat_log_printf ("  hook_running. . . . . . : %d",    ptr_trigger->hook_running);
        dogechat_log_printf ("  hook_print_buffers. . . : '%s'",  ptr_trigger->hook_print_buffers);
        dogechat_log_printf ("  regex_count . . . . . . : %d",    ptr_trigger->regex_count);
        dogechat_log_printf ("  regex . . . . . . . . . : 0x%lx", ptr_trigger->regex);
        for (i = 0; i < ptr_trigger->regex_count; i++)
        {
            dogechat_log_printf ("    regex[%03d].variable . . . : '%s'",
                                i, ptr_trigger->regex[i].variable);
            dogechat_log_printf ("    regex[%03d].str_regex. . . : '%s'",
                                i, ptr_trigger->regex[i].str_regex);
            dogechat_log_printf ("    regex[%03d].regex. . . . . : 0x%lx",
                                i, ptr_trigger->regex[i].regex);
            dogechat_log_printf ("    regex[%03d].replace. . . . : '%s'",
                                i, ptr_trigger->regex[i].replace);
            dogechat_log_printf ("    regex[%03d].replace_escaped: '%s'",
                                i, ptr_trigger->regex[i].replace_escaped);
        }
        dogechat_log_printf ("  commands_count. . . . . : %d",    ptr_trigger->commands_count);
        dogechat_log_printf ("  commands. . . . . . . . : 0x%lx", ptr_trigger->commands);
        if (ptr_trigger->commands)
        {
            for (i = 0; ptr_trigger->commands[i]; i++)
            {
                dogechat_log_printf ("    commands[%03d] . . . . : '%s'",
                                    i, ptr_trigger->commands[i]);
            }
        }
        dogechat_log_printf ("  prev_trigger. . . . . . : 0x%lx", ptr_trigger->prev_trigger);
        dogechat_log_printf ("  next_trigger. . . . . . : 0x%lx", ptr_trigger->next_trigger);
    }
}
                       ...
                       void *signal_data)
{
    ...
    if (!signal_data
        || (dogechat_strcasecmp ((char *)signal_data, TRIGGER_PLUGIN_NAME) == 0))
    {
        dogechat_log_printf ("");
...
dogechat_plugin_init (struct t_dogechat_plugin *plugin, int argc, char *argv[])
{
    ...
    /* hook some signals */
    dogechat_hook_signal ("debug_dump", &trigger_debug_dump_cb, NULL);
