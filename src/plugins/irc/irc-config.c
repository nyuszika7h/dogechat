...
irc_config_get_server_from_option_name (const char *name)
{
    ...
    if (name)
    {
        ...
        if (pos_option)
        {
            server_name = dogechat_strndup (name, pos_option - name);
            if (server_name)
            {
...
irc_config_compute_nick_colors ()
{
    ...
    if (dogechat_config_boolean(irc_config_look_color_nicks_in_nicklist))
        irc_nick_nicklist_set_color_all ();
}

/*
 * Sets nick colors using option "dogechat.color.chat_nick_colors".
 */
...
irc_config_set_nick_colors ()
{
    ...
    {
        dogechat_string_free_split (irc_config_nick_colors);
        irc_config_nick_colors = NULL;
        irc_config_num_nick_colors = 0;
    }

    irc_config_nick_colors =
        dogechat_string_split (
            dogechat_config_string (
                dogechat_config_get ("dogechat.color.chat_nick_colors")),
            ",", 0, 0,
...
irc_config_display_channel_modes_arguments (const char *modes)
{
    ...
    ptr_mode = dogechat_config_string (irc_config_look_item_channel_modes_hide_args);
    if (!ptr_mode)
        return 1;
...
/*
 * Callback for changes on option "dogechat.color.chat_nick_colors".
 */
                                     ...
                                     struct t_config_option *option)
{
    ...
    if (!irc_config_loading
        && (dogechat_config_integer (irc_config_look_display_away) == IRC_CONFIG_DISPLAY_AWAY_CHANNEL))
    {
        dogechat_printf (
            NULL,
                                             ...
                                             struct t_config_option *option)
{
    ...
    {
        irc_config_hashtable_display_join_message = dogechat_hashtable_new (
            32,
        ...
        dogechat_hashtable_remove_all (irc_config_hashtable_display_join_message);

    items = dogechat_string_split (
        dogechat_config_string (irc_config_look_display_join_message),
        ",", 0, 0, &num_items);
    if (items)
    {
        for (i = 0; i < num_items; i++)
        {
            dogechat_hashtable_set (irc_config_hashtable_display_join_message,
                                   items[i], "1");
        }
                                      ...
                                      struct t_config_option *option)
{
         ...
         ptr_server = ptr_server->next_server)
    {
        if (ptr_server->buffer)
            dogechat_buffer_unmerge (ptr_server->buffer, -1);
    }

    /* merge IRC server buffers with core buffer or another buffer */
    if ((dogechat_config_integer (irc_config_look_server_buffer) ==
            ...
            IRC_CONFIG_LOOK_SERVER_BUFFER_MERGE_WITHOUT_CORE))
    {
        ptr_buffer =
            (dogechat_config_integer (irc_config_look_server_buffer) ==
             IRC_CONFIG_LOOK_SERVER_BUFFER_MERGE_WITH_CORE) ?
            dogechat_buffer_search_main () : irc_buffer_search_server_lowest_number ();
        ...
        if (ptr_buffer)
        {
                 ...
                 ptr_server = ptr_server->next_server)
            {
                if (ptr_server->buffer && (ptr_server->buffer != ptr_buffer))
                    dogechat_buffer_merge (ptr_server->buffer, ptr_buffer);
            }
        }
    }
}
         ...
         ptr_server = ptr_server->next_server)
    {
             ...
             ptr_channel = ptr_channel->next_channel)
        {
            if ((ptr_channel->type == IRC_CHANNEL_TYPE_PRIVATE)
                && ptr_channel->buffer)
            {
                dogechat_buffer_unmerge (ptr_channel->buffer, -1);
            }
        }
    }

    /* merge IRC private buffers */
    if ((dogechat_config_integer (irc_config_look_pv_buffer) == IRC_CONFIG_LOOK_PV_BUFFER_MERGE_BY_SERVER)
        || (dogechat_config_integer (irc_config_look_pv_buffer) == IRC_CONFIG_LOOK_PV_BUFFER_MERGE_ALL))
    {
        for (ptr_server = irc_servers; ptr_server;
             ptr_server = ptr_server->next_server)
        {
                 ...
                 ptr_channel = ptr_channel->next_channel)
            {
                    ...
                    && ptr_channel->buffer)
                {
                    ...
                    if (ptr_buffer && (ptr_channel->buffer != ptr_buffer))
                        dogechat_buffer_merge (ptr_channel->buffer, ptr_buffer);
                }
            }
        }
    }
                                         ...
                                         struct t_config_option *option)
{
    ...
    {
        irc_config_hashtable_nick_color_force = dogechat_hashtable_new (
            32,
            DOGECHAT_HASHTABLE_STRING,
            DOGECHAT_HASHTABLE_STRING,
            NULL,
        ...
        dogechat_hashtable_remove_all (irc_config_hashtable_nick_color_force);

    items = dogechat_string_split (
        dogechat_config_string (irc_config_look_nick_color_force),
        ";", 0, 0, &num_items);
    if (items)
    {
        for (i = 0; i < num_items; i++)
        {
            ...
            {
                pos[0] = '\0';
                dogechat_hashtable_set (irc_config_hashtable_nick_color_force,
                                       items[i],
                                       pos + 1);
            }
        }
                                            ...
                                            struct t_config_option *option)
{
    ...
    dogechat_bar_item_update ("buffer_plugin");
                                            ...
                                            struct t_config_option *option)
{
    ...
    {
        dogechat_string_free_split (irc_config_nicks_hide_password);
        irc_config_nicks_hide_password = NULL;
    }
    irc_config_num_nicks_hide_password = 0;

    nicks_hide_password = dogechat_config_string (irc_config_look_nicks_hide_password);
    if (nicks_hide_password && nicks_hide_password[0])
    {
        irc_config_nicks_hide_password = dogechat_string_split (
...
irc_config_change_color_mirc_remap (void *data, struct t_config_option *option)
{
    ...
    {
        irc_config_hashtable_color_mirc_remap = dogechat_hashtable_new (
            32,
            DOGECHAT_HASHTABLE_STRING,
            DOGECHAT_HASHTABLE_STRING,
            NULL,
        ...
        dogechat_hashtable_remove_all (irc_config_hashtable_color_mirc_remap);

    items = dogechat_string_split (
        dogechat_config_string (irc_config_color_mirc_remap),
        ";", 0, 0, &num_items);
    if (items)
    {
        for (i = 0; i < num_items; i++)
        {
            ...
            {
                pos[0] = '\0';
                dogechat_hashtable_set (irc_config_hashtable_color_mirc_remap,
                                       items[i],
                                       pos + 1);
            }
        }
                                       ...
                                       struct t_config_option *option)
{
    ...
    {
        irc_config_hashtable_nick_prefixes = dogechat_hashtable_new (
            32,
            DOGECHAT_HASHTABLE_STRING,
            DOGECHAT_HASHTABLE_STRING,
            NULL,
        ...
        dogechat_hashtable_remove_all (irc_config_hashtable_nick_prefixes);

    items = dogechat_string_split (
        dogechat_config_string (irc_config_color_nick_prefixes),
        ";", 0, 0, &num_items);
    if (items)
    {
        for (i = 0; i < num_items; i++)
        {
            ...
            {
                pos[0] = '\0';
                dogechat_hashtable_set (irc_config_hashtable_nick_prefixes,
                                       items[i],
                                       pos + 1);
            }
        }
                                     ...
                                     struct t_config_option *option)
{
    ...
    time_next_check = (dogechat_config_integer (irc_config_network_lag_check) > 0) ?
        time (NULL) : 0;
                                                 ...
                                                 struct t_config_option *option)
{
    ...
    strcpy (value,
            (dogechat_config_boolean (irc_config_network_send_unknown_commands)) ?
            "1" : "0");
         ...
         ptr_server = ptr_server->next_server)
    {
        if (ptr_server->buffer)
        {
            dogechat_buffer_set (ptr_server->buffer,
...
irc_config_server_default_change_cb (void *data, struct t_config_option *option)
{
    ...
    if (index_option >= 0)
    {
             ...
             ptr_server = ptr_server->next_server)
        {
             ...
             * value)
             */
            if (dogechat_config_option_is_null (ptr_server->options[index_option]))
            {
                switch (index_option)
                {
                    case IRC_SERVER_OPTION_ADDRESSES:
                        irc_server_set_addresses (ptr_server,
                                                  dogechat_config_string (option));
                        break;
    ...
    if (index_option >= 0)
    {
        switch (index_option)
        {
            case IRC_SERVER_OPTION_PROXY:
                ...
                {
                    proxy_found = 0;
                    infolist = dogechat_infolist_get ("proxy", NULL, value);
                    if (infolist)
                    {
                        while (dogechat_infolist_next (infolist))
                        {
                            proxy_name = dogechat_infolist_string (infolist,
                                                                  "name");
                    ...
                    if (!proxy_found)
                    {
                        dogechat_printf (
                            ...
                            dogechat_prefix ("error"), IRC_PLUGIN_NAME, value);
                    }
                }
                break;
            case IRC_SERVER_OPTION_SSL_PRIORITIES:
                ...
                if (pos_error)
                {
                    dogechat_printf (
                        ...
                        dogechat_prefix ("error"), IRC_PLUGIN_NAME, pos_error);
                    return 0;
                }
                break;
            case IRC_SERVER_OPTION_SSL_FINGERPRINT:
                ...
                if (value && value[0])
                {
                    fingerprints = dogechat_string_split (value, ",", 0, 0,
                                                         NULL);
                    if (fingerprints)
                    {
                        rc = 0;
...
irc_config_server_change_cb (void *data, struct t_config_option *option)
{
    ...
    if (index_option >= 0)
    {
        name = dogechat_config_option_get_pointer (option, "name");
                                    ...
                                    const char *option_name, const char *value)
{
            ...
            if (value)
            {
                ptr_option = dogechat_config_new_option (
                    ...
                    _("buffer used to display message received from IRC "
                      "server"),
                    "dogechat|server|current|private", 0, 0, value, value, 0,
                    NULL, NULL, NULL, NULL, NULL, NULL);
                rc = (ptr_option) ?
                    DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE :
                    DOGECHAT_CONFIG_OPTION_SET_ERROR;
            }
            else
                rc = DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE;
        }
    ...
    if (rc == DOGECHAT_CONFIG_OPTION_SET_ERROR)
    {
        dogechat_printf (
            ...
            dogechat_prefix ("error"), IRC_PLUGIN_NAME, option_name, value);
    }
            ...
            if (value)
            {
                ...
                ptr_option = dogechat_config_new_option (
                    ...
                    _("format for CTCP reply or empty string for blocking "
                      ...
                      "$version (DogeChat version), "
                      "$compilation (compilation date), "
                      "$osinfo (info about OS), "
                      "$site (DogeChat site), "
                      "$download (DogeChat site, download page), "
                           ...
                           const char *option_name, const char *value)
{
    ...
    if (option_name)
    {
        if (value && value[0])
        {
            argv = dogechat_string_split (value, ";", 0, 0, &argc);
            argv_eol = dogechat_string_split (value, ";", 1, 0, NULL);
                              ...
                              void *callback_change_data)
{
    ...
    {
        case IRC_SERVER_OPTION_ADDRESSES:
            new_option = dogechat_config_new_option (
                config_file, section,
                option_name, "string",
        ...
        case IRC_SERVER_OPTION_SSL_CERT:
            new_option = dogechat_config_new_option (
                ...
                N_("SSL certificate file used to automatically identify your "
                   "nick (\"%h\" will be replaced by DogeChat home, "
                   "\"~/.dogechat\" by default)"),
                NULL, 0, 0,
                default_value, value,
                null_value_allowed,
        ...
        case IRC_SERVER_OPTION_CAPABILITIES:
            new_option = dogechat_config_new_option (
                ...
                N_("comma-separated list of client capabilities to enable for "
                   ...
                   "capabilities supported by DogeChat) "
        ...
        case IRC_SERVER_OPTION_SASL_KEY:
            new_option = dogechat_config_new_option (
                ...
                N_("file with ECC private key for mechanism "
                   ...
                   "(\"%h\" will be replaced by DogeChat home, "
                   "\"~/.dogechat\" by default)"),
                NULL, 0, 0,
                default_value, value,
                null_value_allowed,
        ...
        case IRC_SERVER_OPTION_AUTOCONNECT:
            new_option = dogechat_config_new_option (
                config_file, section,
                option_name, "boolean",
                N_("automatically connect to server when DogeChat is starting"),
                NULL, 0, 0,
                default_value, value,
                null_value_allowed,
                callback_check_value, callback_check_value_data,
        ...
        case IRC_SERVER_OPTION_CONNECTION_TIMEOUT:
            new_option = dogechat_config_new_option (
                ...
                N_("timeout (in seconds) between TCP connection to server and "
                   ...
                   "001 message is received, DogeChat will disconnect from "
                   "server"),
                NULL, 1, 3600,
                default_value, value,
                null_value_allowed,
        ...
        case IRC_SERVER_OPTION_DEFAULT_MSG_PART:
            new_option = dogechat_config_new_option (
                ...
                N_("default part message (leaving channel) (\"%v\" will be "
                   "replaced by DogeChat version in string)"),
                NULL, 0, 0,
                default_value, value,
                null_value_allowed,
                callback_check_value, callback_check_value_data,
        ...
        case IRC_SERVER_OPTION_DEFAULT_MSG_QUIT:
            new_option = dogechat_config_new_option (
                ...
                N_("default quit message (disconnecting from server) (\"%v\" "
                   "will be replaced by DogeChat version in string)"),
                NULL, 0, 0,
                default_value, value,
                null_value_allowed,
                callback_check_value, callback_check_value_data,
                           ...
                           const char *option_name, const char *value)
{
    ...
    if (option_name)
    {
        ...
        if (pos_option)
        {
            server_name = dogechat_strndup (option_name,
    ...
    if (rc == DOGECHAT_CONFIG_OPTION_SET_ERROR)
    {
        dogechat_printf (
            ...
            dogechat_prefix ("error"), IRC_PLUGIN_NAME, option_name);
    }
                            ...
                            const char *section_name)
{
         ...
         ptr_server = ptr_server->next_server)
    {
        if (!ptr_server->temp_server || irc_config_write_temp_servers)
        {
            for (i = 0; i < IRC_SERVER_NUM_OPTIONS; i++)
            {
                if (!dogechat_config_write_option (config_file,
...
irc_config_init ()
{
    ...
    irc_config_file = dogechat_config_new (IRC_CONFIG_NAME,
                                          &irc_config_reload, NULL);
    if (!irc_config_file)
        return 0;

    /* look */
    ptr_section = dogechat_config_new_section (irc_config_file, "look",
                                              0, 0,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL);
    if (!ptr_section)
    {
        dogechat_config_free (irc_config_file);
        return 0;
    }

    irc_config_look_buffer_open_before_autojoin = dogechat_config_new_option (
        ...
        NULL, NULL, NULL, NULL);
    irc_config_look_buffer_open_before_join = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, NULL, NULL, NULL);
    irc_config_look_buffer_switch_autojoin = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_buffer_switch_join = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_color_nicks_in_names = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, 0, 0, "off", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_color_nicks_in_nicklist = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_color_nicks_in_server_messages = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "color_nicks_in_server_messages", "boolean",
        N_("use nick color in messages from server"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_color_pv_nick_like_channel = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "color_pv_nick_like_channel", "boolean",
        N_("use same nick color for channel and private"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_ctcp_time_format = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_display_away = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, NULL);
    irc_config_look_display_ctcp_blocked = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "display_ctcp_blocked", "boolean",
        N_("display CTCP message even if it is blocked"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_display_ctcp_reply = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "display_ctcp_reply", "boolean",
        N_("display CTCP reply sent by DogeChat"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_display_ctcp_unknown = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "display_ctcp_unknown", "boolean",
        N_("display CTCP message even if it is unknown CTCP"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_display_host_join = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "display_host_join", "boolean",
        N_("display host in join messages"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_display_host_join_local = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "display_host_join_local", "boolean",
        N_("display host in join messages from local client"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_display_host_quit = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "display_host_quit", "boolean",
        N_("display host in part/quit messages"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_display_join_message = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_highlight_pv = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "highlight_pv", "string",
        N_("comma separated list of words to highlight in private buffers "
           ...
           "(it does not affect current buffers), an empty string disables "
            "default highlight on nick, examples: \"$nick\", \"(?-i)$nick\""),
        NULL, 0, 0, "$nick", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_look_highlight_server = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        N_("comma separated list of words to highlight in server buffers "
           ...
           "default highlight on nick, examples: \"$nick\", \"(?-i)$nick\""),
        NULL, 0, 0, "$nick", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_look_highlight_tags_restrict = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_item_away_message = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_item_channel_modes_hide_args = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_item_display_server = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_item_nick_modes = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_item_nick_prefix = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "item_nick_prefix", "boolean",
        N_("display nick prefix in bar item \"input_prompt\""),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL,
        &irc_config_change_bar_item_input_prompt, NULL, NULL, NULL);
    irc_config_look_join_auto_add_chantype = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        N_("automatically add channel type in front of channel name on "
           ...
           "channel type for the server; for example: \"/join dogechat\" will "
           "in fact send: \"/join #dogechat\""),
        NULL, 0, 0, "off", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_msgbuffer_fallback = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, NULL, NULL, NULL);
    irc_config_look_new_channel_position = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_new_pv_position = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        "none|next|near_server", 0, 0, "none",
        NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_look_nick_color_force = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_nick_color_hash = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_nick_color_stop_chars = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        &irc_config_change_look_nick_colors, NULL, NULL, NULL);
    irc_config_look_nick_completion_smart = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, NULL, NULL, NULL);
    irc_config_look_nick_mode = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "nick_mode", "integer",
        ...
        NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_look_nick_mode_empty = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, 0, 0, "off", NULL, 0, NULL, NULL,
        &irc_config_change_bar_item_input_prompt, NULL, NULL, NULL);
    irc_config_look_nicks_hide_password = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_notice_as_pv = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_notice_welcome_redirect = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, 0, 0, "on", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_look_notice_welcome_tags = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, 0, 0, "", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_notify_tags_ison = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, NULL, NULL, NULL);
    irc_config_look_notify_tags_whois = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, 0, 0, "notify_message", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_look_part_closes_buffer = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "part_closes_buffer", "boolean",
        N_("close buffer when /part is issued on a channel"),
        NULL, 0, 0, "off", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_look_pv_buffer = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "pv_buffer", "integer",
    ...
    irc_config_look_pv_tags = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "pv_tags", "string",
        ...
        NULL, NULL, NULL, NULL);
    irc_config_look_raw_messages = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_server_buffer = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_smart_filter = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        N_("filter join/part/quit/nick messages for a nick if not speaking "
           ...
           "\"irc_smart_filter\")"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_look_smart_filter_delay = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_smart_filter_join = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "smart_filter_join", "boolean",
        /* TRANSLATORS: please do not translate "join" */
        N_("enable smart filter for \"join\" messages"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_look_smart_filter_join_unmask = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_look_smart_filter_mode = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    /* color */
    ptr_section = dogechat_config_new_section (irc_config_file, "color",
                                              0, 0,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL);
    ...
    irc_config_color_input_nick = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "input_nick", "color",
        N_("color for nick in input bar"),
        NULL, -1, 0, "lightcyan", NULL, 0, NULL, NULL,
        &irc_config_change_bar_item_input_prompt, NULL, NULL, NULL);
    irc_config_color_item_away = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "item_away", "color",
        N_("color for away item"),
        NULL, -1, 0, "yellow", NULL, 0, NULL, NULL,
        &irc_config_change_color_item_away, NULL, NULL, NULL);
    irc_config_color_item_channel_modes = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_color_item_lag_counting = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_color_item_lag_finished = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "item_lag_finished", "color",
        N_("color for lag indicator, when pong has been received from server"),
        NULL, -1, 0, "yellow", NULL, 0, NULL, NULL,
        &irc_config_change_color_item_lag, NULL, NULL, NULL);
    irc_config_color_item_nick_modes = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_color_message_join = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "message_join", "color",
        N_("color for text in join messages"),
        NULL, -1, 0, "green", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_color_message_quit = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "message_quit", "color",
        N_("color for text in part/quit messages"),
        NULL, -1, 0, "red", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_color_mirc_remap = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "mirc_remap", "string",
        /* TRANSLATORS: please do not translate the list of DogeChat color names at the end of string */
        N_("remap mirc colors in messages using a hashtable: keys are \"fg,bg\" "
           "as integers between -1 (not specified) and 15, values are DogeChat "
           ...
           "DogeChat colors for IRC codes: 0=white, 1=black, 2=blue, 3=green, "
    ...
    irc_config_color_nick_prefixes = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        N_("color for nick prefixes using mode char (o=op, h=halfop, v=voice, "
           ...
           "found, DogeChat will try with next modes received from server "
    ...
    irc_config_color_notice = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "notice", "color",
        N_("color for text \"Notice\" in notices"),
        NULL, -1, 0, "green", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_color_reason_quit = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "reason_quit", "color",
        N_("color for reason in part/quit messages"),
        NULL, -1, 0, "default", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_color_topic_current = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "topic_current", "color",
        ...
        NULL, -1, 0, "default", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_color_topic_new = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "topic_new", "color",
        N_("color for new channel topic (when topic is changed)"),
        NULL, -1, 0, "white", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_color_topic_old = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "topic_old", "color",
    ...
    /* network */
    ptr_section = dogechat_config_new_section (irc_config_file, "network",
                                              0, 0,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL);
    ...
    irc_config_network_autoreconnect_delay_growing = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, NULL, NULL, NULL);
    irc_config_network_autoreconnect_delay_max = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, NULL, NULL, NULL);
    irc_config_network_ban_mask_default = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        N_("default ban mask for commands /ban, /unban and /kickban; variables "
           ...
           "$user does not start with \"~\", otherwise it is set to \"*\"; this "
            "default mask is used only if DogeChat knows the host for the nick"),
        NULL, 0, 0, "*!$ident@$host", NULL, 0, NULL, NULL,
        NULL, NULL, NULL, NULL);
    irc_config_network_channel_encode = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, 0, 0, "off", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_network_colors_receive = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "colors_receive", "boolean",
        N_("when off, colors codes are ignored in incoming messages"),
        NULL, 0, 0, "on", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_network_colors_send = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "colors_send", "boolean",
        ...
        NULL, 0, 0, "on", NULL, 0, NULL, NULL, NULL, NULL, NULL, NULL);
    irc_config_network_lag_check = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "lag_check", "integer",
    ...
    irc_config_network_lag_max = dogechat_config_new_option (
        irc_config_file, ptr_section,
        "lag_max", "integer",
        N_("maximum lag (in seconds): if this lag is reached, DogeChat will "
        ...
        NULL, NULL, NULL, NULL);
    irc_config_network_lag_min_show = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_network_lag_reconnect = dogechat_config_new_option (
        irc_config_file, ptr_section,
        ...
        NULL, NULL, NULL, NULL);
    irc_config_network_lag_refresh_interval = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_network_notify_check_ison = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_network_notify_check_whois = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_network_send_unknown_commands = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    irc_config_network_whois_double_nick = dogechat_config_new_option (
        irc_config_file, ptr_section,
    ...
    /* msgbuffer */
    ptr_section = dogechat_config_new_section (irc_config_file, "msgbuffer",
                                              1, 1,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL,
    ...
    /* CTCP */
    ptr_section = dogechat_config_new_section (irc_config_file, "ctcp",
                                              1, 1,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL,
    ...
    /* ignore */
    ptr_section = dogechat_config_new_section (irc_config_file, "ignore",
                                              0, 0,
    ...
    /* server_default */
    ptr_section = dogechat_config_new_section (irc_config_file, "server_default",
                                              0, 0,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL, NULL, NULL,
                                              NULL, NULL);
    ...
    /* server */
    ptr_section = dogechat_config_new_section (irc_config_file, "server",
                                              0, 0,
    ...
    irc_config_hook_config_nick_colors = dogechat_hook_config (
        "dogechat.color.chat_nick_colors",
...
irc_config_read ()
{
    ...
    irc_config_loading = 1;
    rc = dogechat_config_read (irc_config_file);
    irc_config_loading = 0;
...
irc_config_write (int write_temp_servers)
{
    ...
    return dogechat_config_write (irc_config_file);
}
...
irc_config_free ()
{
    ...
    {
        dogechat_unhook (irc_config_hook_config_nick_colors);
    ...
    if (irc_config_hashtable_display_join_message)
    {
        dogechat_hashtable_free (irc_config_hashtable_display_join_message);
        irc_config_hashtable_display_join_message = NULL;
    }

    if (irc_config_hashtable_nick_color_force)
    {
        dogechat_hashtable_free (irc_config_hashtable_nick_color_force);
        irc_config_hashtable_nick_color_force = NULL;
    }

    if (irc_config_hashtable_nick_prefixes)
    {
        dogechat_hashtable_free (irc_config_hashtable_nick_prefixes);
        irc_config_hashtable_nick_prefixes = NULL;
    }

    if (irc_config_hashtable_color_mirc_remap)
    {
        dogechat_hashtable_free (irc_config_hashtable_color_mirc_remap);
