/*
 * dogechat-python-api.c - python API functions
 *
...
#include "dogechat-python.h"


#define API_DEF_FUNC(__name)                                            \
    { #__name, &dogechat_python_api_##__name, METH_VARARGS, "" }
#define API_FUNC(__name)                                                \
    static PyObject *                                                   \
    dogechat_python_api_##__name (PyObject *self, PyObject *args)
#define API_INIT_FUNC(__init, __name, __ret)                            \
    ...
    {                                                                   \
        DOGECHAT_SCRIPT_MSG_NOT_INIT(PYTHON_CURRENT_SCRIPT_NAME,         \
                                    python_function_name);              \
        __ret;                                                          \
    }
#define API_WRONG_ARGS(__ret)                                           \
    {                                                                   \
        DOGECHAT_SCRIPT_MSG_WRONG_ARGS(PYTHON_CURRENT_SCRIPT_NAME,       \
                                      python_function_name);            \
        __ret;                                                          \
    }
...
#define API_STR2PTR(__string)                                           \
    plugin_script_str2ptr (dogechat_python_plugin,                       \
...
API_FUNC(register)
{
    ...
    if (python_registered_script)
    {
        ...
        dogechat_printf (NULL,
                        ...
                        dogechat_prefix ("error"), PYTHON_PLUGIN_NAME,
    ...
    if (plugin_script_search (dogechat_python_plugin, python_scripts, name))
    {
        ...
        dogechat_printf (NULL,
                        ...
                        dogechat_prefix ("error"), PYTHON_PLUGIN_NAME, name);
        API_RETURN_ERROR;
    }

    /* register script */
    python_current_script = plugin_script_add (dogechat_python_plugin,
    ...
    if (python_current_script)
    {
        ...
        if ((dogechat_python_plugin->debug >= 2) || !python_quiet)
        {
            dogechat_printf (NULL,
...
API_FUNC(string_eval_expression)
{
    ...
    if (!PyArg_ParseTuple (args, "sOOO", &expr, &dict, &dict2, &dict3))
        API_WRONG_ARGS(API_RETURN_EMPTY);
    pointers = dogechat_python_dict_to_hashtable (dict,
                                                 DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                 DOGECHAT_HASHTABLE_STRING,
                                                 DOGECHAT_HASHTABLE_POINTER);
    extra_vars = dogechat_python_dict_to_hashtable (dict2,
                                                   DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                   DOGECHAT_HASHTABLE_STRING,
                                                   DOGECHAT_HASHTABLE_STRING);
    options = dogechat_python_dict_to_hashtable (dict3,
                                                DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                DOGECHAT_HASHTABLE_STRING,
                                                DOGECHAT_HASHTABLE_STRING);
...
API_FUNC(list_add)
{
    char *dogelist, *data, *where, *user_data, *result;
    PyObject *return_value;
    ...
    where = NULL;
    user_data = NULL;
    if (!PyArg_ParseTuple (args, "ssss", &dogelist, &data, &where, &user_data))
        API_WRONG_ARGS(API_RETURN_EMPTY);
...
{
    char *dogelist, *data, *result;
    PyObject *return_value;

    API_INIT_FUNC(1, "list_search", API_RETURN_EMPTY);
    dogelist = NULL;
    data = NULL;
    if (!PyArg_ParseTuple (args, "ss", &dogelist, &data))
        API_WRONG_ARGS(API_RETURN_EMPTY);
...
API_FUNC(list_get)
{
    ...
    dogelist = NULL;
    position = 0;
    if (!PyArg_ParseTuple (args, "si", &dogelist, &position))
        API_WRONG_ARGS(API_RETURN_EMPTY);
...
API_FUNC(list_size)
{
    ...
    dogelist = NULL;
    if (!PyArg_ParseTuple (args, "s", &dogelist))
        API_WRONG_ARGS(API_RETURN_INT(0));
...
API_FUNC(list_remove)
{
    ...
    dogelist = NULL;
    item = NULL;
    if (!PyArg_ParseTuple (args, "ss", &dogelist, &item))
        API_WRONG_ARGS(API_RETURN_ERROR);
...
int
dogechat_python_api_config_reload_cb (void *data,
                                     struct t_config_file *config_file)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        rc = (int *) dogechat_python_exec (script_callback->script,
...
API_FUNC(config_new)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new (dogechat_python_plugin,
                                                       python_current_script,
                                                       name,
                                                       &dogechat_python_api_config_reload_cb,
                                                       function,
                                                       data));
...
int
dogechat_python_api_config_read_cb (void *data,
                                   struct t_config_file *config_file,
...
int
dogechat_python_api_config_section_write_cb (void *data,
                                            struct t_config_file *config_file,
...
int
dogechat_python_api_config_section_write_default_cb (void *data,
                                                    struct t_config_file *config_file,
                                                    const char *section_name)
{
    struct t_plugin_script_cb *script_callback;
...
int
dogechat_python_api_config_section_create_option_cb (void *data,
                                                    struct t_config_file *config_file,
                                                    struct t_config_section *section,
...
int
dogechat_python_api_config_section_delete_option_cb (void *data,
                                                    struct t_config_file *config_file,
                                                    struct t_config_section *section,
...
API_FUNC(config_new_section)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new_section (dogechat_python_plugin,
                                                               python_current_script,
                                                               API_STR2PTR(config_file),
                                                               name,
                                                               user_can_add_options,
                                                               user_can_delete_options,
                                                               &dogechat_python_api_config_read_cb,
                                                               function_read,
                                                               data_read,
                                                               &dogechat_python_api_config_section_write_cb,
                                                               function_write,
                                                               data_write,
                                                               &dogechat_python_api_config_section_write_default_cb,
                                                               function_write_default,
                                                               data_write_default,
                                                               &dogechat_python_api_config_section_create_option_cb,
                                                               function_create_option,
                                                               data_create_option,
                                                               &dogechat_python_api_config_section_delete_option_cb,
...
int
dogechat_python_api_config_option_check_value_cb (void *data,
                                                 struct t_config_option *option,
                                                 const char *value)
{
    struct t_plugin_script_cb *script_callback;
...
void
dogechat_python_api_config_option_change_cb (void *data,
                                            struct t_config_option *option)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
void
dogechat_python_api_config_option_delete_cb (void *data,
                                            struct t_config_option *option)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
API_FUNC(config_new_option)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new_option (dogechat_python_plugin,
                                                              python_current_script,
                                                              API_STR2PTR(config_file),
                                                              API_STR2PTR(section),
                                                              name,
                                                              ...
                                                              value,
                                                              null_value_allowed,
                                                              &dogechat_python_api_config_option_check_value_cb,
                                                              function_check_value,
                                                              data_check_value,
                                                              &dogechat_python_api_config_option_change_cb,
                                                              function_change,
                                                              data_change,
                                                              &dogechat_python_api_config_option_delete_cb,
                                                              function_delete,
                                                              data_delete));
...
API_FUNC(config_option_free)
{
    ...
    plugin_script_api_config_option_free (dogechat_python_plugin,
                                          python_current_script,
                                          API_STR2PTR(option));
...
API_FUNC(config_section_free_options)
{
    ...
    plugin_script_api_config_section_free_options (dogechat_python_plugin,
                                                   python_current_script,
                                                   API_STR2PTR(section));
...
API_FUNC(config_section_free)
{
    ...
    plugin_script_api_config_section_free (dogechat_python_plugin,
                                           python_current_script,
                                           API_STR2PTR(section));
...
API_FUNC(config_free)
{
    ...
    plugin_script_api_config_free (dogechat_python_plugin,
                                   python_current_script,
                                   API_STR2PTR(config_file));
...
API_FUNC(config_get_plugin)
{
    ...
    result = plugin_script_api_config_get_plugin (dogechat_python_plugin,
                                                  python_current_script,
                                                  option);
...
API_FUNC(config_is_set_plugin)
{
    ...
    rc = plugin_script_api_config_is_set_plugin (dogechat_python_plugin,
                                                 python_current_script,
                                                 option);
...
API_FUNC(config_set_plugin)
{
    ...
    rc = plugin_script_api_config_set_plugin (dogechat_python_plugin,
                                              python_current_script,
                                              option,
                                              value);
...
API_FUNC(config_set_desc_plugin)
{
    ...
    plugin_script_api_config_set_desc_plugin (dogechat_python_plugin,
                                              python_current_script,
                                              option,
                                              description);
...
API_FUNC(config_unset_plugin)
{
    ...
    rc = plugin_script_api_config_unset_plugin (dogechat_python_plugin,
                                                python_current_script,
                                                option);
...
API_FUNC(key_bind)
{
    ...
    hashtable = dogechat_python_dict_to_hashtable (dict,
                                                  DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                  DOGECHAT_HASHTABLE_STRING,
                                                  DOGECHAT_HASHTABLE_STRING);
...
API_FUNC(prnt)
{
    ...
    plugin_script_api_printf (dogechat_python_plugin,
                              python_current_script,
                              API_STR2PTR(buffer),
                              "%s", message);
...
API_FUNC(prnt_date_tags)
{
    ...
    plugin_script_api_printf_date_tags (dogechat_python_plugin,
                                        python_current_script,
                                        API_STR2PTR(buffer),
                                        date,
                                        tags,
...
API_FUNC(prnt_y)
{
    ...
    plugin_script_api_printf_y (dogechat_python_plugin,
                                python_current_script,
                                API_STR2PTR(buffer),
                                y,
                                "%s", message);
...
API_FUNC(log_print)
{
    ...
    plugin_script_api_log_printf (dogechat_python_plugin,
                                  python_current_script,
                                  "%s", message);
...
int
dogechat_python_api_hook_command_cb (void *data, struct t_gui_buffer *buffer,
...
API_FUNC(hook_command)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_command (dogechat_python_plugin,
                                                         python_current_script,
                                                         command,
                                                         description,
                                                         arguments,
                                                         args_description,
                                                         completion,
                                                         &dogechat_python_api_hook_command_cb,
                                                         function,
                                                         data));
...
int
dogechat_python_api_hook_command_run_cb (void *data, struct t_gui_buffer *buffer,
                                        const char *command)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
...
API_FUNC(hook_command_run)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_command_run (dogechat_python_plugin,
                                                             python_current_script,
                                                             command,
                                                             &dogechat_python_api_hook_command_run_cb,
                                                             function,
                                                             data));
...
int
dogechat_python_api_hook_timer_cb (void *data, int remaining_calls)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
API_FUNC(hook_timer)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_timer (dogechat_python_plugin,
                                                       python_current_script,
                                                       interval,
                                                       align_second,
                                                       max_calls,
                                                       &dogechat_python_api_hook_timer_cb,
                                                       function,
                                                       data));
...
int
dogechat_python_api_hook_fd_cb (void *data, int fd)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
API_FUNC(hook_fd)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_fd (dogechat_python_plugin,
                                                    python_current_script,
                                                    fd,
                                                    read,
                                                    write,
                                                    exception,
                                                    &dogechat_python_api_hook_fd_cb,
                                                    function,
                                                    data));
...
int
dogechat_python_api_hook_process_cb (void *data,
...
API_FUNC(hook_process)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_process (dogechat_python_plugin,
                                                         python_current_script,
                                                         command,
                                                         timeout,
                                                         &dogechat_python_api_hook_process_cb,
                                                         function,
                                                         data));
...
API_FUNC(hook_process_hashtable)
{
    ...
    if (!PyArg_ParseTuple (args, "sOiss", &command, &dict, &timeout, &function,
                           &data))
        API_WRONG_ARGS(API_RETURN_EMPTY);
    options = dogechat_python_dict_to_hashtable (dict,
                                                DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                                DOGECHAT_HASHTABLE_STRING,
                                                DOGECHAT_HASHTABLE_STRING);

    result = API_PTR2STR(plugin_script_api_hook_process_hashtable (dogechat_python_plugin,
                                                                   python_current_script,
                                                                   command,
                                                                   options,
                                                                   timeout,
...
int
dogechat_python_api_hook_connect_cb (void *data, int status, int gnutls_rc,
...
API_FUNC(hook_connect)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_connect (dogechat_python_plugin,
                                                         python_current_script,
                                                         proxy,
                                                         address,
                                                         port,
                                                         ...
                                                         local_hostname,
                                                         &dogechat_python_api_hook_connect_cb,
                                                         function,
                                                         data));
...
int
dogechat_python_api_hook_print_cb (void *data, struct t_gui_buffer *buffer,
                                  time_t date,
...
API_FUNC(hook_print)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_print (dogechat_python_plugin,
                                                       python_current_script,
                                                       API_STR2PTR(buffer),
                                                       tags,
                                                       message,
                                                       strip_colors,
                                                       &dogechat_python_api_hook_print_cb,
                                                       function,
                                                       data));
...
int
dogechat_python_api_hook_signal_cb (void *data, const char *signal, const char *type_data,
                                   void *signal_data)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
...
API_FUNC(hook_signal)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_signal (dogechat_python_plugin,
                                                        python_current_script,
                                                        signal,
                                                        &dogechat_python_api_hook_signal_cb,
                                                        function,
                                                        data));
...
API_FUNC(hook_signal_send)
{
    ...
    if (strcmp (type_data, DOGECHAT_HOOK_SIGNAL_STRING) == 0)
    {
        rc = dogechat_hook_signal_send (signal, type_data, signal_data);
        API_RETURN_INT(rc);
    }
    else if (strcmp (type_data, DOGECHAT_HOOK_SIGNAL_INT) == 0)
    {
...
int
dogechat_python_api_hook_hsignal_cb (void *data, const char *signal,
                                    struct t_hashtable *hashtable)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        func_argv[0] = (script_callback->data) ? script_callback->data : empty_arg;
        func_argv[1] = (signal) ? (char *)signal : empty_arg;
        func_argv[2] = dogechat_python_hashtable_to_dict (hashtable);
...
API_FUNC(hook_hsignal)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_hsignal (dogechat_python_plugin,
                                                         python_current_script,
                                                         signal,
                                                         &dogechat_python_api_hook_hsignal_cb,
                                                         function,
                                                         data));
...
int
dogechat_python_api_hook_config_cb (void *data, const char *option, const char *value)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
    char empty_arg[1] = { '\0' };
...
API_FUNC(hook_config)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_config (dogechat_python_plugin,
                                                        python_current_script,
                                                        option,
                                                        &dogechat_python_api_hook_config_cb,
                                                        function,
                                                        data));
...
int
dogechat_python_api_hook_completion_cb (void *data, const char *completion_item,
...
API_FUNC(hook_completion)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_completion (dogechat_python_plugin,
                                                            python_current_script,
                                                            completion,
                                                            description,
                                                            &dogechat_python_api_hook_completion_cb,
                                                            function,
                                                            data));
...
char *
dogechat_python_api_hook_modifier_cb (void *data, const char *modifier,
                                     const char *modifier_data, const char *string)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (char *)dogechat_python_exec (script_callback->script,
...
API_FUNC(hook_modifier)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_modifier (dogechat_python_plugin,
                                                          python_current_script,
                                                          modifier,
                                                          &dogechat_python_api_hook_modifier_cb,
                                                          function,
                                                          data));
...
const char *
dogechat_python_api_hook_info_cb (void *data, const char *info_name,
                                 const char *arguments)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (const char *)dogechat_python_exec (script_callback->script,
                                                  DOGECHAT_SCRIPT_EXEC_STRING,
                                                  script_callback->function,
                                                  "sss", func_argv);
    }
...
API_FUNC(hook_info)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_info (dogechat_python_plugin,
                                                      python_current_script,
                                                      info_name,
                                                      description,
                                                      args_description,
                                                      &dogechat_python_api_hook_info_cb,
                                                      function,
                                                      data));
...
struct t_hashtable *
dogechat_python_api_hook_info_hashtable_cb (void *data, const char *info_name,
                                           struct t_hashtable *hashtable)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        ret_hashtable = dogechat_python_exec (script_callback->script,
...
API_FUNC(hook_info_hashtable)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_info_hashtable (dogechat_python_plugin,
                                                                python_current_script,
                                                                info_name,
                                                                description,
                                                                args_description,
                                                                output_description,
                                                                &dogechat_python_api_hook_info_hashtable_cb,
                                                                function,
                                                                data));
...
struct t_infolist *
dogechat_python_api_hook_infolist_cb (void *data, const char *infolist_name,
                                     void *pointer, const char *arguments)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        result = (struct t_infolist *)dogechat_python_exec (script_callback->script,
                                                           DOGECHAT_SCRIPT_EXEC_STRING,
                                                           script_callback->function,
                                                           "ssss", func_argv);
...
API_FUNC(hook_infolist)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_infolist (dogechat_python_plugin,
                                                          python_current_script,
                                                          infolist_name,
                                                          description,
                                                          pointer_description,
                                                          args_description,
                                                          &dogechat_python_api_hook_infolist_cb,
                                                          function,
                                                          data));
...
struct t_hashtable *
dogechat_python_api_hook_focus_cb (void *data,
                                  struct t_hashtable *info)
{
    ...
    {
        func_argv[0] = (script_callback->data) ? script_callback->data : empty_arg;
        func_argv[1] = dogechat_python_hashtable_to_dict (info);
...
API_FUNC(hook_focus)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_focus (dogechat_python_plugin,
                                                       python_current_script,
                                                       area,
                                                       &dogechat_python_api_hook_focus_cb,
                                                       function,
                                                       data));
...
API_FUNC(hook_set)
{
    ...
    dogechat_hook_set (API_STR2PTR(hook),
                      property,
                      value);
...
API_FUNC(unhook)
{
    ...
    plugin_script_api_unhook (dogechat_python_plugin,
                              python_current_script,
                              API_STR2PTR(hook));
...
API_FUNC(unhook_all)
{
    ...
    plugin_script_api_unhook_all (dogechat_python_plugin, python_current_script);

    API_RETURN_OK;
}

int
dogechat_python_api_buffer_input_data_cb (void *data, struct t_gui_buffer *buffer,
...
int
dogechat_python_api_buffer_close_cb (void *data, struct t_gui_buffer *buffer)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
    char empty_arg[1] = { '\0' };
...
API_FUNC(buffer_new)
{
    ...
    result = API_PTR2STR(plugin_script_api_buffer_new (dogechat_python_plugin,
                                                       python_current_script,
                                                       name,
                                                       &dogechat_python_api_buffer_input_data_cb,
                                                       function_input,
                                                       data_input,
                                                       &dogechat_python_api_buffer_close_cb,
                                                       function_close,
                                                       data_close));
...
API_FUNC(buffer_close)
{
    ...
    plugin_script_api_buffer_close (dogechat_python_plugin,
                                    python_current_script,
                                    API_STR2PTR(buffer));
...
API_FUNC(buffer_set)
{
    ...
    dogechat_buffer_set (API_STR2PTR(buffer),
                        property,
                        value);
...
char *
dogechat_python_api_bar_item_build_cb (void *data, struct t_gui_bar_item *item,
                                      ...
                                      struct t_hashtable *extra_info)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        if (strncmp (script_callback->function, "(extra)", 7) == 0)
        {
            ...
            func_argv[4] = dogechat_python_hashtable_to_dict (extra_info);

            ret = (char *)dogechat_python_exec (script_callback->script,
                                               DOGECHAT_SCRIPT_EXEC_STRING,
...
API_FUNC(bar_item_new)
{
    ...
    result = API_PTR2STR(plugin_script_api_bar_item_new (dogechat_python_plugin,
                                                         python_current_script,
                                                         name,
                                                         &dogechat_python_api_bar_item_build_cb,
                                                         function,
                                                         data));
...
API_FUNC(bar_item_remove)
{
    ...
    plugin_script_api_bar_item_remove (dogechat_python_plugin,
                                       python_current_script,
                                       API_STR2PTR(item));
...
API_FUNC(command)
{
    ...
    rc = plugin_script_api_command (dogechat_python_plugin,
                                    python_current_script,
                                    API_STR2PTR(buffer),
                                    command);
...
API_FUNC(info_get_hashtable)
{
    ...
    result_dict = dogechat_python_hashtable_to_dict (result_hashtable);
...
API_FUNC(hdata_hashtable)
{
    ...
    result_dict = dogechat_python_hashtable_to_dict (
...
int
dogechat_python_api_upgrade_read_cb (void *data,
...
API_FUNC(upgrade_read)
{
    ...
    rc = plugin_script_api_upgrade_read (dogechat_python_plugin,
                                         python_current_script,
                                         API_STR2PTR(upgrade_file),
                                         &dogechat_python_api_upgrade_read_cb,
                                         function,
                                         data);
...
PyMethodDef dogechat_python_funcs[] =
{
