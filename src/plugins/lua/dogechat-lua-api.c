/*
 * dogechat-lua-api.c - lua API functions
 *
...
#define API_DEF_FUNC(__name)                                            \
    { #__name, &dogechat_lua_api_##__name }
#define API_FUNC(__name)                                                \
    static int                                                          \
    dogechat_lua_api_##__name (lua_State *L)
#define API_INIT_FUNC(__init, __name, __ret)                            \
    ...
    {                                                                   \
        DOGECHAT_SCRIPT_MSG_NOT_INIT(LUA_CURRENT_SCRIPT_NAME,            \
                                    lua_function_name);                 \
        __ret;                                                          \
    }
#define API_WRONG_ARGS(__ret)                                           \
    {                                                                   \
        DOGECHAT_SCRIPT_MSG_WRONG_ARGS(LUA_CURRENT_SCRIPT_NAME,          \
                                      lua_function_name);               \
        __ret;                                                          \
    }
...
#define API_STR2PTR(__string)                                           \
    plugin_script_str2ptr (dogechat_lua_plugin,                          \
...
API_FUNC(register)
{
    ...
    if (plugin_script_search (dogechat_lua_plugin, lua_scripts, name))
    {
    ...
    /* register script */
    lua_current_script = plugin_script_add (dogechat_lua_plugin,
...
API_FUNC(gettext)
{
    ...
    result = dogechat_gettext (string);
...
API_FUNC(strlen_screen)
{
    ...
    value = dogechat_strlen_screen (string);
...
API_FUNC(string_match)
{
    ...
    value = dogechat_string_match (string, mask, case_sensitive);
...
API_FUNC(string_has_highlight)
{
    ...
    value = dogechat_string_has_highlight (string, highlight_words);
...
API_FUNC(string_has_highlight_regex)
{
    ...
    value = dogechat_string_has_highlight_regex (string, regex);
...
API_FUNC(string_is_command_char)
{
    ...
    value = dogechat_string_is_command_char (string);
...
API_FUNC(string_input_for_buffer)
{
    ...
    result = dogechat_string_input_for_buffer (string);
...
API_FUNC(string_eval_expression)
{
    ...
    pointers = dogechat_lua_tohashtable (L, -3,
                                        DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                        DOGECHAT_HASHTABLE_STRING,
                                        DOGECHAT_HASHTABLE_POINTER);
    extra_vars = dogechat_lua_tohashtable (L, -2,
                                          DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                          DOGECHAT_HASHTABLE_STRING,
                                          DOGECHAT_HASHTABLE_STRING);
    options = dogechat_lua_tohashtable (L, -1,
                                       DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                       DOGECHAT_HASHTABLE_STRING,
                                       DOGECHAT_HASHTABLE_STRING);
...
API_FUNC(mkdir_home)
{
    ...
    if (dogechat_mkdir_home (directory, mode))
        API_RETURN_OK;
...
API_FUNC(mkdir)
{
    ...
    if (dogechat_mkdir (directory, mode))
        API_RETURN_OK;
...
API_FUNC(mkdir_parents)
{
    ...
    if (dogechat_mkdir_parents (directory, mode))
        API_RETURN_OK;
...
API_FUNC(list_add)
{
    const char *dogelist, *data, *where, *user_data;
    char *result;
...
API_FUNC(list_get)
{
    ...
    result = API_PTR2STR(dogechat_list_get (API_STR2PTR(dogelist),
                                           position));
...
API_FUNC(list_set)
{
    ...
    dogechat_list_set (API_STR2PTR(item),
                      new_value);
...
API_FUNC(list_next)
{
    ...
    result = API_PTR2STR(dogechat_list_next (API_STR2PTR(item)));
...
API_FUNC(list_prev)
{
    ...
    result = API_PTR2STR(dogechat_list_prev (API_STR2PTR(item)));
...
API_FUNC(list_string)
{
    ...
    result = dogechat_list_string (API_STR2PTR(item));
...
API_FUNC(list_size)
{
    ...
    dogelist = lua_tostring (L, -1);

    size = dogechat_list_size (API_STR2PTR(dogelist));
...
API_FUNC(list_remove_all)
{
    ...
    dogechat_list_remove_all (API_STR2PTR(dogelist));
...
API_FUNC(list_free)
{
    ...
    dogechat_list_free (API_STR2PTR(dogelist));

    API_RETURN_OK;
}

int
dogechat_lua_api_config_reload_cb (void *data,
                                  struct t_config_file *config_file)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        rc = (int *) dogechat_lua_exec (script_callback->script,
...
API_FUNC(config_new)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new (dogechat_lua_plugin,
                                                       lua_current_script,
                                                       name,
                                                       &dogechat_lua_api_config_reload_cb,
                                                       function,
                                                       data));
...
int
dogechat_lua_api_config_read_cb (void *data,
                                struct t_config_file *config_file,
...
int
dogechat_lua_api_config_section_write_cb (void *data,
                                         struct t_config_file *config_file,
...
int
dogechat_lua_api_config_section_write_default_cb (void *data,
                                                 struct t_config_file *config_file,
                                                 const char *section_name)
{
    struct t_plugin_script_cb *script_callback;
...
int
dogechat_lua_api_config_section_create_option_cb (void *data,
                                                 struct t_config_file *config_file,
                                                 struct t_config_section *section,
...
int
dogechat_lua_api_config_section_delete_option_cb (void *data,
                                                 struct t_config_file *config_file,
                                                 struct t_config_section *section,
...
API_FUNC(config_new_section)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new_section (dogechat_lua_plugin,
                                                               lua_current_script,
                                                               API_STR2PTR(config_file),
                                                               name,
                                                               user_can_add_options,
                                                               user_can_delete_options,
                                                               &dogechat_lua_api_config_read_cb,
                                                               function_read,
                                                               data_read,
                                                               &dogechat_lua_api_config_section_write_cb,
                                                               function_write,
                                                               data_write,
                                                               &dogechat_lua_api_config_section_write_default_cb,
                                                               function_write_default,
                                                               data_write_default,
                                                               &dogechat_lua_api_config_section_create_option_cb,
                                                               function_create_option,
                                                               data_create_option,
                                                               &dogechat_lua_api_config_section_delete_option_cb,
...
int
dogechat_lua_api_config_option_check_value_cb (void *data,
                                              struct t_config_option *option,
                                              const char *value)
{
    struct t_plugin_script_cb *script_callback;
...
void
dogechat_lua_api_config_option_change_cb (void *data,
                                         struct t_config_option *option)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
void
dogechat_lua_api_config_option_delete_cb (void *data,
                                         struct t_config_option *option)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
API_FUNC(config_new_option)
{
    ...
    result = API_PTR2STR(plugin_script_api_config_new_option (dogechat_lua_plugin,
                                                              lua_current_script,
                                                              API_STR2PTR(config_file),
                                                              API_STR2PTR(section),
                                                              name,
                                                              ...
                                                              value,
                                                              null_value_allowed,
                                                              &dogechat_lua_api_config_option_check_value_cb,
                                                              function_check_value,
                                                              data_check_value,
                                                              &dogechat_lua_api_config_option_change_cb,
                                                              function_change,
                                                              data_change,
                                                              &dogechat_lua_api_config_option_delete_cb,
                                                              function_delete,
                                                              data_delete));
...
API_FUNC(config_string_to_boolean)
{
    ...
    value = dogechat_config_string_to_boolean (text);
...
API_FUNC(config_option_is_null)
{
    ...
    value = dogechat_config_option_is_null (API_STR2PTR(option));
...
API_FUNC(config_option_default_is_null)
{
    ...
    value = dogechat_config_option_default_is_null (API_STR2PTR(option));
...
API_FUNC(config_boolean)
{
    ...
    value = dogechat_config_boolean (API_STR2PTR(option));
...
API_FUNC(config_boolean_default)
{
    ...
    value = dogechat_config_boolean_default (API_STR2PTR(option));
...
API_FUNC(config_integer)
{
    ...
    value = dogechat_config_integer (API_STR2PTR(option));
...
API_FUNC(config_integer_default)
{
    ...
    value = dogechat_config_integer_default (API_STR2PTR(option));
...
API_FUNC(config_string)
{
    ...
    result = dogechat_config_string (API_STR2PTR(option));
...
API_FUNC(config_string_default)
{
    ...
    result = dogechat_config_string_default (API_STR2PTR(option));
...
API_FUNC(config_color)
{
    ...
    result = dogechat_config_color (API_STR2PTR(option));
...
API_FUNC(config_color_default)
{
    ...
    result = dogechat_config_color_default (API_STR2PTR(option));
...
API_FUNC(config_write_line)
{
    ...
    dogechat_config_write_line (API_STR2PTR(config_file),
                               option_name,
                               "%s",
                               value);
...
API_FUNC(config_write)
{
    ...
    rc = dogechat_config_write (API_STR2PTR(config_file));
...
API_FUNC(config_read)
{
    ...
    rc = dogechat_config_read (API_STR2PTR(config_file));
...
API_FUNC(config_reload)
{
    ...
    rc = dogechat_config_reload (API_STR2PTR(config_file));
...
API_FUNC(config_option_free)
{
    ...
    plugin_script_api_config_option_free (dogechat_lua_plugin,
                                          lua_current_script,
                                          API_STR2PTR(option));
...
API_FUNC(config_section_free_options)
{
    ...
    plugin_script_api_config_section_free_options (dogechat_lua_plugin,
                                                   lua_current_script,
                                                   API_STR2PTR(section));
...
API_FUNC(config_section_free)
{
    ...
    plugin_script_api_config_section_free (dogechat_lua_plugin,
                                           lua_current_script,
                                           API_STR2PTR(section));
...
API_FUNC(config_free)
{
    ...
    plugin_script_api_config_free (dogechat_lua_plugin,
                                   lua_current_script,
                                   API_STR2PTR(config_file));
...
API_FUNC(config_get)
{
    ...
    result = API_PTR2STR(dogechat_config_get (option));
...
API_FUNC(config_get_plugin)
{
    ...
    result = plugin_script_api_config_get_plugin (dogechat_lua_plugin,
                                                  lua_current_script,
                                                  option);
...
API_FUNC(config_is_set_plugin)
{
    ...
    rc = plugin_script_api_config_is_set_plugin (dogechat_lua_plugin,
                                                 lua_current_script,
                                                 option);
...
API_FUNC(config_set_plugin)
{
    ...
    rc = plugin_script_api_config_set_plugin (dogechat_lua_plugin,
                                              lua_current_script,
                                              option,
                                              value);
...
API_FUNC(config_set_desc_plugin)
{
    ...
    plugin_script_api_config_set_desc_plugin (dogechat_lua_plugin,
                                              lua_current_script,
                                              option,
                                              description);
...
API_FUNC(config_unset_plugin)
{
    ...
    rc = plugin_script_api_config_unset_plugin (dogechat_lua_plugin,
                                                lua_current_script,
                                                option);
...
API_FUNC(key_bind)
{
    ...
    hashtable = dogechat_lua_tohashtable (L, -1,
                                         DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                         DOGECHAT_HASHTABLE_STRING,
                                         DOGECHAT_HASHTABLE_STRING);
...
API_FUNC(print)
{
    ...
    plugin_script_api_printf (dogechat_lua_plugin,
                              lua_current_script,
                              API_STR2PTR(buffer),
                              "%s", message);
...
API_FUNC(print_date_tags)
{
    ...
    plugin_script_api_printf_date_tags (dogechat_lua_plugin,
                                        lua_current_script,
                                        API_STR2PTR(buffer),
                                        date,
                                        tags,
...
API_FUNC(print_y)
{
    ...
    plugin_script_api_printf_y (dogechat_lua_plugin,
                                lua_current_script,
                                API_STR2PTR(buffer),
                                y,
                                "%s", message);
...
API_FUNC(log_print)
{
    ...
    plugin_script_api_log_printf (dogechat_lua_plugin,
                                  lua_current_script,
                                  "%s", message);
...
int
dogechat_lua_api_hook_command_cb (void *data, struct t_gui_buffer *buffer,
...
API_FUNC(hook_command)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_command (dogechat_lua_plugin,
                                                         lua_current_script,
                                                         command,
                                                         description,
                                                         args,
                                                         args_description,
                                                         completion,
                                                         &dogechat_lua_api_hook_command_cb,
                                                         function,
                                                         data));
...
int
dogechat_lua_api_hook_command_run_cb (void *data, struct t_gui_buffer *buffer,
                                     const char *command)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
...
API_FUNC(hook_command_run)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_command_run (dogechat_lua_plugin,
                                                             lua_current_script,
                                                             command,
                                                             &dogechat_lua_api_hook_command_run_cb,
                                                             function,
                                                             data));
...
int
dogechat_lua_api_hook_timer_cb (void *data, int remaining_calls)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
API_FUNC(hook_timer)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_timer (dogechat_lua_plugin,
                                                       lua_current_script,
                                                       interval,
                                                       align_second,
                                                       max_calls,
                                                       &dogechat_lua_api_hook_timer_cb,
                                                       function,
                                                       data));
...
int
dogechat_lua_api_hook_fd_cb (void *data, int fd)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
...
API_FUNC(hook_fd)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_fd (dogechat_lua_plugin,
                                                    lua_current_script,
                                                    fd,
                                                    read,
                                                    write,
                                                    exception,
                                                    &dogechat_lua_api_hook_fd_cb,
                                                    function,
                                                    data));
...
int
dogechat_lua_api_hook_process_cb (void *data,
...
API_FUNC(hook_process)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_process (dogechat_lua_plugin,
                                                         lua_current_script,
                                                         command,
                                                         timeout,
                                                         &dogechat_lua_api_hook_process_cb,
                                                         function,
                                                         data));
...
API_FUNC(hook_process_hashtable)
{
    ...
    options = dogechat_lua_tohashtable (L, -4,
                                       DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                       DOGECHAT_HASHTABLE_STRING,
                                       DOGECHAT_HASHTABLE_STRING);
    timeout = lua_tonumber (L, -3);
    ...
    result = API_PTR2STR(plugin_script_api_hook_process_hashtable (dogechat_lua_plugin,
                                                                   lua_current_script,
                                                                   command,
                                                                   options,
                                                                   timeout,
...
int
dogechat_lua_api_hook_connect_cb (void *data, int status, int gnutls_rc,
...
API_FUNC(hook_connect)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_connect (dogechat_lua_plugin,
                                                         lua_current_script,
                                                         proxy,
                                                         address,
                                                         port,
                                                         ...
                                                         local_hostname,
                                                         &dogechat_lua_api_hook_connect_cb,
                                                         function,
                                                         data));
...
int
dogechat_lua_api_hook_print_cb (void *data, struct t_gui_buffer *buffer,
                               time_t date,
...
API_FUNC(hook_print)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_print (dogechat_lua_plugin,
                                                       lua_current_script,
                                                       API_STR2PTR(buffer),
                                                       tags,
                                                       message,
                                                       strip_colors,
                                                       &dogechat_lua_api_hook_print_cb,
                                                       function,
                                                       data));
...
int
dogechat_lua_api_hook_signal_cb (void *data, const char *signal,
...
API_FUNC(hook_signal)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_signal (dogechat_lua_plugin,
                                                        lua_current_script,
                                                        signal,
                                                        &dogechat_lua_api_hook_signal_cb,
                                                        function,
                                                        data));
...
API_FUNC(hook_signal_send)
{
    ...
    if (strcmp (type_data, DOGECHAT_HOOK_SIGNAL_STRING) == 0)
    {
        ...
        rc = dogechat_hook_signal_send (signal, type_data, (void *)signal_data);
        API_RETURN_INT(rc);
    }
    else if (strcmp (type_data, DOGECHAT_HOOK_SIGNAL_INT) == 0)
    {
        number = lua_tonumber (L, -1);
        rc = dogechat_hook_signal_send (signal, type_data, &number);
        API_RETURN_INT(rc);
    }
    else if (strcmp (type_data, DOGECHAT_HOOK_SIGNAL_POINTER) == 0)
    {
        signal_data = lua_tostring (L, -1);
        rc = dogechat_hook_signal_send (signal, type_data,
...
int
dogechat_lua_api_hook_hsignal_cb (void *data, const char *signal,
                                 struct t_hashtable *hashtable)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
...
API_FUNC(hook_hsignal)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_hsignal (dogechat_lua_plugin,
                                                         lua_current_script,
                                                         signal,
                                                         &dogechat_lua_api_hook_hsignal_cb,
                                                         function,
                                                         data));
...
int
dogechat_lua_api_hook_config_cb (void *data, const char *option,
                                const char *value)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
...
API_FUNC(hook_config)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_config (dogechat_lua_plugin,
                                                        lua_current_script,
                                                        option,
                                                        &dogechat_lua_api_hook_config_cb,
                                                        function,
                                                        data));
...
int
dogechat_lua_api_hook_completion_cb (void *data, const char *completion_item,
...
API_FUNC(hook_completion)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_completion (dogechat_lua_plugin,
                                                            lua_current_script,
                                                            completion,
                                                            description,
                                                            &dogechat_lua_api_hook_completion_cb,
                                                            function,
                                                            data));
...
char *
dogechat_lua_api_hook_modifier_cb (void *data, const char *modifier,
                                  ...
                                  const char *string)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (char *)dogechat_lua_exec (script_callback->script,
...
API_FUNC(hook_modifier)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_modifier (dogechat_lua_plugin,
                                                          lua_current_script,
                                                          modifier,
                                                          &dogechat_lua_api_hook_modifier_cb,
                                                          function,
                                                          data));
...
const char *
dogechat_lua_api_hook_info_cb (void *data, const char *info_name,
                              const char *arguments)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (const char *)dogechat_lua_exec (script_callback->script,
                                               DOGECHAT_SCRIPT_EXEC_STRING,
                                               script_callback->function,
                                               "sss", func_argv);
    }
...
API_FUNC(hook_info)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_info (dogechat_lua_plugin,
                                                      lua_current_script,
                                                      info_name,
                                                      description,
                                                      args_description,
                                                      &dogechat_lua_api_hook_info_cb,
                                                      function,
                                                      data));
...
struct t_hashtable *
dogechat_lua_api_hook_info_hashtable_cb (void *data, const char *info_name,
                                        struct t_hashtable *hashtable)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[3];
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        return (struct t_hashtable *)dogechat_lua_exec (script_callback->script,
...
API_FUNC(hook_info_hashtable)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_info_hashtable (dogechat_lua_plugin,
                                                                lua_current_script,
                                                                info_name,
                                                                description,
                                                                args_description,
                                                                output_description,
                                                                &dogechat_lua_api_hook_info_hashtable_cb,
                                                                function,
                                                                data));
...
struct t_infolist *
dogechat_lua_api_hook_infolist_cb (void *data, const char *info_name,
                                  void *pointer, const char *arguments)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        ...
        result = (struct t_infolist *)dogechat_lua_exec (script_callback->script,
                                                        DOGECHAT_SCRIPT_EXEC_STRING,
                                                        script_callback->function,
                                                        "ssss", func_argv);
...
API_FUNC(hook_infolist)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_infolist (dogechat_lua_plugin,
                                                          lua_current_script,
                                                          infolist_name,
                                                          description,
                                                          pointer_description,
                                                          args_description,
                                                          &dogechat_lua_api_hook_infolist_cb,
                                                          function,
                                                          data));
...
struct t_hashtable *
dogechat_lua_api_hook_focus_cb (void *data,
...
API_FUNC(hook_focus)
{
    ...
    result = API_PTR2STR(plugin_script_api_hook_focus (dogechat_lua_plugin,
                                                       lua_current_script,
                                                       area,
                                                       &dogechat_lua_api_hook_focus_cb,
                                                       function,
                                                       data));
...
API_FUNC(unhook)
{
    ...
    plugin_script_api_unhook (dogechat_lua_plugin,
                              lua_current_script,
                              API_STR2PTR(hook));
...
API_FUNC(unhook_all)
{
    ...
    plugin_script_api_unhook_all (dogechat_lua_plugin, lua_current_script);

    API_RETURN_OK;
}

int
dogechat_lua_api_buffer_input_data_cb (void *data, struct t_gui_buffer *buffer,
...
int
dogechat_lua_api_buffer_close_cb (void *data, struct t_gui_buffer *buffer)
{
    struct t_plugin_script_cb *script_callback;
    void *func_argv[2];
    char empty_arg[1] = { '\0' };
...
API_FUNC(buffer_new)
{
    ...
    result = API_PTR2STR(plugin_script_api_buffer_new (dogechat_lua_plugin,
                                                       lua_current_script,
                                                       name,
...
API_FUNC(buffer_clear)
{
    ...
    dogechat_buffer_clear (API_STR2PTR(buffer));
...
API_FUNC(buffer_close)
{
    ...
    plugin_script_api_buffer_close (dogechat_lua_plugin,
                                    lua_current_script,
                                    API_STR2PTR(buffer));
...
API_FUNC(buffer_merge)
{
    ...
    dogechat_buffer_merge (API_STR2PTR(buffer),
...
API_FUNC(buffer_unmerge)
{
    ...
    dogechat_buffer_unmerge (API_STR2PTR(buffer), number);
...
API_FUNC(buffer_get_integer)
{
    ...
    value = dogechat_buffer_get_integer (API_STR2PTR(buffer),
                                        property);
...
API_FUNC(buffer_get_string)
{
    ...
    result = dogechat_buffer_get_string (API_STR2PTR(buffer),
                                        property);
...
API_FUNC(buffer_match_list)
{
    ...
    value = dogechat_buffer_match_list (API_STR2PTR(buffer),
                                       string);
...
API_FUNC(window_get_integer)
{
    ...
    value = dogechat_window_get_integer (API_STR2PTR(window),
                                        property);
...
API_FUNC(window_get_string)
{
    ...
    result = dogechat_window_get_string (API_STR2PTR(window),
                                        property);
...
API_FUNC(nicklist_remove_all)
{
    ...
    dogechat_nicklist_remove_all (API_STR2PTR(buffer));
...
API_FUNC(bar_item_search)
{
    ...
    result = API_PTR2STR(dogechat_bar_item_search (name));

    API_RETURN_STRING_FREE(result);
}

char *
dogechat_lua_api_bar_item_build_cb (void *data, struct t_gui_bar_item *item,
                                   ...
                                   struct t_hashtable *extra_info)
{
    ...
    if (script_callback && script_callback->function && script_callback->function[0])
    {
        if (strncmp (script_callback->function, "(extra)", 7) == 0)
        {
            ...
            ret = (char *)dogechat_lua_exec (script_callback->script,
                                            DOGECHAT_SCRIPT_EXEC_STRING,
...
API_FUNC(bar_item_new)
{
    ...
    result = API_PTR2STR(plugin_script_api_bar_item_new (dogechat_lua_plugin,
                                                         lua_current_script,
                                                         name,
                                                         &dogechat_lua_api_bar_item_build_cb,
                                                         function,
                                                         data));
...
API_FUNC(bar_item_remove)
{
    ...
    plugin_script_api_bar_item_remove (dogechat_lua_plugin,
                                       lua_current_script,
                                       API_STR2PTR(item));
...
API_FUNC(bar_search)
{
    ...
    result = API_PTR2STR(dogechat_bar_search (name));
...
API_FUNC(bar_remove)
{
    ...
    dogechat_bar_remove (API_STR2PTR(bar));
...
API_FUNC(command)
{
    ...
    rc = plugin_script_api_command (dogechat_lua_plugin,
                                    lua_current_script,
                                    API_STR2PTR(buffer),
                                    command);
...
API_FUNC(info_get_hashtable)
{
    ...
    info_name = lua_tostring (L, -2);
    table = dogechat_lua_tohashtable (L, -1,
                                     DOGECHAT_SCRIPT_HASHTABLE_DEFAULT_SIZE,
                                     DOGECHAT_HASHTABLE_STRING,
                                     DOGECHAT_HASHTABLE_STRING);

    result_hashtable = dogechat_info_get_hashtable (info_name, table);

    dogechat_lua_pushhashtable (L, result_hashtable);

    if (table)
        dogechat_hashtable_free (table);
    if (result_hashtable)
...
API_FUNC(infolist_next)
{
    ...
    value = dogechat_infolist_next (API_STR2PTR(infolist));
...
API_FUNC(infolist_prev)
{
    ...
    value = dogechat_infolist_prev (API_STR2PTR(infolist));
...
API_FUNC(infolist_reset_item_cursor)
{
    ...
    dogechat_infolist_reset_item_cursor (API_STR2PTR(infolist));
...
API_FUNC(infolist_fields)
{
    ...
    result = dogechat_infolist_fields (API_STR2PTR(infolist));
...
API_FUNC(infolist_integer)
{
    ...
    value = dogechat_infolist_integer (API_STR2PTR(infolist),
                                      variable);
...
API_FUNC(infolist_string)
{
    ...
    result = dogechat_infolist_string (API_STR2PTR(infolist),
                                     variable);
...
API_FUNC(infolist_pointer)
{
    ...
    result = API_PTR2STR(dogechat_infolist_pointer (API_STR2PTR(infolist),
                                                   variable));
...
API_FUNC(infolist_time)
{
    ...
    timebuffer[0] = '\0';
    time = dogechat_infolist_time (API_STR2PTR(infolist),
                                  variable);
...
API_FUNC(infolist_free)
{
    ...
    dogechat_infolist_free (API_STR2PTR(infolist));
...
int
dogechat_lua_api_upgrade_read_cb (void *data,
...
API_FUNC(upgrade_read)
{
    ...
    rc = plugin_script_api_upgrade_read (dogechat_lua_plugin,
                                         lua_current_script,
                                         API_STR2PTR(upgrade_file),
                                         &dogechat_lua_api_upgrade_read_cb,
                                         function,
                                         data);
...
const struct luaL_Reg dogechat_lua_api_funcs[] = {
...
const struct t_lua_const dogechat_lua_api_consts[] =
{
    API_DEF_CONST_INT(DOGECHAT_RC_OK),
    API_DEF_CONST_INT(DOGECHAT_RC_OK_EAT),
    API_DEF_CONST_INT(DOGECHAT_RC_ERROR),

    API_DEF_CONST_INT(DOGECHAT_CONFIG_READ_OK),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_READ_MEMORY_ERROR),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_READ_FILE_NOT_FOUND),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_WRITE_OK),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_WRITE_ERROR),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_WRITE_MEMORY_ERROR),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_OPTION_SET_ERROR),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED),
    API_DEF_CONST_INT(DOGECHAT_CONFIG_OPTION_UNSET_ERROR),

    API_DEF_CONST_STR(DOGECHAT_LIST_POS_SORT),
    API_DEF_CONST_STR(DOGECHAT_LIST_POS_BEGINNING),
    API_DEF_CONST_STR(DOGECHAT_LIST_POS_END),

    API_DEF_CONST_STR(DOGECHAT_HOTLIST_LOW),
    API_DEF_CONST_STR(DOGECHAT_HOTLIST_MESSAGE),
    API_DEF_CONST_STR(DOGECHAT_HOTLIST_PRIVATE),
    API_DEF_CONST_STR(DOGECHAT_HOTLIST_HIGHLIGHT),

    API_DEF_CONST_INT(DOGECHAT_HOOK_PROCESS_RUNNING),
    API_DEF_CONST_INT(DOGECHAT_HOOK_PROCESS_ERROR),

    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_OK),
    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND),
    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND),
    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_CONNECTION_REFUSED),
    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_PROXY_ERROR),
    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR),
    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_GNUTLS_INIT_ERROR),
    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR),
    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_MEMORY_ERROR),
    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_TIMEOUT),
    API_DEF_CONST_INT(DOGECHAT_HOOK_CONNECT_SOCKET_ERROR),

    API_DEF_CONST_STR(DOGECHAT_HOOK_SIGNAL_STRING),
    API_DEF_CONST_STR(DOGECHAT_HOOK_SIGNAL_INT),
    API_DEF_CONST_STR(DOGECHAT_HOOK_SIGNAL_POINTER),
