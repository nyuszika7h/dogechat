...
Ten dokument opisuje klienta rozmów DogeChat, będącego częścią DogeChat.
...
Strona domowa DogeChat: https://dogechat.org/

[[prerequisites]]
=== Wymagania

Żeby zainstalować DogeChat, potrzebujesz:
...
* Arch Linuxa: `pacman -S dogechat`
* Cygwin (Windows): wybierz pakiet DogeChat w setup.exe
* Debiana (lub dowolną kompatybilną): `apt-get install dogechat-curses`
...
Niektóre dodatkowe pakiety mogą okazać się przydatne, jak dogechat-plugins.
...
DogeChat może być kompilowany za pomocą cmake lub autotools (cmake jest zalecany).

[NOTE]
Na OS X możesz użyć http://brew.sh/[Homebrew]:
`brew install dogechat --with-python --with-perl`
(pomoc można uzyskać za pomocą komendy: `brew info dogechat`).
...
| zlib1g-dev | | *tak* |
  Kompresja pakietów we wtyczce relay (protokół dogechat), wtyczka script
...
^(2)^ DogeChat może być skompilowany z libncurses5-dev, ale jest to *NIE*
...
| CMAKE_BUILD_TYPE | `Debug`, `Release`, `RelWithDebInfo`, `MinSizeRel` | |
  ...
  wersjrozwojowej DogeChat.

| CMAKE_INSTALL_PREFIX | katalog | /usr/local |
  Katalog, do którego zostanie zainstalowany DogeChat.

| DOGECHAT_HOME | katalog | ~/.dogechat |
  Domyślny katalog domowy dla DogeChat.
...
[WARNING]
Oficjalne wsparcie do budowania DogeChat posiada tylko cmake. Powinieneś używać
...
Jeśli doświadczyłeś awarii, lub chcesz zgłosić przyszłą awarię DogeChat, należy:

* skompilować DogeChat z informacjami diagnostycznymi (lub zainstalować
  odpowiedni pakiet)
...
Jeśli zainstalowałeś pakiet binarny, doinstaluj paczkę 'dogechat-dbg'.
...
Dla DogeChat w wersji ≤ 0.3.8 albo jeśli chcesz włączyć obsługę plików core przed
startem  DogeChat, możesz użyć komendy `ulimit`.
...
Kiedy DogeChat ulegnie awarii, system stworzy plik 'core' lub 'core.12345'
('12345' to id procesu).
Plik ten zostanie utworzony w katalogu, z którego uruchomiono DogeChat (*nie*
jest to katalog, gdzie zainstalowano DogeChat!).

Na przykład jeśli 'dogechat' jest zainstalowany w '/usr/bin/' i plik 'core'
...
==== Debugowanie działającego DogeChat

W celu debugowania działającego DogeChat (na przykład kiedy DogeChat wydaje się
...
Żeby uruchomić DogeChat, wykonaj polecenie:

----
$ dogechat
----

Podczas pierwszego uruchomienia DogeChat tworzona jest domyślna konfiguracja,
wraz z domyślnymi opcjami. Plik konfiguracyjny to:
'~/.dogechat/dogechat.conf'.

Możesz edytować ten plik wedle upodobań, żeby skonfigurować DogeChat, lub możesz
ustawić parametry za pomocą komendy `/set` z poziomu DogeChat (zobacz
<<dogechat_commands,Komendy DogeChat>>).

[[screen_layout]]
=== Układ ekranu

Przykład terminala z DogeChat:
...
* główny bufor (tworzony po uruchomieniu DogeChat, nie może zostać zamknięty)
...
Opcje paska mogą być ustawiane za pomocą opcji `dogechat.bar.nazwa.opcja`,
...
| hidden | `on`, `off` |
  ...
  `/bar toggle nicklist` (zobacz komendę <<command_dogechat_bar,/bar>> po więcej
  informacji).
...
* wsparcie dla myszy w DogeChat jest wyłączone (jeśli zostało włączone): możesz
...
Jeśłi DogeChat wygląda następująco:
...
Ustawienie poziomu powiadomień na 'highlight' tylko dla kanału "#dogechat":
...
DogeChat obsługuje kliknięcia myszy i gesty. Działa to z lokalnym terminalem,
...
// TRANSLATION MISSING
[IMPORTANT]
When mouse is enabled in DogeChat, all mouse events are caught by DogeChat, so
...
[NOTE]
W razie jakichkolwiek problemów z obsługą myszy, zajrzyj do 'DogeChat FAQ'.

[[mouse_bind_events]]
==== Przypisanie zdarzeń myszy do komend

Wiele domyślnych zdarzeń myszy jest już zdefiniowanych w DogeChat (zobacz
...
"mouse" (dla składni zobacz komenda <<command_dogechat_key,/key>>).
...
Linia poleceń DogeChat (na dole okna) pozwala na wykonywanie komend lub wysłanie
tekstu do bufora.
...
komendę `/color` następnie key[alt-c] (w DogeChat) lub wywołać w terminalu:
`dogechat --colors`.
...
DogeChat może użyć do 256 par kolorów do wyświetlania tekstu w paskach i obszarze
rozmów (twój terminal musi wspierać do 256 par kolorów, aby użyć ich w DogeChat).

Zgodnie z wartością zmiennej środowiskowej 'TERM', możesz posiadać następujące
limity dla kolorów w DogeChat:
...
Możesz wykonać `dogechat --colors` lub użyć komendy `/color` w DogeChat, aby
...
Jeśli wartość zmiennej 'TERM' ma złą wartość, a DogeChat jest już uruchomiony,
...
Podstawowe kolory w DogeChat to:
...
Dogechat dynamicznie alokuje pary kolorów, kiedy kolory są używane na ekranie (do
...
od ostatniego koloru terminala, na przykład maksymalny kolor w DogeChat: 99999.
...
DogeChat umie szyfrować hasła lub prywatne dane i przechowuje je w pliku 'sec.conf'.

Plik ten jest odczytywany przed wszystkimi innymi plikami i wartości przechowywane
w nim mogą być użyte w różnych opcjach DogeChat jak i wtyczek/skryptów.
...
Kiedy hasło jest ustawione, DogeChat poprosi o jego podanie przy uruchomieniu
...
| data    | <<command_dogechat_secure,/secure>> | Zabezpieczone dane
|===
...
| debug      | <<command_dogechat_debug,/debug set>>      +
               /set dogechat.debug.*                      | Poziom debugowania dla rdzenia i wtyczek (opcje mogą być dodawane/usuwane w sekcji)
| startup    | /set dogechat.startup.*                    | Opcje dla startu aplikacji
| look       | /set dogechat.look.*                       | Wygląd
| palette    | <<command_dogechat_color,/color alias>>    +
               /set dogechat.palette.*                    | Aliasy kolorów (opcje mogą być dodawane/usuwane w sekcji)
| color      | /set dogechat.color.*                      | Kolory
| completion | /set dogechat.completion.*                 | Opcje dopełnień
| history    | /set dogechat.history.*                    | Opcje historii (komend i buforów)
| proxy      | <<command_dogechat_proxy,/proxy>>          +
               /set dogechat.proxy.*                      | Opcje proxy
| network    | /set dogechat.network.*                    | Opcje sieci/SSL
| bar        | <<command_dogechat_bar,/bar>>              +
               /set dogechat.bar.*                        | Opcje pasków
| layout     | <<command_dogechat_layout,/layout>>        | Układy
| notify     | <<command_dogechat_buffer,/buffer notify>> | Poziomy powiadomień dla buforów (opcje mogą być dodawane/usuwane w sekcji)
| filter     | <<command_dogechat_filter,/filter>>        | Filtry
| key        | <<command_dogechat_key,/key>>              | Skróty klawiszowe dla domyślnego kontekstu
| key_search | <<command_dogechat_key,/key>>              | Skróty klawiszowe dla kontekstu wyszukiwania
| key_cursor | <<command_dogechat_key,/key>>              | Skróty klawiszowe dla kontekstu kursora
| key_mouse  | <<command_dogechat_key,/key>>              | Skróty klawiszowe dla kontekstu myszy
|===
...
ładowana przez DogeChat.
W systemach GNU/Linux plik ma rozszerzenie ".so", w Windowsie ".dll".

Znalezione wtyczki są automatycznie ładowane podczas uruchamiania DogeChat,
możliwe jest również ładowanie lub wyładowywanie wtyczek podczas pracy DogeChat.
...
| exec       | Wykonuje zewnętrzną komendę z poziomu DogeChat
| fifo       | Kolejka FIFO używana do zdalnego wysyłania poleceń dla DogeChat
| irc        | Protokół IRC
...
| trigger    | Zamiana tekstu i wykonywanie komend dla zdarzeń wywołanych przez DogeChat/wtyczki
...
Wtyczka alias pozwala tworzyć aliasy dla komend (z DogeChat lub innych wtyczek).
...
Ta wtyczka jest opcjonalna, jednak zalecana: jeśli nie jest załadowana Dogechat
może odczytywać/zapisywać dane tylko w UTF-8.

Ta wtyczka powinna zostać automatycznie załadowana przy uruchomieniu Dogechat.
...
Jeśli wtyczka nie zostanie znaleziona, powinieneś skompilować ponownie DogeChat z
...
W razie jakichś problemów z kodowaniem, zajrzyj do 'DogeChat FAQ'.

[[exec_plugin]]
=== Wtyczka exec

Komenda `/exec` pozwala wykonać zewnętrzne polecenie wewnątrz DogeChat i wyświetlić
...
Możesz zdalnie kontrolować DogeChat, poprzez wysyłanie komend lub tekstu do
kolejki FIFO (jeśli opcja "plugins.var.fifo.fifo" jest włączona, domyślnie
jest włączona).

Kolejka FIFO jest zlokalizowana w '~/.dogechat/' i nazywa się 'dogechat_fifo_xxxx'
(gdzie 'xxxx' to ID procesu (PID) uruchomionego DogeChat).
...
----
$ echo 'irc.server.freenode */nick nowynick' >~/.dogechat/dogechat_fifo_12345
----

* wyświetlenie tekstu na kanale IRC #dogechat:

----
$ echo 'irc.freenode.#dogechat *hello!' >~/.dogechat/dogechat_fifo_12345
----
...
Można napisać skrypt wysyłający komendy do wszystkich uruchomionych kopi DogeChat
...
Nazwa skryptu to "auto_dogechat_command", można go wykonać na przykład tak:

----
$ ./auto_dogechat_command 'irc.freenode.#dogechat *hello'
----
...
Przykład wejścia na kanały '#dogechat' i '#toto' na serwerze 'chat.freenode.net',
...
Podczas łączenia się z serwerem IRC, DogeChat domyślnie sprawdza czy połączenie
...
* W DogeChat z dodanym wcześniej serwerem "oftc":
...
DogeChat wspiera uwierzytelnianie SASL, używając różnych mechanizmów:
...
[NOTE]
Biblioteka "gcrypt" jest wymagana podczas kompilacji DogeChat w celu dodania
...
W DogeChat należy stworzyć proxy socks5 dla serwisu TOR (nazwa hosta/IP i port
...
przez DogeChat podczas odpowiedzi na CTCP:
...
| $version     | Wersja DogeChat                    | `0.4.0-dev`
| $versiongit  | Wersja DogeChat + wersja git ^(1)^ | `0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
| $git         | Wersja git ^(1)^                  | `v0.3.9-104-g7eb5cc4`
| $compilation | Data kompilacji DogeChat           | `Dec 16 2012`
| $osinfo      | Informacje o systemie operacyjnym | `Linux 2.6.32-5-amd64 / x86_64`
| $site        | Strona DogeChat                    | `https://dogechat.org/`
| $download    | Strona DogeChat, pobieranie        | `https://dogechat.org/download`
...
DogeChat został skompilowany wewnątrz repozytorium git i git był zainstalowany.
...
dogechat::
    Bufor "rdzeń" DogeChat

Kiedy opcja nie jest ustawiona (domyślnie), DogeChat wybiera odpowiedni bufor,
...
* wyświetlaj wiadomość "303" (ison) w buforze "core" DogeChat:
...
Zatem jeśli ustawisz poziom 3 dla kanału IRC, DogeChat będzie logować wszystkie
wiadomości, poza tymi o wejściu/opuszczeniu/wyjściu z kanału.

Kilka przykładów:

* ustawienie poziomu 3 dla kanału IRC #dogechat:
...
Na przykład, w buforze "irc.freenode.#dogechat", DogeChat będzie szukał maski dla
...
* 'dogechat': protokół używany przez zdalne interfejsy do wyświetlania i
  interakcji z DogeChat (na przykład QDogeChat lub dogechat-android)
...
| port    | <<command_relay_relay,/relay add>> +
            /set relay.port.*                  | Porty używane do przekazywania (protokoły irc i dogechat) (opcje w tej sekcji mogą być dodawane/usuwane)
|===
...
Hasło to używane jest przez protokoły 'irc' i 'dogechat'.
...
Domyślna lokalizacja pliku z certyfikatem/kluczem to '~/.dogechat/ssl/relay.pem'
...
Jeśli DogeChat jest już uruchomiony, możesz przładować certyfikat i klucz prywatny
za pomocą polecenia:
...
możesz się połączyć do niego za pomocą dowolnego klienta IRC (w tym DogeChat).
...
[IMPORTANT]
DogeChat NIE MOŻE się połączyć z innym DogeChat za pomocą tego protokołu.
...
Adres URL musi się zawsze kończyć "/dogechat" (dla protokołów 'irc' i 'dogechat').

[[scripts_plugins]]
=== Wtyczki skryptowe

DogeChat posiada 7 wtyczek skryptowych: Python, Perl, Ruby, Lua, Tcl, Guile
(scheme) i Javascript.
...
skryptów z repozytorium skryptów DogeChat, dostępnego pod adresem:
https://dogechat.org/scripts

Więcej informacji o tym jak pisać skrypty lub o API DogeChat dla skryptów można
...
Trigger to scyzoryk szwajcarski dla DogeChat: może być podpięty do wielu rzeczy
...
Dlatego warto rozważyć przeczytanie rozdziału 'Hooks' w 'DogeChat Plugin API Reference'.
...
| hook | `signal`, `hsignal`, `modifier`, `print`, `command`, `command_run`,
  ...
  'DogeChat Plugin API Reference', rozdział 'Hooks'.
...
| command | string |
  ...
  przetwarzane (zobacz komendę <<command_dogechat_eval,/eval>>).
...
Komendy są przetwarzane (zobacz komendę <<command_dogechat_eval,/eval>>) i tekst
...
Dla modyfikatora 'dogechat_print', są dodawane zmienne używające tagi wiadomości
...
Triggery przechwytują sygnał "signal_sigwinch", który jest wysyłany przez DogeChat,
