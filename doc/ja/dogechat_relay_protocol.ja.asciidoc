...
この文書は DogeChat リレープロトコルについて述べたものです: リレープロトコルとは、DogeChat
...
* 'リレー': これは relay プラグインを備えた DogeChat を指し、 "サーバ"
...
[NOTE]
この文書で述べる全てのクライアントは'リレー' プラグインの 'dogechat' プロトコルを使っています。また'リレー'
...
* 'id': 'リレー' からの応答に含まれる任意指定のメッセージ識別子;
  ...
  (アンダースコアが最初についている識別子は DogeChat 'event' メッセージ用に予約されています)
...
* 'option': 以下のうちの 1 つ:
** 'password': 'リレー' の認証用パスワード (DogeChat の
...
----
# 初期化、デフォルト設定の zlib を使用 (DogeChat がサポートする場合)
...
* 'buffer': ポインタ ('0x12345') またはバッファの完全な名前 (例:
  'core.dogechat' または 'irc.freenode.#dogechat')
...
# irc.freenode.#dogechat のニックネームリストを要求
...
** 'upgrade': DogeChat アップグレードに関するシグナルを受信 (アップグレード、アップグレードの終了);
   名前が "*" のバッファに対してのみ利用可能
   _(DogeChat バージョン 0.4.1 以上で利用可)_
...
# #dogechat チャンネルを同期、ニックネームリストは受信しない
sync irc.freenode.#dogechat buffer

# 一般的なシグナル + #dogechat チャンネルに対する全てのシグナルを取得
...
このため "sync *"、"sync irc.freenode.#dogechat"、"desync *" の順に送信した場合、DogeChat
は #dogechat チャンネルに対するアップデートを送信し続けます
...
# #dogechat チャンネルのニックネームリストの同期を中止 (バッファは同期する)
desync irc.freenode.#dogechat nicklist

# #dogechat チャンネルの同期を中止
...
テストコマンド: DogeChat は様々な種類のオブジェクトを返します。
...
DogeChat にメッセージ "_pong" と同じ引数を持つ返信 ping を送信。
...
| _upgrade | upgrade | (空) |
  DogeChat のアップグレード中 | DogeChat との同期を中止 (または切断)

| _upgrade_ended | upgrade | (空) |
  DogeChat のアップグレード終了 | DogeChat との同期および再同期
|===

[[message_buffer_opened]]
==== _buffer_opened

このメッセージは DogeChat が "buffer_opened"
...
| full_name       | string    | 完全な名前 (例: 'irc.freenode.#dogechat')
| short_name      | string    | 短い名前 (例: '#dogechat')
...
例: freenode の '#dogechat' チャンネルに参加、新しいバッファは
...
このメッセージは DogeChat が "buffer_moved"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ 'irc.freenode.#dogechat' を番号 2 に移動:
...
このメッセージは DogeChat が "buffer_merged"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ 'irc.freenode.#dogechat' をバッファ #2 とマージ:
...
このメッセージは DogeChat が "buffer_unmerged"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ 'irc.freenode.#dogechat' をアンマージ:
...
このメッセージは DogeChat が "buffer_hidden"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ 'irc.freenode.#dogechat' を隠す:
...
このメッセージは DogeChat が "buffer_unhidden"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ 'irc.freenode.#dogechat' を隠すことを止める:
...
このメッセージは DogeChat が "buffer_renamed"
シグナルを送信する際にクライアントに送られます。
...
このメッセージは DogeChat が "buffer_title_changed"
シグナルを送信する際にクライアントに送られます。
...
このメッセージは DogeChat が "buffer_cleared"
シグナルを送信する際にクライアントに送られます。
...
例: バッファ 'irc.freenode.#dogechat' をクリア:
...
このメッセージは DogeChat が "buffer_type_changed"
シグナルを送信する際にクライアントに送られます。
...
このメッセージは DogeChat が "buffer_localvar_added"
シグナルを送信する際にクライアントに送られます。
...
例: 'irc.freenode.#dogechat' にローカル変数 'test' を追加:
...
このメッセージは DogeChat が "buffer_localvar_changed"
シグナルを送信する際にクライアントに送られます。
...
例: 'irc.freenode.#dogechat' に含まれるローカル変数 'test' を更新:
...
このメッセージは DogeChat が "buffer_localvar_removed"
シグナルを送信する際にクライアントに送られます。
...
例: 'irc.freenode.#dogechat' からローカル変数 'test' を削除:
...
このメッセージは DogeChat が "buffer_line_added"
シグナルを送信する際にクライアントに送られます。
...
| date         | time             | メッセージの日付
| date_printed | time             | DogeChat メッセージを表示した日付
...
例: バッファ 'irc.freenode.#dogechat' でニックネーム 'FlashCode' からの新しいメッセージ 'hello!':
...
このメッセージは DogeChat が "buffer_closing"
シグナルを送信する際にクライアントに送られます。
...
例: DogeChat がバッファ 'irc.freenode.#dogechat' を閉じる:
...
例: バッファ 'irc.freenode.#dogechat' のニックネームリスト:
...
_DogeChat バージョン 0.3.8 以上で利用可。_

このメッセージは DogeChat がアップグレード処理を始める際にクライアントに送られます。

メッセージにデータは含まれません。

クライアントは DogeChat との同期を中止するか
('desync' コマンドを送信)、DogeChat から切断することを推奨します
(これはアップグレードの後はすべてのポインタが変わるからです)。

[NOTE]
DogeChat のアップグレード中、ソケットは開いたままです
...
クライアントは DogeChat との同期を再開することを推奨します:
...
2 つのバッファ (dogechat コアと freenode サーバ) と
...
2 つのバッファ (dogechat コアと freenode サーバ) を持つインフォリストの例:
