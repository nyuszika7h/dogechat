...
[NOTE]
この文書は DogeChat バージョン 0.3.0 以上向けに書かれたものですが、
最新安定版の DogeChat に対しても利用できます。
...
[[dogechat_name]]
=== "DogeChat" という名前の由来は何ですか?

"Doge" は再帰的頭字語で、"Doge Enhanced Environment" の略です。
そのため、完全な名前は "Doge Enhanced Environment for Chat" です。

"Doge" はまた "とても小さい" を意味します。(もちろん、
その他の意味もありますが、それは DogeChat とは関係ありませんよ!)。

[[why_choose_dogechat]]
=== DogeChat を選ぶ理由は何ですか? X-Chat や Irssi は最高なのに...

DogeChat は非常に軽く、革新的な機能を備えているからです。

こののページに関するより詳しい情報: https://dogechat.org/about/features

[[compilation_install]]
== コンパイル / インストール

[[gui]]
=== DogeChat には多くの GUI があると聞きました。コンパイル/利用方法を教えてください。

いくつかのリモート GUI が利用可能です、機能のページに載っているリンクをご覧ください:
https://dogechat.org/about/features

[[compile_git]]
=== git リポジトリをクローンした後に DogeChat をコンパイルできません。

DogeChat をコンパイルするには cmake を使うことが推奨されています。
...
[[compile_osx]]
=== どうすれば OS X に DogeChat をインストールできますか?
...
[[lost]]
=== DogeChat を起動しました。でも何をすればいいかわかりません。
...
* dogechat が libncursesw にリンクされていることの確認 (警告: 全てではありませんが、
  ほとんどのディストリビューションで必要です): `ldd /path/to/dogechat`
* `/plugin` コマンドで "Charset" プラグインがロード済みであることの確認
  (ロードされていない場合、"dogechat-plugins" パッケージが必要かもしれません) 。
...
* 'dogechat.look.eat_newline_glitch' オプションが off であることを確認してください
  (このオプションは表示上のバグを引き起こす可能性があります)

[NOTE]
DogeChat は UTF-8 ロケールを推奨します。ISO 又はその他のロケールを使う場合、
*全ての* 設定 (ターミナル、screen、..) が ISO であり、
UTF-8 *でない* ことを確認してください。

[[unicode_chars]]
=== ターミナルで表示されるユニコード文字が DogeChat では表示されない場合があります、なぜでしょうか?
...
DogeChat を起動した場所に依存しますが、以下の値を持つはずです:

* DogeChat を screen および tmux
  を使わずにローカルまたはリモートマシンで実行している場合、使用中のターミナルに依存します:
  'xterm' 、'xterm-256color' 、'rxvt-unicode' 、'rxvt-256color' 、...
* DogeChat を screen 内で実行している場合、'screen' または 'screen-256color' です、
* DogeChat を tmux 内で実行している場合、'tmux' 、'tmux-256color' 、
  'screen' 、'screen-256color' です。

もし必要であれば、TERM 変数を修正してください: `export TERM="xxx"`

[[screen_weird_chars]]
=== dogechat を screen/tmux の中で使っている場合、おかしなランダム文字列が表示されます。どうすれば直りますか。
...
[[osx_display_broken]]
=== OS X 上で DogeChat をコンパイルした場合、"(null)" という文字列が至る所に表示されます。

ncursesw を自分でコンパイルした場合は、標準の (システムに元から存在する)
ncurses を使ってみてください。

さらに OS X では、Homebrew パッケージマネージャを使って
DogeChat をインストールすることをお勧めします。
...
DogeChat は自動的にデフォルトバー "input" を作成します:
...
DogeChat ≥ 1.0 では、最小限表示を使うことができます (デフォルトキー: key[alt-l])。
...
デフォルトでは、DogeChat はそれぞれの行の最初に時間とプレフィックス、
...
DogeChat ≥ 0.3.6 では、"eat_newline_glitch" オプションを有効化できます。
...
=== DogeChat が出力するメッセージの言語を、再起動せずに変更したいです。このようなことは可能ですか。

DogeChat ≥ 1.0 では、再起動せずに変更できます:
...
[[use_256_colors]]
=== どうすれば DogeChat で 256 色が使えますか。

256 色は DogeChat ≥ 0.3.4 でサポートされます。
...
2 つのコマンドを使って変数の値を変更してください (DogeChat バージョン 1.0 以上で可能):
...
マウスのサポートは DogeChat 0.3.6 以上からです。
...
[[mouse_select_paste]]
=== DogeChat でマウスが利用可能な場合、ターミナルのテキストを選択したり、貼り付けるにはどうすれば良いですか。

DogeChat でマウスが利用可能な場合、key[shift] キーを押しながらターミナルをクリックして選択してください、
...
Mac OS X をお使いの場合、必ず `curl-ca-bundle` をインストールして、DogeChat
で証明書へのパスを設定してください:
...
異なる優先順位文字列を試してみてください (DogeChat 0.3.5 以上)、
"xxx" はサーバ名に書き換えてください。
...
オプション 'dogechat.network.gnutls_ca_file' に証明書ファイルへのパスを設定してください。
...
DogeChat バージョン 0.4.1 以上の場合、'irc.look.display_join_message'
...
この様な場合には、Bitlbee が離席状態を通知することを禁止し、DogeChat にニックネームリスト中に含まれる離席中のニックネームに対して特別な色を使わせることができます。
...
[NOTE]
DogeChat 0.3.3 以下では、オプション名が 'irc.network.away_check' と
...
古い DogeChat をお使いの場合、'beep.pl' や 'launcher.pl' 等のスクリプトを使ってください。
...
[[openbsd_plugins]]
=== OpenBSD と DogeChat を使っている場合、プラグインが 1 つもロードされません。
...
[[memory_usage]]
=== どうすれば DogeChat のメモリ使用量を節約できますか。
...
* SSL を*使わない*なら、証明書を読み込まないでください: オプション
  'dogechat.network.gnutls_ca_file' に空文字列を設定してください。
* 'dogechat.history.max_buffer_lines_number' オプションの値を減らすか、
  'dogechat.history.max_buffer_lines_minutes' オプションに値を設定してください。
* 'dogechat.history.max_commands' オプションの値を減らしてください。

[[cpu_usage]]
=== どうすれば DogeChat の CPU 負荷を低減できますか?

以下に挙げる <<memory_usage,memory>> に関するヒントに従ってください:

* "nicklist" バーを非表示にする: `/bar hide nicklist`
* ステータスバー時間に秒の表示を止める:
  `/set dogechat.look.item_time_format "%H:%M"` (これはデフォルト値です)
...
以下のページを参照してください: https://dogechat.org/dev/support

[[gdb_error_threads]]
=== gdb で DogeChat を実行する場合、スレッドに関するエラーが発生します。

gdb で DogeChat を実行する場合、以下のエラーが出るかもしれません:
...
(システム上の libpthread と DogeChat へのパスを変更):
...
[[supported_os]]
=== DogeChat をサポートするプラットフォームのリストを教えてください。他のオペレーティングシステムへの移植予定はありますか。

完全なリストはこのページにあります: https://dogechat.org/download

我々は可能な限り多くのプラットフォームに移植することに最善を尽くしています。
我々が持っていない OS で DogeChat をテストすることを歓迎します。

[[help_developers]]
=== どうすれば DogeChat 開発者を手伝えますか。
...
[[donate]]
=== DogeChat 開発者にお金か何かを贈りたいのですが。

開発補助のためお金を贈ることができます。
詳しいことは https://dogechat.org/about/donate に載っています。
