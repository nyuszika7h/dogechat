...
このマニュアルは DogeChat チャットクライアントについての文書で、これは DogeChat の一部です。
...
DogeChat のホームページ: https://dogechat.org/

[[prerequisites]]
=== 事前に必要なもの

DogeChat をインストールするには、以下のものが必要です:

* GNU/Linux が稼動しているシステム (ソースパッケージを使う場合は、
  コンパイラツールも必要)、又は互換 OS
* 'root' 特権 (DogeChat をシステムディレクトリにインストールする場合)
...
* Cygwin (Windows): setup.exe で DogeChat パッケージを選択してください
...
例えば dogechat-plugins 等の追加パッケージを使うとより便利になるかもしれません。
...
DogeChat は cmake または autotools を使ってコンパイルできます (cmake を使うことが推奨されています)。

[NOTE]
OS X では、http://brew.sh/[Homebrew] を使ってください:
`brew install dogechat --with-python --with-perl`
(ヘルプを見るには: `brew info dogechat`)。
...
| zlib1g-dev | | *必須* |
  relay プラグインでパケットを圧縮 (dogechat プロトコル)、スクリプトプラグイン
...
^(2)^ DogeChat は libncurses5-dev でもコンパイル可能ですが、これは推奨*されません*
...
[WARNING]
cmake 以外を用いた DogeChat のビルドは公式にサポートされません。cmake
...
DogeChat がクラッシュした場合、または DogeChat
...
バイナリパッケージをインストールする場合は、'dogechat-dbg' パッケージをインストールしてください。

[[core_files]]
==== Core ファイル

'core' ファイルを有効化するには、<<option_dogechat.startup.sys_rlimit,dogechat.startup.sys_rlimit>>
オプションを使ってください:
...
DogeChat バージョン 0.3.8 以下または DogeChat の実行前に core
...
DogeChat がクラッシュすると、'core' または 'core.12345' ファイルが作られます
('12345' はプロセス番号です)。
このファイルは DogeChat を起動したディレクトリに作られます (これは
DogeChat がインストールされているディレクトリでは*ありません*!)。

例えば、'dogechat' が '/usr/bin/' にインストールされ、'core' ファイルが
...
起動している DogeChat をデバッグするには (例えば DogeChat がフリーズしているような場合)、gdb
の引数にプロセス番号を与えて起動します ('12345' は dogechat プロセスの PID に変更してください):
...
DogeChat を起動させるには、以下コマンドを実行:

----
$ dogechat
----

初めて DogeChat を起動する場合、デフォルトのオプションで設定ファイルが作成されます。
このデフォルト設定ファイルは
'~/.dogechat/dogechat.conf' です。

DogeChat を設定するにはこのファイルを編集するか、
DogeChat 内で `/set` コマンドにパラメータを付けて設定します
(<<dogechat_commands,DogeChat コマンド>>を参照)。

[[screen_layout]]
=== スクリーンレイアウト

DogeChat を起動したターミナルの例:
...
* コアバッファ (動時にy DogeChat が作成、閉じることはできない)
...
バーオプションは `dogechat.bar.name.option` オプションで設定します。ここで
...
| hidden | `on`、`off` |
  ...
  `/bar toggle nicklist` (<<command_dogechat_bar,/bar>> コマンドを参照)。
...
* 式: ブール値として評価
  (<<command_dogechat_eval,/eval>> コマンドを参照)
...
* DogeChat マウスサポートを無効化 (有効化されていた場合):
...
(また、デフォルトでは入力が変更された場合に最小限表示を終了します、<<option_dogechat.look.bare_display_exit_on_input,dogechat.look.bare_display_exit_on_input>>
オプションを参照)。
...
デフォルトの通知レベルは
<<option_dogechat.look.buffer_notify_default,dogechat.look.buffer_notify_default>> オプションで指定し、
デフォルトは 'all' です。
...
"#dogechat" チャンネルだけに対して、通知レベルを 'highlight' に設定する場合:
...
DogeChat はマウスクリックとマウスジェスチャーをサポートしています。
...
key[alt-%] キーにマウスを 10 秒間無効化する機能を割り当てるには:
...
[IMPORTANT]
DogeChat でマウスを有効化した場合、すべてのマウスイベントは DogeChat
...
[NOTE]
マウスに関するトラブルがあれば 'DogeChat FAQ' を参照してください。

[[mouse_bind_events]]
==== コマンドに対してマウスイベントを割り当てる

DogeChat はデフォルトマウスイベントの多くを定義しています
(<<key_bindings_mouse_context,"マウス" モード用のキー>>を参照)。

`/key` コマンドで "mouse" コンテキストを指定することで割り当てを追加、変更できます
(詳しい使い方は <<command_dogechat_key,/key>> コマンドを参照)。
...
DogeChat コマンドライン (ウィンドウの一番下にあります)
...
[NOTE]
端末で利用可能なすべての色を表示するには、DogeChat で `/color` を実行した後
key[alt-c] を入力するか、端末で以下のコマンドを実行してください: `dogechat --colors`。
...
DogeChat ではバーやチャットエリアにおけるテキスト表示に 256 個の色ペアを利用できます
(この機能を利用するには DogeChat が実行されているターミナルが 256 色表示に対応している必要があります)。

'TERM' 環境変数の値によって、DogeChat
...
`dogechat --colors` を実行するか、`/color` コマンドをin DogeChat
...
'TERM' 変数が間違った値に設定された状態で DogeChat が起動完了している場合は、以下の
...
DogeChat はスクリーンに色が表示された時点で色ペアを動的に割り当てます
(バッファとバーを表示する時点で)。
...
例えば、DogeChat における色番号の最大値は 99999 です。
...
| data       | <<command_dogechat_secure,/secure>> | 保護データ
|===
...
| debug      | <<command_dogechat_debug,/debug set>>      +
               /set dogechat.debug.*                      | core とプラグインのデバッグレベル (オプションをセクションに追加/削除出来ます)
| startup    | /set dogechat.startup.*                    | 起動オプション
| look       | /set dogechat.look.*                       | 外観
| palette    | <<command_dogechat_color,/color alias>>    +
               /set dogechat.palette.*                    | 色の別名 (オプションをセクションに追加/削除出来ます)
| color      | /set dogechat.color.*                      | 色
| completion | /set dogechat.completion.*                 | 補完オプション
| history    | /set dogechat.history.*                    | 履歴オプション (コマンドとバッファ)
| proxy      | <<command_dogechat_proxy,/proxy>>          +
               /set dogechat.proxy.*                      | プロキシオプション
| network    | /set dogechat.network.*                    | ネットワーク/SSL オプション
| bar        | <<command_dogechat_bar,/bar>>              +
               /set dogechat.bar.*                        | バーオプション
| layout     | <<command_dogechat_layout,/layout>>        | レイアウト
| notify     | <<command_dogechat_buffer,/buffer notify>> | バッファに対する通知レベル (オプションをセクションに追加/削除出来ます)
| filter     | <<command_dogechat_filter,/filter>>        | フィルタ
| key        | <<command_dogechat_key,/key>>              | デフォルトコンテキストのキー
| key_search | <<command_dogechat_key,/key>>              | 検索コンテキストのキー
| key_cursor | <<command_dogechat_key,/key>>              | カーソルコンテキストのキー
| key_mouse  | <<command_dogechat_key,/key>>              | マウスコンテキストのキー
|===
...
プラグインは DogeChat によって読み込まれます。
GNU/Linux の場合、プラグインファイルは ".so" という拡張子を持ち、Windows の場合、".dll" です。

見つかったプラグインは DogeChat の起動時に自動的に読み込まれます。DogeChat
...
| exec       | DogeChat 内部から外部コマンドを実行
| fifo       | 外部から DogeChat にコマンドを送信するための FIFO パイプ
| irc        | IRC チャットプロトコル
...
| trigger    | DogeChat およびプラグインが発生させたイベントに対するテキスト置換とコマンド実行
| xfer       | ファイル転送とダイレクトチャット
|===

API を使ったプラグインやスクリプトの開発についてより詳しく学ぶには、'DogeChat
プラグイン API リファレンス' 又は 'DogeChat スクリプト製作ガイド' を参照してください。

[[alias_plugin]]
=== Alias プラグイン

Alias プラグインを使うことで、コマンドの別名を定義できます (DogeChat
...
このプラグインがロードされていない場合、DogeChat が読み書きできるデータは UTF-8 データのみになります。

Charset プラグインは DogeChat によって自動的にロードされるべきです。
...
文字コードに関する問題があれば、'DogeChat FAQ' を参照してください。

[[exec_plugin]]
=== exec プラグイン

`/exec` コマンドを使うことで DogeChat
...
外部から DogeChat を操作するには、FIFO パイプにコマンドやテキストを書き込んでください ("plugins.var.fifo.fifo"
オプションが有効化されている必要がありますが、デフォルトで有効化されているはずです)。

FIFO パイプは '~/.dogechat/' の下にあり、'dogechat_fifo_xxxx'
のような名前になっています ('xxxx' は実行中の DogeChat のプロセス ID (PID) です)。
...
* IRC の #dogechat チャンネルに対してテキストを送信:
...
このスクリプトを "auto_dogechat_command" というファイル名で保存した場合、以下のように実行できます:
...
で稼働中の IRC サーバ上の '#dogechat' と '#toto' チャンネルに参加する例:
...
SSL を使って IRC サーバに接続する場合、DogeChat
...
* DogeChat では、"oftc" サーバが既に追加されています:
...
DogeChat は SASL 認証をサポートします、以下の認証メカニズムを利用できます:
...
DogeChat のコンパイル時に必要です (<<dependencies,依存関係>>を参照)。
...
DogeChat で TOR サービスを使った socks5 プロキシを作成してください
...
CTCP 応答時に自動的に DogeChat によって展開されます:
...
| $version     | DogeChat バージョン                        | `0.4.0-dev`
| $versiongit  | DogeChat バージョン + git バージョン ^(1)^ | `0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
| $git         | git バージョン ^(1)^                      | `v0.3.9-104-g7eb5cc4`
| $compilation | DogeChat コンパイル日時                    | `Dec 16 2012`
| $osinfo      | OS に関する情報                           | `Linux 2.6.32-5-amd64 / x86_64`
| $site        | DogeChat ウェブサイト                      | `https://dogechat.org/`
| $download    | DogeChat ウェブサイトのダウンロードページ  | `https://dogechat.org/download`
...
DogeChat をコンパイルし、git がインストールされている場合のみ値が設定されます。
...
オプションが設定されていない (デフォルトの) 場合、DogeChat
...
* "303" (ison) メッセージを DogeChat "core" バッファに表示:
...
従って、IRC チャンネルに対してレベル 3 を設定した場合、DogeChat
は参加/退出/終了メッセージを除いて全てのメッセージを保存します。

例:

* IRC チャンネル #dogechat に対してレベル 3 を設定:
...
例えば "irc.freenode.#dogechat" バッファの場合、DogeChat
...
* 'irc': IRC プロキシ: IRC サーバに対する接続を、単一または複数の IRC
  クライアントで共有するために用います。
* 'dogechat': DogeChat の表示にリモートインターフェイス
  (例えば QDogeChat) を使う場合に利用されるプロトコル
...
| port       | <<command_relay_relay,/relay add>> +
               /set relay.port.*                  | リレーに使うポート(irc や dogechat プロトコル) (オプションをセクションに追加/削除出来ます)
|===
...
このパスワードは 'irc' と 'dogechat' プロトコルで利用されます。
...
証明書/秘密鍵ファイルへのデフォルトパスは '~/.dogechat/ssl/relay.pem' です
...
サーバのふりをして、他の IRC クライアント (DogeChat 自身も) は DogeChat に接続できます。
...
Relay プラグインは DogeChat プロトコルを使ってリモートインターフェイスに対してデータを送信できます。

リモートインターフェイスを使って接続できます、ダウンロードページのリストを御覧ください:
https://dogechat.org/download

[IMPORTANT]
このプロトコルを使った場合 DogeChat から他の DogeChat に接続することはできません。
...
の最後には必ず "/dogechat" をつけます ('irc' と 'dogechat' プロトコルの場合)。

[[scripts_plugins]]
=== スクリプトプラグイン

DogeChat は 7 種類のスクリプトプラグインを備えています: Python、Perl、Ruby、Lua、Tcl、Guile
(scheme)、Javascript。
これらのプラグインでそれぞれの言語で書かれたスクリプトのロード、実行、アンロードができます。

"スクリプト" と呼ばれるほかのプラグインはスクリプトマネージャで、任意の言語で書かれたスクリプトをロード
/ アンロードしたり、DogeChat スクリプトリポジトリ (https://dogechat.org/scripts から参照可能)
のスクリプトをインストール / 削除するために利用します。

スクリプトの書き方やスクリプト用の DogeChat API についての詳しい情報は、
'DogeChat スクリプト製作ガイド' を参照してください。
...
| hook | `signal`、`hsignal`、`modifier`、`print`、`command`、`command_run`、
  `timer`、`config`、`focus` |
  トリガの中で使われるフック。より詳しい情報は、'DogeChat
...
| conditions | 文字列 |
  ...
  <<command_dogechat_eval,/eval>> を参照)。
...
| command | 文字列 |
  ...
  内容は評価されます (コマンド <<command_dogechat_eval,/eval>> を参照)。
...
コマンドは評価され (コマンド <<command_dogechat_eval,/eval>> を参照)
...
'dogechat_print' 修飾子では、メッセージタグを使う変数 (下の
...
(変数は 'dogechat_print' 修飾子でも格納されます):
...
DogeChat は SIGWINCH を受けとたった際 (ターミナルのサイズが変更された際)
...
サポートを依頼する前に、必ず DogeChat に付属するドキュメントと FAQ を読んでください。
...
他の方法でサポートを受けるには、以下のページをご覧ください: https://dogechat.org/dev/support
