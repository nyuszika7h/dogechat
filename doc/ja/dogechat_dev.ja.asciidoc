...
このマニュアルは DogeChat の内部構造について書かれています:
...
DogeChat リポジトリは GitHub organization の "dogechat" にあります:
...
scripts::
    dogechat.org に投稿された '公式' スクリプト

dogechat.org::
    DogeChat ウェブサイトのソースコード: https://dogechat.org/

dogercd::
    IRC テストサーバ

qdogechat::
    DogeChat の Qt リモート GUI。

このマニュアルは 'dogechat' リポジトリだけを説明しています。
...
|       exec/       | exec プラグイン
|       fifo/       | fifo プラグイン (DogeChat にコマンドを送信する FIFO パイプ)
...
DogeChat "core" は以下のディレクトリに配置されています:
...
| パス/ファイル名               | 説明
| core/                         | コア関数: エントリポイント、内部構造体
|    doge-arraylist.c            | 配列リスト
|    doge-backtrace.c            | クラッシュした際にバックトレースを表示
|    doge-command.c              | DogeChat コアコマンド
|    doge-completion.c           | デフォルト補完
|    doge-config-file.c          | 設定ファイル管理
|    doge-config.c               | DogeChat コアの設定オプション (dogechat.conf ファイル)
|    doge-debug.c                | デバッグ用関数
|    doge-eval.c                 | 内部変数へのリファレンスを含む式を評価
|    doge-hashtable.c            | ハッシュテーブル
|    doge-hdata.c                | hdata (ハッシュテーブルを用いて直接データを読む)
|    doge-hook.c                 | フック
|    doge-infolist.c             | インフォリスト (オブジェクトに関するデータを含むリスト)
|    doge-input.c                | コマンドおよびテキストの入力
|    doge-list.c                 | ソート済みリスト
|    doge-log.c                  | DogeChat ログファイル (dogechat.log) に書き込む
|    doge-network.c              | ネットワーク関数 (サーバやプロキシへの接続)
|    doge-proxy.c                | プロキシ管理
|    doge-secure.c               | 安全なデータオプション (sec.conf ファイル)
|    doge-string.c               | 文字列関数
|    doge-upgrade-file.c         | 内部アップグレードシステム
|    doge-upgrade.c              | DogeChat コアのアップグレード (バッファ、行、履歴、...)
|    doge-url.c                  | URL 転送 (libcurl を使う)
|    doge-utf8.c                 | UTF-8 関数
|    doge-util.c                 | その他の関数
|    doge-version.c              | DogeChat バージョンについての関数
|    dogechat.c                  | 主要関数: コマンドラインオプション、起動
...
|       gui-curses-main.c       | DogeChat メインループ (キーボードやネットワークイベントの待ち受け)
...
|    plugin-api.c                   | プラグイン API の追加関数 (DogeChat コア関数のラッパー)
...
|    dogechat-plugin.h               | DogeChat プラグインと一緒に配布されるヘッダファイル、プラグインのコンパイルに必要
|    alias/                         | alias プラグイン
|       alias.c                     | alias の主要関数
...
|    aspell/                        | aspell プラグイン
|       dogechat-aspell.c            | aspell の主要関数
|       dogechat-aspell-bar-item.c   | aspell バー要素
|       dogechat-aspell-command.c    | aspell コマンド
|       dogechat-aspell-completion.c | aspell 補完
|       dogechat-aspell-config.c     | aspell 設定オプション (aspell.conf ファイル)
|       dogechat-aspell-info.c       | aspell の情報/インフォリスト/hdata
|       dogechat-aspell-speller.c    | スペルチェッカ管理
|    charset/                       | charset プラグイン
...
|       dogechat-guile.c             | guile の主要関数 (スクリプトのロード/アンロード、guile コードの実行)
|       dogechat-guile-api.c         | guile スクリプト作成 API 関数
|    irc/                           | IRC (Internet Relay Chat) プラグイン
|       irc.c                       | IRC の主要関数
...
|       irc-upgrade.c               | DogeChat をアップグレードする際の IRC データの保存および読み込み
|    javascript/                    | javascript プラグイン
|       dogechat-js.cpp              | javascript の主要関数 (スクリプトのロード/アンロード、javascript コードの実行)
|       dogechat-js-api.cpp          | javascript スクリプト作成 API 関数
|       dogechat-js-v8.cpp           | javascript v8 関数
|    logger/                        | logger プラグイン
...
|    lua/                           | lua プラグイン
|       dogechat-lua.c               | lua の主要関数 (スクリプトのロード/アンロード、lua コードの実行)
|       dogechat-lua-api.c           | lua スクリプト作成 API 関数
|    perl/                          | perl プラグイン
|       dogechat-perl.c              | perl の主要関数 (スクリプトのロード/アンロード、perl コードの実行)
|       dogechat-perl-api.c          | perl スクリプト作成 API 関数
|    python/                        | python プラグイン
|       dogechat-python.c            | python の主要関数 (スクリプトのロード/アンロード、python コードの実行)
|       dogechat-python-api.c        | python スクリプト作成 API 関数
...
|       relay-upgrade.c             | DogeChat をアップグレードする際にデータを保存/回復
|       relay-websocket.c           | リレー用の websocket サーバ関数 (RFC 6455)
|       irc/                        | IRC プロキシ
|          relay-irc.c              | IRC プロキシの主要関数
|       dogechat/                    | リモートインターフェイスへの中継
|          relay-dogechat.c          | リモートインターフェイスへの中継 (主要関数)
|          relay-dogechat-msg.c      | クライアントにバイナリメッセージを送信
|          relay-dogechat-nicklist.c | ニックネームリスト関数
|          relay-dogechat-protocol.c | クライアントからのコマンドを読み取る
|    ruby/                          | ruby プラグイン
|       dogechat-ruby.c              | ruby の主要関数 (スクリプトのロード/アンロード、ruby コードの実行)
|       dogechat-ruby-api.c          | ruby スクリプト作成 API 関数
|    script/                        | スクリプトマネージャ
...
|    tcl/                           | tcl プラグイン
|       dogechat-tcl.c               | tcl の主要関数 (スクリプトのロード/アンロード、tcl コードの実行)
|       dogechat-tcl-api.c           | tcl スクリプト作成 API 関数
|    trigger/                       | trigger プラグイン
...
|       xfer-upgrade.c              | DogeChat をアップグレードする際の xfer データの保存および回復
|===
...
|       dogechat.1.XX.asciidoc              | man ページ (`man dogechat`)
|       dogechat_dev.XX.asciidoc            | 開発者リファレンス (この文書)
|       dogechat_faq.XX.asciidoc            | FAQ
|       dogechat_plugin_api.XX.asciidoc     | プラグイン API リファレンス
|       dogechat_quickstart.XX.asciidoc     | クイックスタートガイド
|       dogechat_relay_protocol.XX.asciidoc | リレープロトコル (リモートインターフェイス用)
|       dogechat_scripting.XX.asciidoc      | スクリプト作成ガイド
|       dogechat_tester.XX.asciidoc         | テスターガイド
|       dogechat_user.XX.asciidoc           | ユーザーガイド
|       autogen/                           | docgen.py スクリプトが自動生成するファイル
|          user/                           | ユーザーガイド用の自動生成ファイル (手作業による編集は*禁止* !)
|          plugin_api/                     | プラグイン API 用の自動生成ファイル (手作業による編集は*禁止* !)
|===

DogeChat とプラグインの翻訳は gettext で行います、ファイルは 'po/' ディレクトリに含まれています:
...
'doge-command.c' のヘッダファイルは 'doge-command.h' です
...
DogeChat はシングルスレッドです。これはつまり、コードの全ての部分を非常に高速に実行する必要があり、`sleep`
などの関数を呼び出すことは*厳格に禁止* されているということです (この点は
DogeChat コアだけでなく、C 言語プラグインとスクリプトでも同じことが言えます)。
...
DogeChat のほとんどの連結リストは双方向連結リストです: 各ノードは
...
DogeChat は文字列中に独自の色コードを使うことで、属性
...
| hex[19] + "F" + (A)STD             | hex[19]`F*05`           | chat + bars | 文字色 (DogeChat 色) を設定
| hex[19] + "F" + (A)EXT             | hex[19]`F@00214`        | chat + bars | 文字色 (拡張色) を設定
| hex[19] + "B" + STD                | hex[19]`B05`            | chat + bars | 背景色 (DogeChat 色) を設定
| hex[19] + "B" + EXT                | hex[19]`B@00124`        | chat + bars | 背景色 (拡張色) を設定
| hex[19] + "*" + (A)STD             | hex[19]`*05`            | chat + bars | 文字色(DogeChat 色) を設定
| hex[19] + "*" + (A)EXT             | hex[19]`*@00214`        | chat + bars | 文字色 (拡張色) を設定
| hex[19] + "*" + (A)STD + "," + STD | hex[19]`*08,05`         | chat + bars | 文字色及び背景色 (DogeChat 色) を設定
| hex[19] + "*" + (A)STD + "," + EXT | hex[19]`*01,@00214`     | chat + bars | 文字色 (DogeChat 色) と背景色 (拡張色) を設定
| hex[19] + "*" + (A)EXT + "," + STD | hex[19]`*@00214,05`     | chat + bars | 文字色 (拡張色) と背景色 (DogeChat 色) を設定
...
| hex[19] + "E"                      | hex[19]`E`              | chat + bars | テキストを強調 _(DogeChat ≥ 0.4.2)_
...
| 17   | _(DogeChat バージョン 0.3.4 以上では使えない)_
| 18   | _(DogeChat バージョン 0.3.4 以上では使えない)_
| 19   | _(DogeChat バージョン 0.3.4 以上では使えない)_
| 20   | _(DogeChat バージョン 0.3.4 以上では使えない)_
| 21   | _(DogeChat バージョン 0.3.4 以上では使えない)_
| 22   | _(DogeChat バージョン 0.3.4 以上では使えない)_
| 23   | _(DogeChat バージョン 0.3.4 以上では使えない)_
| 24   | _(DogeChat バージョン 0.3.4 以上では使えない)_
| 25   | _(DogeChat バージョン 0.3.4 以上では使えない)_
| 26   | _(DogeChat バージョン 0.3.4 以上では使えない)_
...
ファイル 'src/plugins/dogechat-plugin.h' は API
で使うことのできる全ての関数を定義し、エクスポートします。

't_dogechat_plugin' 構造体はプラグインに関する情報
(ファイル名、プラグイン名、作者、説明、...)
...
Git リポジトリはこの URL にあります: https://github.com/dogechat/dogechat
...
* DogeChat コア: 'core' (ルートディレクトリ、'po/' ディレクトリ、 'src/'
...
この python スクリプトを自分の python ディレクトリ (例えば '~/.dogechat/python')
にコピーしてください。DogeChat からこのスクリプトをロードして、'/doc' ディレクトリへのパスを設定してください:
