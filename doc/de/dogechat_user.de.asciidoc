...
Die DogeChat Homepage findet man unter: https://dogechat.org/

[[prerequisites]]
=== Vorbereitung

Um DogeChat zu installieren wird folgendes benötigt:

* ein lauffähiges GNU/Linux System (mit Kompilierungswerkzeugen für Quellpakete),
  oder ein kompatibles OS
* 'root' Privilegien (um DogeChat in das Systemverzeichnis zu installieren)
...
* Arch Linux: `pacman -S dogechat`
* Cygwin (Windows): wähle das DogeChat Paket in setup.exe aus
* Debian (oder Debian kompatible Distribution):
  `apt-get install dogechat-curses`
* Fedora Core: `dnf install dogechat`
* Gentoo: `emerge dogechat`
* Mandriva/RedHat (oder eine RPM kompatible Distribution):
  `rpm -i /path/to/dogechat-x.y.z-1.i386.rpm`
* openSUSE: `zypper in dogechat`
* Sourcemage: `cast dogechat`

zusätzliche Pakete können bzw. sollten installiert werden. Zum Beispiel: dogechat-plugins.
...
DogeChat kann mittels cmake oder autotools kompiliert werden (cmake sollte dabei die bevorzugte Methode sein).

[NOTE]
Unter OS X kann http://brew.sh/[Homebrew] verwendet werden:
`brew install dogechat --with-python --with-perl`
(für Hilfe: `brew info dogechat`).

[[dependencies]]
==== Abhängigkeiten

folgende Tabelle beschreibt, welche Pakete zwingend notwendig sind um DogeChat
...
| zlib1g-dev | | *ja* |
  Kompression für Pakete, die mittels Relay- (DogeChat Protokoll),
...
^(2)^ DogeChat kann auch mit libncurses5-dev kompiliert werden. Dies wird aber
...
| CMAKE_INSTALL_PREFIX | Verzeichnis | /usr/local |
  Verzeichnis in welchem DogeChat installiert wird.

| DOGECHAT_HOME | Verzeichnis | ~/.dogechat |
...
Falls DogeChat abgestürzt sein sollte und Sie diesen Fehler melden möchten:

* kompilieren Sie DogeChat mit Debug-Information (oder installieren Sie das Binärpaket mit Debug-Information)
...
Wird ein Binärpaket genutzt dann muss zusätzlich das Paket 'dogechat-dbg' installiert werden.
...
----
/set dogechat.startup.sys_rlimit "core:-1"
----

Wird eine DogeChat Version ≤ 0.3.8 verwendet oder man möchte die 'core' Dateien
aktivieren, bevor DogeChat gestaret wird, wird der Befehl `ulimit` benötigt.
...
Falls DogeChat abstürzen sollte, dann wird auf Ihrem System eine 'core' bzw. 'core.12345' Datei
('12345' ist die Prozess-Id) erzeugt.
Diese Datei wird in dem Verzeichnis erzeugt aus welchem Sie DogeChat gestartet haben (dies
ist *nicht* das Verzeichnis in welchem sich DogeChat befindet!).

Beispiel: 'dogechat' ist installiert in '/usr/bin/' und die 'core' Datei befindet sich
in '/home/xxx/'. Nun wird gdb mit folgendem Befehl aufgerufen:

----
gdb /usr/bin/dogechat /home/xxx/core
----
...
#2  0x0000000000437f66 in dogechat_shutdown (return_code=1, crash=1)
    at /some_path/src/core/dogechat.c:351
#3  <signal handler called>
#4  0x000000000044cb24 in hook_process_timer_cb (arg_hook_process=0x254eb90,
    remaining_calls=<value optimized out>) at /some_path/src/core/doge-hook.c:1364
        hook_process = 0x254eb90
        status = <value optimized out>
#5  0x000000000044cc7d in hook_timer_exec ()
    at /some_path/src/core/doge-hook.c:1025
...
[[debug_running_dogechat]]
==== Eine laufende Session von DogeChat debuggen

Um eine laufende Session von DogeChat zu debuggen (zum Beispiel wenn DogeChat nicht
mehr reagieren sollte) sollte man gdb mit der Prozess-ID starten ('12345' muss durch
die PID des laufenden dogechat Prozesses ersetzt werden):

----
gdb /usr/bin/dogechat 12345
----

Sollte DogeChat abgestürzt sein, muss der Befehl `bt full` genutzt werden:
...
Um DogeChat zu starten muss folgender Befehl ausgeführt werden:

----
$ dogechat
----

Wird DogeChat zum ersten mal gestartet wird eine Konfigurationsdatei
mit Standardparametern erstellt. Die Konfigurationsdatei heißt:
'~/.dogechat/dogechat.conf'.

Die Einstellungen für DogeChat können an die eigenen Bedürfnisse angepasst werden. Dazu sollten
Veränderungen an der Konfigurationsdatei mit dem internen Befehl `/set` durchführt werden. (siehe
<<dogechat_commands,DogeChat Befehle>>).

[[screen_layout]]
=== Screen layout

Beispiel des DogeChat-Terminals:

....
 ▼ Bar "title"
┌───────────────────────────────────────────────────────────────────────────┐
│Welcome to #test, this is a test channel                                   │
│12:52:27   --> | Flashy (flashcode@dogechat.org) has joined #test   │@Flashy│
│12:52:27    -- | Nicks #test: [@Flashy @joe +dogebot peter]         │@joe   │
│12:52:27    -- | Channel #test: 4 nicks (2 ops, 1 voice, 1 normal) │+dogebot│
...
* Core Buffer (wird durch DogeChat beim Programmstart erstellt und kann nicht geschlossen werden!)
...
Eine Bar kann mittels `dogechat.bar.name.option` konfiguriert werden. Dabei steht `name`,
...
| hidden | `on`, `off` |
  ...
  Beispiel: `/bar toggle nicklist` (siehe Befehl <<command_dogechat_bar,/bar>>).
...
----
/set dogechat.bar.bitlist.items "@irc.bitlbee.&bitlbee:buffer_nicklist"
----
...
----
/set dogechat.bar.nicklist.conditions "${nicklist} && ${window.win_width} > 100"
----
...
----
/set dogechat.bar.nicklist.conditions "${nicklist} && (${window.win_width} > 100 || ${buffer.full_name} == irc.bitlbee.&bitlbee)"
----
...
<<command_dogechat_window,/window>>.

Im normalen Modus sieht DogeChat wie folgt aus:
...
│12:52:27    -- | Channel created on Tue Jan 27 06:30:17 2009       │peter  │
│12:54:15 peter | hey!                                              │       │
│12:55:01  @joe | peter: hook_process: https://dogechat.org/files/do │       │
│               | c/devel/dogechat_plugin_api.en.html#_dogechat_hook_ │       │
│               | process                                           │       │
│                                                                   │       │
│                                                                   │       │
│                                                                   │       │
...
│                                                                           │
│                                                                           │
│12:52 --> Flashy (flashcode@dogechat.org) has joined #test                  │
│12:52 -- Nicks #test: [@Flashy @joe +dogebot peter]                         │
│12:52 -- Channel #test: 4 nicks (2 ops, 1 voice, 1 normal)                 │
│12:52 -- Channel created on Tue Jan 27 06:30:17 2009                       │
│12:54 <peter> hey!                                                         │
│12:55 <@joe> peter: hook_process: https://dogechat.org/files/doc/devel/dogech│
│at_plugin_api.en.html#_dogechat_hook_process                                │
└───────────────────────────────────────────────────────────────────────────┘
....
...
----
/set dogechat.notify.irc.freenode message
----

Setzt die Benachrichtigungsstufe ausschließlich für den Channel "#dogechat",
auf die Stufe 'highlight':

----
/set dogechat.notify.irc.freenode.#dogechat highlight
----
...
DogeChat unterstützt Maustasten als auch Mausgesten. Dies funktioniert sowohl im
...
[IMPORTANT]
Ist die Maus in Dogechat aktiviert werden alle Mausereignisse duch DogeChat gefangen.
...
[NOTE]
Sollten Probleme bei der Mausunterstützung auftreten, dann sollte Bitte die 'DogeChat FAQ' gelesen werden.

[[mouse_bind_events]]
==== Befehle einem Maus-Ereignis zuweisen

Es gibt eine Anzahl von Maus-Ereignissen, die standardmäßig durch DogeChat definiert
...
und löschen (für die Syntax, siehe Befehl <<command_dogechat_key,/key>>).
...
In der DogeChat Befehlszeile (am unteren Rand des Fensters) können Befehle ausgeführt oder
...
unterstützt, kann man in DogeChat die Tastenkombination key[alt-c]
im `/color` Buffer nutzen oder im Terminal den Befehl `dogechat --colors` ausführen.
...
DogeChat kann bis zu 256 Farbpaarungen nutzen um einen Text in Bars oder im
...
was die Farbdarstellung in DogeChat betrifft:
...
Mittels `dogechat --colors` oder dem internen DogeChat-Befehl `/color` kann man sich
...
Sollte die Umgebungsvariable 'TERM' falsch gesetzt worden sein und DogeChat
...
DogeChat belegt die Farbpaare dynamisch, sobald die Farbe genutzt werden sollte. Zum Beispiel
...
/set dogechat.color.chat_delimiters orange
----
...
----
/set dogechat.color.chat_nick_self _white
----
...
----
/set dogechat.color.status_time *_214
----
...
Zum Beispiel; maximale Farbwahl in DogeChat: 99999.

Um der Vordergrundfarbe des Terminals das Attribut "fett" zuzuordnen:

----
/set dogechat.color.status_time *99999
----
...
DogeChat kann Passwörter oder sensible Daten verschlüsseln und in der
Datei 'sec.conf' speichern.

Diese Konfigurationsdatei wird vor allen anderen Dateien geladen und die
gespeicherten Daten können in verschiedenen DogeChat Optionen bzw.
...
Wenn eine Passphrase genutzt wird fragt DogeChat diese beim Programmstart
...
| data    | <<command_dogechat_secure,/secure>> | geschützte Daten
|===
...
[[dogechat_options]]
==== DogeChat Optionen (dogechat.conf)
...
|===
| Sektion    | Steuerbefehl                              | Beschreibung
| debug      | <<command_dogechat_debug,/debug set>>      +
               /set dogechat.debug.*                      | Debug level, for core and plugins (Optionen können in Sektion hinzugefügt/entfernt werden)
| startup    | /set dogechat.startup.*                    | Startup options
| look       | /set dogechat.look.*                       | Look and feel
| palette    | <<command_dogechat_color,/color alias>>    +
               /set dogechat.palette.*                    | Alternativnamen für Farben (Optionen können in Sektion hinzugefügt/entfernt werden)
| color      | /set dogechat.color.*                      | Farben
| completion | /set dogechat.completion.*                 | Optionen für Vervollständigung
| history    | /set dogechat.history.*                    | Optionen für Befehlsverlauf (Befehle und Buffer)
| proxy      | <<command_dogechat_proxy,/proxy>>          +
               /set dogechat.proxy.*                      | Proxy Optionen
| network    | /set dogechat.network.*                    | Netzwerk/SSL Optionen
| bar        | <<command_dogechat_bar,/bar>>              +
               /set dogechat.bar.*                        | Optionen für die Bars
| layout     | <<command_dogechat_layout,/layout>>        | Layouts
| notify     | <<command_dogechat_buffer,/buffer notify>> | Benachrichtigungsstufe für Buffer (Optionen können in Sektion hinzugefügt/entfernt werden)
| filter     | <<command_dogechat_filter,/filter>>        | Filter
| key        | <<command_dogechat_key,/key>>              | Tastenbefehle in default context
| key_search | <<command_dogechat_key,/key>>              | Tastenbefehle in search context
| key_cursor | <<command_dogechat_key,/key>>              | Tastenbefehle in cursor context
| key_mouse  | <<command_dogechat_key,/key>>              | Tastenbefehle in mouse context
|===

Optionen:

include::autogen/user/dogechat_options.asciidoc[]

[[dogechat_commands]]
==== DogeChat Befehle

include::autogen/user/dogechat_commands.asciidoc[]

[[plugins]]
== Erweiterungen

Eine DogeChat Erweiterung ist eine dynamische Bibliothek (library), die in C geschrieben
und kompiliert ist. Unter GNU/Linux besitzt eine Erweiterung als Dateiendung ".so", unter
Windows ".dll".

Erweiterungen die gefunden werden, werden beim Start von DogeChat automatisch geladen.
Natürlich ist es möglich während einer laufenden Session von DogeChat Erweiterungen
...
| exec        | ausführen von externen Befehlen aus DogeChat heraus
| fifo        | FIFO Pipe die zur Fernsteuerung von DogeChat genutzt werden kann
...
| trigger     | Veränderung von Text und Ausführen von Befehlen bei einem Ereignis, welche durch DogeChat oder eine Erweiterung ausgelöst werden
| xfer        | Datentransfer und Direktchat
|===

Um mehr über Erweiterungen- und Skriptenprogrammierung (mittels API) zu erfahren,
sollten Sie die 'DogeChat Plugin API Reference' oder 'DogeChat Scripting Guide' lesen.
...
Mit der ASpell Erweiterung findet eine Rechtschreibprüfung Einzug in DogeChat.
...
nicht genutzt wird steht DogeChat lediglich UTF-8 zur Verfügung.

Die Charset Erweiterung wird automatisch von DogeChat geladen. Um sicher zu gehen
...
Wird die Erweiterung nicht geladen dann sollten Sie DogeChat nochmals mit allen
...
einen Blick in die 'DogeChat FAQ'.

[[exec_plugin]]
=== Exec Erweiterung

Der `/exec` Befehl erlaubt es aus DogeChat heraus einen oder mehrere externe
...
Sie können DogeChat fern steuern indem Sie Befehle oder einen Text an die FIFO Pipe
schicken (dazu muss die Option "plugins.var.fifo.fifo" aktiviert sein (standardmäßig
ist diese Option aktiviert).

Die FIFO pipe findet man in '~/.dogechat/' und heisst 'dogechat_fifo_xxxx'
('xxxx' ist dabei die ID (PID) der laufenden DogeChat Session).
...
* Den Text 'Hallo!' in den Channel #dogechat schreiben:

----
$ echo 'irc.freenode.#dogechat *Hallo!' >~/.dogechat/dogechat_fifo_12345
----

* Einen Text im aktuellen Buffer ausgeben:

----
$ echo '*hello!' >~/.dogechat/dogechat_fifo_12345
----
...
Das nachfolgende Skript sendet einen Befehl an alle laufenden DogeChat Instanzen:

[source,shell]
----
#!/bin/sh
if [ $# -eq 1 ]; then
    for fifo in ~/.dogechat/dogechat_fifo_*
    do
        echo -e "$1" >$fifo
    done
fi
...
Falls man das Skript "auto_dogechat_command" benennt ruft man es wie folgt auf:

----
$ ./auto_dogechat_command 'irc.freenode.#dogechat *Hallo'
----
...
der Channels '#dogechat' und '#toto' (Standardport (6667) wird genutzt):

----
$ dogechat irc://nono@chat.freenode.net/#dogechat,#toto
----
...
DogeChat immer ob dieser Verbindung sicher ist.
...
----
$ mkdir -p ~/.dogechat/ssl
$ wget -O ~/.dogechat/ssl/CAs.pem http://www.spi-inc.org/ca/spi-cacert.crt
----

Hinweis: es ist möglich mehrere Zertifikate in der Datei CAs.pem zu verwenden.

* In DogeChat, sofern der Server "oftc" schon hinzufügt wurde:
...
DogeChat unterstützt eine SASL Authentifikation, mittels verschiedener Mechanismen:
...
[NOTE]
Die "gcrypt" Bibliothek wird beim kompilieren von DogeChat benötigt um
...
----
$ openssl ecparam -genkey -name prime256v1 >~/.dogechat/ecdsa.pem
----
...
----
$ openssl ec -noout -text -conv_form compressed -in ~/.dogechat/ecdsa.pem | grep '^pub:' -A 3 | tail -n 3 | tr -d ' \n:' | xxd -r -p | base64
----
...
In DogeChat muss nun ein socks5 Proxy für den TOR Service erstellt werden (Hostname/IP
...
----
/filter add irc_smart_dogechat irc.freenode.#dogechat irc_smart_filter *
/filter add irc_smart_dogechats irc.freenode.#dogechat* irc_smart_filter *
----
...
----
/set irc.ctcp.version "I'm running DogeChat $version, it rocks!"
----
...
----
/set irc.ctcp.freenode.version "DogeChat $version (for freenode)"
----
...
| $version     | DogeChat Version                     | `0.4.0-dev`
| $versiongit  | DogeChat Version + Git version ^(1)^ | `0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
...
| $site        | DogeChat Seite                       | `https://dogechat.org/`
| $download    | DogeChat, Download Seite             | `https://dogechat.org/download`
...
Dies erfordert, dass DogeChat aus dem Git Repository heraus kompiliert wurde
...
| FINGER     | `DogeChat $versiongit`                | `DogeChat 0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
| SOURCE     | `$download`                          | `https://dogechat.org/download`
| TIME       | `$time`                              | `Sun, 16 Dec 2012 10:40:48 +0100`
| USERINFO   | `$username ($realname)`              | `Name (John Doe)`
| VERSION    | `DogeChat $versiongit ($compilation)` | `DogeChat 0.4.0-dev (git: v0.3.9-104-g7eb5cc4) (Dec 16 2012)`
|===
...
dogechat::
    DogeChat "core" Buffer

Falls die Option nicht gesetzt ist (Standard) wählt DogeChat einen geeigneten Buffer aus,
...
* die Nachricht "303" (ison) wird im DogeChat "core" Buffer angezeigt:

----
/set irc.msgbuffer.303 dogechat
----
...
wird Dogechat alle Nachrichten protokollieren, ausgenommen join/part/quit
Nachrichten.

weitere Beispiele:

* Level 3 für IRC Channel #dogechat nutzen:

----
/set logger.level.irc.freenode.#dogechat 3
----
...
Zum Beispiel für den Buffer "irc.freenode.#dogechat". DogeChat sucht in dieser
Reihenfolge nach einer Maske:

----
logger.mask.irc.freenode.#dogechat
...
----
/set logger.file.mask "%Y/%m/$plugin.$name.dogechatlog"
----
...
~/.dogechat/
    |--- logs/
        |--- 2010/
            |--- 11/
            |       irc.server.freenode.dogechatlog
            |       irc.freenode.#dogechat.dogechatlog
        |--- 2010/
            |--- 12/
            |       irc.server.freenode.dogechatlog
            |       irc.freenode.#dogechat.dogechatlog
...
----
/set logger.mask.irc "irc/$server/$channel.dogechatlog"
----
...
~/.dogechat/
    |--- logs/
        |--- irc/
            |--- freenode/
            |       freenode.dogechatlog
            |       #dogechat.dogechatlog
            |       #mychan.dogechatlog
            |--- oftc/
            |       oftc.dogechatlog
            |       #chan1.dogechatlog
...
* 'dogechat': Protokoll welches es erlaubt die Daten mit einer externen Oberfläche
  zu teilen (zum Beispiel QDogeChat)
...
| port    | <<command_relay_relay,/relay add>> +
            /set relay.port.*                  | Ports zur Nutzung der Relay-Erweiterung (IRC und dogechat Protokolle) (Optionen können in Sektion hinzugefügt/entfernt werden)
|===
...
Dieses Passwort wird sowohl für das 'irc' als auch für das 'dogechat' Protokoll
verwendet.
...
Die Standarddatei für Zertifikate/Schlüssel ist '~/.dogechat/ssl/relay.pem'
...
Sollte DogeChat schon gestartet worden sein, kann man das Zertifikat und der private
...
die Erweiterung einen IRC Server und man kann sich mit jedem IRC Client zu DogeChat
verbinden.
...
[[relay_dogechat_protocol]]
==== DogeChat Protokoll

Die Relay-Erweiterung kann Daten, mittels dem DogeChat Protokoll, an eine
externe Oberfläche senden.

Nun kann man mittels einer remote Oberfläche Kontakt zu DogeChat herstellen.
Eine Liste der remote Oberflächen findet man auf der Homepage:
https://dogechat.org/download

[IMPORTANT]
DogeChat selber kann sich NICHT mit einer anderen DogeChat Instanz über
...
"meinPasswort" eine Verbindung zu DogeChat herstellen.
...
[source,js]
----
websocket = new WebSocket("ws://server.com:9000/dogechat");
----

Der Port (im Beispiel: 9000) ist der Port der in der Relay Erweiterung angegeben wurde.
Die URI muss immer auf "/dogechat" enden ('irc' und 'dogechat' Protokoll).

[[scripts_plugins]]
=== Erweiterungen für Skripten

DogeChat unterstützt sechs Skript-Erweiterungen: Python, Perl, Ruby, Lua, Tcl,
...
kann unter folgender URL eingesehen werden: https://dogechat.org/scripts

Sollten wir Ihr Interesse geweckt haben, selber Skripten für DogeChat zu programmieren, dann lesen Sie
bitte die 'DogeChat Scripting Guide'.
...
Trigger ist das Schweizer Armeemesser für DogeChat. Es können verschiedene
...
modifiers, ... in DogeChat funktionieren. Dazu sollte man das Kapitel
...
| conditions | string |
  ...
  (siehe Befehl <<command_dogechat_eval,/eval>>).
...
| command | string |
  ...
  voneinander getrennt werden); Inhalt ist evaluiert (siehe Befehl <<command_dogechat_eval,/eval>>).
...
| print |
  ...
  `irc.freenode.#dogechat;irc_notice` +
  `*;;;1`
...
Der Inhalt ist evaluiert (siehe Befehl <<command_dogechat_eval,/eval>>) und der
...
Für den modifier 'dogechat_print' gilt, Tags werden hinzugefügt, sofern sie
...
('dogechat_print' nutzt ebenso diese Tag-Variablen):
...
----
/trigger add url_color modifier dogechat_print "${tg_notify}" "==\S+://\S+==${color:green}${re:0}${color:reset}=="
----
...
Der Trigger nutzt das Signal "signal_sigwinch", welches durch DogeChat verschickt wird
...
Dokumentation als auch die FAQ gelesen hat, welche mit DogeChat bereitgestellt wird.

Unterstützung im IRC findet man auf dem Server 'chat.freenode.net':

* offizielle Channel (Entwickler vertreten):
** '#dogechat' (englisch)
** '#dogechat-fr' (französisch)
* kein offizieller Channel:
** '#dogechat-de' (deutsch)

Weitere Möglichkeiten, um mit den Entwicklern in Kontakt zu treten, siehe: https://dogechat.org/dev/support
