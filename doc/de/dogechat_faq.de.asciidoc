= DogeChat FAQ (häufig gestellte Fragen)
...
[NOTE]
Diese Dokumentation bezieht sich auf die DogeChat Version ≥ 0.3.0. Es sollte aber
vorzugsweise immer die aktuell veröffentlichte Version von DogeChat genutzt werden.
...
[[dogechat_name]]
=== Woher kommt eigentlich der Name "DogeChat"?

"Doge" ist ein sich selbst nutzendes Akronym und bedeutet "Doge Enhanced Environment".
Somit lautet der vollständige Name "Doge Enhanced Environment for Chat".

"Doge" bedeutet außerdem "sehr klein" (und ja, es gibt noch eine andere Bedeutung des Wortes.
Aber diese Bedeutung trifft nicht auf DogeChat zu!)

[[why_choose_dogechat]]
=== Weshalb sollte man DogeChat nutzen? - X-Chat und Irssi sind doch auch sehr gut...

DogeChat ist sehr Speicher schonend und besticht durch innovative Funktionen.
...
[[gui]]
=== Ich habe gehört es gibt verschiedene GUIs für DogeChat. Wie kann ich diese kompilieren bzw. nutzen?

Die verschiedenen grafischen Oberflächen findet man auf folgender Seite:
https://dogechat.org/about/features

[[compile_git]]
=== Ich kann DogeChat nach Cloning des git Repository nicht kompilieren - weshalb?

Es wird empfohlen cmake zum kompilieren von DogeChat zu nutzen.

Falls Du DogeChat mit Hilfe der autotools kompilieren solltest (dich also nicht
...
[[compile_osx]]
=== Wie installiere ich DogeChat auf OS X?
...
DogeChat kann durch folgenden Befehl installiert werden:
...
[[lost]]
=== Ich habe DogeChat gestartet, komme aber nicht mehr weiter - was kann ich tun?
...
* überprüfe, ob DogeChat mit libncursesw verlinkt ist (Warnung: Dies ist bei den
  meisten Distributionen nötig - jedoch nicht bei allen): `ldd /path/to/dogechat`
* prüfe mit dem Befehl `/plugin`, ob die "Charset" Erweiterung geladen ist (falls
  dies nicht der Fall sein sollte benötigst Du wahrscheinlich noch das Paket "dogechat-plugins")
...
** Solltest Du screen nutzten, prüfe ob screen im UTF-8 Modus läuft
   ...
   screen)
* überprüfe die Option 'dogechat.look.eat_newline_glitch' (diese Option kann zu
  Darstellungsfehlern führen, wenn sie aktiviert ist)

[NOTE]
Für Dogechat wird die Nutzung von UTF-8 locale empfohlen. Nutzt Du jedoch ISO
...
[[unicode_chars]]
=== Einige Unicode-Zeichen werden im Terminal angezeigt aber in DogeChat nicht, warum?
...
Abhängig davon von wo DogeChat gestartet wird solltest Du folgenden Wert nutzen:

* läuft DogeChat auf einem lokalen oder entfernten Rechner, ohne screen oder tmux,
  ist entscheidend welcher Terminal genutzt wird: 'xterm', 'xterm-256color', 'rxvt-unicode',
  'rxvt-256color', ...
* wird DogeChat in screen gestartet: 'screen' oder 'screen-256color',
* wird DogeChat in tmux gestartet: 'tmux', 'tmux-256color',
  'screen' oder 'screen-256color'.

Falls nötig korrigiere den Wert Deiner Variablen TERM: `export TERM="xxx"`.

[[screen_weird_chars]]
=== Wenn ich Dogechat mit screen/tmux nutze habe ich merkwürdige Zeichen, wie kann ich dies beheben?
...
[[osx_display_broken]]
=== Ich habe DogeChat unter OS X kompiliert und erhalte nur "(null)" als Textausgabe. Was läuft da schief?
...
Darüber hinaus ist es unter OS X ratsam DogeChat mittels des Homebrew package manager zu installieren.
...
Falls Du doch nicht zufrieden sein solltest, dann lösche einfach die neue Bar. DogeChat erstellt
...
Ab DogeChat ≥ 1.0 kann ein einfacher Anzeigemodus genutzt werden (Standardtastenbelegung: key[alt-l]).
...
DogeChat stellt standardmäßig die Uhrzeit als auch einen Präfix in jeder
...
Ab der DogeChat Version ≥ 0.3.6, kann die Option "eat_newline_glitch" aktiviert
...
=== Ich möchte die Meldungen von Dogechat in einer anderen Sprache ausgeben lassen, ohne DogeChat zu beenden. Ist dies möglich?
...
und mit einern älteren Version von DogeChat:
...
[[use_256_colors]]
=== Wie kann ich 256 Farben in DogeChat nutzen?

256 Farben werden ab der DogeChat Version ≥ 0.3.4 unterstützt.
...
Falls die Umgebungsvariable 'TERM' einen falschen Wert haben sollte und DogeChat
schon gestartet wurde, kann man die Umgebungsvariable mit folgenden DogeChat Befehlen
anpassen (DogeChat ≥ 1.0):
...
Mit der DogeChat Version 0.3.4 muss der Befehl `/color` genutzt werden, um neue Farben zu erstellen.
...
Um den speziellen Code während des Startvorgangs von DogeChat zu aktivieren:
...
Eine Unterstützung von Mausfunktionen ist seit DogeChat ≥ 0.3.6 verfügbar.
...
Falls die Maus von DogeChat nicht erkannt werden sollte muss die TERM Variable
...
Man sollte versuchen eine andere Priorität zu nutzen (nur DogeChat ≥ 0.3.5); Im Beispiel
...
Die Option 'dogechat.network.gnutls_ca_file' sollte auf die Zertifikationsdatei zeigen:
...
Der Befehl `/filter` ist ein Kern-Befehl (A.d.Ü.: wird in DogeChat ausgeführt und
...
Ab DogeChat ≥ 0.4.1 kann man auswählen welche Server Nachrichten beim Betreten
...
Mitteilungen überflutet werden möchte), kann man DogeChat anweisen abwesende Nicks
...
Um sich in DogeChat anzeigen zu lassen welche Nicks abwesend sind siehe:
...
[NOTE]
Für Dogechat ≤ 0.3.3, lauten die Optionen 'irc.network.away_check' und
'irc.network.away_check_max_nicks'.

[[highlight_notification]]
=== Wie kann ich mich benachrichtigen lassen, falls mich jemand in einem Channel highlighted

Seit DogeChat ≥ 1.0 gibt es standardmäßig einen Trigger, "beep", der an das Terminal
...
Nutzt man eine ältere Version von DogeChat, können z.B. die Skripten 'beep.pl' oder 'launcher.pl'
genutzt werden.

Beim 'launcher.pl' Skript müssen noch die Befehle angepasst werden:

----
/set plugins.var.perl.launcher.signal.dogechat_highlight "/Pfad/zum/Befehl Argumente"
----
...
[[openbsd_plugins]]
=== Ich benutze OpenBSD aber DogeChat lädt keine Erweiterungen - weshalb?
...
Die Skripten für DogeChat sind mit anderer IRC-Clients nicht kompatibel und vice versa.

[[settings]]
== Einstellungen

[[memory_usage]]
=== Wie kann ich DogeChat dazu bringen weniger Speicher zu verbrauchen?

Damit DogeChat weniger Speicher benötigt, solltest Du folgende Tipps umsetzen:
...
* der Wert der Option 'dogechat.history.max_buffer_lines_number' sollte möglichst
  niedrig eingestellt werden oder die Option 'dogechat.history.max_buffer_lines_minutes'
  verwendet werden.
* nutze einen kleineren Wert für die Option 'dogechat.history.max_commands'

[[cpu_usage]]
=== Wie kann ich DogeChat dazu bewegen die CPU weniger zu beanspruchen?
...
* die Sekundenanzeigen in der Statusbar sollte deaktiviert werden:
  `/set dogechat.look.item_time_format "%H:%M"` (dies ist die Standardeinstellung)
...
siehe: https://dogechat.org/dev/support

[[gdb_error_threads]]
=== Wenn man DogeChat in gdb startet, kann ein Thread-Fehler auftreten. Was beudeutet das?

Falls man DogeChat in gdb ausführt, kann folgende Fehlermeldung erscheinen:

----
$ gdb /Pfad/zu/dogechat
(gdb) run
...
libpthread und DogeChat muss natürlich an das eigene System angepasst werden):

----
$ LD_PRELOAD=/lib/libpthread.so.0 gdb /Pfad/zu/dogechat
(gdb) run
----

[[supported_os]]
=== Auf welchen Plattformen läuft DogeChat und wird es noch auf andere Betriebssystemen portiert?

Eine vollständige Liste der Portierungen findest Du unter: https://dogechat.org/download

Wir geben unser Bestes um DogeChat auf möglichst viele Plattformen zu portieren.
Aber um DogeChat auf fremden Betriebssystemen zu testen, zu denen wir keinen Zugang haben,
ist Hilfe gerne gesehen.

[[help_developers]]
=== Ich möchte den Entwicklern von DogeChat helfen. Was kann ich tun?
...
[[donate]]
=== Kann ich Geld oder anderes an die DogeChat Entwickler spenden?

Du kannst uns Geld zur Unterstützung der weiteren Entwicklung spenden.
Details hierzu gibt es auf: https://dogechat.org/about/donate
