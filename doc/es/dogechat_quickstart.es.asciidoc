...
DogeChat tiene ayuda para todos los comandos, solo teclee:
...
DogeChat usará inmediatamente el nuevo valor asignado (*no* es necesario
reiniciar DogeChat después de aplicar cambios a la configuración).
...
* mostrará todas las opciones de DogeChat y sus plugins:

----
/set
----

* mostrará solo las opciones de DogeChat:
...
DogeChat o usando el comando `/save` para forzar la forzar a DogeChat a
...
Llamaremos "Núcleo de DogeChat" a la aplicación que solo se usa para
mostrar información en pantalla e interactuar con el usuario, eso es lo
único que hace el núcleo de DogeChat sin no tiene plugins. (para los
...
DogeChat usa las opciones por defecto para todos los servidores
...
Para cada opción de "server", DogeChat usa estos valores previamente
asignados siempre y cuando no sean Nulos. Si no hay nada asignado,
DogeChat usara los valores predeterminados ("irc.server_default.xxx")
...
Para que el servidor se conecte cuando DogeChat inicie:
...
DogeChat usa muchas teclas por defecto. Las mismas, están bien
...
un paquete separado (como dogechat-plugin).
Los plugins se cargan de manera automática cuando son encontrados por DogeChat
(por favor, refierase a la documentación de DogeChat para ver como cargar/descargar
plugins y scripts).
...
Ahora puede usar DogeChat y leer las FAQ/documentación para cada pregunta
en: https://dogechat.org/doc

Disfrute de DogeChat!
