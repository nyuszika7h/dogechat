= Guida per l'Utente di DogeChat
...
La home page di DogeChat si trova qui: https://dogechat.org/

[[prerequisites]]
=== Requisiti

Per poter installare DogeChat, è necessario:

* un sistema GNU/Linux in esecuzione (con gli strumenti di
  compilazione per i sorgenti), oppure OS compatibile
* privilegi di 'root' (per installare DogeChat in una directory di
  sistema)
// TRANSLATION MISSING
...
// TRANSLATION MISSING
* Cygwin (Windows): select DogeChat packages in setup.exe
...
* Mandriva/RedHat (o qualunque distribuzione compatibile con RPM):
  `rpm -i /path/per/dogechat-x.y.z-1.i386.rpm`
...
DogeChat può essere compilato con cmake oppure autotools
...
// TRANSLATION MISSING
| zlib1g-dev | | *sì* |
  Compression of packets in relay plugin (dogechat protocol), script plugin
...
^(2)^ DogeChat può essere compilato con libncurses5-dev, ma *NON* è raccomandato
...
| CMAKE_BUILD_TYPE | `Debug`, `Release`, `RelWithDebInfo`, `MinSizeRel` | |
  ...
  running development version of DogeChat.

| CMAKE_INSTALL_PREFIX | directory | /usr/local |
  The directory where DogeChat will be installed.

| DOGECHAT_HOME | directory | ~/.dogechat |
  The default home directory when running DogeChat.
...
// TRANSLATION MISSING
[WARNING]
Only cmake is officially supported to build DogeChat. You should use autotools
...
futuro di DogeChat, è necessario:
...
Se è stato installato il pacchetto binario, installare il pacchetto 'dogechat-gdb'.
...
// TRANSLATION MISSING
For DogeChat ≤ 0.3.8 or if you want to enable core files even before DogeChat
...
In caso di errore di DogeChat, il sistema creerà un file 'core' oppure 'core.12345'
('12345' è l'id del processo).
Questo file viene creato nella directory in cui è in esecuzione DogeChat (e
*non* la directory dove DogeChat è installato!).

Ad esempio, se 'dogechat' è installato in '/usr/bin/' ed il file 'core'
...
==== Debug di DogeChat durante l'esecuzione

Per eseguire il debug di un'istanza di DogeChat in esecuzione (ad esempio se
DogeChat sembra sia bloccato), è possibile usare gdb con l'id del processo
(sostituire '12345' con il PID del processo dogechat):
...
[[running_dogechat]]
=== Esecuzione di DogeChat

Argomenti a riga di comando:

include::cmdline_options.it.asciidoc[]

Per avviare DogeChat, digitare questo comando:

----
$ dogechat
----

Alla prima esecuzione di DogeChat, viene creato un file di configurazione
predefinito. Il file di configurazione viene creato nella directory:
'~/.dogechat/dogechat.conf'.

È possibile modificare questo file per configurare DogeChat a seconda delle
proprie esigenze, oppure impostando i parametri con il comando `/set`
all'interno di DogeChat (consultare <<dogechat_commands,comandi di DogeChat>>).

[[screen_layout]]
=== Layout dello schermo

Esempio di terminale con DogeChat:
...
* buffer core (creato by DogeChat all'avvio, non può essere chiuso)
...
The bar options can be set with options `dogechat.bar.name.option` where `name`
...
| hidden | `on`, `off` |
  ...
  example: `/bar toggle nicklist` (see command <<command_dogechat_bar,/bar>>).
...
* the DogeChat mouse support is disabled (if it was enabled): you can use your
...
Imposta il livello di notifica 'highlight' solo sul canale '#dogechat':
...
DogeChat supporta i click e le azioni del mouse. Funziona con il terminale
...
[NOTE]
Per qualunque problema con il mouse, consultare 'FAQ di DogeChat'.

[[mouse_bind_events]]
==== Associare gli eventi del mouse a comandi

Molti eventi del mouse predefiniti sono già definiti da DogeChat (consultare
...
La riga di comando di DogeChat (nella parte inferiore della finestra) consente di
...
key[alt-c] in DogeChat or run this command in terminal: `dogechat --colors`.
...
DogeChat può usare fino a 256 coppie di colore per visualizzare il testo nelle barre e
nell'area di chat(il terminale deve supportare 256 colori per essere utilizzati in DogeChat).

In base al valore della variabile di ambiente 'TERM', potrebbero verificarsi i
seguenti limiti per i colori in DogeChat:
...
È possibile eseguire `dogechat --colors` o utilizzare il comando `/color` in
DogeChat per visualizzare i limiti del proprio ambiente.
...
I colori base in DogeChat sono:
...
DogeChat alloca dinamicamente le coppie colore quando vengono usati i colori
...
numero massimo in DogeChat: 99999.
...
DogeChat can encrypt passwords or private data using secured data, stored in file
'sec.conf'.

This configuration file is read before any other file, and the values stored
inside can be used in various DogeChat or plugins/scripts options.
...
When a passphrase is set, DogeChat will ask you to enter it on startup (but not
on `/upgrade`).
...
| data    | <<command_dogechat_secure,/secure>> | Secured data
|===
...
[[dogechat_options]]
==== Opzioni di DogeChat (dogechat.conf)
...
| debug      | <<command_dogechat_debug,/debug set>>      +
               /set dogechat.debug.*                      | Debug level, for core and plugins (options can be added/removed in section)
...
| palette    | <<command_dogechat_color,/color alias>>    +
               /set dogechat.palette.*                    | Color aliases (options can be added/removed in section)
| color      | /set dogechat.color.*                      | Colors
| completion | /set dogechat.completion.*                 | Completion options
| history    | /set dogechat.history.*                    | History options (commands and buffers)
| proxy      | <<command_dogechat_proxy,/proxy>>          +
               /set dogechat.proxy.*                      | Proxy options
| network    | /set dogechat.network.*                    | Network/SSL options
| bar        | <<command_dogechat_bar,/bar>>              +
               /set dogechat.bar.*                        | Bar options
| layout     | <<command_dogechat_layout,/layout>>        | Layouts
| notify     | <<command_dogechat_buffer,/buffer notify>> | Notify levels for buffers (options can be added/removed in section)
| filter     | <<command_dogechat_filter,/filter>>        | Filters
| key        | <<command_dogechat_key,/key>>              | Keys in default context
| key_search | <<command_dogechat_key,/key>>              | Keys in search context
| key_cursor | <<command_dogechat_key,/key>>              | Keys in cursor context
| key_mouse  | <<command_dogechat_key,/key>>              | Keys in mouse context
|===
...
I plugin trovati vengono caricati automaticamente all'avvio di DogeChat, ed
...
// TRANSLATION MISSING
| exec       | Execution of external commands in DogeChat
| fifo       | pipe FIFO utilizzata per inviare comandi da remoto su DogeChat
...
// TRANSLATION MISSING
| trigger    | Text replacement and command execution on events triggered by DogeChat/plugins
| xfer       | Trasferimento file e chat diretta
|===

Per saperne di più riguardo lo sviluppo di plugin o script (tramite le API), per
favore consultare 'Riferimento API dei Plugin per DogeChat' oppure
'Guida allo Scripting di DogeChat'.

[[alias_plugin]]
=== Plugin Alias

Il plugin Alias consente di creare alias per i comandi (da DogeChat o altri
plugin).
...
Questo plugin è opzionale, ma raccomandato: se non caricato, DogeChat
può leggere/scrivere soltanto dati UTF-8.

Il plugin Charset dovrebbe essere caricato automaticamente da DogeChat.
...
Se il plugin non viene trovato, allora è necessario ricompilare DogeChat
...
le 'Domande Frequenti di DogeChat'.

// TRANSLATION MISSING
[[exec_plugin]]
=== Exec plugin

The `/exec` command lets you execute external commands inside DogeChat and
...
È possibile controllare da remoto DogeChat, inviando comandi o del testo ad una
pipe FIFO (se l'opzione "plugins.var.fifo.fifo" è abilitata, e lo è per default).

La pipe FIFO si trova in '~/.dogechat/' ed è chiamata 'dogechat_fifo_xxxx'
(dove 'xxxx' è l'ID del processo (PID) dell'istanza di DogeChat in esecuzione).
...
----
$ echo 'irc.server.freenode */nick nuovonick' >~/.dogechat/dogechat_fifo_12345
----

* visualizazre del testo sul canale IRC #dogechat:
...
DogeChat in esecuzione, per esempio:
...
Se lo script viene chiamato "auto_dogechat_command", può essere
eseguito con:

----
$ ./auto_dogechat_command 'irc.freenode.#dogechat *ciao'
----
...
Esempio per entrare in '#dogechat' e '#tizio' sul server 'chat.freenode.net',
porta predefinita (6667), con il nick 'caio':

----
$ dogechat irc://caio@chat.freenode.net/#dogechat,#tizio
----
...
Al momento della connessione al server IRC con SSL, DogeChat verifica in
...
// TRANSLATION MISSING
* In DogeChat, with "oftc" server already added:
...
// TRANSLATION MISSING
DogeChat supports SASL authentication, using different mechanisms:
...
[NOTE]
La libreria "gcrypt" è richiesta per compilare DogeChat al fine di usare il
...
In DogeChat è necessario creare un proxy socks5 per il servizio TOR (nome
...
----
/set irc.ctcp.version "Uso DogeChat $version, fico!"
----
...
----
/set irc.ctcp.freenode.version "DogeChat $version (per freenode)"
----
...
automaticamente da DogeChat quando si risponde alle CTCP
...
| $version     | Versione di DogeChat                     | `0.4.0-dev`
| $versiongit  | Versione di DogeChat + git version ^(1)^ | `0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
| $git         | Git version ^(1)^                       | `v0.3.9-104-g7eb5cc4`
| $compilation | Data di compilazione di DogeChat         | `Dec 16 2012`
| $osinfo      | Informazioni sull'OS                    | `Linux 2.6.32-5-amd64 / x86_64`
| $site        | Sito di DogeChat                         | `https://dogechat.org/`
| $download    | Sito di DogeChat, pagina di download     | `https://dogechat.org/download`
...
if DogeChat has been compiled inside the git repository and if git was installed.
...
dogechat::
    Il buffer "core" di DogeChat

Quando l'opzione non è impostata (default), DogeChat sceglie il buffer
...
* visualizza messaggio "303" (ison) sul buffer "core" di DogeChat:
...
Se viene impostato il livello 3 per un canale IRC, DogeChat registrerà
tutti i messaggi, ma non quelli di entrata/uscita/disconnessione.

Alcuni esempi:

* imposta il livello 3 per il canale IRC #dogechat:
...
Ad esempio, sul buffer "irc.freenode.#dogechat", DogeChat cercherà un mask con il
...
* 'irc': proxy IRC: usato per condividere le connessioni ai server IRC con uno
  o più client IRC
* 'dogechat': protocollo usato dalle interfacce remote per mostrare ed interagire
  con DogeChat (ad esempio QDogeChat)
...
| port    | <<command_relay_relay,/relay add>> +
            /set relay.port.*                  | Ports used for relay (irc and dogechat protocols) (options can be added/removed in section)
|===
...
This password is used for 'irc' and 'dogechat' protocols.
...
Il file predefinito per il certificato/chiave è '~/.dogechat/ssl/relay.pem'
...
Se DogeChat è già in esecuzione, è possibile ricaricare il certificato e la
...
you can connect to DogeChat with any other IRC client (including DogeChat itself).
...
[IMPORTANT]
// TRANSLATION MISSING
DogeChat itself can NOT connect to another DogeChat with this protocol.
...
The URI must always end with "/dogechat" (for 'irc' and 'dogechat' protocols).

[[scripts_plugins]]
=== Plugin per gli script

DogeChat fornisce 7 plugin per lo scripting: Python, Perl, Ruby, Lua, Tcl, Guile
(scheme), Javascript.
...
scripts of any language, and install/remove scripts of DogeChat scripts
repository, which are visible at this URL: https://dogechat.org/scripts

Per maggiori informazioni su come scrivere gli script, o le API DogeChat
per gli script, consultare la 'Guida allo Scripting di DogeChat'.
...
Trigger is the Swiss Army knife for DogeChat: it can hook many things (signal,
...
| hook | `signal`, `hsignal`, `modifier`, `print`, `command`, `command_run`,
  ...
  'DogeChat Plugin API Reference', chapter 'Hooks'.
...
| command | string |
  ...
  evaluated (see command <<command_dogechat_eval,/eval>>).
...
It is evaluated (see command <<command_dogechat_eval,/eval>>) and text replaced
...
For the 'dogechat_print' modifier, variables using message tags are added (see
...
Variables set using tags in message (they are set in modifier 'dogechat_print'
too):
...
The triggers catch the signal "signal_sigwinch", which is sent by DogeChat when
...
// TRANSLATION MISSING
For other ways of support, see: https://dogechat.org/dev/support
