= FAQ di DogeChat (Domande Frequenti)
...
[NOTE]
Questo documento è stato scritto per le versioni di DogeChat ≥ 0.3.0, ma dovrebbe
essere usato preferibilmente con l'ultima versione stabile di DogeChat.
...
[[dogechat_name]]
=== Da dove viene il nome "DogeChat"?

"Doge" è un acronimo ricorsivo che sta per "Doge Enhanced Environment".
Quindi il nome completo è "Doge Enhanced Envoronment for Chat."

"Doge" significa anche "molto piccolo" (e sì, c'è un altro significato, ma
non c'entra con DogeChat!).

[[why_choose_dogechat]]
=== Perché scegliere DogeChat? X-Chat ed Irssi vanno così bene...

// TRANSLATION MISSING
Because DogeChat is very light and brings innovating features.

Maggiori informazioni su https://dogechat.org/about/features

[[compilation_install]]
== Compilazione / installazione

[[gui]]
=== Ho sentito parlare di molte interfacce per DogeChat. Come si possono compilare/utilizzare?
...
[[compile_git]]
=== Non è possibile compilare DogeChat dopo aver clonato il repository git, perché?

Il metodo raccomandato per compilare DogeChat è utilizzando cmake.
...
[[lost]]
=== Ho avviato DogeChat, ma mi sono perso, cosa posso fare?
...
Si raccomanda ai nuovi utenti di leggere la guida rapida: https://dogechat.org/doc
...
* verificare che dogechat abbia un link con libncursesw (attenzione:
  necessario su molte distribuzioni ma non tutte): `ldd /path/di/dogechat`
* verificare che il plugin "charset" sia caricato con il comando `plugin` (se non
  lo è, probabilmente è necessario il pacchetto "dogechat-plugins")
...
[NOTE]
Si raccomanda il locale UTF-8 per DogeChat. Se si utilizza ISO o un altro
...
A seconda di dove viene eseguito DogeChat, si dovrebbe avere:
...
[[screen_weird_chars]]
=== Quando uso dogechat con screen/tmux, appaiono dei caratteri strani, come posso risolvere il problema?
...
[[osx_display_broken]]
=== Ho compilato DogeChat su OS X, e vedo "(null)" dovunque sullo schermo, cosa non va?

Se ncursesw è stato compilato manualmente, utilizzare ncurses standard (incluse
col sistema).

Inoltre, su OS X, si raccomanda di installare DogeChat con il gestore pacchetti
Homebrew.
...
Se non si è soddisfatti del risultato, basta eliminare la nuova barra, DogeChat
...
Con le impostazioni predefinite, DogeChat mostra l'ora ed il prefisso per ogni
...
Con DogeChat ≥ 0.3.6, si può abilitare l'opzione "eat_newline_glitch", in
...
=== Voglio cambiare la lingua utilizzata da DogeChat per i messaggi, ma senza uscire da DogeChat, è possibile?
...
[[use_256_colors]]
=== Come posso usare 256 colori in DogeChat?

I 256 colori sono supportati nelle versioni di DogeChat ≥ 0.3.4.
...
Il mouse è supportato con le versioni di DogeChat ≥ 0.3.6.
...
[[mouse_select_paste]]
=== Come posso selezionare o incollare del testo nel terminale quando il mouse è abilitato in DogeChat?

Quando il mouse è abilitato in DogeChat, è possibile usare il modificatore
...
Provare una stringa di priorità diversa (solo DogeChat ≥ 0.3.5), sostituendo
...
Impostare l'opzione 'dogechat.network.gnutls_ca_file' con il file dei certificati:
...
modificare questo comportamento e consentire a DogeChat di utilizzare un
...
Per verificare i nick assenti in DogeChat, consultare la domanda
...
[NOTE]
Per DogeChat ≤ 0.3.3, le opzioni sono 'irc.network.away_check' e
...
----
/set plugins.var.perl.launcher.signal.dogechat_highlight "/path/del/comando argomenti"
----
...
[[openbsd_plugins]]
=== Uso OpenBSD e DogeChat non carica nessun plugin, cosa c'è che non va?
...
[[memory_usage]]
=== Come posso ottimizzare DogeChat per poter utilizzare meno memoria?
...
* ridurre il valore dell'opzione 'dogechat.history.max_buffer_lines_number'  oppure
  impostare il valore dell'opzione 'dogechat.history.max_buffer_lines_minutes'
* ridurre il valore dell'opzione 'dogechat.history.max_commands'
...
[[gdb_error_threads]]
=== Quando eseguo DogeChat in gdb, c'è un errore riguardo ai thread, cosa posso fare?

Quando viene eseguito DogeChat all'interno di gdb, potrebbe verificarsi
questo errore:
...
path di libpthread e DogeChat con i path del proprio sistema):
...
[[supported_os]]
=== Qual è la lista delle piattaforme supportate da DogeChat? Verrà effettuato il port su altri sistemi operativi?

La lista completa è su questa pagina: https://dogechat.org/download

Facciamo del nostro meglio per portarlo su più piattaforme possibili. L'aiuto
per gli OS che non abbiamo, e su cui testare DogeChat, è ben accetto.

[[help_developers]]
=== Voglio aiutare gli sviluppatori di DogeChat. Cosa posso fare?
...
[[donate]]
=== Posso donare denaro o altre cose agli sviluppatori di DogeChat?

È possibile donare denaro per aiutare lo sviluppo.
Maggiori dettagli su https://dogechat.org/about/donate
