...
Questo manuale documenta le API per i plugin di DogeChat, utilizzate
dai plugin C per interagire con il core di DogeChat.

[[plugins_in_dogechat]]
== Plugin in DogeChat

Un plugin è un programma C che può richiamare le funzioni di DogeChat
definite in un'interfaccia.

Questo programma C non richiede i sorgenti di DogeChat per essere compilato
e può essere caricato dinamicamente in DogeChat con il comano `/plugin`.
...
Il plugin deve includere il file "dogechat-plugin.h" (disponibile nel codice
sorgente di DogeChat).
Il file definisce strutture e tipi utilizzati per comunicare con DogeChat.
...
DOGECHAT_PLUGIN_NAME("nome")::
    nome del plugin

DOGECHAT_PLUGIN_DESCRIPTION("descrizione")::
    breve descrizione del plugin

DOGECHAT_PLUGIN_VERSION("1.0")::
    versione del plugin

DOGECHAT_PLUGIN_LICENSE("GPL3")::
    licenza del plugin

// TRANSLATION MISSING
DOGECHAT_PLUGIN_PRIORITY(1000)::
...
[source,C]
----
int dogechat_plugin_init (struct t_dogechat_plugin *plugin,
                         int argc, char *argv[]);
----

Argomenti:

* 'plugin': puntatore alla struttura del plugin di DogeChat
...
* 'DOGECHAT_RC_OK' se l'operazione ha successo (il plugin
  verrà caricato)
* 'DOGECHAT_RC_ERROR' se c'è un errore (il plugin NON
  verrà caricato)
...
// TRANSLATION MISSING
When plugins are auto-loaded (for example on startup), DogeChat first loads all
...
The default DogeChat plugins are initialized in this order:
...
scaricato da DogeChat.

Prototipo:

[source,C]
----
int dogechat_plugin_end (struct t_dogechat_plugin *plugin);
----

Argomenti:

* 'plugin': puntatore alla struttura plugin di DogeChat

Valori restituiti:

* 'DOGECHAT_RC_OK' se l'operazione ha successo
* 'DOGECHAT_RC_ERROR' se c'è un errore

[[compile_plugin]]
=== Compilazione del plugin

La compilazione non richiede i sorgenti di DogeChat, è richiesto solo
il file 'dogechat-plugin.h'.
...
esempio '/usr/local/lib/dogechat/plugins') oppure nella cartella
plugin dell'utente (ad esempio '/home/xxx/.dogechat/plugins').
...
DOGECHAT_PLUGIN_NAME("double");
DOGECHAT_PLUGIN_DESCRIPTION("Test plugin for DogeChat");
DOGECHAT_PLUGIN_AUTHOR("Sébastien Helleu <flashcode@flashtux.org>");
DOGECHAT_PLUGIN_VERSION("0.1");
DOGECHAT_PLUGIN_LICENSE("GPL3");

struct t_dogechat_plugin *dogechat_plugin = NULL;
                   ...
                   char **argv, char **argv_eol)
{
    ...
    if (argc > 1)
    {
        dogechat_command (NULL, argv_eol[1]);
        dogechat_command (NULL, argv_eol[1]);
    }
...
int
dogechat_plugin_init (struct t_dogechat_plugin *plugin,
                     int argc, char *argv[])
{
    dogechat_plugin = plugin;

    dogechat_hook_command ("double",
...
[source,C]
----
const char *dogechat_plugin_get_name (struct t_dogechat_plugin *plugin);
----

Argomenti:

* 'plugin': puntatore alla struttura plugin di DogeChat (può essere NULL)

Valore restituito:

* nome del plugin, "core" per il core di DogeChat (se il puntatore al plugin
  è NULL)
...
[source,C]
----
const char *name = dogechat_plugin_get_name (plugin);
----
...
----
# prototipo
name = dogechat.plugin_get_name(plugin)

# esempio
plugin = dogechat.buffer_get_pointer(dogechat.current_buffer(), "plugin")
name = dogechat.plugin_get_name(plugin)
----
...
[source,C]
----
void dogechat_charset_set (const char *charset);
----
...
[source,C]
----
dogechat_charset_set ("iso-8859-1");
----
...
----
# prototipo
dogechat.charset_set(charset)

# esempio
dogechat.charset_set("iso-8859-1")
----
...
[source,C]
----
char *dogechat_iconv_to_internal (const char *charset, const char *string);
----
...
[source,C]
----
char *str = dogechat_iconv_to_internal ("iso-8859-1", "iso string: é à");
/* ... */
free (str);
----
...
----
# prototipo
str = dogechat.iconv_to_internal(charset, string)

# esempio
str = dogechat.iconv_to_internal("iso-8859-1", "iso string: é à")
----

==== iconv_from_internal

Converte la stringa dal set caratteri interno di DogeChat (UTF-8)
in un'altra.
...
[source,C]
----
char *dogechat_iconv_from_internal (const char *charset, const char *string);
----
...
[source,C]
----
char *str = dogechat_iconv_from_internal ("iso-8859-1", "utf-8 string: é à");
/* ... */
free (str);
----
...
----
# prototipo
str = dogechat.iconv_from_internal(charset, string)

# esempio
str = dogechat.iconv_from_internal("iso-8859-1", "utf-8 string: é à")
----
...
[source,C]
----
const char *dogechat_gettext (const char *string);
----
...
[source,C]
----
char *str = dogechat_gettext ("hello");
----
...
----
# prototipo
str = dogechat.gettext(string)

# esempio
str = dogechat.gettext("hello")
----
...
[source,C]
----
const char *dogechat_ngettext (const char *string, const char *plural,
                              int count);
----
...
[source,C]
----
char *str = dogechat_ngettext ("file", "files", num_files);
----
...
----
# prototipo
str = dogechat.ngettext(string, plural, count)

# esempio
num_files = 2
str = dogechat.ngettext("file", "files", num_files)
----
...
[source,C]
----
char *dogechat_strndup (const char *string, int length);
----
...
[source,C]
----
char *str = dogechat_strndup ("abcdef", 3);  /* result: "abc" */
/* ... */
free (str);
----
...
[source,C]
----
void dogechat_string_tolower (char *string);
----
...
----
char str[] = "AbCdé";
dogechat_string_tolower (str);  /* str ora è: "abcdé" */
----
...
[source,C]
----
void dogechat_string_toupper (char *string);
----
...
----
char str[] = "AbCdé";
dogechat_string_toupper (str);  /* str ora è: "ABCDé" */
----
...
[source,C]
----
int dogechat_strcasecmp (const char *string1, const char *string2);
----
...
[source,C]
----
int diff = dogechat_strcasecmp ("aaa", "CCC");  /* == -2 */
----
...
// TRANSLATION MISSING
_DogeChat ≥ 0.3.7, updated in 1.0._
...
[source,C]
----
int dogechat_strcasecmp_range (const char *string1, const char *string2, int range);
----
...
[source,C]
----
int diff = dogechat_strcasecmp_range ("nick{away}", "NICK[away]", 29);  /* == 0 */
----
...
[source,C]
----
int dogechat_strncasecmp (const char *string1, const char *string2, int max);
----
...
[source,C]
----
int diff = dogechat_strncasecmp ("aabb", "aacc", 2);  /* == 0 */
----
...
[source,C]
----
int dogechat_strncasecmp_range (const char *string1, const char *string2, int max, int range);
----
...
[source,C]
----
int diff = dogechat_strncasecmp_range ("nick{away}", "NICK[away]", 6, 29);  /* == 0 */
----
...
[source,C]
----
int dogechat_strcmp_ignore_chars (const char *string1, const char *string2,
...
[source,C]
----
int diff = dogechat_strcmp_ignore_chars ("a-b", "--a-e", "-", 1);  /* == -3 */
----
...
[source,C]
----
const char *dogechat_strcasestr (const char *string, const char *search);
----
...
* puntatore alla stringa trovata, o NULL se non trovata
  (_DogeChat ≥ 1.3_: pointer returned is a 'const char *' instead of 'char *')

Esempio in C:

[source,C]
----
const char *pos = dogechat_strcasestr ("aBcDeF", "de");  /* risultato: puntatore a "DeF" */
----
...
<<_utf8_strlen_screen,dogechat_utf8_strlen_screen>>).

Prototipo:

[source,C]
----
int dogechat_strlen_screen (const char *string);
----
...
[source,C]
----
int length_on_screen = dogechat_strlen_screen ("é");  /* == 1 */
----
...
----
# prototipo
length = dogechat.strlen_screen(string)

# esempio
length = dogechat.strlen_screen("é")  # 1
----
...
[source,C]
----
int dogechat_string_match (const char *string, const char *mask,
                          int case_sensitive);
----
...
[source,C]
----
int match1 = dogechat_string_match ("abcdef", "abc*", 0);   /* == 1 */
int match2 = dogechat_string_match ("abcdef", "*dd*", 0);   /* == 0 */
int match3 = dogechat_string_match ("abcdef", "*def", 0);   /* == 1 */
int match4 = dogechat_string_match ("abcdef", "*de*", 0);   /* == 1 */
int match5 = dogechat_string_match ("abcdef", "*b*d*", 0);  /* == 1 */
----
...
----
# prototipo
match = dogechat.string_match(string, mask, case_sensitive)

# esempio
match1 = dogechat.string_match("abcdef", "abc*", 0)   # 1
match2 = dogechat.string_match("abcdef", "*dd*", 0)   # 0
match3 = dogechat.string_match("abcdef", "*def", 0)   # 1
match4 = dogechat.string_match("abcdef", "*de*", 0)   # 1
match5 = dogechat.string_match("abcdef", "*b*d*", 0)  # 1
----
...
[source,C]
----
char *dogechat_string_expand_home (const char *path);
----
...
[source,C]
----
char *str = dogechat_string_expand_home ("~/file.txt");
...
. replace leading `%h` by DogeChat home directory,
. replace leading `~` by user home directory (call to
  <<_string_expand_home,dogechat_string_expand_home>>),
. evaluate variables
  (see <<_string_eval_expression,dogechat_string_eval_expression>>).

Prototipo:

[source,C]
----
char *dogechat_string_eval_path_home (const char *path,
...
* 'pointers': hashtable for call to function
  <<_string_eval_expression,dogechat_string_eval_expression>>
...
[source,C]
----
char *str = dogechat_string_expand_home ("%h/test");
/* result: "/home/xxx/.dogechat/test" */
/* ... */
free (str);
----
...
----
# prototipo
path = dogechat.string_eval_path_home(path, pointers, extra_vars, options)

# esempio
path = dogechat.string_eval_path_home("%h/test", "", "", "")
# path == "/home/xxx/.dogechat/test"
----
...
[source,C]
----
char *dogechat_string_remove_quotes (const char *string, const char *quotes);
----
...
[source,C]
----
char *str = dogechat_string_remove_quotes (string, " 'Non posso' ", "'");
/* risultato: "Non posso" */
/* ... */
free (str);
----
...
[source,C]
----
char *dogechat_string_strip (const char *string, int left, int right,
                            const char *chars);
----
...
[source,C]
----
char *str = dogechat_string_strip (".abc -", 0, 1, "- .");  /* risultato: ".abc" */
/* ... */
free (str);
----
...
[source,C]
----
char *dogechat_string_convert_escaped_chars (const char *string);
----
...
[source,C]
----
char *str = dogechat_string_convert_escaped_chars ("snowman: \\u2603");
/* str == "snowman: ☃" */
/* ... */
free (str);
----
...
[source,C]
----
char *dogechat_string_mask_to_regex (const char *mask);
----
...
[source,C]
----
char *str_regex = dogechat_string_mask_to_regex ("test*mask");
/* result: "test.*mask" */
/* ... */
free (str_regex);
----
...
----
# prototipo
regex = dogechat.string_mask_to_regex(mask)

# esempio
regex = dogechat.string_mask_to_regex("test*mask")  # "test.*mask"
----
...
[source,C]
----
const char *dogechat_string_regex_flags (const char *regex, int default_flags, int *flags)
----
...
int flags;
const char *ptr_regex = dogechat_string_regex_flags (regex, REG_EXTENDED, &flags);
...
<<_string_regex_flags,dogechat_string_regex_flags>>).

Prototipo:

[source,C]
----
int dogechat_string_regcomp (void *preg, const char *regex, int default_flags)
----
...
----
regex_t my_regex;
if (dogechat_string_regcomp (&my_regex, "(?i)test", REG_EXTENDED) != 0)
{
    /* error */
}
----
...
[source,C]
----
int dogechat_string_has_highlight (const char *string,
...
[source,C]
----
int hl = dogechat_string_has_highlight ("my test string", "test,word2");  /* == 1 */
----
...
----
# prototipo
highlight = dogechat.string_has_highlight(string, highlight_words)

# esempio
highlight = dogechat.string_has_highlight("my test string", "test,word2")  # 1
----
...
[source,C]
----
int dogechat_string_has_highlight_regex (const char *string, const char *regex);
----
...
[source,C]
----
int hl = dogechat_string_has_highlight_regex ("my test string", "test|word2");  /* == 1 */
----
...
----
# prototipo
highlight = dogechat.string_has_highlight_regex(string, regex)

# esempio
highlight = dogechat.string_has_highlight_regex("my test string", "test|word2")  # 1
----
...
[source,C]
----
char *dogechat_string_replace (const char *string, const char *search,
                              const char *replace);
----
...
[source,C]
----
char *str = dogechat_string_replace ("test", "s", "x");  /* result: "text" */
/* ... */
free (str);
----
...
[source,C]
----
char *dogechat_string_replace_regex (const char *string, void *regex,
...
* 'regex': pointer to a regular expression ('regex_t' structure) compiled with
  DogeChat function <<_string_regcomp,dogechat_string_regcomp>> or regcomp
  (see `man regcomp`)
...
regex_t my_regex;
char *string;
if (dogechat_string_regcomp (&my_regex, "([0-9]{4})-([0-9]{2})-([0-9]{2})",
                            REG_EXTENDED) == 0)
{
    string = dogechat_string_replace_regex ("date: 2014-02-14", &my_regex,
                                           "$3/$2/$1", '$', NULL, NULL);
...
[source,C]
----
char **dogechat_string_split (const char *string, const char *separators,
...
** 2: come il punto 1, ma non rimuove i delimitatori alla fine della stringa
   prima della divisione _(DogeChat ≥ 0.3.6)_
...
* array di stringhe, NULL se si verifica un problema (deve essere liberata chiamando
  <<_string_free_split,dogechat_string_free_split>> dopo l'uso)
...
char **argv;
int argc;
argv = dogechat_string_split ("abc de  fghi", " ", 0, 0, &argc);
/* result: argv[0] == "abc"
           argv[1] == "de"
           argv[2] == "fghi"
           argv[3] == NULL
...
argv = dogechat_string_split ("abc de  fghi", " ", 1, 0, &argc);
...
[source,C]
----
char **dogechat_string_split_shell (const char *string, int *num_items);
----
...
char **argv;
int argc;
argv = dogechat_string_split_shell ("test 'first arg'  \"second arg\"", &argc);
/* result: argv[0] == "test"
           argv[1] == "first arg"
           argv[2] == "second arg"
           argv[3] == NULL
...
[source,C]
----
void dogechat_string_free_split (char **split_string);
----

Argomenti:

* 'split_string': stringa divisa dalla funzione
  <<_string_split,dogechat_string_split>>
...
char *argv;
int argc;
argv = dogechat_string_split (string, " ", 0, 0, &argc);
/* ... */
dogechat_string_free_split (argv);
----
...
[source,C]
----
char *dogechat_string_build_with_split_string (char **split_string,
...
char **argv;
int argc;
argv = dogechat_string_split ("abc def ghi", " ", 0, 0, &argc);
char *str = dogechat_string_build_with_split_string (argv, ";");
/* str == "abc;def;ghi" */
/* ... */
free (str);
----
...
[source,C]
----
char **dogechat_string_split_command (const char *command, char separator);
----
...
* array di stringhe, NULL in caso di problemi (deve essere liberata chiamando
  <<_free_split_command,dogechat_free_split_command>> dopo l'uso)

Esempio in C:

[source,C]
----
char **argv = dogechat_string_split_command ("/command1 arg;/command2", ';');
/* result: argv[0] == "/command1 arg"
           argv[1] == "/command2"
*/
dogechat_free_split_command (argv);
----
...
[source,C]
----
void dogechat_string_free_split_command (char **split_command);
----

Argomenti:

* 'split_command': comando diviso da
  <<_string_split_command,dogechat_string_split_command>>
...
[source,C]
----
char *dogechat_string_format_size (unsigned long long size);
----
...
char *str = dogechat_string_format_size (0);  /* str == "0 bytes" */
/* ... */
free (str);

char *str = dogechat_string_format_size (1);  /* str == "1 byte" */
/* ... */
free (str);

char *str = dogechat_string_format_size (200);  /* str == "200 bytes" */
/* ... */
free (str);

char *str = dogechat_string_format_size (15200);  /* str == "15.2 KB" */
/* ... */
free (str);

char *str = dogechat_string_format_size (2097152);  /* str == "2.10 MB" */
/* ... */
free (str);
----
...
Rimuove i colori di DogeChat da una stringa.

Prototipo:

[source,C]
----
char *dogechat_string_remove_color (const char *string,
...
* 'string': stringa
* 'replacement': se non NULL e non vuota, i codici colore di DogeChat sono
  sostituiti dal primo carattere di questa stringa, altrimenti i codici colori di
  DogeChat ed i caratteri seguenti (se correlate al colore) sono rimossi dalla
  stringa
...
char *str = dogechat_string_remove_color (my_string1, NULL);
/* ... */
free (str);

/* sostituisce i codici colore con "?" */
char *str = dogechat_string_remove_color (my_string2, "?");
/* ... */
free (str);
----
...
----
# prototipo
str = dogechat.string_remove_color(string, replacement)

# esempio
str = dogechat.string_remove_color(my_string, "?")
----
...
[source,C]
----
void dogechat_string_encode_base64 (const char *from, int length, char *to);
----
...
dogechat_string_encode_base64 (string, strlen (string), result);
/* result == "YWJjZGVmZ2g=" */
----
...
[source,C]
----
int dogechat_string_decode_base64 (const char *from, char *to);
----
...
int length;
length = dogechat_string_decode_base64 (string, result);
...
char *dump = dogechat_string_hex_dump (string, strlen (string), 8, " >> ", NULL);
...
[source,C]
----
int dogechat_string_is_command_char (const char *string);
----
...
[source,C]
----
int command_char1 = dogechat_string_is_command_char ("/test");  /* == 1 */
int command_char2 = dogechat_string_is_command_char ("test");   /* == 0 */
----
...
----
# prototipo
is_cmdchar = dogechat.string_is_command_char(string)

# esempi
command_char1 = dogechat.string_is_command_char("/test")  # == 1
command_char2 = dogechat.string_is_command_char("test")   # == 0
----
...
[source,C]
----
const char *dogechat_string_input_for_buffer (const char *string);
----
...
[source,C]
----
const char *str1 = dogechat_string_input_for_buffer ("test");    /* "test"  */
const char *str2 = dogechat_string_input_for_buffer ("/test");   /* NULL    */
const char *str3 = dogechat_string_input_for_buffer ("//test");  /* "/test" */
----
...
----
# prototipo
str = dogechat.string_input_for_buffer(string)

# esempi
str1 = dogechat.string_input_for_buffer("test")    # "test"
str2 = dogechat.string_input_for_buffer("/test")   # ""
str3 = dogechat.string_input_for_buffer("//test")  # "/test"
----

==== string_eval_expression

// TRANSLATION MISSING
_DogeChat ≥ 0.4.0, updated in 0.4.2, 1.0, 1.1, 1.2 and 1.3._
...
[source,C]
----
char *dogechat_string_eval_expression (const char *expr,
                                      struct t_hashtable *pointers,
                                      struct t_hashtable *extra_vars,
                                      struct t_hashtable *options);
----
...
** 'regex': pointer to a regular expression ('regex_t' structure) compiled with
   DogeChat function <<_string_regcomp,dogechat_string_regcomp>> or
...
| `${eval:xxx}` +
  (_DogeChat ≥ 1.3_) |
  String to evaluate |
  `${eval:${date:${dogechat.look.buffer_time_format}}}` |
...
| `${color:name}` |
  DogeChat color code (the name of color has optional attributes),
  see function <<_color,dogechat_color>> for supported formats |
  `${color:red}red text` +
...
| `${info:name}` +
  `${info:name,arguments}` |
  Info from DogeChat or a plugin, see function
  <<_info_get,dogechat_info_get>> |
  `${info:version}` +
...
----
/* conditions */
struct t_hashtable *options1 = dogechat_hashtable_new (8,
                                                      DOGECHAT_HASHTABLE_STRING,
                                                      DOGECHAT_HASHTABLE_STRING,
                                                      NULL,
                                                      NULL);
dogechat_hashtable_set (options1, "type", "condition");
char *str1 = dogechat_string_eval_expression ("${window.win_width} > 100", NULL, NULL, options1);  /* "1" */
char *str2 = dogechat_string_eval_expression ("abc =~ def", NULL, NULL, options1);                 /* "0" */

/* simple expression */
char *str3 = dogechat_string_eval_expression ("${buffer.full_name}", NULL, NULL, NULL);  /* "core.dogechat" */

/* replace with regex */
struct t_hashtable *options2 = dogechat_hashtable_new (8,
                                                      DOGECHAT_HASHTABLE_STRING,
                                                      DOGECHAT_HASHTABLE_STRING,
                                                      NULL,
                                                      NULL);
/* add brackets around URLs */
dogechat_hashtable_set (options2, "regex", "\\w+://\\S+");
dogechat_hashtable_set (options2, "regex_replace", "[ ${re:0} ]");
char *str4 = dogechat_string_eval_expression ("test: https://dogechat.org", NULL, NULL, NULL);  /* "test: [ https://dogechat.org ]" */

/* hide passwords */
dogechat_hashtable_set (options2, "regex", "(password=)(\\S+)");
dogechat_hashtable_set (options2, "regex_replace", "${re:1}${hide:*,${re:2}}");
char *str5 = dogechat_string_eval_expression ("password=abc password=def", NULL, NULL, NULL);  /* "password=*** password=***" */
----
...
----
# prototipo
str = dogechat.string_eval_expression(expr, pointers, extra_vars, options)

# esempi

# conditions
str1 = dogechat.string_eval_expression("${window.win_width} > 100", {}, {}, {"type": "condition"})  # "1"
str2 = dogechat.string_eval_expression("abc =~ def", {}, {}, {"type": "condition"})                 # "0"

# simple expression
str3 = dogechat.string_eval_expression("${buffer.full_name}", {}, {}, {}) # "core.dogechat"
...
str4 = dogechat.string_eval_expression("test: https://dogechat.org", {}, {}, options)  # "test: [ https://dogechat.org ]"
...
str5 = dogechat.string_eval_expression("password=abc password=def", {}, {}, options)  # "password=*** password=***"
----
...
[source,C]
----
int dogechat_utf8_has_8bits (const char *string);
----
...
[source,C]
----
if (dogechat_utf8_has_8bits (string))
{
    /* ... */
}
----
...
[source,C]
----
int dogechat_utf8_is_valid (const char *string, int length, char **error);
----
...
----
char *error;
if (dogechat_utf8_is_valid (string, -1, &error))
{
    /* ... */
}
else
...
[source,C]
----
void dogechat_utf8_normalize (char *string, char replacement);
----
...
[source,C]
----
dogechat_utf8_normalize (string, '?');
----
...
[source,C]
----
const char *dogechat_utf8_prev_char (const char *string_start,
                                    const char *string);
----
...
[source,C]
----
const char *prev_char = dogechat_utf8_prev_char (string, ptr_in_string);
----
...
[source,C]
----
const char *dogechat_utf8_next_char (const char *string);
----
...
[source,C]
----
const char *next_char = dogechat_utf8_next_char (string);
----
...
[source,C]
----
int dogechat_utf8_char_int (const char *string);
----
...
[source,C]
----
int char_int = dogechat_utf8_char_int ("être");  /* "ê" come intero */
----
...
[source,C]
----
int dogechat_utf8_char_size (const char *string);
----
...
[source,C]
----
int char_size = dogechat_utf8_char_size ("être");  /* == 2 */
----
...
[source,C]
----
int dogechat_utf8_strlen (const char *string);
----
...
[source,C]
----
int length = dogechat_utf8_strlen ("chêne");  /* == 5 */
----
...
[source,C]
----
int dogechat_utf8_strnlen (const char *string, int bytes);
----
...
[source,C]
----
int length = dogechat_utf8_strnlen ("chêne", 4);  /* == 3 */
----
...
[source,C]
----
int dogechat_utf8_strlen_screen (const char *string);
----
...
[source,C]
----
int length_on_screen = dogechat_utf8_strlen_screen ("é");  /* == 1 */
----
...
[source,C]
----
int dogechat_utf8_charcmp (const char *string1, const char *string2);
----
...
[source,C]
----
int diff = dogechat_utf8_charcmp ("aaa", "ccc");  /* == -2 */
----
...
[source,C]
----
int dogechat_utf8_charcasecmp (const char *string1, const char *string2);
----
...
[source,C]
----
int diff = dogechat_utf8_charcasecmp ("aaa", "CCC");  /* == -2 */
----
...
[source,C]
----
int dogechat_utf8_char_size_screen (const char *string);
----
...
[source,C]
----
int length_on_screen = dogechat_utf8_char_size_screen ("é");  /* == 1 */
----
...
[source,C]
----
const char *dogechat_utf8_add_offset (const char *string, int offset);
----
...
const char *str2 = dogechat_utf8_add_offset (str, 3);  /* points to "ne" */
----
...
[source,C]
----
int dogechat_utf8_real_pos (const char *string, int pos);
----
...
[source,C]
----
int pos = dogechat_utf8_real_pos ("chêne", 3);  /* == 4 */
----
...
[source,C]
----
int dogechat_utf8_pos (const char *string, int real_pos);
----
...
[source,C]
----
int pos = dogechat_utf8_pos ("chêne", 4);  /* == 3 */
----
...
[source,C]
----
char *dogechat_utf8_strndup (const char *string, int length);
----
...
[source,C]
----
char *string = dogechat_utf8_strndup ("chêne", 3);  /* restituisce "chê" */
/* ... */
free (string);
----
...
Crea una cartella nella home di DogeChat.

Prototipo:

[source,C]
----
int dogechat_mkdir_home (char *directory, int mode);
----
...
[source,C]
----
if (!dogechat_mkdir_home ("temp", 0755))
{
    /* errore */
}
----
...
----
# prototipo
dogechat.mkdir_home(directory, mode)

# esempio
dogechat.mkdir_home("temp", 0755)
----
...
[source,C]
----
int dogechat_mkdir (char *directory, int mode);
----
...
[source,C]
----
if (!dogechat_mkdir ("/tmp/mydir", 0755))
{
    /* errore */
}
----
...
----
# prototipo
dogechat.mkdir(directory, mode)

# esempio
dogechat.mkdir("/tmp/mydir", 0755)
----
...
[source,C]
----
int dogechat_mkdir_parents (char *directory, int mode);
----
...
[source,C]
----
if (!dogechat_mkdir_parents ("/tmp/my/dir", 0755))
{
    /* errore */
}
----
...
----
# prototipo
dogechat.mkdir_parents(directory, mode)

# esempio
dogechat.mkdir_parents("/tmp/my/dir", 0755)
----
...
[source,C]
----
void dogechat_exec_on_files (const char *directory,
                            int hidden_files,
                            void *data,
...
* 'data': puntatore fornito alla callback quando chiamata da DogeChat
...
...
dogechat_exec_on_files ("/tmp", 0, NULL, &callback);
----
...
[source,C]
----
char *dogechat_file_get_content (const char *filename);
----
...
content = dogechat_file_get_content ("/tmp/test.txt");
/* ... */
free (content);
----
...
[source,C]
----
int dogechat_util_timeval_cmp (struct timeval *tv1, struct timeval *tv2);
----
...
[source,C]
----
if (dogechat_util_timeval_cmp (&tv1, &tv2) > 0)
{
    /* tv1 > tv2 */
}
----
...
[source,C]
----
long long dogechat_util_timeval_diff (struct timeval *tv1, struct timeval *tv2);
----
...
// TRANSLATION MISSING
[NOTE]
With DogeChat ≤ 1.0, the returned value was in milliseconds.

Esempio in C:

[source,C]
----
long long diff = dogechat_util_timeval_diff (&tv1, &tv2);
----
...
[source,C]
----
void dogechat_util_timeval_add (struct timeval *tv, long long interval);
----
...
// TRANSLATION MISSING
[NOTE]
With DogeChat ≤ 1.0, the interval was expressed in milliseconds.

Esempio in C:

[source,C]
----
dogechat_util_timeval_add (&tv, 2000000);  /* aggiunge 2 secondi */
----
...
// TRANSLATION MISSING
_DogeChat ≥ 0.3.2, updated in 1.3._

// TRANSLATION MISSING
Get date/time as a string built with "strftime" and the format defined in
option 'dogechat.look.time_format'.

Prototipo:

[source,C]
----
const char *dogechat_util_get_time_string (const time_t *date);
----
...
dogechat_printf (NULL, "date: %s",
                dogechat_util_get_time_string (&date));
----
...
// TRANSLATION MISSING
Convert a string with DogeChat version to a number.

Prototipo:

[source,C]
----
int dogechat_util_version_number (const char *version);
----

Argomenti:

// TRANSLATION MISSING
* 'version': DogeChat version as string (example: "0.3.9" or "0.3.9-dev")

Esempio in C:

[source,C]
----
version_number = dogechat_util_version_number ("0.3.8");      /* == 0x00030800 */
version_number = dogechat_util_version_number ("0.3.9-dev");  /* == 0x00030900 */
version_number = dogechat_util_version_number ("0.3.9-rc1");  /* == 0x00030900 */
version_number = dogechat_util_version_number ("0.3.9");      /* == 0x00030900 */
----
...
[source,C]
----
struct t_dogelist *dogechat_list_new ();
----
...
[source,C]
----
struct t_dogelist *list = dogechat_list_new ();
----
...
[source,C]
----
struct t_dogelist_item *dogechat_list_add (struct t_dogelist *dogelist,
                                         const char *data,
                                         const char *where,
                                         void *user_data);
----
...
* 'dogelist': puntatore alla lista
* 'data': dati da inserire nella lista
* 'where': posizione nella lista:
** 'DOGECHAT_LIST_POS_SORT': aggiunge alla lista, mantenendola ordinata
** 'DOGECHAT_LIST_POS_BEGINNING': aggiunge all'inizio della lista
** 'DOGECHAT_LIST_POS_END': aggiunge alla fine della lista
...
[source,C]
----
struct t_dogelist_item *my_item =
    dogechat_list_add (list, "my data", DOGECHAT_LIST_POS_SORT, NULL);
----
...
----
# prototipo
item = dogechat.list_add(list, data, where, user_data)

# esempio
item = dogechat.list_add(list, "my data", dogechat.DOGECHAT_LIST_POS_SORT, "")
----
...
[source,C]
----
struct t_dogelist_item *dogechat_list_search (struct t_dogelist *dogelist,
                                            const char *data);
----
...
[source,C]
----
struct t_dogelist_item *item = dogechat_list_search (list, "my data");
----
...
----
# prototipo
item = dogechat.list_search(list, data)

# esempio
item = dogechat.list_search(list, "my data")
----
...
[source,C]
----
int dogechat_list_search_pos (struct t_dogelist *dogelist,
                             const char *data);
----
...
[source,C]
----
int pos_item = dogechat_list_search_pos (list, "my data");
----
...
----
# prototipo
pos_item = dogechat.list_search_pos(list, data)

# esempio
pos_item = dogechat.list_search_pos(list, "my data")
----
...
[source,C]
----
struct t_dogelist_item *dogechat_list_casesearch (struct t_dogelist *dogelist,
                                                const char *data);
----
...
[source,C]
----
struct t_dogelist_item *item = dogechat_list_casesearch (list, "my data");
----
...
----
# prototipo
item = dogechat.list_casesearch(list, data)

# esempio
item = dogechat.list_casesearch(list, "my data")
----
...
[source,C]
----
int dogechat_list_casesearch_pos (struct t_dogelist *dogelist,
                                 const char *data);
----
...
[source,C]
----
int pos_item = dogechat_list_casesearch_pos (list, "my data");
----
...
----
# prototipo
pos_item = dogechat.list_casesearch_pos(list, data)

# esempio
pos_item = dogechat.list_casesearch_pos(list, "my data")
----
...
[source,C]
----
struct t_dogelist_item *dogechat_list_get (struct t_dogelist *dogelist,
                                         int position);
----
...
[source,C]
----
struct t_dogelist_item *item = dogechat_list_get (list, 0);  /* primo elemento */
----
...
----
# prototipo
item = dogechat.list_get(list, position)

# esempio
item = dogechat.list_get(list, 0)
----
...
[source,C]
----
void dogechat_list_set (struct t_dogelist_item *item, const char *value);
----
...
[source,C]
----
dogechat_list_set (item, "nuovi dati");
----
...
----
# prototipo
dogechat.list_set(item, value)

# esempio
dogechat.list_set(item, "nuovi dati")
----
...
[source,C]
----
struct t_dogelist_item *dogechat_list_next (struct t_dogelist_item *item);
----
...
[source,C]
----
struct t_dogelist_item *next_item = dogechat_list_next (item);
----
...
[source,C]
----
struct t_dogelist_item *dogechat_list_prev (struct t_dogelist_item *item);
----
...
[source,C]
----
struct t_dogelist_item *prev_item = dogechat_list_prev (item);
----
...
[source,C]
----
const char *dogechat_list_string (struct t_dogelist_item *item);
----
...
[source,C]
----
dogechat_printf (NULL, "valore dell'elemento: %s", dogechat_list_string (item));
----
...
----
# prototipo
value = dogechat.list_string(item)

# esempio
dogechat.prnt("", "valore dell'elemento: %s" % dogechat.list_string(item))
----
...
[source,C]
----
char *dogechat_list_size (struct t_dogelist *dogelist);
----
...
[source,C]
----
dogechat_printf (NULL, "dimensione della lista: %d", dogechat_list_size (list));
----
...
----
# prototipo
size = dogechat.list_size(list)

# esempio
dogechat.prnt("", "dimensione della lista: %d" % dogechat.list_size(list))
----
...
[source,C]
----
void dogechat_list_remove (struct t_dogelist *dogelist,
...
[source,C]
----
dogechat_list_remove (list, item);
----
...
----
# prototipo
dogechat.list_remove(list, item)
...
[source,C]
----
void dogechat_list_remove_all (struct t_dogelist *dogelist);
----
...
[source,C]
----
void dogechat_list_free (struct t_dogelist *dogelist);
----
...
[source,C]
----
struct t_hashtable *dogechat_hashtable_new (int size,
...
[source,C]
----
struct t_hashtable *hashtable = dogechat_hashtable_new (8,
                                                       DOGECHAT_HASHTABLE_STRING,
                                                       DOGECHAT_HASHTABLE_STRING,
                                                       NULL,
                                                       NULL);
...
// TRANSLATION MISSING
_DogeChat ≥ 0.3.3, updated in 0.4.2._
...
[source,C]
----
struct t_hashtable_item *dogechat_hashtable_set_with_size (struct t_hashtable *hashtable,
...
[source,C]
----
dogechat_hashtable_set_with_size (hashtable, "my_key", 0,
...
[source,C]
----
struct t_hashtable_item *dogechat_hashtable_set (struct t_hashtable *hashtable,
...
[source,C]
----
dogechat_hashtable_set (hashtable, "my_key", "my_value");
----
...
[source,C]
----
void *dogechat_hashtable_get (struct t_hashtable *hashtable, void *key);
----
...
[source,C]
----
void *value = dogechat_hashtable_get (hashtable, "my_key");
----
...
[source,C]
----
int dogechat_hashtable_has_key (struct t_hashtable *hashtable, void *key);
----
...
[source,C]
----
if (dogechat_hashtable_has_key (hashtable, "my_key"))
{
...
[source,C]
----
void dogechat_hashtable_map (struct t_hashtable *hashtable,
        ...
        const void *key, const void *value)
{
    ...
    dogechat_printf (NULL, "key: '%s', value: '%s'",
                    (const char *)key,
                    (const char *)value);
}
/* ... */
dogechat_hashtable_map (hashtable, &map_cb, NULL);
----
...
[source,C]
----
void dogechat_hashtable_map_string (struct t_hashtable *hashtable,
                                   void (*callback_map)(void *data,
                                                        struct t_hashtable *hashtable,
                                                        const char *key,
                                                        const char *value),
...
/* ... */
dogechat_hashtable_map_string (hashtable, &map_cb, NULL);
----
...
[source,C]
----
struct t_hashtable *dogechat_hashtable_dup (struct t_hashtable *hashtable);
----
...
[source,C]
----
struct t_hashtable *new_hashtable = dogechat_hashtable_dup (hashtable);
----
...
[source,C]
----
int dogechat_hashtable_get_integer (struct t_hashtable *hashtable,
                                   void *property);
----
...
[source,C]
----
int items_count = dogechat_hashtable_get_integer (hashtable, "items_count");
----
...
[source,C]
----
const char *dogechat_hashtable_get_string (struct t_hashtable *hashtable,
                                          const char *property);
----
...
[source,C]
----
dogechat_printf (NULL, "keys are type: %s",
                dogechat_hashtable_get_string (hashtable, "type_keys"));
dogechat_printf (NULL, "list of keys: %s",
                dogechat_hashtable_get_string (hashtable, "keys"));
----
...
[source,C]
----
void dogechat_hashtable_set_pointer (struct t_hashtable *hashtable,
...
dogechat_hashtable_set_pointer (hashtable, "callback_free_value", &my_free_value_cb);
dogechat_hashtable_set_pointer (hashtable, "callback_free_key", &my_free_key_cb);
----
...
[source,C]
----
int dogechat_hashtable_add_to_infolist (struct t_hashtable *hashtable,
...
[source,C]
----
dogechat_hashtable_add_to_infolist (hashtable, infolist_item, "testhash");
...
[source,C]
----
void dogechat_hashtable_remove (struct t_hashtable *hashtable, const void *key);
----
...
[source,C]
----
dogechat_hashtable_remove (hashtable, "my_key");
----
...
[source,C]
----
void dogechat_hashtable_remove_all (struct t_hashtable *hashtable);
----
...
[source,C]
----
dogechat_hashtable_remove_all (hashtable);
----
...
[source,C]
----
void dogechat_hashtable_free (struct t_hashtable *hashtable);
----
...
[source,C]
----
struct t_config_file *dogechat_config_new (const char *name,
...
** valore restituito:
*** 'DOGECHAT_CONFIG_READ_OK'
*** 'DOGECHAT_CONFIG_READ_MEMORY_ERROR'
*** 'DOGECHAT_CONFIG_READ_FILE_NOT_FOUND'
* 'callback_reload_data': puntatore fornito per ricaricare il callback
  quando richiesto da DogeChat
...
la funzione <<_config_write,dogechat_config_write>>.
Si dovrebbe chiamare questa funzione solo dopo aver aggiunto alcune sezioni
(con <<_config_new_section,dogechat_config_new_section>>) e le
opzioni (con <<_config_new_option,dogechat_config_new_option>>).
...
struct t_config_file *config_file = dogechat_config_new ("test",
                                                        &my_config_reload_cb,
                                                        NULL);
----
...
----
# prototipo
config_file = dogechat.config_new(name, calback_reload, callback_reload_data)
...
config_file = dogechat.config_new("test", "my_config_reload_cb", "")
----
...
[source,C]
----
struct t_config_section *dogechat_config_new_section (
    struct t_config_file *config_file,
    const char *name,
...
*** 'DOGECHAT_CONFIG_READ_MEMORY_ERROR'
*** 'DOGECHAT_CONFIG_READ_FILE_NOT_FOUND'
* 'callback_read_data': puntatore fornito alla callback quando chiamata da DogeChat
...
** 'const char *option_name': nome dell'opzione
** valore restituito:
*** 'DOGECHAT_CONFIG_WRITE_OK'
*** 'DOGECHAT_CONFIG_WRITE_ERROR'
*** 'DOGECHAT_CONFIG_WRITE_MEMORY_ERROR'
* callback_write_data: puntatore fornito alla callback quando chiamata da DogeChat
...
** 'const char *value': valore
** valore restituito:
*** 'DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED'
*** 'DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE'
*** 'DOGECHAT_CONFIG_OPTION_SET_ERROR'
*** 'DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND'
...
** valore restituito:
*** 'DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET'
*** 'DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET'
*** 'DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED'
*** 'DOGECHAT_CONFIG_OPTION_UNSET_ERROR'
                    ...
                    const char *value)
{
    ...
    return DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED;
...
struct t_config_section *new_section1 =
    dogechat_config_new_section (config_file, "section1", 0, 0,
...
struct t_config_section *new_section2 =
    dogechat_config_new_section (config_file, "section2", 1, 1,
...
----
# prototipo
section = dogechat.config_new_section(config_file, name,
...
def my_section_read_cb(data, config_file, section, option_name, value):
    # ...
    return dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED
    # return dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE
    # return dogechat.DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND
    # return dogechat.DOGECHAT_CONFIG_OPTION_SET_ERROR

def my_section_write_cb(data, config_file, section_name):
    # ...
    return dogechat.DOGECHAT_CONFIG_WRITE_OK
...
def my_section_delete_option_cb(data, config_file, section, option):
    # ...
    return dogechat.DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED

section = dogechat.config_new_section(config_file, "section1", 1, 1,
    "my_section_read_cb", "",
    "my_section_write_cb", "",
...
[source,C]
----
struct t_config_section *dogechat_config_search_section (
    struct t_config_file *config_file,
    const char *section_name);
----
...
[source,C]
----
struct t_config_section *section = dogechat_config_search_section (config_file,
                                                                  "section");
----
...
----
# prototipo
section = dogechat.config_search_section(config_file, section_name)

# esempio
section = dogechat.config_search_section(config_file, "section")
----
...
[source,C]
----
struct t_config_option *dogechat_config_new_option (
    struct t_config_file *config_file,
    struct t_config_section *section,
    const char *name,
    const char *type,
...
// TRANSLATION MISSING
* 'name': nome dell'opzione; with DogeChat ≥ 1.4, the name can include a parent
...
struct t_config_option *option1 =
    dogechat_config_new_option (config_file, section, "option1", "boolean",
...
struct t_config_option *option2 =
    dogechat_config_new_option (config_file, section, "option2", "integer",
...
struct t_config_option *option3 =
    dogechat_config_new_option (config_file, section, "option3", "integer",
...
struct t_config_option *option4 =
    dogechat_config_new_option (config_file, section, "option4", "string",
...
struct t_config_option *option5 =
    dogechat_config_new_option (config_file, section, "option5", "color",
...
----
# prototipo
option = dogechat.config_new_option(config_file, section, name, type, description,
...
option1 = dogechat.config_new_option(config_file, section, "option1", "boolean",
    "My option, type boolean",
    "", 0, 0, "on", "on", 0,
    "", "",
    "", "",
...
option2 = dogechat.config_new_option(config_file, section, "option2", "integer",
    "My option, type integer",
    "", 0, 100, "15", "15", 0,
    "", "",
    "", "",
...
option3 = dogechat.config_new_option(config_file, section, "option3", "integer",
    "My option, type integer (with string values)",
...
option4 = dogechat.config_new_option(config_file, section, "option4", "string",
    "My option, type string",
    "", 0, 0, "test", "test", 1,
...
option5 = dogechat.config_new_option(config_file, section, "option5", "color",
    "My option, type color",
...
'DogeChat Scripting Guide' for more info _(fixed in version 0.4.1)_.
...
[source,C]
----
struct t_config_option *dogechat_config_search_option (
    struct t_config_file *config_file,
    struct t_config_section *section,
    const char *option_name);
----
...
----
struct t_config_option *option =
    dogechat_config_search_option (config_file, section, "option");
----
...
----
# prototipo
option = dogechat.config_search_option(config_file, section, option_name)

# esempio
option = dogechat.config_search_option(config_file, section, "option")
----
...
[source,C]
----
void dogechat_config_search_section_option (struct t_config_file *config_file,
                                           struct t_config_section *section,
                                           const char *option_name,
...
dogechat_config_search_section_option(config_file,
                                     section,
                                     "option",
                                     &ptr_section,
                                     &ptr_option);
...
[source,C]
----
void dogechat_config_search_with_string (const char *option_name,
                                        struct t_config_file **config_file,
                                        struct t_config_section **section,
                                        struct t_config_option **option,
...
dogechat_config_search_with_string ("file.section.option",
                                   &ptr_config_file,
                                   &ptr_section,
                                   &ptr_option,
                                   &option_name);
...
[source,C]
----
int dogechat_config_string_to_boolean (const char *text);
----
...
[source,C]
----
if (dogechat_config_string_to_boolean (option_value))
{
    /* il valore è "true" */
}
else
...
----
# prototipo
value = dogechat.config_string_to_boolean(text)

# esempio
if dogechat.config_string_to_boolean(text):
    # ...
----
...
[source,C]
----
int dogechat_config_option_reset (struct t_config_option *option,
                                 int run_callback);
----
...
* 'DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED' se il valore dell'opzione è stato
  resettato
* 'DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE' se il valore non è stato
  modificato
* 'DOGECHAT_CONFIG_OPTION_SET_ERROR' in caso di errore

Esempio in C:

[source,C]
----
switch (dogechat_config_option_reset (option, 1))
{
...
----
# prototipo
rc = dogechat.config_option_reset(option, run_callback)

# esempio
rc = dogechat.config_option_reset(option, 1)
if rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_ERROR:
    # ...
----
...
[source,C]
----
int dogechat_config_option_set (struct t_config_option *option,
...
* 'DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED' se il valore dell'opzione è
  cambiato
* 'DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE' se il valore non è cambiato
* 'DOGECHAT_CONFIG_OPTION_SET_ERROR' in caso di errore
...
[source,C]
----
switch (dogechat_config_option_set (option, "new_value", 1))
{
    case DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
        /* .... */
        break;
...
----
# prototipo
rc = dogechat.config_option_set(option, value, run_callback)

# esempio
rc = dogechat.config_option_set(option, "new_value", 1)
if rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
...
[source,C]
----
int dogechat_config_option_set_null (struct t_config_option *option,
                                    int run_callback);
----
...
(consultare <<_config_new_option,dogechat_config_new_option>>).
...
[source,C]
----
switch (dogechat_config_option_set_null (option, 1))
{
    case DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
        /* .... */
        break;
...
----
# prototipo
rc = dogechat.config_option_set_null(option, run_callback)

# esempio
rc = dogechat.config_option_set_null(option, 1)
if rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
...
[source,C]
----
int dogechat_config_option_unset (struct t_config_option *option);
----
...
* 'DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET' se il valore dell'opzione non è
  stato ripristinato
* 'DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET' se il valore dell'opzione è stato
  ripristinato
* 'DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED' se l'opzione è stata rimossa
* 'DOGECHAT_CONFIG_OPTION_UNSET_ERROR' in caso di errore

Esempio in C:

[source,C]
----
switch (dogechat_config_option_unset (option))
{
...
----
# prototipo
rc = dogechat.config_option_unset(option)

# esempio
rc = dogechat.config_option_unset(option)
if rc == dogechat.DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_UNSET_ERROR:
    # ...
----
...
[source,C]
----
void dogechat_config_option_rename (struct t_config_option *option,
                                   const char *new_name);
----
...
[source,C]
----
dogechat_config_option_rename (option, "new_name");
----
...
----
# prototipo
dogechat.config_option_rename(option, new_name)
...
[source,C]
----
void *dogechat_config_option_get_pointer (struct t_config_option *option,
                                         const char *property);
----
...
// TRANSLATION MISSING
** 'parent_name': name of parent option ('char *') _(DogeChat ≥ 1.4)_
...
[source,C]
----
char *description = dogechat_config_option_get_pointer (option, "description");
----
...
[source,C]
----
int dogechat_config_option_is_null (struct t_config_option *option);
----
...
[source,C]
----
if (dogechat_config_option_is_null (option))
{
    /* il valore è "null" */
}
else
...
----
# prototipo
dogechat.config_option_is_null(option)

# esempio
if dogechat.config_option_is_null(option):
    # ...
----
...
[source,C]
----
int dogechat_config_option_default_is_null (struct t_config_option *option);
----
...
[source,C]
----
if (dogechat_config_option_default_is_null (option))
{
...
----
# prototipo
dogechat.config_option_default_is_null(option)

# esempio
if dogechat.config_option_default_is_null(option):
    # ...
----
...
[source,C]
----
int dogechat_config_boolean (struct t_config_option *option);
----
...
[source,C]
----
struct t_config_option *option = dogechat_config_get ("plugin.section.option");
...
----
# prototipo
value = dogechat.config_boolean(option)

# esempio
option = dogechat.config_get("plugin.section.option")
if dogechat.config_boolean(option):
    # ...
----
...
[source,C]
----
int dogechat_config_boolean_default (struct t_config_option *option);
----
...
----
struct t_config_option *option = dogechat_config_get ("plugin.section.option");
if (dogechat_config_boolean_default (option))
{
    /* il valore è "true" */
}
else
...
----
# prototipo
value = dogechat.config_boolean_default(option)

# esempio
option = dogechat.config_get("plugin.section.option")
if dogechat.config_boolean_default(option):
    # ...
----
...
[source,C]
----
int dogechat_config_integer (struct t_config_option *option);
----
...
----
struct t_config_option *option = dogechat_config_get ("plugin.section.option");
int value = dogechat_config_integer (option);
----
...
----
# prototipo
value = dogechat.config_integer(option)
...
[source,C]
----
int dogechat_config_integer_default (struct t_config_option *option);
----
...
----
struct t_config_option *option = dogechat_config_get ("plugin.section.option");
int value = dogechat_config_integer_default (option);
----
...
----
# prototipo
value = dogechat.config_integer_default(option)
...
[source,C]
----
const char *dogechat_config_string (struct t_config_option *option);
----
...
----
struct t_config_option *option = dogechat_config_get ("plugin.section.option");
const char *value = dogechat_config_string (option);
----
...
----
# prototipo
value = dogechat.config_string(option)
...
[source,C]
----
const char *dogechat_config_string_default (struct t_config_option *option);
----
...
----
struct t_config_option *option = dogechat_config_get ("plugin.section.option");
const char *value = dogechat_config_string_default (option);
----
...
----
# prototipo
value = dogechat.config_string_default(option)
...
[source,C]
----
const char *dogechat_config_color (struct t_config_option *option);
----
...
----
struct t_config_option *option = dogechat_config_get ("plugin.section.option");
const char *color = dogechat_config_color (option);
----
...
----
# prototipo
value = dogechat.config_color(option)
...
[source,C]
----
const char *dogechat_config_color_default (struct t_config_option *option);
----
...
----
struct t_config_option *option = dogechat_config_get ("plugin.section.option");
const char *color = dogechat_config_color_default (option);
----
...
----
# prototipo
value = dogechat.config_color_default(option)
...
[source,C]
----
void dogechat_config_write_option (struct t_config_file *config_file,
                                  struct t_config_option *option);
----
...
my_section_write_cb (void *data, struct t_config_file *config_file,
                     const char *section_name)
{
    dogechat_config_write_line (config_file, "my_section", NULL);

    dogechat_config_write_option (config_file, option);
...
----
# prototipo
dogechat.config_write_option(config_file, option)

# esempio
def my_section_write_cb(data, config_file, section_name):
    dogechat.config_write_line(config_file, "my_section", "")
    dogechat.config_write_option(config_file, option)
    return dogechat.DOGECHAT_RC_OK
----
...
[source,C]
----
void dogechat_config_write_line (struct t_config_file *config_file,
                                const char *option_name,
                                const char *value, ...);
----
                     ...
                     const char *section_name)
{
    ...
    dogechat_config_write_line (config_file, "option", "%s;%d",
                               "value", 123);
...
----
# prototipo
dogechat.config_write_line(config_file, option_name, value)

# esempio
def my_section_write_cb(data, config_file, section_name):
    dogechat.config_write_line(config_file, "my_section", "")
    dogechat.config_write_line(config_file, "option", "value")
    return dogechat.DOGECHAT_RC_OK
----
...
[source,C]
----
int dogechat_config_write (struct t_config_file *config_file);
----
...
* 'DOGECHAT_CONFIG_WRITE_OK' se la configurazione è stata scritta
* 'DOGECHAT_CONFIG_WRITE_MEMORY_ERROR' se non c'è memoria sufficiente
* 'DOGECHAT_CONFIG_WRITE_ERROR' se si è verificato un altro errore

Esempio in C:

[source,C]
----
switch (dogechat_config_write (config_file))
{
    case DOGECHAT_CONFIG_WRITE_OK:
        /* ... */
        break;
    case DOGECHAT_CONFIG_WRITE_MEMORY_ERROR:
        /* ... */
        break;
    case DOGECHAT_CONFIG_WRITE_ERROR:
        /* ... */
        break;
}
----
...
----
# prototipo
rc = dogechat.config_write(config_file)

# esempio
rc = dogechat.config_write(config_file)
if rc == dogechat.DOGECHAT_CONFIG_WRITE_OK:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_WRITE_MEMORY_ERROR:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_WRITE_ERROR:
    # ...
----
...
[source,C]
----
int dogechat_config_read (struct t_config_file *config_file);
----
...
* 'DOGECHAT_CONFIG_READ_OK' se la configurazione è stata caricata
* 'DOGECHAT_CONFIG_READ_MEMORY_ERROR' se non c'è memoria sufficiente
* 'DOGECHAT_CONFIG_READ_FILE_NOT_FOUND' se il file non è stato trovato

Esempio in C:

[source,C]
----
switch (dogechat_config_read (config_file))
{
    case DOGECHAT_CONFIG_READ_OK:
        /* ... */
        break;
    case DOGECHAT_CONFIG_READ_MEMORY_ERROR:
        /* ... */
        break;
    case DOGECHAT_CONFIG_READ_FILE_NOT_FOUND:
        /* ... */
        break;
}
----
...
----
# prototipo
rc = dogechat.config_read(config_file)

# esempio
rc = dogechat.config_read(config_file)
if rc == dogechat.DOGECHAT_CONFIG_READ_OK:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_READ_MEMORY_ERROR:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_READ_FILE_NOT_FOUND:
    # ...
----
...
[source,C]
----
int dogechat_config_reload (struct t_config_file *config_file);
----
...
* 'DOGECHAT_CONFIG_READ_OK' se il file di configurazione è stato ricaricato
* 'DOGECHAT_CONFIG_READ_MEMORY_ERROR' se non c'è memoria sufficiente
* 'DOGECHAT_CONFIG_READ_FILE_NOT_FOUND' se il file non è stato trovato
...
[source,C]
----
switch (dogechat_config_reload (config_file))
{
    case DOGECHAT_CONFIG_READ_OK:
        /* ... */
        break;
...
----
# prototipo
rc = dogechat.config_reload(config_file)
...
[source,C]
----
void dogechat_config_option_free (struct t_config_option *option);
----
...
----
# prototipo
dogechat.config_option_free(option)
...
[source,C]
----
void dogechat_config_section_free_options (struct t_config_section *section);
----
...
----
# prototipo
dogechat.config_section_free_options(section)
...
[source,C]
----
void dogechat_config_section_free (struct t_config_section *section);
----
...
----
# prototipo
dogechat.config_section_free(section)
...
[source,C]
----
void dogechat_config_free (struct t_config_file *config_file);
----
...
----
# prototipo
dogechat.config_free(config_file)
...
[source,C]
----
struct t_config_option *dogechat_config_get (const char *option_name);
----
...
[source,C]
----
struct t_config_option *option = dogechat_config_get ("dogechat.look.item_time_format");
----
...
----
# prototipo
option = dogechat.config_get(option_name)

# esempio
option = dogechat.config_get("dogechat.look.item_time_format")
----
...
[source,C]
----
const char *dogechat_config_get_plugin (const char *option_name);
----

Argomenti:

* 'option_name': nome dell'opzione, DogeChat aggiungerà il prefisso
...
char *value = dogechat_config_get_plugin ("option");
----
...
----
# prototipo
value = dogechat.config_get_plugin(option_name)

# esempio
value = dogechat.config_get_plugin("option")
----
...
[source,C]
----
int dogechat_config_is_set_plugin (const char *option_name);
----

Argomenti:

* 'option_name': nome dell'opzione, DogeChat aggiunge il prefisso "plugins.var.xxx."
...
[source,C]
----
if (dogechat_config_is_set_plugin ("option"))
{
...
----
# prototipo
value = dogechat.config_is_set_plugin(option_name)

# esempio
if dogechat.config_is_set_plugin("option"):
    # l'opzione è impostata
    # ...
else:
    # l'opzione non esiste
...
[source,C]
----
int dogechat_config_set_plugin (const char *option_name, const char *value);
----
...
* 'DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED' se il valore dell'opzione è stato modificato
* 'DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE' se il valore non è cambiato
* 'DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND' se l'opzione non è stata trovata
* 'DOGECHAT_CONFIG_OPTION_SET_ERROR' in caso di errore
...
[source,C]
----
switch (dogechat_config_set_plugin ("option", "test_value"))
{
    case DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
        /* ... */
        break;
...
----
# prototipo
rc = dogechat.config_set_plugin(option_name, value)

# esempio
rc = dogechat.config_set_plugin("option", "test_value")
if rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_ERROR:
    # ...
----
...
[source,C]
----
void dogechat_config_set_desc_plugin (const char *option_name,
                                     const char *description);
----

Argomenti:

* 'option_name': nome dell'opzione, DogeChat aggiungerà il prefisso "plugins.desc.xxx."
  (dove "xxx" è il nome del plugin corrente)
...
[source,C]
----
dogechat_config_set_desc_plugin ("option", "description of option");
----
...
----
# prototipo
dogechat.config_set_desc_plugin(option_name, description)

# esempio
version = dogechat.info_get("version_number", "") or 0
if int(version) >= 0x00030500:
    dogechat.config_set_desc_plugin("option", "description of option")
----
...
[source,C]
----
int dogechat_config_unset_plugin (const char *option_name);
----
...
[source,C]
----
switch (dogechat_config_unset_plugin ("option"))
{
    case DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET:
        /* ... */
        break;
...
----
# prototipo
rc = dogechat.config_unset_plugin(option_name)

# esempio
rc = dogechat.config_unset_plugin("option")
if rc == dogechat.DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET:
    # ...
...
una combinazione tasti, usare <<_key_unbind,dogechat_key_unbind>>.

Prototipo:

[source,C]
----
int dogechat_key_bind (const char *context, struct t_hashtable *keys);
----
...
[source,C]
----
struct t_hashtable *keys = dogechat_hashtable_new (8,
                                                  DOGECHAT_HASHTABLE_STRING,
                                                  DOGECHAT_HASHTABLE_STRING,
                                                  NULL,
                                                  NULL);
if (keys)
{
    dogechat_hashtable_set (keys, "@chat(plugin.buffer):button1", "hsignal:test_mouse");
    dogechat_hashtable_set (keys, "@chat(plugin.buffer):wheelup", "/mycommand up");
    dogechat_hashtable_set (keys, "@chat(plugin.buffer):wheeldown", "/mycommand down");
    dogechat_key_bind ("mouse", keys);
    dogechat_hashtable_free (keys);
}
----
...
----
# prototipo
num_keys = dogechat.key_bind(context, keys)
...
[source,C]
----
int dogechat_key_unbind (const char *context, const char *key);
----

Argomenti:

* 'context': contesto per i tasti (consultare <<_key_bind,dogechat_key_bind>>)
...
dogechat_key_unbind ("mouse", "@chat(plugin.buffer):button1");

/* rimuove tutti i tasti con la zona "chat(plugin.buffer)" */
dogechat_key_unbind ("mouse", "area:chat(plugin.buffer)");
----
...
----
# prototipo
num_keys = dogechat.key_unbind(context, key)

# esempi

# rimuove un singolo tasto
dogechat.key_unbind("mouse", "@chat(plugin.buffer):button1")

# rimuove tutti i tasti con la zona "chat(python.test)"
dogechat.key_unbind("mouse", "area:chat(python.test)")
----
...
[source,C]
----
const char *dogechat_prefix (const char *prefix);
----
...
[source,C]
----
dogechat_printf (NULL, "%sQuesto è un errore...", dogechat_prefix ("error"));
----
...
----
# prototipo
value = dogechat.prefix(prefix)

# esempio
dogechat.prnt("", "%sQuesto è un errore..." % dogechat.prefix("error"))
----
...
[source,C]
----
const char *dogechat_color (const char *color_name);
----
...
// TRANSLATION MISSING
** DogeChat color option name (from dogechat.color.xxx), for example
   'chat_delimiters'
// TRANSLATION MISSING
** option name (format: file.section.option), for example
   'irc.color.message_quit' (_DogeChat ≥ 1.2_)
...
*** 'emphasis': toggle the emphasis for text (note: this should be used only in
    bars, because DogeChat uses text emphasis when searching text in buffer)
    _(DogeChat ≥ 0.4.2)_
...
*** 'resetcolor': ripristina colore (mantiene attributi) _(DogeChat ≥ 0.3.6)_
...
* `|` : mantiene attributi: non ripristina
  grassetto/inverso/corsivo/sottolineato al cambio di colore _(DogeChat ≥ 0.3.6)_
...
[source,C]
----
dogechat_printf (NULL, "Color: %sblue %sdefault color %syellow on red",
                dogechat_color ("blue"),
                dogechat_color ("chat"),
                dogechat_color ("yellow,red"));
----
...
----
# prototipo
value = dogechat.color(color_name)

# esempio
dogechat.prnt("", "Color: %sblue %sdefault color %syellow on red"
    % (dogechat.color("blue"), dogechat.color("chat"), dogechat.color("yellow,red")))
----
...
[source,C]
----
void dogechat_printf (struct t_gui_buffer *buffer, const char *message, ...);
----

Argomenti:

* 'buffer': puntatore al buffer, se NULL il messaggio viene visualizzato sul buffer di DogeChat
...
[source,C]
----
dogechat_printf (NULL, "Benvenuto sul buffer di DogeChat");
dogechat_printf (buffer, "Benvenuto su questo buffer");
dogechat_printf (buffer, "%sQuesto è un errore!", dogechat_prefix ("error"));
dogechat_printf (buffer, " \tMessaggio senza prefisso ma con \t alcune \t tabulazioni");
dogechat_printf (buffer, "\t\tMessage without time/alignment");
dogechat_printf (buffer, "\t\t");  /* empty line (without time) */
----
...
----
# prototipo
dogechat.prnt(buffer, message)

# esempio
dogechat.prnt("", "Benvenuto sul buffer di DogeChat")
dogechat.prnt(buffer, "Benvenuto su questo buffer")
dogechat.prnt(buffer, "%sQuesto è un errore!" % dogechat.prefix("error"))
dogechat.prnt(buffer, " \tMessaggio senza prefisso ma con \t alcune \t tabulazioni")
dogechat.prnt(buffer, "\t\tMessage without time/alignment")
dogechat.prnt(buffer, "\t\t")  # empty line (without time)
----
...
[source,C]
----
void dogechat_printf_date (struct t_gui_buffer *buffer, time_t date,
                          const char *message, ...);
----
...
[source,C]
----
dogechat_printf_date (NULL, time (NULL) - 120, "Ciao, 2 minuti fa");
----
...
[source,C]
----
void dogechat_printf_tags (struct t_gui_buffer *buffer, const char *tags,
                          const char *message, ...);
----
...
[source,C]
----
dogechat_printf_tags (NULL, "notify_message",
...
[source,C]
----
void dogechat_printf_date_tags (struct t_gui_buffer *buffer, time_t date,
...
[source,C]
----
dogechat_printf_date_tags (NULL, time (NULL) - 120, "notify_message",
...
----
# prototipo
dogechat.prnt_date_tags(buffer, date, tags, message)

# esempio
time = int(time.time())
dogechat.prnt_date_tags("", time - 120, "notify_message",
    "Messaggio 2 minuti fa, con tag 'notify_message'")
----
...
[source,C]
----
void dogechat_printf_y (struct t_gui_buffer *buffer, int y,
                       const char *message, ...);
----
...
[source,C]
----
dogechat_printf_y (buffer, 2, "Mio messaggio sulla terza riga");
----
...
----
# prototipo
dogechat.prnt_y(buffer, y, message)

# esempio
dogechat.prnt_y("", 2, "Mio messaggio sulla terza riga")
----
...
Scrive un messaggio nel file di log di DogeChat (dogechat.log).

Prototipo:

[source,C]
----
void dogechat_log_printf (const char *message, ...);
----
...
[source,C]
----
dogechat_log_printf ("Mio messaggio nel file di log");
----
...
----
# prototipo
dogechat.log_print(message)

# esempio
dogechat.log_print("Mio messaggio nel file di log")
----
...
dogechat_hook_modifier ("2000|input_text_display", &modifier_cb, NULL);
----
...
[source,C]
----
struct t_hook *dogechat_hook_command (const char *command,
                                     const char *description,
                                     const char *args,
...
*** 'DOGECHAT_RC_OK'
*** 'DOGECHAT_RC_ERROR'
* 'callback_data': puntatore fornito dalla callback quando chiamata da DogeChat
...
struct t_hook *my_command_hook =
    dogechat_hook_command (/* nome comando */
                          "myfilter",
                          /* description */
...
----
# prototipo
hook = dogechat.hook_command(command, description, args, args_description,
...
Hook su un comando quando eseguito da DogeChat.

Prototipo:

[source,C]
----
struct t_hook *dogechat_hook_command_run (const char *command,
                                         int (*callback)(void *data,
                                                         struct t_gui_buffer *buffer,
                                                         const char *command),
                                         void *callback_data);
...
*** 'DOGECHAT_RC_OK_EAT'
*** 'DOGECHAT_RC_ERROR'
* 'callback_data': puntatore fornito alla callback quando chiamata da DogeChat

[NOTE]
La callback può restituire 'DOGECHAT_RC_OK' o 'DOGECHAT_RC_OK_EAT' (il comando
non verrà eseguito da DogeChat dopo la callback).
...
my_command_run_cb (void *data, struct t_gui_buffer *buffer,
                   const char *command)
{
    dogechat_printf (NULL, "Sto mangiando il completamento!");
    return DOGECHAT_RC_OK_EAT;
}

struct t_hook *my_command_run_hook =
    dogechat_hook_command_run ("/input complete*",
...
----
# prototipo
hook = dogechat.hook_command_run(command, callback, callback_data)

# esempio
def my_command_run_cb(data, buffer, command):
    dogechat.prnt("", "Sto mangiando il completamento!")
    return dogechat.DOGECHAT_RC_OK_EAT

hook = dogechat.hook_command_run("/input complete*", "my_command_run_cb", "")
----
...
[source,C]
----
struct t_hook *dogechat_hook_timer (long interval,
                                   int align_second,
                                   int max_calls,
                                   int (*callback)(void *data,
                                                   int remaining_calls),
...
*** 'DOGECHAT_RC_OK'
*** 'DOGECHAT_RC_ERROR'
* 'callback_data': pointer given to callback when it is called by DogeChat
...
struct t_hook *my_timer_hook =
    dogechat_hook_timer (20 * 1000, 0, 0, &my_timer_cb, NULL);
----
...
----
# prototipo
hook = dogechat.hook_timer(interval, align_second, max_calls, callback, callback_data)
...
# timer chiamato ogni 20 secondi
hook = dogechat.hook_timer(20 * 1000, 0, 0, "my_timer_cb", "")
----
...
[source,C]
----
struct t_hook *dogechat_hook_fd (int fd,
                                int flag_read,
                                int flag_write,
                                int flag_exception,
                                int (*callback)(void *data,
...
* 'flag_exception': 1 = cattura l'eccezione evento (event), 0 = ignora
  (_DogeChat ≥ 1.3_: this argument is ignored and not used any more)
...
*** 'DOGECHAT_RC_OK'
*** 'DOGECHAT_RC_ERROR'
* 'callback_data': puntatore fornito alla calback quando chiamata da DogeChat
...
/* ... */
struct t_hook *my_fd_hook = dogechat_hook_fd (sock, 1, 0, 0, &my_fd_cb, NULL);
----
...
----
# prototipo
hook = dogechat.hook_fd(fd, flag_read, flag_write, flag_exception, callback, callback_data)
...
sock = ...
hook = dogechat.hook_fd(sock, 1, 0, 0, "my_fd_cb", "")
----
...
DogeChat makes an automatic split of command and arguments (like the shell does).
If the split is not correct (according to quotes in your command), or if you
want to use shell, you can use function
<<_hook_process_hashtable,dogechat_hook_process_hashtable>> with
arguments in the hashtable 'options' _(DogeChat ≥ 0.4.0)_.

Prototipo:

[source,C]
----
struct t_hook *dogechat_hook_process (const char *command,
                                     int timeout,
                                     int (*callback)(void *data,
                                                     const char *command,
                                                     int return_code,
...
*** '< 0': 'DOGECHAT_HOOK_PROCESS_RUNNING' (dati disponibili, ma figlio
    ancora in esecuzione) o 'DOGECHAT_HOOK_PROCESS_ERROR' (errore nella
...
Quando il comando ha terminato, o se viene raggiunto il timeout, DogeChat
...
scaricare il contenuto dell'URL _(DogeChat ≥ 0.3.7)_. Le opzioni per
un URL sono disponibili con la funzione
<<_hook_process_hashtable,dogechat_hook_process_hashtable>>.

// TRANSLATION MISSING
[TIP]
If you want to retrieve infos about DogeChat (like current stable version,
latest git commit, ...), you can use URLs on page https://dogechat.org/dev/info
               ...
               const char *out, const char *err)
{
    ...
    {
        dogechat_printf (NULL, "Errore con il comando '%s'", command);
        return DOGECHAT_RC_OK;
    }

    if (return_code >= 0)
    {
        dogechat_printf (NULL, "return_code = %d", return_code);
    }

    if (out)
    {
        dogechat_printf (NULL, "stdout: %s", out);
    }

    if (err)
    {
        dogechat_printf (NULL, "stderr: %s", err);
    }

    return DOGECHAT_RC_OK;
}

struct t_hook *my_process_hook = dogechat_hook_process ("ls", 5000,
                                                       &my_process_cb, NULL);
----
...
----
# prototipo
hook = dogechat.hook_process(command, timeout, callback, callback_data)

# esempio
def my_process_cb(data, command, return_code, out, err):
    if return_code == dogechat.DOGECHAT_HOOK_PROCESS_ERROR:
        dogechat.prnt("", "Error with command '%s'" % command)
        return dogechat.DOGECHAT_RC_OK
    if return_code >= 0:
        dogechat.prnt("", "return_code = %d" % return_code)
    if out != "":
        dogechat.prnt("", "stdout: %s" % out)
    if err != "":
        dogechat.prnt("", "stderr: %s" % err)
    return dogechat.DOGECHAT_RC_OK

hook = dogechat.hook_process("ls", 5000, "my_process_cb", "")
----
...
[source,C]
----
struct t_hook *dogechat_hook_process_hashtable (const char *command,
                                               struct t_hashtable *options,
                                               int timeout,
                                               int (*callback)(void *data,
                                                               const char *command,
...
<<_hook_process,dogechat_hook_process>>, con un argomento aggiuntivo:
...
| stdin +
  ...
  (see function <<_hook_set,dogechat_hook_set>>)
               ...
               const char *out, const char *err)
{
    if (return_code == DOGECHAT_HOOK_PROCESS_ERROR)
    {
        dogechat_printf (NULL, "Error with command '%s'", command);
        return DOGECHAT_RC_OK;
    }
...
struct t_hashtable *options = dogechat_hashtable_new (8,
                                                     DOGECHAT_HASHTABLE_STRING,
                                                     DOGECHAT_HASHTABLE_STRING,
                                                     NULL,
                                                     NULL);
if (options)
{
    dogechat_hashtable_set (options, "file_out", "/tmp/dogechat.org.html");
    struct t_hook *my_process_hook = dogechat_hook_process_hashtable ("url:https://dogechat.org/",
                                                                     options,
                                                                     20000,
                                                                     &my_process_cb, NULL);
...
struct t_hashtable *options_cmd1 = dogechat_hashtable_new (8,
                                                          DOGECHAT_HASHTABLE_STRING,
                                                          DOGECHAT_HASHTABLE_STRING,
                                                          NULL,
                                                          NULL);
if (options_cmd1)
{
    dogechat_hashtable_set (options_cmd1, "arg1", "-from");
    dogechat_hashtable_set (options_cmd1, "arg2", nick);
    dogechat_hashtable_set (options_cmd1, "arg3", "-msg");
    dogechat_hashtable_set (options_cmd1, "arg4", message);  /* unsafe argument */
    struct t_hook *my_process_hook = dogechat_hook_process_hashtable ("my-notify-command",
                                                                     options_cmd1,
                                                                     20000,
                                                                     &my_process_cb, NULL);
    dogechat_hashtable_free (options_cmd1);
}

/* example 3: call shell to execute a command (command must be SAFE) */
struct t_hashtable *options_cmd2 = dogechat_hashtable_new (8,
                                                          DOGECHAT_HASHTABLE_STRING,
                                                          DOGECHAT_HASHTABLE_STRING,
                                                          NULL,
                                                          NULL);
if (options_cmd2)
{
    dogechat_hashtable_set (options_cmd2, "arg1", "-c");
    dogechat_hashtable_set (options_cmd2, "arg2", "ls -l /tmp | grep something");
    struct t_hook *my_process_hook = dogechat_hook_process_hashtable ("sh",
                                                                     options_cmd2,
                                                                     20000,
                                                                     &my_process_cb, NULL);
    dogechat_hashtable_free (options_cmd2);
}
----
...
----
# prototipo
hook = dogechat.hook_process_hashtable(command, options, timeout, callback, callback_data)
...
# example 1: download URL
hook1 = dogechat.hook_process_hashtable("url:https://dogechat.org/",
                                       {"file_out": "/tmp/dogechat.org.html"},
                                       20000, "my_process_cb", "")

# example 2: execute a notify program with a message from someone
hook2 = dogechat.hook_process_hashtable("my-notify-command",
                                       {"arg1": "-from",
                                        "arg2": nick,
                                        "arg3": "-msg",
...
# example 3: call shell to execute a command (command must be SAFE)
hook3 = dogechat.hook_process_hashtable("sh",
                                       {"arg1": "-c",
...
[source,C]
----
struct t_hook *dogechat_hook_connect (const char *proxy,
                                     const char *address,
                                     int port,
                                     int ipv6,
                                     int retry,
...
*** 'DOGECHAT_HOOK_CONNECT_OK': connessione avvenuta con successo
*** 'DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND': indirizzo non trovato
*** 'DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND': indirizzo IP non trovato
*** 'DOGECHAT_HOOK_CONNECT_CONNECTION_REFUSED': connessione rifiutata
*** 'DOGECHAT_HOOK_CONNECT_PROXY_ERROR': errore con il proxy
*** 'DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR': errore con il nome host locale
*** 'DOGECHAT_HOOK_CONNECT_GNUTLS_INIT_ERROR': errore inizializzazione GnuTLS
*** 'DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR': errore di handshake GnuTLS
*** 'DOGECHAT_HOOK_CONNECT_MEMORY_ERROR': memoria insufficiente
*** 'DOGECHAT_HOOK_CONNECT_TIMEOUT': timeout
*** 'DOGECHAT_HOOK_CONNECT_SOCKET_ERROR': errore nella creazione socket
...
struct t_hook *my_connect_hook = dogechat_hook_connect (NULL,
                                                       "my.server.org", 1234,
                                                       1, 0,
                                                       NULL, NULL, 0,  /* GnuTLS */
                                                       NULL,
...
----
# prototipo
hook = dogechat.hook_connect(proxy, address, port, ipv6, retry, local_hostname,
...
def my_connect_cb(data, status, gnutls_rc, sock, error, ip_address):
    if status == DOGECHAT_HOOK_CONNECT_OK:
        # ...
    elif status == DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND:
        # ...
    elif status == DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND:
        # ...
    elif status == DOGECHAT_HOOK_CONNECT_CONNECTION_REFUSED:
        # ...
    elif status == DOGECHAT_HOOK_CONNECT_PROXY_ERROR:
        # ...
    elif status == DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR:
        # ...
    elif status == DOGECHAT_HOOK_CONNECT_GNUTLS_INIT_ERROR:
        # ...
    elif status == DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR:
        # ...
    elif status == DOGECHAT_HOOK_CONNECT_MEMORY_ERROR:
        # ...
    elif status == DOGECHAT_HOOK_CONNECT_TIMEOUT:
        # ...
    elif status == DOGECHAT_HOOK_CONNECT_SOCKET_ERROR:
        # ...
    return dogechat.DOGECHAT_RC_OK

hook = dogechat.hook_connect("", "my.server.org", 1234, 1, 0, "",
                            "my_connect_cb", "")
----
...
[source,C]
----
struct t_hook *dogechat_hook_print (struct t_gui_buffer *buffer,
                                   const char *tags,
                                   const char *message,
                                   int strip_colors,
                                   int (*callback)(void *data,
...
// TRANSLATION MISSING
** with DogeChat ≥ 0.4.3: comma-separated list of tags that must be in message
   (logical "or"); it is possible to combine many tags as a logical "and" with
   separator "+"; wildcard "*" is allowed in tags
// TRANSLATION MISSING
** with DogeChat ≤ 0.4.2: comma-separated list of tags that must all be in
   message (logical "and")
...
// TRANSLATION MISSING
[IMPORTANT]
In scripts, with DogeChat ≥ 1.0, the callback arguments 'displayed' and
'highlight' are integers (with DogeChat ≤ 0.4.3, they were strings). +
...
struct t_hook *my_print_hook =
    dogechat_hook_print (NULL, NULL, NULL, 1, &my_print_cb, NULL);
----
...
----
# prototipo
hook = dogechat.hook_print(buffer, tags, message, strip_colors, callback, callback_data)
...
# cattura tutti i messaggi, su tutti i buffer, senza colore
hook = dogechat.hook_print("", "", "", 1, "my_print_cb", "")
----
...
[source,C]
----
struct t_hook *dogechat_hook_signal (const char *signal,
                                    int (*callback)(void *data,
                                                    const char *signal,
...
*** 'DOGECHAT_HOOK_SIGNAL_STRING': stringa
*** 'DOGECHAT_HOOK_SIGNAL_INT': numero intero
*** 'DOGECHAT_HOOK_SIGNAL_POINTER': puntatore
** 'void *signal_data': dati inviati con il segnale
** valore restituito:
*** 'DOGECHAT_RC_OK'
// TRANSLATION MISSING
*** 'DOGECHAT_RC_OK_EAT' (stop sending the signal immediately)
    _(DogeChat ≥ 0.4.0)_
*** 'DOGECHAT_RC_ERROR'
* 'callback_data': puntatore fornito alla callback quando chiamata da DogeChat
...
// TRANSLATION MISSING
List of signals sent by DogeChat and plugins:
...
| dogechat | buffer_opened |
  Puntatore: buffer |
  Buffer aperto

| dogechat | buffer_closing |
  Puntatore: buffer |
  Chiusura del buffer

| dogechat | buffer_closed |
  Puntatore: buffer |
  Buffer chiuso

// TRANSLATION MISSING
| dogechat | buffer_cleared |
  Puntatore: buffer |
  Buffer cleared

// TRANSLATION MISSING
| dogechat | buffer_hidden |
  Pointer: buffer |
  Buffer hidden

// TRANSLATION MISSING
| dogechat | buffer_unhidden |
  Pointer: buffer |
  Buffer unhidden

| dogechat | buffer_line_added +
  _(DogeChat ≥ 0.3.7)_ |
  Puntatore: riga |
  Riga aggiunta in un buffer

| dogechat | buffer_lines_hidden |
  Puntatore: buffer |
  Righe nascoste nel buffer

| dogechat | buffer_localvar_added |
  Puntatore: buffer |
  Variabili locali aggiunte

| dogechat | buffer_localvar_changed |
  Puntatore: buffer |
  Variabili locali modificate

| dogechat | buffer_localvar_removed |
  Puntatore: buffer |
  Variabili locali rimosse

// TRANSLATION MISSING
| dogechat | buffer_merged |
  Puntatore: buffer |
  Buffer merged

// TRANSLATION MISSING
| dogechat | buffer_unmerged |
  Puntatore: buffer |
  Buffer unmerged

| dogechat | buffer_moved |
  Puntatore: buffer |
  Buffer spostato

| dogechat | buffer_renamed |
  Puntatore: buffer |
  Buffer rinominato

| dogechat | buffer_switch |
  Puntatore: buffer |
  Passaggio tra buffer

| dogechat | buffer_title_changed |
  Puntatore: buffer |
  Titolo del buffer modificato

| dogechat | buffer_type_changed |
  Puntatore: buffer |
  Tipo di buffer modificato

// TRANSLATION MISSING
| dogechat | buffer_zoomed +
  _(DogeChat ≥ 0.4.3)_ |
  Puntatore: buffer |
  Merged buffer zoomed

// TRANSLATION MISSING
| dogechat | buffer_unzoomed +
  _(DogeChat ≥ 0.4.3)_ |
  Puntatore: buffer |
  Merged buffer unzoomed

| dogechat | day_changed +
  _(DogeChat ≥ 0.3.2)_ |
...
| dogechat | filter_added |
  Puntatore: filtro |
  Filtro aggiunto

| dogechat | filter_removing |
  Puntatore: filtro |
  Rimozione del filtro

| dogechat | filter_removed |
  - |
  Filtro rimosso

| dogechat | filters_enabled |
  - |
  Filtri abilitati

| dogechat | filters_disabled |
  - |
  Filtri disabilitati

| dogechat | hotlist_changed |
  - |
  Hotlist modificata

| dogechat | input_paste_pending |
  - |
  Incolla testo in attesa

| dogechat | input_search |
  Puntatore: buffer |
  Ricerca testo nel buffer

| dogechat | input_text_changed |
  Puntatore: buffer |
  Testo in input modificato

| dogechat | input_text_cursor_moved |
  Puntatore: buffer |
...
| dogechat | key_pressed |
  String: tasto digitato |
  Tasto digitato

// TRANSLATION MISSING
| dogechat | key_combo_default +
  _(DogeChat ≥ 1.0)_ |
  String: key combo |
  Key combo in 'default' context

// TRANSLATION MISSING
| dogechat | key_combo_search +
  _(DogeChat ≥ 1.0)_ |
  String: key combo |
  Key combo in 'search' context

// TRANSLATION MISSING
| dogechat | key_combo_cursor +
  _(DogeChat ≥ 1.0)_ |
  String: key combo |
  Key combo in 'cursor' context

// TRANSLATION MISSING
| dogechat | mouse_enabled +
  _(DogeChat ≥ 1.1)_ |
  - |
  Mouse enabled

// TRANSLATION MISSING
| dogechat | mouse_disabled +
  _(DogeChat ≥ 1.1)_ |
  - |
  Mouse disabled

// TRANSLATION MISSING
| dogechat | nicklist_group_added +
  _(DogeChat ≥ 0.3.2)_ |
  String: buffer pointer + "," + group name |
  Group added in nicklist

// TRANSLATION MISSING
| dogechat | nicklist_group_changed +
  _(DogeChat ≥ 0.3.4)_ |
  String: buffer pointer + "," + group name |
  Group changed in nicklist

// TRANSLATION MISSING
| dogechat | nicklist_group_removing +
  _(DogeChat ≥ 0.4.1)_ |
  String: buffer pointer + "," + group name |
  Removing group from nicklist

// TRANSLATION MISSING
| dogechat | nicklist_group_removed +
  _(DogeChat ≥ 0.3.2)_ |
  String: buffer pointer + "," + group name |
  Group removed from nicklist

// TRANSLATION MISSING
| dogechat | nicklist_nick_added +
  _(DogeChat ≥ 0.3.2)_ |
  String: buffer pointer + "," + nick name |
  Nick added in nicklist

// TRANSLATION MISSING
| dogechat | nicklist_nick_changed +
  _(DogeChat ≥ 0.3.4)_ |
  String: buffer pointer + "," + nick name |
  Nick changed in nicklist

// TRANSLATION MISSING
| dogechat | nicklist_nick_removing +
  _(DogeChat ≥ 0.4.1)_ |
  String: buffer pointer + "," + nick name |
  Removing nick from nicklist

// TRANSLATION MISSING
| dogechat | nicklist_nick_removed +
  _(DogeChat ≥ 0.3.2)_ |
  String: buffer pointer + "," + nick name |
  Nick removed from nicklist

| dogechat | partial_completion |
  - |
  Completamento parziale avvenuto

// TRANSLATION MISSING
| dogechat | plugin_loaded +
  _(DogeChat ≥ 0.3.9)_ |
  String: path to plugin loaded |
  Plugin loaded

// TRANSLATION MISSING
| dogechat | plugin_unloaded +
  _(DogeChat ≥ 0.3.9)_ |
...
// TRANSLATION MISSING
| dogechat | signal_sighup +
  _(DogeChat ≥ 1.3)_ |
  - |
  Signal SIGHUP received

// TRANSLATION MISSING
| dogechat | signal_sigquit +
  _(DogeChat ≥ 1.2)_ |
  - |
  Signal SIGQUIT received (quit request with core dump)

// TRANSLATION MISSING
| dogechat | signal_sigterm +
  _(DogeChat ≥ 1.2)_ |
  - |
  Signal SIGTERM received (graceful termination of DogeChat process)

// TRANSLATION MISSING
| dogechat | signal_sigwinch +
  _(DogeChat ≥ 0.4.3)_ |
  - |
...
| dogechat | upgrade_ended +
  _(DogeChat ≥ 0.3.4)_ |
  - |
  Fine del processo di aggiornamento (comando `/upgrade`)

| dogechat | dogechat_highlight |
  String: messaggio con prefisso |
  Evento accaduto

| dogechat | dogechat_pv |
  String: messaggio con prefisso |
  Messaggio privato visualizzato

// TRANSLATION MISSING
| dogechat | window_closing +
  _(DogeChat ≥ 0.3.6)_ |
  Puntatore: finestra |
  Closing window

// TRANSLATION MISSING
| dogechat | window_closed +
  _(DogeChat ≥ 0.3.6)_ |
  Puntatore: finestra |
  Window closed

// TRANSLATION MISSING
| dogechat | window_opened +
  _(DogeChat ≥ 0.4.1)_ |
  Puntatore: finestra |
  Window opened

| dogechat | window_scrolled |
  Puntatore: finestra |
  Scroll nella finestra

| dogechat | window_switch +
  _(DogeChat ≥ 0.3.7)_ |
  Puntatore: finestra |
  Passaggio alla finestra

| dogechat | window_zoom |
  Puntatore: finestra corrente |
  Massimizzazione della finestra

| dogechat | window_zoomed |
  Puntatore: finestra corrente |
  Finestra massimizzata

| dogechat | window_unzoom |
  Puntatore: finestra corrente |
  Minimizzazione della finestra

| dogechat | window_unzoomed |
  Puntatore: finestra corrente |
...
struct t_hook *my_signal_hook = dogechat_hook_signal ("quit",
                                                     &my_signal_cb, NULL);
----
...
----
# prototipo
hook = dogechat.hook_signal(signal, callback, callback_data)
...
# cattura il segnale "quit"
hook = dogechat.hook_signal("quit", "my_signal_cb", "")
----
...
[source,C]
----
int dogechat_hook_signal_send (const char *signal, const char *type_data,
                              void *signal_data);
----
...
* 'type_data': tipo di dati inviati con il segnale (consultare
  <<_hook_signal,dogechat_hook_signal>>)
* 'signal_data': dati inviati con il segnale

// TRANSLATION MISSING
Return value _(DogeChat ≥ 1.0)_:

// TRANSLATION MISSING
* return code of last callback executed ('DOGECHAT_RC_OK' if no callback was
  executed):
** 'DOGECHAT_RC_OK'
** 'DOGECHAT_RC_OK_EAT'
** 'DOGECHAT_RC_ERROR'
...
[source,C]
----
int rc = dogechat_hook_signal_send ("my_signal", DOGECHAT_HOOK_SIGNAL_STRING, my_string);
----
...
----
# prototipo
rc = dogechat.hook_signal_send(signal, type_data, signal_data)

# esempio
rc = dogechat.hook_signal_send("my_signal", dogechat.DOGECHAT_HOOK_SIGNAL_STRING, my_string)
----
...
[source,C]
----
dogechat_hook_signal_send ("logger_backlog", DOGECHAT_HOOK_SIGNAL_POINTER, buffer);
----
...
[source,python]
----
dogechat.hook_signal_send("logger_backlog", dogechat.DOGECHAT_HOOK_SIGNAL_POINTER, buffer)
----
...
. sposta il nuovo script nella cartella '~/.dogechat/xxx/' (dove 'xxx' è il
  linguaggio)
. crea un link al nuovo script nella cartella '~/.dogechat/xxx/autoload/'
...
[source,C]
----
dogechat_hook_signal_send ("python_script_install", DOGECHAT_HOOK_SIGNAL_STRING,
                          "/home/xxx/.dogechat/test.py");
----
...
[source,python]
----
dogechat.hook_signal_send("python_script_install", DOGECHAT_HOOK_SIGNAL_STRING,
                         "/home/xxx/.dogechat/test.py")
----
...
dogechat_hook_signal_send ("python_script_remove", DOGECHAT_HOOK_SIGNAL_STRING,
                          "test.py,script.py");
----
...
----
# scarica e rimuove gli script test.py e script.py
dogechat.hook_signal_send("python_script_remove", DOGECHAT_HOOK_SIGNAL_STRING,
                         "test.py,script.py")
----
...
[source,C]
----
/* dice "Hello!" sul server freenode, canale #dogechat */
dogechat_hook_signal_send ("irc_input_send", DOGECHAT_HOOK_SIGNAL_STRING,
                          "freenode;#dogechat;1;;Hello!");
...
[source,python]
----
# dice "Hello!" sul server freenode server, canale #dogechat
dogechat.hook_signal_send("irc_input_send", dogechat.DOGECHAT_HOOK_SIGNAL_STRING,
                         "freenode;#dogechat;1;;Hello!")
...
[source,C]
----
struct t_hook *dogechat_hook_hsignal (const char *signal,
                                     int (*callback)(void *data,
                                                     const char *signal,
                                                     struct t_hashtable *hashtable),
                                     void *callback_data);
...
// TRANSLATION MISSING
List of hsignals sent by DogeChat and plugins:
...
struct t_hook *my_hsignal_hook = dogechat_hook_hsignal ("test",
                                                       &my_hsignal_cb, NULL);
----
...
----
# prototipo
hook = dogechat.hook_hsignal(signal, callback, callback_data)
...
hook = dogechat.hook_hsignal("test", "my_hsignal_cb", "")
----

==== hook_hsignal_send

// TRANSLATION MISSING
_DogeChat ≥ 0.3.4, updated in 1.0._
...
[source,C]
----
int dogechat_hook_hsignal_send (const char *signal, struct t_hashtable *hashtable);
----
...
if (hashtable)
{
    dogechat_hashtable_set (hashtable, "key", "value");
    rc = dogechat_hook_hsignal_send ("my_hsignal", hashtable);
    dogechat_hashtable_free (hashtable);
}
----
...
----
# prototipo
rc = dogechat.hook_hsignal_send(signal, hashtable)

# esempio
rc = dogechat.hook_hsignal_send("my_hsignal", {"key": "value"})
----
...
{
    dogechat_printf (NULL, "error = %s", dogechat_hashtable_get (hashtable, "error"));
    dogechat_printf (NULL, "output = %s", dogechat_hashtable_get (hashtable, "output"));
    return DOGECHAT_RC_OK;
}

dogechat_hook_hsignal ("irc_redirection_test_whois", &test_whois_cb, NULL);
struct t_hashtable *hashtable = dogechat_hashtable_new (8,
                                                       DOGECHAT_HASHTABLE_STRING,
                                                       DOGECHAT_HASHTABLE_STRING,
                                                       NULL,
...
if (hashtable)
{
    dogechat_hashtable_set (hashtable, "server", "freenode");
    dogechat_hashtable_set (hashtable, "pattern", "whois");
    dogechat_hashtable_set (hashtable, "signal", "test");
    dogechat_hashtable_set (hashtable, "string", "FlashCode");
    dogechat_hook_hsignal_send ("irc_redirect_command", hashtable);
    dogechat_hook_signal_send ("irc_input_send", DOGECHAT_HOOK_SIGNAL_STRING,
                              "freenode;;2;;/whois FlashCode");
    dogechat_hashtable_free (hashtable);
}
...
def test_whois_cb(data, signal, hashtable):
    dogechat.prnt("", "error = %s" % hashtable["error"])
    dogechat.prnt("", "output = %s" % hashtable["output"])
    return dogechat.DOGECHAT_RC_OK

dogechat.hook_hsignal ("irc_redirection_test_whois", "test_whois_cb", "")
dogechat.hook_hsignal_send("irc_redirect_command",
...
if (hashtable)
{
    dogechat_hashtable_set (hashtable, "pattern", "my_whois");
    dogechat_hashtable_set (hashtable, "timeout", "30");
    dogechat_hashtable_set (hashtable, "cmd_start", "311:1");
    dogechat_hashtable_set (hashtable, "cmd_stop", "318:1,401:1,402:1,431:1,461");
    dogechat_hashtable_set (hashtable, "cmd_extra", "318:1");
    dogechat_hook_hsignal_send ("irc_redirect_pattern", hashtable);
    /*
...
[source,python]
----
dogechat.hook_hsignal_send("irc_redirect_pattern",
...
[source,C]
----
struct t_hook *dogechat_hook_config (const char *option,
                                    int (*callback)(void *data,
                                                    const char *option,
                                                    const char *value),
                                    void *callback_data);
...
* 'option': opzione, il formato è il nome completo, come usato con il comando
  `/set` (ad esempio: `dogechat.look.item_time_format`), wildcard "*" is allowed
...
/* cattura le modifiche dell'opzione "dogechat.look.item_time_format" */
struct t_hook *my_config_hook = dogechat_hook_config ("dogechat.look.item_time_format",
                                                     &my_config_cb, NULL);
----
...
----
# prototipo
hook = dogechat.hook_config(option, callback, callback_data)
...
# cattura le modifiche dell'opzione "dogechat.look.item_time_format"
hook = dogechat.hook_config("dogechat.look.item_time_format", "my_config_cb", "")
----
...
[source,C]
----
struct t_hook *dogechat_hook_completion (const char *completion_item,
                                        const char *description,
                                        int (*callback)(void *data,
...
** 'struct t_gui_completion *completion': struttura usata per aggiungere
   ...
   <<_hook_completion_list_add,dogechat_hook_completion_list_add>>)
** valore restituito:
*** 'DOGECHAT_RC_OK'
*** 'DOGECHAT_RC_ERROR'
* 'callback_data': puntatore fornito alla callback quando chiamata da DogeChat
...
[NOTE]
I nomi del completamento sono globali (condivisi tra DogeChat e plugin). Si
...
<<_hook_command_run,dogechat_hook_command_run>> with command:
"/input complete_next" (and you must return 'DOGECHAT_RC_OK_EAT' if your callback
has updated the command line, so that DogeChat will not perform the completion).
...
my_completion_cb (void *data, const char *completion_item,
                  ...
                  struct t_gui_completion *completion)
{
    dogechat_hook_completion_list_add (completion, "word1",
                                      0, DOGECHAT_LIST_POS_SORT);
    dogechat_hook_completion_list_add (completion, "test_word2",
                                      0, DOGECHAT_LIST_POS_SORT);
    return DOGECHAT_RC_OK;
}

struct t_hook *my_completion_hook = dogechat_hook_completion ("plugin_item",
...
----
# prototipo
hook = dogechat.hook_completion(completion_item, description, callback, callback_data)

# esempio
def my_completion_cb(data, completion_item, buffer, completion):
    dogechat.hook_completion_list_add(completion, "word1", 0, dogechat.DOGECHAT_LIST_POS_SORT)
    dogechat.hook_completion_list_add(completion, "test_word2", 0, dogechat.DOGECHAT_LIST_POS_SORT)
    return dogechat.DOGECHAT_RC_OK

hook = dogechat.hook_completion("plugin_item", "my custom completion!",
                               "my_completion_cb", "")
----
...
[source,C]
----
const char *dogechat_hook_completion_get_string (struct t_gui_completion *completion,
                                                const char *property);
----
                  ...
                  struct t_gui_completion *completion)
{
    ...
    const char *args = dogechat_hook_completion_get_string (completion, "args");
...
----
# prototipo
value = dogechat.hook_completion_get_string(completion, property)

# esempio
def my_completion_cb(data, completion_item, buffer, completion):
    # ottiene l'argomento del comando
    args = dogechat.hook_completion_get_string(completion, "args")
    # completamento che dipende dagli argomenti
    # ...
    return dogechat.DOGECHAT_RC_OK
----
...
[source,C]
----
void dogechat_hook_completion_list_add (struct t_gui_completion *completion,
                                       const char *word,
                                       int nick_completion,
                                       const char *where);
----
...
** 'DOGECHAT_LIST_POS_SORT': qualunque posizione, per mantenere
   la lista ordinata
** 'DOGECHAT_LIST_POS_BEGINNING': inizio della lista
** 'DOGECHAT_LIST_POS_END': fine della lista

Esempio in C: consultare <<_hook_completion,dogechat_hook_completion>>.
...
----
# prototipo
dogechat.hook_completion_list_add(completion, word, nick_completion, where)
...
[source,C]
----
struct t_hook *dogechat_hook_modifier (const char *modifier,
...
List of modifiers used by DogeChat and plugins:
...
| irc_color_decode |
  ...
  Qualsiasi stringa |
  String with IRC colors converted to DogeChat colors (or IRC colors removed)
...
| color_decode_ansi +
  ...
  "1" per mantenere i colori, "0" per rimuovere i colori |
  Qualsiasi stringa |
  String with ANSI colors converted to DogeChat colors (or ANSI colors removed)
...
struct t_hook *my_modifier_hook = dogechat_hook_modifier ("dogechat_print",
                                                         &my_modifier_cb, NULL);
----
...
----
# prototipo
hook = dogechat.hook_modifier(modifier, callback, callback_data)

# esempio
def my_modifier_cb(data, modifier, modifier_data, string):
    return "%s xxx" % string

hook = dogechat.hook_modifier("dogechat_print", "my_modifier_cb", "")
----
...
[source,C]
----
char *dogechat_hook_modifier_exec (const char *modifier,
                                  const char *modifier_data,
                                  const char *string);
----
...
[source,C]
----
char *new_string = dogechat_hook_modifier_exec ("my_modifier",
                                               my_data, my_string);
----
...
----
# prototipo
dogechat.hook_modifier_exec(modifier, modifier_data, string)

# esempio
dogechat.hook_modifier_exec("my_modifier", my_data, my_string)
----
...
[source,C]
----
struct t_hook *dogechat_hook_info (const char *info_name,
                                  const char *description,
                                  const char *args_description,
...
struct t_hook *my_info_hook = dogechat_hook_info ("my_info",
                                                 "Some info",
...
----
# prototipo
hook = dogechat.hook_info(info_name, description, args_description, callback, callback_data)

# esempio
def my_info_cb(data, info_name, arguments):
    return "some_info"

hook = dogechat.hook_info("my_info", "Some info", "Info about arguments",
                         "my_info_cb", "")
----
...
[source,C]
----
struct t_hook *dogechat_hook_info_hashtable (const char *info_name,
                                            const char *description,
                                            const char *args_description,
...
struct t_hook *my_info_hook = dogechat_hook_info_hashtable ("my_info_hashtable",
                                                           "Some info",
...
----
# prototipo
hook = dogechat.hook_info_hashtable(info_name, description, args_description,
...
hook = dogechat.hook_info_hashtable("my_info_hashtable", "Some info",
                                   "Info about input hashtable",
                                   "Info about output hashtable",
...
[source,C]
----
struct t_hook *dogechat_hook_infolist (const char *infolist_name,
                                      const char *description,
...
struct t_hook *my_infolist = dogechat_hook_infolist ("my_infolist",
...
----
# prototipo
hook = dogechat.hook_infolist(infolist_name, description, pointer_description,
...
hook = dogechat.hook_infolist("my_infolist", "Infolist with some data",
...
[source,C]
----
struct t_hook *dogechat_hook_hdata (const char *hdata_name,
                                   const char *description,
...
/* add hdata "my_hdata" */
struct t_hook *my_hdata = dogechat_hook_hdata ("my_hdata",
...
[source,C]
----
struct t_hook *dogechat_hook_focus (const char *area,
                                   struct t_hashtable *(*callback)(void *data,
...
| _buffer_name | Nome del buffer |
  "dogechat", "freenode.#dogechat", ... | ""

| _buffer_full_name | Nome completo del buffer |
  "core.dogechat", "irc.freenode.#dogechat", ... | ""
...
^(4)^ Data/ora in cui DogeChat aggiunge una riga nel buffer (maggiore o uguale a
"chat_line_date").
...
struct t_hook *my_focus = dogechat_hook_focus ("buffer_nicklist",
...
----
# prototipo
hook = dogechat.hook_focus(area, callback, callback_data)
...
hook = dogechat.hook_focus("buffer_nicklist", "my_focus_nicklist_cb", "")
----

==== hook_set

_DogeChat ≥ 0.3.9 (script: DogeChat ≥ 0.4.3)._
...
[source,C]
----
void dogechat_hook_set (struct t_hook *hook, const char *property,
                       const char *value);
----

Argomenti:

* 'hook': qualcosa su cui è presente un hook con "dogechat_hook_xxx()"
// TRANSLATION MISSING
...
----
struct t_hook *my_command_hook =
    dogechat_hook_command ("abcd", "description",
                          "args", "description args",
                          "", &my_command_cb, NULL);
dogechat_hook_set (my_command_hook, "subplugin", "test");
----
...
----
# prototipo
dogechat.hook_set(hook, property, value)
...
hook = dogechat.hook_process_hashtable("/path/to/command", {"stdin": "1"},
                                      20000, "my_process_cb", "")
dogechat.hook_set(hook, "stdin", "data sent to stdin of child process")
dogechat.hook_set(hook, "stdin_close", "")  # optional
----
...
[source,C]
----
void dogechat_unhook (struct t_hook *hook);
----
...
[source,C]
----
struct t_hook *my_hook = dogechat_hook_command ( /* ... */ );
/* ... */
dogechat_unhook (my_hook);
----
...
----
# prototipo
dogechat.unhook(hook)

# esempio
dogechat.unhook(my_hook)
----
...
[source,C]
----
struct t_gui_buffer *dogechat_buffer_new (const char *name,
...
{
    dogechat_printf (buffer, "Testo: %s", input_data);
    return DOGECHAT_RC_OK;
}
...
{
    dogechat_printf (NULL, "Il buffer '%s' verrà chiuso!",
                    dogechat_buffer_get_string (buffer, "name"));
    return DOGECHAT_RC_OK;
}

struct t_gui_buffer *my_buffer = dogechat_buffer_new ("my_buffer",
                                                     &my_input_cb, NULL,
                                                     &my_close_cb, NULL);
----
...
----
# prototipo
buffer = dogechat.buffer_new(name, input_callback, input_callback_data,
    close_callback, close_callback_data)

# esempio
def my_input_cb(data, buffer, input_data):
    dogechat.prnt(buffer, "Testo: %s" % input_data)
    return dogechat.DOGECHAT_RC_OK

def my_close_cb(data, buffer):
    dogechat.prnt("", "Il buffer '%s' verrà chiuso!" % dogechat.buffer_get_string(buffer, "name"))
    return dogechat.DOGECHAT_RC_OK

buffer = dogechat.buffer_new("my_buffer", "my_input_cb", "", "my_close_cb", "")
----
...
[source,C]
----
struct t_gui_buffer *dogechat_current_buffer ();
----
...
[source,C]
----
dogechat_printf (dogechat_current_buffer (), "Testo sul buffer corrente");
----
...
# esempio
dogechat.prnt(dogechat.current_buffer(), "Testo sul buffer corrente")
----
...
[source,C]
----
struct t_gui_buffer *dogechat_buffer_search (const char *plugin,
                                            const char *name);
----
...
** `==`: the name used is the buffer full name (for example:
   `irc.freenode.#dogechat` instead of `freenode.#dogechat`)
   _(DogeChat ≥ 1.0)_
// TRANSLATION MISSING
* 'name': name of buffer, if it is NULL or empty string, the current buffer is
  ...
  `(?i)`, the search is case insensitive _(DogeChat ≥ 1.0)_
...
[source,C]
----
struct t_gui_buffer *buffer1 = dogechat_buffer_search ("irc", "freenode.#dogechat");
struct t_gui_buffer *buffer2 = dogechat_buffer_search ("==", "irc.freenode.#test");  /* DogeChat ≥ 1.0 */
----
...
----
# prototipo
buffer = dogechat.buffer_search(plugin, name)

# esempio
buffer = dogechat.buffer_search("my_plugin", "my_buffer")
----

==== buffer_search_main

Cerca nel buffer principale di DogeChat (per primo nel buffer 'core', il primo
visualizzato all'avvio di DogeChat).

Prototipo:

[source,C]
----
struct t_gui_buffer *dogechat_buffer_search_main ();
----

Valore restituito:

* puntatore al buffer principale di DogeChat (buffer 'core')

Esempio in C:

[source,C]
----
struct t_gui_buffer *dogechat_buffer = dogechat_buffer_search_main ();
----
...
----
# prototipo
buffer = dogechat.buffer_search_main()
...
[source,C]
----
void dogechat_buffer_clear (struct t_gui_buffer *buffer);
----
...
[source,C]
----
struct t_gui_buffer *my_buffer = dogechat_buffer_search ("my_plugin",
                                                        "my_buffer");
if (my_buffer)
{
    dogechat_buffer_clear (my_buffer);
}
----
...
[source,C]
----
void dogechat_buffer_close (struct t_gui_buffer *buffer);
----
...
struct t_gui_buffer *my_buffer = dogechat_buffer_new ("my_buffer",
                                                     ...
                                                     &my_close_cb, NULL);
/* ... */
dogechat_buffer_close (my_buffer);
----
...
con lo stesso numero, e DogeChat visualizzerà le righe di entrambi i
...
[source,C]
----
void dogechat_buffer_merge (struct t_gui_buffer *buffer,
...
[source,C]
----
/* merge current buffer with dogechat "core" buffer */
dogechat_buffer_merge (dogechat_current_buffer (),
                      dogechat_buffer_search_main ());
----
...
----
# prototipo
dogechat.buffer_merge(buffer, target_buffer)

# esempio
# merge current buffer with DogeChat "core" buffer
dogechat.buffer_merge(dogechat.current_buffer(), dogechat.buffer_search_main())
----
...
[source,C]
----
void dogechat_buffer_unmerge (struct t_gui_buffer *buffer,
                             int number);
----
...
[source,C]
----
dogechat_buffer_unmerge (dogechat_current_buffer (), 1);
----
...
----
# prototipo
dogechat.buffer_unmerge(buffer, number)

# esempio
dogechat.buffer_unmerge(dogechat.current_buffer(), 1)
----
...
[source,C]
----
int dogechat_buffer_get_integer (struct t_gui_buffer *buffer,
                                const char *property);
----
...
[source,C]
----
dogechat_printf (NULL, "my buffer number is: %d",
                dogechat_buffer_get_integer (my_buffer, "number"));
----
...
----
# prototipo
value = dogechat.buffer_get_integer(buffer, property)

# esempio
dogechat.prnt("", "my buffer number is: %d" % dogechat.buffer_get_integer(my_buffer, "number"))
----
...
[source,C]
----
const char *dogechat_buffer_get_string (struct t_gui_buffer *buffer,
                                       const char *property);
----
...
** 'plugin': nome del plugin che ha creato questo buffer ("core"
   per il buffer principale di DogeChat)
** 'name': nome del buffer
** 'full_name': nome completo del buffer ("plugin.nome") _(DogeChat ≥ 0.3.7)_
...
[source,C]
----
dogechat_printf (NULL, "name / short name of buffer are: %s / %s",
                dogechat_buffer_get_string (my_buffer, "name"),
                dogechat_buffer_get_string (my_buffer, "short_name"));
----
...
----
# prototipo
value = dogechat.buffer_get_string(buffer, property)

# esempio
dogechat.prnt("", "name / short name of buffer are: %s / %s"
    % (dogechat.buffer_get_string(my_buffer, "name"),
    dogechat.buffer_get_string(my_buffer, "short_name")))
----
...
[source,C]
----
void *dogechat_buffer_pointer (struct t_gui_buffer *buffer,
                              const char *property);
----
...
[source,C]
----
dogechat_printf (NULL, "plugin pointer of my buffer: %lx",
                dogechat_buffer_get_pointer (my_buffer, "plugin"));
----
...
----
# prototipo
value = dogechat.buffer_get_pointer(buffer, property)

# esempio
dogechat.prnt("", "plugin pointer of my buffer: %s" % dogechat.buffer_get_pointer(my_buffer, "plugin"))
----
...
[source,C]
----
void dogechat_buffer_set (struct t_gui_buffer *buffer, const char *property,
                         const char *value);
----
...
// TRANSLATION MISSING
| hotlist | "+", "-", DOGECHAT_HOTLIST_LOW, DOGECHAT_HOTLIST_MESSAGE,
  DOGECHAT_HOTLIST_PRIVATE, DOGECHAT_HOTLIST_HIGHLIGHT, "-1" |
  ...
  "-1": remove buffer from hotlist _(DogeChat ≥ 1.0)_
...
| type | "formatted" oppure "free" |
  ...
  'clear' is forced to "0" _(DogeChat ≥ 1.0)_
...
| clear +
  ...
  "1" to let user clear the buffer (default for a new buffer)
   ...
   the function <<_buffer_clear,dogechat_buffer_clear>>)
...
dogechat_buffer_set (my_buffer, "name", "my_new_name");

/* aggiunge una nuova variabile locale "tizio" con il valore "abc" */
dogechat_buffer_set (my_buffer, "localvar_set_tizio", "abc");

/* rimuove la variabile locale "tizio" */
dogechat_buffer_set (my_buffer, "localvar_del_tizio", NULL);
----
...
----
# prototipo
dogechat.buffer_set(buffer, property, value)

# esempi

# disabilita hotlist (per tutti i buffer)
dogechat.buffer_set("", "hotlist", "-")

# abilita nuovamente hotlist
dogechat.buffer_set("", "hotlist", "+")

# cambia il nome buffer
dogechat.buffer_set(my_buffer, "name", "my_new_name")

# aggiunge una nuova variabile locale "tizio" con il valore "abc"
dogechat.buffer_set(my_buffer, "localvar_set_tizio", "abc")

# rimuove la variabile locale "tizio"
dogechat.buffer_set(my_buffer, "localvar_del_tizio", "")
----
...
[source,C]
----
void dogechat_buffer_set_pointer (struct t_gui_buffer *buffer, const char *property,
                                 void *pointer);
----
...
** 'nickcmp_callback': set nick comparison callback function (this callback is
   called when searching nick in nicklist) _(DogeChat ≥ 0.3.9)_
// TRANSLATION MISSING
...
dogechat_buffer_set_pointer (my_buffer, "close_callback", &my_close_cb);
----
...
[source,C]
----
char *dogechat_buffer_string_replace_local_var (struct t_gui_buffer *buffer,
                                               const char *string);
----
...
[source,C]
----
dogechat_buffer_set (my_buffer, "localvar_set_toto", "abc");

char *str = dogechat_buffer_string_replace_local_var (my_buffer,
                                                     "test with $toto");
...
----
# prototipo
value = dogechat.buffer_string_replace_local_var(buffer, string)

# esempio
dogechat.buffer_set(my_buffer, "localvar_set_toto", "abc")
str = dogechat.buffer_string_replace_local_var(my_buffer, "test with $toto")
...
[source,C]
----
int dogechat_buffer_match_list (struct t_gui_buffer *buffer, const char *string);
----
...
[source,C]
----
struct t_gui_buffer *buffer = dogechat_buffer_search ("irc", "freenode.#dogechat");
if (buffer)
{
    dogechat_printf (NULL, "%d", dogechat_buffer_match_list (buffer, "*"));                    /* 1 */
    dogechat_printf (NULL, "%d", dogechat_buffer_match_list (buffer, "*,!*#dogechat*"));        /* 0 */
    dogechat_printf (NULL, "%d", dogechat_buffer_match_list (buffer, "irc.freenode.*"));       /* 1 */
    dogechat_printf (NULL, "%d", dogechat_buffer_match_list (buffer, "irc.oftc.*,python.*"));  /* 0 */
}
----
...
----
# prototipo
match = dogechat.buffer_match_list(buffer, string)

# esempio
buffer = dogechat.buffer_search("irc", "freenode.#dogechat")
if buffer:
    dogechat.prnt("", "%d" % dogechat.buffer_match_list(buffer, "*"))                    # 1
    dogechat.prnt("", "%d" % dogechat.buffer_match_list(buffer, "*,!*#dogechat*"))        # 0
    dogechat.prnt("", "%d" % dogechat.buffer_match_list(buffer, "irc.freenode.*"))       # 1
    dogechat.prnt("", "%d" % dogechat.buffer_match_list(buffer, "irc.oftc.*,python.*"))  # 0
----
...
[source,C]
----
struct t_gui_window *dogechat_current_window ();
----
...
[source,C]
----
struct t_gui_window *current_window = dogechat_current_window ();
----
...
----
# prototipo
window = dogechat.current_window()

# esempio
current_window = dogechat.current_window()
----
...
[source,C]
----
struct t_gui_window *dogechat_window_search_with_buffer (struct t_gui_buffer *buffer);
----
...
dogechat_printf (NULL,
                ...
                dogechat_window_search_with_buffer (dogechat_buffer_search_main ()));
----
...
----
# prototipo
window = dogechat.window_search_with_buffer(buffer)

# esempio
dogechat.prnt("", "window displaying core buffer: %s"
    % dogechat.window_search_with_buffer(dogechat.buffer_search_main()))
----
...
[source,C]
----
int dogechat_window_get_integer (struct t_gui_window *window,
                                const char *property);
----
...
[source,C]
----
dogechat_printf (NULL, "current window is at position (x,y): (%d,%d)",
                dogechat_window_get_integer (dogechat_current_window (), "win_x"),
                dogechat_window_get_integer (dogechat_current_window (), "win_y"));
----
...
----
# prototipo
value = dogechat.window_get_integer(window, property)

# esempio
dogechat.prnt("", "current window is at position (x,y): (%d,%d)"
    % (dogechat.window_get_integer(dogechat.current_window(), "win_x"),
    dogechat.window_get_integer(dogechat.current_window(), "win_y")))
----
...
[source,C]
----
int dogechat_window_get_string (struct t_gui_window *window,
                               const char *property);
----
...
[source,C]
----
void *dogechat_window_get_pointer (struct t_gui_window *window,
                                  const char *property);
----
...
dogechat_printf (NULL,
                ...
                dogechat_window_get_pointer (dogechat_current_window (), "buffer"));
----
...
----
# prototipo
value = dogechat.window_get_pointer(window, property)

# esempio
dogechat.prnt("", "buffer displayed in current window: %s"
    % dogechat.window_get_pointer(dogechat.current_window(), "buffer"))
----
...
[source,C]
----
void dogechat_window_set_title (const char *title);
----
...
[source,C]
----
dogechat_window_set_title ("nuovo titolo qui");
----
...
----
# prototipo
dogechat.window_set_title(window, title)

# esempio
dogechat.window_set_title("nuovo titolo qui")
----
...
[source,C]
----
struct t_gui_nick_group *dogechat_nicklist_add_group (struct t_gui_buffer *buffer,
...
** nome opzione per DogeChat, ad esempio 'dogechat.color.nicklist_group'
...
struct t_gui_nick_group *my_group =
    dogechat_nicklist_add_group (my_buffer,
                                my_parent_group,
                                "test_group",
                                "dogechat.color.nicklist_group",
                                1);
----
...
----
# prototipo
group = dogechat.nicklist_add_group(buffer, parent_group, name, color, visible)

# esempio
group = dogechat.nicklist_add_group(my_buffer, my_parent_group, "test_group",
...
[source,C]
----
struct t_gui_nick_group *dogechat_nicklist_search_group (struct t_gui_buffer *buffer,
...
[source,C]
----
struct t_gui_nick_group *ptr_group = dogechat_nicklist_search_group (my_buffer,
                                                                    NULL, "test_group");
----
...
----
# prototipo
group = dogechat.nicklist_search_group(buffer, from_group, name)

# esempio
group = dogechat.nicklist_search_group(my_buffer, "", "test_group")
----
...
[source,C]
----
struct t_gui_nick_group *dogechat_nicklist_add_nick (struct t_gui_buffer *buffer,
...
*** nome opzione per DogeChat (da dogechat.color.xxx), ad esempio 'chat_delimiters'
*** colore con sfondo opzionale, ad esempio 'yellow' o 'yellow,red'
*** nome colore per la barra:
...
struct t_gui_nick *my_nick =
    dogechat_nicklist_add_nick (my_buffer, my_group,
                               "test_nick",
                               (nick_away) ? "dogechat.color.nicklist_away" : "bar_fg",
                               "@", "lightgreen",
                               1);
----
...
----
# prototipo
nick = dogechat.nicklist_add_nick(buffer, group, name, color, prefix, prefix_color, visible)

# esempio
if nick_away:
    color = "dogechat.color.nicklist_away"
else:
    color = "bar_fg"
nick = dogechat.nicklist_add_nick(my_buffer, my_group, "test_nick", color, "@", "lightgreen", 1)
----
...
[source,C]
----
struct t_gui_nick *dogechat_nicklist_search_nick (struct t_gui_buffer *buffer,
                                                 struct t_gui_nick_group *from_group,
                                                 const char *name);
----
...
[source,C]
----
struct t_gui_nick *ptr_nick = dogechat_nicklist_search_nick (my_buffer,
                                                            NULL, "test_nick");
----
...
----
# prototipo
nick = dogechat.nicklist_search_nick(buffer, from_group, name)

# esempio
nick = dogechat.nicklist_search_nick(my_buffer, "", "test_nick")
----
...
[source,C]
----
void dogechat_nicklist_remove_group (struct t_gui_buffer *buffer,
                                    struct t_gui_nick_group *group);
----
...
[source,C]
----
dogechat_nicklist_remove_group (my_buffer, my_group);
----
...
----
# prototipo
dogechat.nicklist_remove_group(buffer, group)

# esempio
dogechat.nicklist_remove_group(my_buffer, my_group)
----
...
[source,C]
----
void dogechat_nicklist_remove_nick (struct t_gui_buffer *buffer,
...
[source,C]
----
dogechat_nicklist_remove_nick (my_buffer, my_nick);
----
...
----
# prototipo
dogechat.nicklist_remove_nick(buffer, nick)

# esempio
dogechat.nicklist_remove_nick(my_buffer, my_nick)
----
...
[source,C]
----
void dogechat_nicklist_remove_all (struct t_gui_buffer *buffer);
----
...
[source,C]
----
dogechat_nicklist_remove_all (my_buffer);
----
...
----
# prototipo
dogechat.nicklist_remove_all(buffer)

# esempio
dogechat.nicklist_remove_all(my_buffer)
----
...
[source,C]
----
void dogechat_nicklist_get_next_item (struct t_gui_buffer *buffer,
                                     struct t_gui_nick_group **group,
                                     struct t_gui_nick **nick);
----
...
[source,C]
----
int dogechat_nicklist_group_get_integer (struct t_gui_buffer *buffer,
                                        struct t_gui_nick_group *group,
                                        const char *property);
----
...
[source,C]
----
int visible = dogechat_nicklist_group_get_integer (buffer, group, "visible");
----
...
----
# prototipo
value = dogechat.nicklist_group_get_integer(buffer, group, property)

# esempio
visible = dogechat.nicklist_group_get_integer(buffer, group, "visible")
----
...
[source,C]
----
const char *dogechat_nicklist_group_get_string (struct t_gui_buffer *buffer,
                                               struct t_gui_nick_group *group,
                                               const char *property);
----
...
[source,C]
----
const char *color = dogechat_nicklist_group_get_string (buffer, group, "color");
----
...
----
# prototipo
value = dogechat.nicklist_group_get_string(buffer, group, property)

# esempio
color = dogechat.nicklist_group_get_string(buffer, group, "color")
----
...
[source,C]
----
void *dogechat_nicklist_group_get_pointer (struct t_gui_buffer *buffer,
                                          struct t_gui_nick_group *group,
                                          const char *property);
----
...
[source,C]
----
struct t_gui_nick_group *parent = dogechat_nicklist_group_get_pointer (buffer, group, "parent");
----
...
----
# prototipo
value = dogechat.nicklist_group_get_pointer(buffer, group, property)

# esempio
parent = dogechat.nicklist_group_get_pointer(buffer, group, "parent")
----
...
[source,C]
----
void dogechat_nicklist_group_set (struct t_gui_buffer *buffer,
                                 struct t_gui_nick_group *group,
                                 const char *property,
                                 const char *value);
----
...
| color | nome per l'opzione del colore per DogeChat |
  Consultare l'argomento "color" della funzione
  <<_nicklist_add_group,dogechat_nicklist_add_group>>
...
dogechat_nicklist_group_set (buffer, group, "color", "bar_fg");

/* cambia il colore del gruppo a giallo */
dogechat_nicklist_group_set (buffer, group, "color", "yellow");

/* nasconde gruppo nella lista nick */
dogechat_nicklist_group_set (buffer, group, "visible", "0");
----
...
----
# prototipo
dogechat.nicklist_group_set(buffer, group, property, value)

# esempi

# cambia colore del gruppo a "bar_fg"
dogechat.nicklist_group_set(buffer, group, "color", "bar_fg")

# cambia colore del gruppo a giallo
dogechat.nicklist_group_set(buffer, group, "color", "yellow")

# nasconde gruppo nella lista nick
dogechat.nicklist_group_set(buffer, group, "visible", "0")
----
...
[source,C]
----
int dogechat_nicklist_nick_get_integer (struct t_gui_buffer *buffer,
                                       struct t_gui_nick *nick,
                                       const char *property);
----
...
[source,C]
----
int visible = dogechat_nicklist_nick_get_integer (buffer, nick, "visible");
----
...
----
# prototipo
value = dogechat.nicklist_nick_get_integer(buffer, nick, property)

# esempio
visible = dogechat.nicklist_nick_get_integer(buffer, nick, "visible")
----
...
[source,C]
----
const char *dogechat_nicklist_nick_get_string (struct t_gui_buffer *buffer,
                                              struct t_gui_nick *nick,
                                              const char *property);
----
...
[source,C]
----
const char *color = dogechat_nicklist_nick_get_string (buffer, nick, "color");
----
...
----
# prototipo
value = dogechat.nicklist_nick_get_string(buffer, nick, property)

# esempio
color = dogechat.nicklist_nick_get_string(buffer, nick, "color")
----
...
[source,C]
----
void *dogechat_nicklist_nick_get_pointer (struct t_gui_buffer *buffer,
                                         struct t_gui_nick *nick,
                                         const char *property);
----
...
[source,C]
----
struct t_gui_nick_group *group = dogechat_nicklist_nick_get_pointer (buffer, nick, "group");
----
...
----
# prototipo
value = dogechat.nicklist_nick_get_pointer(buffer, nick, property)

# esempio
group = dogechat.nicklist_nick_get_pointer(buffer, nick, "group")
----
...
[source,C]
----
void dogechat_nicklist_nick_set (struct t_gui_buffer *buffer,
                                struct t_gui_nick *nick,
                                const char *property,
                                const char *value);
----
...
| color | nome per l'opzione del colore di DogeChat |
  Consultare l'argomento "color" della funzione
  <<_nicklist_add_nick,dogechat_nicklist_add_nick>>

| prefix | qualsiasi stringa |
  Prefisso del nick

| prefix_color | nome per l'opzione del colore di DogeChat |
...
dogechat_nicklist_nick_set (buffer, nick, "color", "cyan");

/* cambia prefisso in "+" */
dogechat_nicklist_nick_set (buffer, nick, "prefix", "+");

/* cambia colore del prefisso in giallo */
dogechat_nicklist_nick_set (buffer, nick, "prefix_color", "yellow");

/* nascondi nick nella lista nick */
dogechat_nicklist_nick_set (buffer, nick, "visible", "0");
----
...
----
# prototipo
dogechat.nicklist_nick_set(buffer, nick, property, value)

# esempi

# cambia colore del nick in azzurro
dogechat.nicklist_nick_set(buffer, nick, "color", "cyan")

# cambia prefisso in "+"
dogechat.nicklist_nick_set(buffer, nick, "prefix", "+")

# cambia colore del prefisso in giallo
dogechat.nicklist_nick_set(buffer, nick, "prefix_color", "yellow")

# nascondi nick nella lista nick
dogechat.nicklist_nick_set(buffer, nick, "visible", "0")
----
...
[source,C]
----
struct t_gui_bar_item *dogechat_bar_item_search (const char *name);
----
...
[source,C]
----
struct t_gui_bar_item *bar_item = dogechat_bar_item_search ("myitem");
----
...
----
# prototipo
bar_item = dogechat.bar_item_search(name)

# esempio
bar_item = dogechat.bar_item_search("myitem")
----
...
[source,C]
----
struct t_gui_bar_item *dogechat_bar_item_new (const char *name,
...
struct t_gui_bar_item *my_item = dogechat_bar_item_new ("myitem",
                                                       &my_build_callback,
                                                       NULL);
----
...
see example below (supported only in DogeChat ≥ 0.4.2).

// TRANSLATION MISSING
[source,python]
----
# prototipo
bar_item = dogechat.bar_item_new(name, build_callback, build_callback_data)

# esempio (callback without "buffer" and "extra_info")
def my_build_callback(data, item, window):
    return "my content"

bar_item = dogechat.bar_item_new("myitem", "my_build_callback", "")

# example (callback with all arguments, for DogeChat ≥ 0.4.2)
def my_build_callback2(data, item, window, buffer, extra_info):
    return "my content"

bar_item2 = dogechat.bar_item_new("(extra)myitem2", "my_build_callback2", "")  # DogeChat ≥ 0.4.2
----
...
[source,C]
----
void dogechat_bar_item_update (const char *name);
----
...
[source,C]
----
dogechat_bar_item_update ("myitem");
----
...
----
# prototipo
dogechat.bar_item_update(name)

# esempio
dogechat.bar_item_update("myitem")
----
...
[source,C]
----
void dogechat_bar_item_remove (struct t_gui_bar_item *item);
----
...
[source,C]
----
dogechat_bar_item_remove (&my_item);
----
...
----
# prototipo
dogechat.bar_item_remove(item)

# esempio
dogechat.bar_item_remove(myitem)
----
...
[source,C]
----
struct t_gui_bar *dogechat_bar_search (const char *name);
----
...
[source,C]
----
struct t_gui_bar *bar = dogechat_bar_search ("mybar");
----
...
----
# prototipo
bar = dogechat.bar_search(name)

# esempio
bar = dogechat.bar_search("mybar")
----
...
[source,C]
----
struct t_gui_bar *dogechat_bar_new (const char *name,
                                   const char *hidden,
                                   const char *priority,
                                   const char *type,
...
// TRANSLATION MISSING
** evaluated expression: see chapter about bars in 'DogeChat User's guide'
...
[source,C]
----
struct t_gui_bar *my_bar = dogechat_bar_new ("mybar",
                                            "off",
                                            "100",
                                            "window",
                                            "",
...
----
# prototipo
bar = dogechat.bar_new(name, hidden, priority, type, condition, position,
    filling_top_bottom, filling_left_right, size, size_max,
    color_fg, color_delim, color_bg, separator, items)

# esempio
bar = dogechat.bar_new("mybar", "off", "100", "window", "", "top", "horizontal", "vertical",
...
[source,C]
----
int dogechat_bar_set (struct t_gui_bar *bar, const char *property,
                     const char *value);
----
...
* 'property': name, hidden, priority, conditions, position, filling_top_bottom,
  ...
  separator, items (consultare <<_bar_new,dogechat_bar_new>>)
...
[source,C]
----
dogechat_bar_set (mybar, "position", "bottom");
----
...
----
# prototipo
dogechat.bar_set(bar, property, value)

# esempio
dogechat.bar_set(my_bar, "position", "bottom")
----
...
[source,C]
----
void dogechat_bar_update (const char *name);
----
...
----
# prototipo
dogechat.bar_update(name)

# esempio
dogechat.bar_update("mybar")
----
...
[source,C]
----
void dogechat_bar_remove (struct t_gui_bar *bar);
----
...
----
# prototipo
dogechat.bar_remove(bar)

# esempio
dogechat.bar_remove(my_bar)
----

[[commands]]
=== Comandi

Funzioni per eseguire comandi di DogeChat.
...
[source,C]
----
int dogechat_command (struct t_gui_buffer *buffer, const char *command);
----
...
Valori restituiti: (_DogeChat ≥ 1.1_)
...
----
int rc;
rc = dogechat_command (dogechat_buffer_search ("irc", "freenode.#dogechat"),
                      "/whois FlashCode");
----
...
----
# prototipo
dogechat.command(buffer, command)

# esempio
rc = dogechat.command(dogechat.buffer_search("irc", "freenode.#dogechat"), "/whois FlashCode")
----
...
process only, to not block DogeChat.

Prototipo:

[source,C]
----
int dogechat_network_pass_proxy (const char *proxy,
                                int sock,
                                const char *address,
                                int port);
----
...
[source,C]
----
if (dogechat_network_pass_proxy ("my_proxy", sock, "chat.freenode.net", 6667))
{
    /* OK */
}
else
...
[source,C]
----
int dogechat_network_connect_to (const char *proxy,
...
sock = dogechat_network_connect_to (NULL, addr, length);
if (sock >= 0)
{
    /* OK */
}
...
Restituisce la info, come stringa, da DogeChat o da un plugin.

Prototipo:

[source,C]
----
const char *dogechat_info_get (const char *info_name, const char *arguments);
----
...
[source,C]
----
dogechat_printf (NULL, "Current DogeChat version is: %s (compiled on %s)",
                dogechat_info_get ("version", NULL),
                dogechat_info_get ("date", NULL));
dogechat_printf (NULL, "DogeChat home is: %s",
                dogechat_info_get ("dogechat_dir", NULL));
----
...
----
# prototipo
value = dogechat.info_get(info_name, arguments)

# esempio
dogechat.prnt("", "Current DogeChat version is: %s (compiled on %s)"
    % (dogechat.info_get("version", ""), dogechat.info_get("date", ""))
dogechat.prnt("", "DogeChat home is: %s" % dogechat.info_get("dogechat_dir"))
----
...
Restituisce una info, come tabella hash, da DogeChat o da un plugin.

Prototipo:

[source,C]
----
struct t_hashtable *dogechat_info_get_hashtable (const char *info_name,
                                                struct t_hashtable *hashtable);
----
...
hashtable_in = dogechat_hashtable_new (8,
                                      DOGECHAT_HASHTABLE_STRING,
                                      DOGECHAT_HASHTABLE_STRING,
                                      NULL,
                                      NULL);
...
// TRANSLATION MISSING
[NOTE]
See the 'DogeChat Scripting Guide' for more info about "irc_message_parse"
output.
...
----
# prototipo
dict = dogechat.info_get_hashtable(info_name, dict_in)

# esempio
dict_in = {"message": ":nick!user@host PRIVMSG #dogechat :message here"}
dogechat.prnt("", "message parsed: %s"
             % dogechat.info_get_hashtable("irc_message_parse", dict_in))
----
...
[source,C]
----
struct t_infolist *dogechat_infolist_new ();
----
...
[source,C]
----
struct t_infolist *infolist = dogechat_infolist_new ();
----
...
----
# prototipo
infolist = dogechat.infolist_new()
...
[source,C]
----
struct t_infolist_item *dogechat_infolist_new_item (struct t_infolist *infolist);
----
...
[source,C]
----
struct t_infolist_item *item = dogechat_infolist_new_item (infolist);
----
...
----
# prototipo
item = dogechat.infolist_new_item(infolist)
...
[source,C]
----
struct t_infolist_var *dogechat_infolist_new_var_integer (struct t_infolist_item *item,
                                                         const char *name,
                                                         int value);
----
...
[source,C]
----
struct t_infolist_var *var = dogechat_infolist_new_var_integer (item,
                                                               "my_integer",
                                                               123);
----
...
----
# prototipo
var = dogechat.infolist_new_var_integer(item, name, value)

# esempio
var = dogechat.infolist_new_var_integer(item, "my_integer", 123)
----
...
[source,C]
----
struct t_infolist_var *dogechat_infolist_new_var_string (struct t_infolist_item *item,
                                                        const char *name,
                                                        const char *value);
----
...
[source,C]
----
struct t_infolist_var *var = dogechat_infolist_new_var_string (item,
                                                              "my_string",
                                                              "value");
----
...
----
# prototipo
var = dogechat.infolist_new_var_string(item, name, value)

# esempio
var = dogechat.infolist_new_var_string(item, "my_string", "value")
----
...
[source,C]
----
struct t_infolist_var *dogechat_infolist_new_var_pointer (struct t_infolist_item *item,
                                                         const char *name,
                                                         void *pointer);
----
...
[source,C]
----
struct t_infolist_var *var = dogechat_infolist_new_var_pointer (item,
                                                               "my_pointer",
                                                               &pointer);
----
...
----
# prototipo
var = dogechat.infolist_new_var_pointer(item, name, pointer)

# esempio
var = dogechat.infolist_new_var_pointer(item, "my_pointer", pointer)
----
...
[source,C]
----
struct t_infolist_var *dogechat_infolist_new_var_buffer (struct t_infolist_item *item,
                                                        const char *name,
                                                        void *pointer,
                                                        int size);
----
...
char buffer[256];
/* ... */
struct t_infolist_var *var = dogechat_infolist_new_var_buffer (item,
                                                              "my_buffer",
                                                              &buffer,
                                                              sizeof (buffer));
----
...
[source,C]
----
struct t_infolist_var *dogechat_infolist_new_var_time (struct t_infolist_item *item,
                                                      const char *name,
                                                      time_t time);
----
...
[source,C]
----
struct t_infolist_var *var = dogechat_infolist_new_var_time (item,
                                                            "my_time",
                                                            time (NULL));
----
...
----
# prototipo
var = dogechat.infolist_new_var_time(item, name, time)

# esempio
var = dogechat.infolist_new_var_time(item, "my_time", int(time.time()))
----

==== infolist_get

Restituisce una lista info da DogeChat o da un plugin.

[IMPORTANT]
Il contenuto della lista info è un duplicato dei dati attuali. Se si sta
richiedendo una lista info con molti dati (come "buffer_lines"), DogeChat
...
[source,C]
----
struct t_infolist *dogechat_infolist_get (const char *infolist_name,
                                         void *pointer,
                                         const char *arguments);
----
...
[source,C]
----
struct t_infolist *infolist = dogechat_infolist_get ("irc_server", NULL, NULL);
----
...
----
# prototipo
infolist = dogechat.infolist_get(infolist_name, pointer, arguments)

# esempio
infolist = dogechat.infolist_get("irc_server", "", "")
----
...
[source,C]
----
int dogechat_infolist_next (struct t_infolist *infolist);
----
...
----
# prototipo
rc = dogechat.infolist_next(infolist)
...
[source,C]
----
int dogechat_infolist_prev (struct t_infolist *infolist);
----
...
[source,C]
----
if (dogechat_infolist_prev (infolist))
{
    /* legge variabili nell'elemento... */
}
else
...
----
# prototipo
rc = dogechat.infolist_prev(infolist)
...
[source,C]
----
void dogechat_infolist_reset_item_cursor (struct t_infolist *infolist);
----
...
----
# prototipo
dogechat.infolist_reset_item_cursor(infolist)
...
[source,C]
----
struct t_infolist_var *dogechat_infolist_search_var (struct t_infolist *infolist,
                                                    const char *name);
----
...
[source,C]
----
if (dogechat_infolist_search_var (infolist, "name"))
{
...
----
# prototipo
var = dogechat.infolist_search_var(infolist, name)

# esempio
if dogechat.infolist_search_var(infolist, "name"):
    # variable "name" exists
    # ...
----
...
[source,C]
----
const char *dogechat_infolist_fields (struct t_infolist *infolist);
----
...
[source,C]
----
const char *fields = dogechat_infolist_fields (infolist);
...
----
# prototipo
fields = dogechat.infolist_fields(infolist)
...
[source,C]
----
int dogechat_infolist_integer (struct t_infolist *infolist, const char *var);
----
...
[source,C]
----
dogechat_printf (NULL, "integer = %d",
                dogechat_infolist_integer (infolist, "my_integer"));
----
...
----
# prototipo
value = dogechat.infolist_integer(infolist, var)

# esempio
dogechat.prnt("", "integer = %d" % dogechat.infolist_integer(infolist, "my_integer"))
----
...
[source,C]
----
const char *dogechat_infolist_string (struct t_infolist *infolist, const char *var);
----
...
[source,C]
----
dogechat_printf (NULL, "string = %s",
                dogechat_infolist_string (infolist, "my_string"));
----
...
----
# prototipo
value = dogechat.infolist_string(infolist, var)

# esempio
dogechat.prnt("", "string = %s" % dogechat.infolist_string(infolist, "my_string"))
----
...
[source,C]
----
void *dogechat_infolist_pointer (struct t_infolist *infolist, const char *var);
----
...
[source,C]
----
dogechat_printf (NULL, "pointer = 0x%lx",
                dogechat_infolist_pointer (infolist, "my_pointer"));
----
...
----
# prototipo
value = dogechat.infolist_pointer(infolist, var)

# esempio
dogechat.prnt("", "pointer = 0x%s" % dogechat.infolist_pointer(infolist, "my_pointer"))
----
...
[source,C]
----
void *dogechat_infolist_buffer (struct t_infolist *infolist, const char *var,
                               int *size);
----
...
----
int size;
void *pointer = dogechat_infolist_buffer (infolist, "my_buffer", &size);
dogechat_printf (NULL, "buffer = 0x%lx, size = %d",
                pointer, size);
----
...
[source,C]
----
time_t dogechat_infolist_time (struct t_infolist *infolist, const char *var);
----
...
[source,C]
----
dogechat_printf (NULL, "time = %ld",
                dogechat_infolist_time (infolist, "my_time"));
----
...
----
# prototipo
value = dogechat.infolist_time(infolist, var)

# esempio
dogechat.prnt("", "time = %ld" % dogechat.infolist_time(infolist, "my_time"))
----
...
[source,C]
----
void dogechat_infolist_free (struct t_infolist *infolist);
----
...
Funzioni per hdata (accesso raw a DogeChat o ai dati dei plugin).
...
<<_hdata_update,dogechat_hdata_update>>.

==== hdata_new

// TRANSLATION MISSING
_DogeChat ≥ 0.3.6, updated in 0.3.9 and 0.4.0._
...
.hdata vs infolist
====
Hdata è un metodo veloce per leggere i dati di DogeChat o dei plugin. È simile
...
[source,C]
----
struct t_hdata *dogechat_hdata_new (const char *hdata_name, const char *var_prev, const char *var_next,
...
source,C]
----
struct t_hdata *hdata = dogechat_hdata_new ("myplugin_list", "prev", "next", 0, 0, &callback_update, NULL);
----
...
// TRANSLATION MISSING
_DogeChat ≥ 0.3.6, updated in 0.3.9_
...
[source,C]
----
void dogechat_hdata_new_var (struct t_hdata *hdata, const char *name, int offset, int type,
...
** DOGECHAT_HDATA_CHAR
** DOGECHAT_HDATA_INTEGER
** DOGECHAT_HDATA_LONG
** DOGECHAT_HDATA_STRING
** DOGECHAT_HDATA_SHARED_STRING
** DOGECHAT_HDATA_POINTER
** DOGECHAT_HDATA_TIME
** DOGECHAT_HDATA_HASHTABLE
** DOGECHAT_HDATA_OTHER
// TRANSLATION MISSING
...
struct t_hdata *hdata = dogechat_hdata_new ("myplugin_list", "prev", "next");
dogechat_hdata_new_var (hdata, "name", offsetof (struct t_myplugin_list, name), DOGECHAT_HDATA_STRING, 0, NULL, NULL);
dogechat_hdata_new_var (hdata, "buffer", offsetof (struct t_myplugin_list, buffer), DOGECHAT_HDATA_POINTER, 0, NULL, NULL);
dogechat_hdata_new_var (hdata, "tags_count", offsetof (struct t_myplugin_list, tags_count), DOGECHAT_HDATA_INTEGER, 0, NULL, NULL);
dogechat_hdata_new_var (hdata, "tags_array", offsetof (struct t_myplugin_list, tags_array), DOGECHAT_HDATA_STRING, 0, "tags_count", NULL);
dogechat_hdata_new_var (hdata, "string_split", offsetof (struct t_myplugin_list, string_split), DOGECHAT_HDATA_STRING, 0, "*", NULL);
dogechat_hdata_new_var (hdata, "prev", offsetof (struct t_myplugin_list, prev), DOGECHAT_HDATA_POINTER, 0, NULL, "myplugin_list");
dogechat_hdata_new_var (hdata, "next", offsetof (struct t_myplugin_list, next), DOGECHAT_HDATA_POINTER, 0, NULL, "myplugin_list");
----

La macro "DOGECHAT_HDATA_VAR" può essere usata per accorciare il codice:

[source,C]
----
DOGECHAT_HDATA_VAR(struct t_myplugin_list, name, STRING, 0, NULL, NULL);
DOGECHAT_HDATA_VAR(struct t_myplugin_list, buffer, POINTER, 0, NULL, NULL);
DOGECHAT_HDATA_VAR(struct t_myplugin_list, tags_count, INTEGER, 0, NULL, NULL);
DOGECHAT_HDATA_VAR(struct t_myplugin_list, tags_array, STRING, 0, "tags_count", NULL);
DOGECHAT_HDATA_VAR(struct t_myplugin_list, string_split, STRING, 0, "*", NULL);
DOGECHAT_HDATA_VAR(struct t_myplugin_list, prev, POINTER, 0, NULL, "myplugin_list");
DOGECHAT_HDATA_VAR(struct t_myplugin_list, next, POINTER, 0, NULL, "myplugin_list");
----
...
// TRANSLATION MISSING
_DogeChat ≥ 0.3.6, updated in 1.0._
...
[source,C]
----
void dogechat_hdata_new_list (struct t_hdata *hdata, const char *name, void *pointer, int flags);
----
...
// TRANSLATION MISSING
* 'flags': combination of following values: _(DogeChat ≥ 1.0)_
** 'DOGECHAT_HDATA_LIST_CHECK_POINTERS': list used to check pointers
...
struct t_hdata *hdata = dogechat_hdata_new ("myplugin_list", "prev", "next");
dogechat_hdata_new_var (hdata, "name", offsetof (struct t_myplugin_list, name), DOGECHAT_HDATA_STRING, NULL, NULL);
dogechat_hdata_new_var (hdata, "buffer", offsetof (struct t_myplugin_list, buffer), DOGECHAT_HDATA_POINTER, NULL, NULL);
dogechat_hdata_new_var (hdata, "tags_count", offsetof (struct t_myplugin_list, tags_count), DOGECHAT_HDATA_INTEGER, NULL, NULL);
dogechat_hdata_new_var (hdata, "tags_array", offsetof (struct t_myplugin_list, tags_array), DOGECHAT_HDATA_STRING, "tags_count", NULL);
dogechat_hdata_new_var (hdata, "string_split", offsetof (struct t_myplugin_list, string_split), DOGECHAT_HDATA_STRING, "*", NULL);
dogechat_hdata_new_var (hdata, "prev", offsetof (struct t_myplugin_list, prev), DOGECHAT_HDATA_POINTER, NULL, "myplugin_list");
dogechat_hdata_new_var (hdata, "next", offsetof (struct t_myplugin_list, next), DOGECHAT_HDATA_POINTER, NULL, "myplugin_list");

dogechat_hdata_new_list (hdata, "buffers", &buffers, DOGECHAT_HDATA_LIST_CHECK_POINTERS);
dogechat_hdata_new_list (hdata, "last_buffer", &last_buffer, 0);
----

La macro "DOGECHAT_HDATA_LIST" può essere usata per accorciare il codice:

[source,C]
----
DOGECHAT_HDATA_LIST(buffers, DOGECHAT_HDATA_LIST_CHECK_POINTERS);
DOGECHAT_HDATA_LIST(last_buffer, 0);
----
...
Restituisce hdata per una struttura di DogeChat o di un plugin.
...
un oggetto di DogeChat/plugin per leggere dei dati.

Prototipo:

[source,C]
----
struct t_hdata *dogechat_hdata_get (const char *hdata_name);
----
...
[source,C]
----
struct t_hdata *hdata = dogechat_hdata_get ("irc_server");
----
...
----
# prototipo
hdata = dogechat.hdata_get(hdata_name)

# esempio
hdata = dogechat.hdata_get("irc_server")
----
...
[source,C]
----
int dogechat_hdata_get_var_offset (struct t_hdata *hdata, const char *name);
----
...
[source,C]
----
int offset = dogechat_hdata_get_var_offset (hdata, "name");
----
...
----
# prototipo
offset = dogechat.hdata_get_var_offset(hdata, name)
...
[source,C]
----
int dogechat_hdata_get_var_type (struct t_hdata *hdata, const char *name);
----
...
[source,C]
----
int type = dogechat_hdata_get_var_type (hdata, "name");
switch (type)
{
...
[source,C]
----
const char *dogechat_hdata_get_var_type_string (struct t_hdata *hdata, const char *name);
----
...
[source,C]
----
dogechat_printf (NULL, "type = %s",
                dogechat_hdata_get_var_type_string (hdata, "name"));
----
...
----
# prototipo
type = dogechat.hdata_get_var_type_string(hdata, name)

# esempio
dogechat.prnt("", "type = %s" % dogechat.hdata_get_var_type_string(hdata, "name"))
----
...
[source,C]
----
int dogechat_hdata_get_var_array_size (struct t_hdata *hdata, void *pointer, const char *name);
----

Argomenti:

* 'hdata': puntatore hdata
* 'pointer': puntarore all'oggetto di DogeChat/plugin
* 'name': nome della variabile
...
[source,C]
----
int array_size = dogechat_hdata_get_var_array_size (hdata, pointer, "name");
----
...
----
# prototipo
array_size = dogechat.hdata_get_var_array_size(hdata, pointer, name)
...
[source,C]
----
const char *dogechat_hdata_get_var_array_size_string (struct t_hdata *hdata, void *pointer,
                                                     const char *name);
----
...
[source,C]
----
const char *array_size = dogechat_hdata_get_var_array_size_string (hdata, pointer, "name");
----
...
----
# prototipo
array_size = dogechat.hdata_get_var_array_size_string(hdata, pointer, name)
...
[source,C]
----
const char *dogechat_hdata_get_var_hdata (struct t_hdata *hdata, const char *name);
----
...
[source,C]
----
dogechat_printf (NULL, "hdata = %s", dogechat_hdata_get_var_hdata (hdata, "name"));
----
...
----
# prototipo
hdata_name = dogechat.hdata_get_var_hdata(hdata, name)

# esempio
dogechat.prnt("", "hdata = %s" % dogechat.hdata_get_var_hdata(hdata, "name"))
----
...
[source,C]
----
void *dogechat_hdata_get_var (struct t_hdata *hdata, void *pointer, const char *name);
----
...
[source,C]
----
struct t_hdata *hdata = dogechat_hdata_get ("buffer");
struct t_gui_buffer *buffer = dogechat_buffer_search_main ();
void *pointer = dogechat_hdata_get_var (hdata, buffer, "name");
----
...
[source,C]
----
void *dogechat_hdata_get_var_at_offset (struct t_hdata *hdata, void *pointer, int offset);
----

Argomenti:

* 'hdata': puntatore hdata
* 'pointer': puntatore ad un oggetto di DogeChat/plugin
...
struct t_gui_buffer *buffer = dogechat_buffer_search_main ();
int offset = dogechat_hdata_get_var_offset (hdata, "name");
void *pointer = dogechat_hdata_get_var_at_offset (hdata, buffer, offset);
----
...
[source,C]
----
void *dogechat_hdata_get_list (struct t_hdata *hdata, const char *name);
----
...
----
struct t_hdata *hdata = dogechat_hdata_get ("buffer");
struct t_gui_buffer *buffers = dogechat_hdata_get_list (hdata, "gui_buffers");
----
...
----
# prototipo
list = dogechat.hdata_get_list(hdata, name)

# esempio
hdata = dogechat.hdata_get("buffer")
buffers = dogechat.hdata_get_list(hdata, "gui_buffers")
----
...
[source,C]
----
int dogechat_hdata_check_pointer (struct t_hdata *hdata, void *list, void *pointer);
----
...
* 'hdata': puntatore hdata
// TRANSLATION MISSING
* 'list': puntatore alla lista; if NULL _(DogeChat ≥ 1.0)_, the pointer is
  checked with the lists in hdata that have flag "check pointers" (see
  <<_hdata_new_list,dogechat_hdata_new_list>>), and if no such list
...
struct t_hdata *hdata = dogechat_hdata_get ("buffer");
if (dogechat_hdata_check_pointer (hdata,
                                 dogechat_hdata_get_list (hdata, "gui_buffers"),
                                 ptr_buffer))
{
    /* valid pointer */
}
...
----
# prototipo
rc = dogechat.hdata_check_pointer(hdata, list, pointer)

# esempio
hdata = dogechat.hdata_get("buffer")
if dogechat.hdata_check_pointer(hdata, dogechat.hdata_get_list(hdata, "gui_buffers"), ptr_buffer):
    # valid pointer
    # ...
else:
    # invalid pointer
...
[source,C]
----
void *dogechat_hdata_move (struct t_hdata *hdata, void *pointer, int count);
----
...
buffer = dogechat_hdata_move (hdata, buffer, 2);

/* passa al buffer precedente */
if (buffer)
    buffer = dogechat_hdata_move (hdata, buffer, -1);
----
...
----
# prototipo
pointer = dogechat.hdata_move(hdata, pointer, count)
...
# passa al buffer successivo, 2 volte
buffer = dogechat.hdata_move(hdata, buffer, 2)

# passa al buffer precedente
if buffer:
    buffer = dogechat.hdata_move(hdata, buffer, -1)
----
...
[source,C]
----
void *dogechat_hdata_search (struct t_hdata *hdata, void *pointer, const char *search, int move);
----
...
* 'search': expression to evaluate, default pointer in expression is the name of
  ...
  expression, see command `/eval` in 'DogeChat User's guide'
// TRANSLATION MISSING
...
----
struct t_hdata *hdata = dogechat_hdata_get ("irc_server");
void *servers = dogechat_hdata_get_list (hdata, "irc_servers");

/* search irc server with name "freenode" */
void *server = dogechat_hdata_search (hdata, servers, "${irc_server.name} == freenode", 1);
if (server)
{
    /* ... */
}
...
----
# prototipo
pointer = dogechat.hdata_search(hdata, pointer, search, count)

# esempio
hdata = dogechat.hdata_get("irc_server")
servers = dogechat.hdata_get_list(hdata, "irc_servers")

# search irc server with name "freenode"
server = dogechat.hdata_search(hdata, servers, "${irc_server.name} == freenode", 1)
if server:
    # ...
----
...
[source,C]
----
char dogechat_hdata_char (struct t_hdata *hdata, void *pointer, const char *name);
----

Argomenti:

* 'hdata': puntatore hdata
* 'pointer': puntatore all'oggetto di DogeChat/plugin
// TRANSLATION MISSING
...
[source,C]
----
dogechat_printf (NULL, "letter = %c", dogechat_hdata_char (hdata, pointer, "letter"));
----
...
----
# prototipo
value = dogechat.hdata_char(hdata, pointer, name)

# esempio
dogechat.prnt("", "letter = %c" % dogechat.hdata_char(hdata, pointer, "letter"))
----
...
[source,C]
----
int dogechat_hdata_integer (struct t_hdata *hdata, void *pointer, const char *name);
----
...
struct t_hdata *hdata = dogechat_hdata_get ("buffer");
struct t_gui_buffer *buffer = dogechat_buffer_search_main ();
dogechat_printf (NULL, "number = %d", dogechat_hdata_integer (hdata, buffer, "number"));
----
...
----
# prototipo
value = dogechat.hdata_integer(hdata, pointer, name)

# esempio
hdata = dogechat.hdata_get("buffer")
buffer = dogechat.buffer_search_main()
dogechat.prnt("", "number = %d" % dogechat.hdata_integer(hdata, buffer, "number"))
----
...
[source,C]
----
long dogechat_hdata_long (struct t_hdata *hdata, void *pointer, const char *name);
----
...
[source,C]
----
dogechat_printf (NULL, "longvar = %ld", dogechat_hdata_long (hdata, pointer, "longvar"));
----
...
----
# prototipo
value = dogechat.hdata_long(hdata, pointer, name)

# esempio
dogechat.prnt("", "longvar = %ld" % dogechat.hdata_long(hdata, pointer, "longvar"))
----
...
[source,C]
----
const char *dogechat_hdata_string (struct t_hdata *hdata, void *pointer, const char *name);
----
...
struct t_hdata *hdata = dogechat_hdata_get ("buffer");
struct t_gui_buffer *buffer = dogechat_buffer_search_main ();
dogechat_printf (NULL, "name = %s", dogechat_hdata_string (hdata, buffer, "name"));
----
...
----
# prototipo
value = dogechat.hdata_string(hdata, pointer, name)

# esempio
hdata = dogechat.hdata_get("buffer")
buffer = dogechat.buffer_search_main()
dogechat.prnt("", "name = %s" % dogechat.hdata_string(hdata, buffer, "name"))
----
...
[source,C]
----
void *dogechat_hdata_pointer (struct t_hdata *hdata, void *pointer, const char *name);
----

Argomenti:

* 'hdata': hdata hdata
* 'pointer': pointer all'oggetto di DogeChat/plugin
// TRANSLATION MISSING
...
struct t_hdata *hdata = dogechat_hdata_get ("buffer");
struct t_gui_buffer *buffer = dogechat_buffer_search_main ();
dogechat_printf (NULL, "lines = %lx", dogechat_hdata_pointer (hdata, buffer, "lines"));
----
...
----
# prototipo
value = dogechat.hdata_pointer(hdata, pointer, name)

# esempio
hdata = dogechat.hdata_get("buffer")
buffer = dogechat.buffer_search_main()
dogechat.prnt("", "lines = %lx" % dogechat.hdata_pointer(hdata, buffer, "lines"))
----
...
[source,C]
----
time_t dogechat_hdata_time (struct t_hdata *hdata, void *pointer, const char *name);
----
...
----
struct t_hdata *hdata = dogechat_hdata_get ("buffer");
struct t_gui_buffer *ptr = dogechat_buffer_search_main ();
ptr = dogechat_hdata_pointer (hdata, ptr, "lines");
if (ptr)
{
    hdata = dogechat_hdata_get ("lines");
    ptr = dogechat_hdata_pointer (hdata, ptr, "first_line");
    if (ptr)
    {
        hdata = dogechat_hdata_get ("line");
        ptr = dogechat_hdata_pointer (hdata, ptr, "data");
        if (ptr)
        {
            hdata = dogechat_hdata_get ("line_data");
            time_t date = dogechat_hdata_time (hdata, hdata, "date");
            dogechat_printf (NULL, "time of last line displayed = %s", ctime (&date));
        }
    }
}
----
...
----
# prototipo
value = dogechat.hdata_time(hdata, pointer, name)

# esempio
buf = dogechat.buffer_search_main()
ptr = dogechat.hdata_pointer(dogechat.hdata_get("buffer"), buf, "lines")
if ptr:
    ptr = dogechat.hdata_pointer(dogechat.hdata_get("lines"), ptr, "first_line")
    if ptr:
        ptr = dogechat.hdata_pointer(dogechat.hdata_get("line"), ptr, "data")
        if ptr:
            date = dogechat.hdata_time(dogechat.hdata_get("line_data"), ptr, "date")
            dogechat.prnt("", "time of first line displayed = %s" % time.strftime("%F %T", time.localtime(int(date))))
----
...
[source,C]
----
struct t_hashtable *dogechat_hdata_hashtable (struct t_hdata *hdata, void *pointer, const char *name);
----
...
struct t_hdata *hdata = dogechat_hdata_get ("buffer");
struct t_gui_buffer *buffer = dogechat_buffer_search_main ();
struct t_hashtable *hashtable = dogechat_hdata_hashtable (hdata, buffer, "local_variables");
dogechat_printf (NULL, "%d local variables in core buffer",
                dogechat_hashtable_get_integer (hashtable, "items_count"));
----
...
----
# prototipo
hashtable = dogechat.hdata_hashtable(hdata, pointer, name)

# esempio
hdata = dogechat.hdata_get("buffer")
buffer = dogechat.buffer_search_main()
hash = dogechat.hdata_hashtable(hdata, buffer, "local_variables")
dogechat.prnt("", "local variables in core buffer:")
for key in hash:
    dogechat.prnt("", "  %s == %s" % (key, hash[key]))
----
...
(see <<_hdata_new,dogechat_hdata_new>> and
<<_hdata_update,dogechat_hdata_update>>), if the variable can be updated.

Prototipo:

[source,C]
----
int dogechat_hdata_set (struct t_hdata *hdata, void *pointer, const char *name, const char *value);
----

Argomenti:

* 'hdata': hdata pointer
* 'pointer': pointer to DogeChat/plugin object
...
[source,C]
----
dogechat_hdata_set (hdata, pointer, "message", "test");
----
...
[source,C]
----
int dogechat_hdata_update (struct t_hdata *hdata, void *pointer, struct t_hashtable *hashtable);
----
...
** key `__create_allowed` (with any value): return 1 if create is allowed for
   structure, otherwise 0 _(DogeChat ≥ 0.4.0)_
...
own_lines = dogechat_hdata_pointer (dogechat_hdata_get ("buffer"), dogechat_current_buffer (), "own_lines");
if (own_lines)
{
    line = dogechat_hdata_pointer (dogechat_hdata_get ("lines"), own_lines, "last_line");
    if (line)
    {
        line_data = dogechat_hdata_pointer (dogechat_hdata_get ("line"), line, "data");
        hdata = dogechat_hdata_get ("line_data");
        hashtable = dogechat_hashtable_new (8,
                                           DOGECHAT_HASHTABLE_STRING,
                                           DOGECHAT_HASHTABLE_STRING,
                                           NULL,
                                           NULL);
        if (hashtable)
        {
            snprintf (str_date, sizeof (str_date), "%ld", ((long int)dogechat_hdata_time (hdata, line_data, "date")) - 3600);
            dogechat_hashtable_set (hashtable, "date", str_date);
            dogechat_hdata_update (hdata, line_data, hashtable);
            dogechat_hashtable_free (hashtable);
        }
    }
}
...
----
# prototipo
count = dogechat.hdata_update(hdata, pointer, hashtable)

# example: subtract one hour on last message displayed in current buffer
own_lines = dogechat.hdata_pointer(dogechat.hdata_get("buffer"), dogechat.current_buffer(), "own_lines")
if own_lines:
    line = dogechat.hdata_pointer(dogechat.hdata_get("lines"), own_lines, "last_line")
    if line:
        line_data = dogechat.hdata_pointer(dogechat.hdata_get("line"), line, "data")
        hdata = dogechat.hdata_get("line_data")
        dogechat.hdata_update(hdata, line_data, {"date": str(dogechat.hdata_time(hdata, line_data, "date") - 3600)})
----
...
[source,C]
----
const char *dogechat_hdata_get_string (struct t_hdata *hdata, const char *property);
----
...
[source,C]
----
dogechat_printf (NULL, "variables in hdata: %s", dogechat_hdata_get_string (hdata, "var_keys"));
dogechat_printf (NULL, "lists in hdata: %s", dogechat_hdata_get_string (hdata, "list_keys"));
----
...
----
# prototipo
value = dogechat.hdata_get_string(hdata, property)

# esempio
dogechat.prnt("", "variables in hdata: %s" % dogechat.hdata_get_string(hdata, "var_keys"))
dogechat.prnt("", "lists in hdata: %s" % dogechat.hdata_get_string(hdata, "list_keys"))
----

[[upgrade]]
=== Aggiornamento

Funzioni per l'aggiornamento di DogeChat (comando "/upgrade").
...
[source,C]
----
struct t_upgrade_file *dogechat_upgrade_new (const char *filename, int write);
----
...
[source,C]
----
struct t_upgrade_file *upgrade_file = dogechat_upgrade_new ("my_file", 1);
----
...
----
# prototipo
upgrade_file = dogechat.upgrade_new(filename, write)

# esempio
upgrade_file = dogechat.upgrade_new("my_file", 1)
----
...
[source,C]
----
int dogechat_upgrade_write_object (struct t_upgrade_file *upgrade_file,
                                  int object_id,
...
[source,C]
----
if (dogechat_upgrade_write_object (upgrade_file, 1, &infolist))
{
    /* ok */
}
else
...
----
# prototipo
rc = dogechat.upgrade_write_object(upgrade_file, object_id, infolist)

# esempio
dogechat.upgrade_write_object(upgrade_file, 1, infolist)
----
...
[source,C]
----
int dogechat_upgrade_read (struct t_upgrade_file *upgrade_file,
                          int (*callback_read)(void *data,
...
dogechat_upgrade_read (upgrade_file, &my_upgrade_read_cb, NULL);
----
...
----
# prototipo
rc = dogechat.upgrade_read(upgrade_file, callback_read, callback_read_data)
...
dogechat.upgrade_read(upgrade_file, "my_upgrade_read_cb", ""))
----
...
[source,C]
----
void dogechat_upgrade_close (struct t_upgrade_file *upgrade_file);
----
...
----
# prototipo
dogechat.upgrade_close(upgrade_file)
