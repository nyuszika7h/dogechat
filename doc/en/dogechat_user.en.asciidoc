...
DogeChat homepage is here: https://dogechat.org/

[[prerequisites]]
=== Prerequisites

In order to install DogeChat, you need:

* a running GNU/Linux system (with compiler tools for source
  package), or a compatible OS
* 'root' privileges (to install DogeChat in a system directory)
...
Some additional packages may be useful, like dogechat-plugins.
...
DogeChat can be compiled with cmake or autotools (cmake is recommended way).
...
^(2)^ DogeChat can compile with libncurses5-dev, but it is *NOT* recommended (you
...
If you experienced crashes, or if you want to report any future DogeChat crash,
you have to:
...
If you installed a binary package, then install package 'dogechat-dbg'.
...
When DogeChat crashes, your system will create a file 'core' or 'core.12345'
('12345' is process id).
This file is created in directory where you have run DogeChat (this is *not*
directory where DogeChat is installed!).

For example if 'dogechat' is installed in '/usr/bin/' and 'core' file is
...
==== Debug a running DogeChat

To debug a running DogeChat (for example if DogeChat seems blocked), you can use
gdb with process id (replace '12345' with PID of dogechat process):
...
To start DogeChat, issue this command:

----
$ dogechat
----

When you run DogeChat for the first time, a default configuration file is
created, with default options. The default configuration file is:
'~/.dogechat/dogechat.conf'.

You can edit this file at your convenience to configure DogeChat or you can set
parameters with `/set` command in DogeChat (see
<<dogechat_commands,DogeChat commands>>).

[[screen_layout]]
=== Screen layout

Example of terminal with DogeChat:
...
* core buffer (created by DogeChat on startup, can not be closed)
...
Set notify level 'highlight' on channel "#dogechat" only:
...
DogeChat supports mouse clicks and gestures. It is working with local terminal,
...
[NOTE]
For any problem with mouse, please look at 'DogeChat FAQ'.

[[mouse_bind_events]]
==== Bind mouse events to commands

Many default mouse events are already defined by DogeChat (see
<<key_bindings_mouse_context,keys for "mouse" context>>).

You can change or add bindings with the command `/key` using context "mouse"
(for syntax, see command <<command_dogechat_key,/key>>).
...
DogeChat command line (at the bottom of window) lets you execute commands or
send text to buffer.
...
DogeChat can use up to 256 color pairs to display text in bars and chat area
(your terminal must support 256 colors to use them in DogeChat).

According to value of 'TERM' environment variable, you may have following limits
for colors in DogeChat:
...
You can run `dogechat --colors` or use command `/color` in DogeChat to
...
DogeChat dynamically allocates color pairs when colors are used on screen (to
...
greater than last terminal color, for example max color in DogeChat: 99999.
...
[[dogechat_options]]
==== DogeChat options (dogechat.conf)
...
Plugins found are automatically loaded when DogeChat is starting, and it is
possible to load or unload plugins while DogeChat is running.
...
| fifo       | FIFO pipe used to remotely send commands to DogeChat
...
'DogeChat Plugin API Reference' or 'DogeChat Scripting Guide'.

[[alias_plugin]]
=== Alias plugin

Alias plugin lets you create alias for commands (from DogeChat or other
plugins).
...
This plugin is optional, but recommended: if it's not loaded, DogeChat
can only read/write UTF-8 data.

Charset plugin should be autoloaded by DogeChat. To be sure plugin is loaded,
try:
...
If plugin is not found, then you should compile again DogeChat with
...
For any problem with charsets, please look at 'DogeChat FAQ'.
...
You can remote control DogeChat, by sending commands or text to a FIFO pipe (if
option "plugins.var.fifo.fifo" is enabled, it is by default).

The FIFO pipe is located in '~/.dogechat/' and is called 'dogechat_fifo_xxxx'
(where 'xxxx' is the process ID (PID) of running DogeChat).
...
* display text on IRC #dogechat channel:
...
You can write a script to send command to all running DogeChat at same time,
for example:
...
If the script is called "auto_dogechat_command", you can run it with:
...
Example to join '#dogechat' and '#toto' on server 'chat.freenode.net' server,
...
When connecting to IRC server with SSL, DogeChat checks by default that the
...
[NOTE]
The "gcrypt" library is required when compiling DogeChat in order to use
...
In DogeChat you need to add a socks5 proxy for TOR service (hostname/IP and
...
Following codes can be used in strings and are automatically expanded by DogeChat
...
| $compilation | DogeChat compilation date            | `Dec 16 2012`
| $osinfo      | Info about OS                       | `Linux 2.6.32-5-amd64 / x86_64`
| $site        | DogeChat site                        | `https://dogechat.org/`
| $download    | DogeChat site, download page         | `https://dogechat.org/download`
...
When option is not set (default), DogeChat chooses appropriate buffer, commonly
...
* display message "303" (ison) on DogeChat "core" buffer:
...
So if you set level 3 for an IRC channel, DogeChat will log all messages,
but not join/part/quit.

Some examples:

* set level 3 for IRC channel #dogechat:
...
For example, on buffer "irc.freenode.#dogechat", DogeChat will search a mask with
...
* 'irc': IRC proxy: used to share connections to IRC servers with one or many
  other IRC clients
* 'dogechat': protocol used by remote interfaces to display and interact with
  DogeChat (for example QDogeChat or dogechat-android)
...
The default file for certificate/key is '~/.dogechat/ssl/relay.pem'
...
If DogeChat is already running, you can reload the certificate and private key
with command:
...
The Relay plugin can send data to remote interface using DogeChat protocol.
...
DogeChat provides 7 scripting plugins: Python, Perl, Ruby, Lua, Tcl, Guile
...
For more information about how to write scripts, or DogeChat API for
scripts, please read 'DogeChat Scripting Guide'.
