...
This manual documents DogeChat plugins API, used by C plugins to interact with
DogeChat core.

[[plugins_in_dogechat]]
== Plugins in DogeChat

A plugin is a C program which can call DogeChat functions defined in an
interface.

This C program does not need DogeChat sources to compile and can be dynamically
loaded into DogeChat with command `/plugin`.
...
The plugin has to include "dogechat-plugin.h" file (available in DogeChat source
code).
This file defines structures and types used to communicate with DogeChat.
...
This function is called when plugin is loaded by DogeChat.
...
* 'plugin': pointer to DogeChat plugin structure
* 'argc': number of arguments for plugin (given on command line by user)
* 'argv': arguments for plugin

Return value:

* 'DOGECHAT_RC_OK' if successful (plugin will be loaded)
* 'DOGECHAT_RC_ERROR' if error (plugin will NOT be loaded)
...
This function is called when plugin is unloaded by DogeChat.
...
* 'DOGECHAT_RC_OK' if successful
* 'DOGECHAT_RC_ERROR' if error

[[compile_plugin]]
=== Compile plugin

Compile does not need DogeChat sources, only file 'dogechat-plugin.h' is
required.
...
'/usr/local/lib/dogechat/plugins') or into user's plugins directory (for example
'/home/xxx/.dogechat/plugins').
...
* 'plugin': pointer to DogeChat plugin structure (can be NULL)

Return value:

* name of plugin, "core" for DogeChat core (if plugin pointer is NULL)
...
Convert string to DogeChat internal charset (UTF-8).
...
Convert string from internal DogeChat charset (UTF-8) to another.
...
----
char str[] = "AbCdé";
dogechat_string_tolower (str);  /* str is now: "abcdé" */
----
...
----
char str[] = "AbCdé";
dogechat_string_toupper (str);  /* str is now: "ABCDé" */
----
...
[source,C]
----
const char *pos = dogechat_strcasestr ("aBcDeF", "de");  /* result: pointer to "DeF" */
----
...
[source,C]
----
char *str = dogechat_string_remove_quotes (string, " 'I can't' ", "'");
/* result: "I can't" */
/* ... */
free (str);
----
...
[source,C]
----
char *str = dogechat_string_strip (".abc -", 0, 1, "- .");  /* result: ".abc" */
/* ... */
free (str);
----
...
* array of strings, NULL if problem (must be freed by calling
  <<_string_free_split,dogechat_string_free_split>> after use)
...
* array of strings, NULL if problem (must be freed by calling
  <<_free_split_command,dogechat_free_split_command>> after use)
...
Remove DogeChat colors from a string.
...
* 'string': string
* 'replacement': if not NULL and not empty, DogeChat color codes are replaced by
  first char of this string, otherwise DogeChat color codes and following chars
...
[source,C]
----
int char_int = dogechat_utf8_char_int ("être");  /* "ê" as integer */
----
...
[source,C]
----
char *string = dogechat_utf8_strndup ("chêne", 3);  /* returns "chê" */
/* ... */
free (string);
----
...
Create a directory in DogeChat home.
...
* 'data': pointer given to callback when it is called by DogeChat
...
[source,C]
----
dogechat_util_timeval_add (&tv, 2000000);  /* add 2 seconds */
----
...
* 'dogelist': list pointer
* 'data': data to insert in list
* 'where': position in list:
** 'DOGECHAT_LIST_POS_SORT': add in list, keeping list sorted
** 'DOGECHAT_LIST_POS_BEGINNING': add to beginning of list
** 'DOGECHAT_LIST_POS_END': add to end of list
...
[source,C]
----
struct t_dogelist_item *item = dogechat_list_get (list, 0);  /* first item */
----
...
[source,C]
----
dogechat_list_set (item, "new data");
----
...
# example
dogechat.list_set(item, "new data")
----
...
[source,C]
----
dogechat_printf (NULL, "value of item: %s", dogechat_list_string (item));
----
...
# example
dogechat.prnt("", "value of item: %s" % dogechat.list_string(item))
----
...
[source,C]
----
dogechat_printf (NULL, "size of list: %d", dogechat_list_size (list));
----
...
# example
dogechat.prnt("", "size of list: %d" % dogechat.list_size(list))
----
...
function <<_config_write,dogechat_config_write>>.
You should call this function only after adding some sections (with
<<_config_new_section,dogechat_config_new_section>>) and options (with
...
*** 'DOGECHAT_CONFIG_READ_MEMORY_ERROR'
*** 'DOGECHAT_CONFIG_READ_FILE_NOT_FOUND'
* 'callback_read_data': pointer given to callback when it is called by DogeChat
...
* callback_write_data: pointer given to callback when it is called by DogeChat
...
* 'name': name of option; with DogeChat ≥ 1.4, the name can include a parent
...
* 'DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED' if option value has been reset
* 'DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE' if value was not changed
* 'DOGECHAT_CONFIG_OPTION_SET_ERROR' if an error occurred
...
* 'DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED' if option value has been changed
* 'DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE' if value was not changed
* 'DOGECHAT_CONFIG_OPTION_SET_ERROR' if an error occurred
...
* 'DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET' if option value has not been reset
* 'DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET' if option value has been reset
* 'DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED' if option has been removed
* 'DOGECHAT_CONFIG_OPTION_UNSET_ERROR' if an error occurred
...
* 'DOGECHAT_CONFIG_WRITE_OK' if configuration was written
* 'DOGECHAT_CONFIG_WRITE_MEMORY_ERROR' if there was not enough memory
* 'DOGECHAT_CONFIG_WRITE_ERROR' if another error occurred
...
* 'DOGECHAT_CONFIG_READ_OK' if configuration was loaded
* 'DOGECHAT_CONFIG_READ_MEMORY_ERROR' if there was not enough memory
* 'DOGECHAT_CONFIG_READ_FILE_NOT_FOUND' if file was not found
...
* 'DOGECHAT_CONFIG_READ_OK' if configuration was reloaded
* 'DOGECHAT_CONFIG_READ_MEMORY_ERROR' if there was not enough memory
* 'DOGECHAT_CONFIG_READ_FILE_NOT_FOUND' if file was not found
...
* 'option_name': option name, DogeChat will add prefix "plugins.var.xxx."
...
* 'DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED' if option value has been changed
* 'DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE' if value was not changed
* 'DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND' if option was not found
* 'DOGECHAT_CONFIG_OPTION_SET_ERROR' if other error occurred
...
* 'option_name': option name, DogeChat will add prefix "plugins.desc.xxx."
  (where "xxx" is current plugin name)
...
* 'context': context for keys (see <<_key_bind,dogechat_key_bind>>)
...
[source,C]
----
dogechat_printf (NULL, "%sThis is an error...", dogechat_prefix ("error"));
----
...
# example
dogechat.prnt("", "%sThis is an error..." % dogechat.prefix("error"))
----
...
*** 'resetcolor': reset color (keep attributes) _(DogeChat ≥ 0.3.6)_
...
* `|` : keep attributes: do not reset bold/reverse/italic/underlined when
  changing color _(DogeChat ≥ 0.3.6)_
...
* 'buffer': buffer pointer, if NULL, message is displayed on DogeChat buffer
...
[source,C]
----
dogechat_printf (NULL, "Hello on DogeChat buffer");
dogechat_printf (buffer, "Hello on this buffer");
dogechat_printf (buffer, "%sThis is an error!", dogechat_prefix ("error"));
dogechat_printf (buffer, " \tMessage without prefix but with \t some \t tabs");
...
# example
dogechat.prnt("", "Hello on DogeChat buffer")
dogechat.prnt(buffer, "Hello on this buffer")
dogechat.prnt(buffer, "%sThis is an error!" % dogechat.prefix("error"))
dogechat.prnt(buffer, " \tMessage without prefix but with \t some \t tabs")
...
[source,C]
----
dogechat_printf_date (NULL, time (NULL) - 120, "Hello, 2 minutes ago");
----
...
[source,C]
----
dogechat_printf_y (buffer, 2, "My message on third line");
----
...
# example
dogechat.prnt_y("", 2, "My message on third line")
----
...
Write a message in DogeChat log file (dogechat.log).
...
[source,C]
----
dogechat_log_printf ("My message in log file");
----
...
# example
dogechat.log_print("My message in log file")
----
...
struct t_hook *my_command_hook =
    dogechat_hook_command (/* command name */
                          "myfilter",
                          /* description */
...
Hook a command when DogeChat runs it.
...
[NOTE]
Callback can return 'DOGECHAT_RC_OK' or 'DOGECHAT_RC_OK_EAT' (command will not
be executed by DogeChat after callback).
...
my_command_run_cb (void *data, struct t_gui_buffer *buffer,
                   const char *command)
{
    dogechat_printf (NULL, "I'm eating the completion!");
...
def my_command_run_cb(data, buffer, command):
    dogechat.prnt("", "I'm eating the completion!")
...
DogeChat makes an automatic split of the command and its arguments (like the
shell does). +
...
* 'command': command to launch in child process or URL _(DogeChat ≥ 0.3.7)_
  (see below)
...
**** '3': not enough memory
**** '4': error with a file
*** '< 0': 'DOGECHAT_HOOK_PROCESS_RUNNING' (data available, but child still
    running) or 'DOGECHAT_HOOK_PROCESS_ERROR' (error when launching command)
...
When command has ended, or if timeout is reached, DogeChat will automatically
unhook (and kill process if it is still running).

The command can be an URL with format: "url:http://www.example.com", to download
content of URL _(DogeChat ≥ 0.3.7)_. Options are possible for URL with
function <<_hook_process_hashtable,dogechat_hook_process_hashtable>>.
...
Arguments are the same as function <<_hook_process,dogechat_hook_process>>,
...
if (options_cmd1)
{
    ...
    dogechat_hashtable_set (options_cmd1, "arg4", message);  /* untrusted argument */
...
*** 'DOGECHAT_HOOK_CONNECT_OK': connection OK
*** 'DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND': address not found
*** 'DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND': IP address not found
*** 'DOGECHAT_HOOK_CONNECT_CONNECTION_REFUSED': connection refused
*** 'DOGECHAT_HOOK_CONNECT_PROXY_ERROR': error with proxy
*** 'DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR': error with local hostname
*** 'DOGECHAT_HOOK_CONNECT_GNUTLS_INIT_ERROR': GnuTLS init error
*** 'DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR': GnuTLS handshake error
*** 'DOGECHAT_HOOK_CONNECT_MEMORY_ERROR': insufficient memory
*** 'DOGECHAT_HOOK_CONNECT_TIMEOUT': timeout
*** 'DOGECHAT_HOOK_CONNECT_SOCKET_ERROR': unable to create socket
...
*** 'DOGECHAT_HOOK_SIGNAL_STRING': string
*** 'DOGECHAT_HOOK_SIGNAL_INT': integer number
*** 'DOGECHAT_HOOK_SIGNAL_POINTER': pointer
...
. move new script to directory '~/.dogechat/xxx/' (where 'xxx' is language)
. create link to new script in directory '~/.dogechat/xxx/autoload/'
. load new script
...
** '2': queue with low priority (like messages automatically sent by DogeChat)
* semicolon
...
[source,C]
----
/* say "Hello!" on freenode server, #dogechat channel */
...
* 'option': option, format is full name, as used with command `/set` (for
  example: `dogechat.look.item_time_format`), wildcard "*" is allowed
  (priority allowed, see note about <<hook_priority,priority>>)
...
/* catch changes to option "dogechat.look.item_time_format" */
...
[NOTE]
Completion names are global (shared across DogeChat and plugins). So it is
...
** 'DOGECHAT_LIST_POS_SORT': any position, to keep list sorted
** 'DOGECHAT_LIST_POS_BEGINNING': beginning of list
** 'DOGECHAT_LIST_POS_END': end of list

C example: see <<_hook_completion,dogechat_hook_completion>>.
...
* 'modifier': modifier name, list of modifiers used by Dogechat or plugins
  (priority allowed, see note about <<hook_priority,priority>>)
  (see table below)
...
^(4)^ It is date when DogeChat adds line in buffer (greater or equal to
"_chat_line_date").
...
* 'hook': something hooked with "dogechat_hook_xxx()"
...
{
    dogechat_printf (buffer, "Text: %s", input_data);
    return DOGECHAT_RC_OK;
}
...
{
    dogechat_printf (NULL, "Buffer '%s' will be closed!",
...
def my_input_cb(data, buffer, input_data):
    dogechat.prnt(buffer, "Text: %s" % input_data)
    return dogechat.DOGECHAT_RC_OK

def my_close_cb(data, buffer):
    dogechat.prnt("", "Buffer '%s' will be closed!" % dogechat.buffer_get_string(buffer, "name"))
    return dogechat.DOGECHAT_RC_OK
...
[source,C]
----
dogechat_printf (dogechat_current_buffer (), "Text on current buffer");
----
...
# example
dogechat.prnt(dogechat.current_buffer(), "Text on current buffer")
----
...
Search DogeChat main buffer ('core' buffer, first buffer displayed when DogeChat
is starting).
...
* pointer to DogeChat main buffer ('core' buffer)
...
with same number, and DogeChat will display lines from both buffers (mixed
lines).
...
* 'buffer': buffer pointer
* 'property': property name:
** 'plugin': name of plugin which created this buffer ("core" for DogeChat main
   buffer)
** 'name': name of buffer
** 'full_name': full name of buffer ("plugin.name") _(DogeChat ≥ 0.3.7)_
...
* 'buffer': buffer pointer
* 'property': property name:
** 'plugin': pointer to plugin which created this buffer (NULL for DogeChat main
   buffer)
...
dogechat_buffer_set (my_buffer, "localvar_del_toto", NULL);
----
...
[source,C]
----
dogechat_window_set_title ("new title here");
----
...
# example
dogechat.window_set_title("new title here")
----
...
** DogeChat option name, for example 'dogechat.color.nicklist_group'
...
* 'name': nick name
* 'color': color option name:
*** DogeChat option name (from dogechat.color.xxx), for example 'chat_delimiters'
*** color with optional background, for example 'yellow' or 'yellow,red'
*** bar color name:
**** 'bar_fg': foreground color for bar
**** 'bar_delim': delimiters color for bar
...
| color | DogeChat color option name |
...
| prefix_color | DogeChat color option name |
...
* 'property': name, hidden, priority, conditions, position, filling_top_bottom,
  ...
  separator, items (see <<_bar_new,dogechat_bar_new>>)
* 'value': new value for property
...
Functions for executing DogeChat commands.
...
Return info, as string, from DogeChat or a plugin.
...
Return info, as hashtable, from DogeChat or a plugin.
...
Return infolist from DogeChat or a plugin.

[IMPORTANT]
Content of infolist is a duplication of actual data. So if you are asking
infolist with lot of data (like "buffer_lines"), DogeChat will allocate memory
...
Functions for hdata (raw access to DogeChat or plugins data).
...
.hdata vs infolist
====
Hdata is a fast way to read DogeChat or plugins data. It is similar to
...
_DogeChat ≥ 0.3.6, updated in 0.3.9._
...
The macro "DOGECHAT_HDATA_VAR" can be used to shorten code:
...
The macro "DOGECHAT_HDATA_LIST" can be used to shorten code:
...
Return hdata for a DogeChat or plugin structure.
...
DogeChat/plugin object to read some data.
...
# example
dogechat.prnt("", "type = %s" % dogechat.hdata_get_var_type_string("name"))
----
...
* 'hdata': hdata pointer
* 'list': list pointer; if NULL _(DogeChat ≥ 1.0)_, the pointer is checked with
...
* 'hdata': hdata pointer
* 'pointer': pointer to a DogeChat/plugin object
...
Functions for upgrading DogeChat (command "/upgrade").
