...
Ce manuel documente l'API DogeChat des extensions, utilisée par les extensions
en C pour interagir avec le cœur de DogeChat.

[[plugins_in_dogechat]]
== Extensions dans DogeChat

Une extension est un programme C qui peut appeler des fonctions DogeChat
définies dans une interface.

Ce programme C n'a pas besoin des sources DogeChat pour être compilé et peut
être dynamiquement chargé dans DogeChat avec la commande `/plugin`.
...
L'extension doit inclure le fichier "dogechat-plugin.h" (disponible dans le code
source DogeChat).
...
DOGECHAT_PLUGIN_NAME("name")::
    nom de l'extension

DOGECHAT_PLUGIN_DESCRIPTION("description")::
...
Cette fonction est appelée quand l'extension est chargée par DogeChat.
...
* 'plugin' : pointeur vers la structure d'extension DogeChat
...
* 'DOGECHAT_RC_OK' si ok (l'extension sera chargée)
* 'DOGECHAT_RC_ERROR' si erreur (l'extension ne sera PAS chargée)

[[plugin_priority]]
===== Priorité de l'extension

Lorsque les extensions sont automatiquement chargées (par exemple au
démarrage), DogeChat charge d'abord toutes les extensions, puis appelle les
...
Les extensions par défaut de DogeChat sont initialisées dans cet ordre :

include::autogen/plugin_api/plugins_priority.asciidoc[]

==== dogechat_plugin_end

Cette fonction est appelée quand l'extension est déchargée par DogeChat.
...
* 'DOGECHAT_RC_OK' si ok
* 'DOGECHAT_RC_ERROR' si erreur

[[compile_plugin]]
=== Compilation de l'extension

La compilation ne nécessite pas les sources de DogeChat, seul le fichier
 'dogechat-plugin.h' est requis.
...
exemple '/usr/local/lib/dogechat/plugins') ou dans le répertoire utilisateur des
extensions (par exemple '/home/xxx/.dogechat/plugins').
...
DOGECHAT_PLUGIN_DESCRIPTION("Extension de test pour DogeChat");
...
* 'plugin' : pointeur vers la structure d'extension DogeChat (peut être NULL)

Valeur de retour :

* nom de l'extension, "core" pour le cœur de DogeChat (si le pointeur vers
...
[source,C]
----
char *str = dogechat_iconv_to_internal ("iso-8859-1", "chaîne iso : é à");
/* ... */
free (str);
----
...
# exemple
str = dogechat.iconv_to_internal("iso-8859-1", "chaîne iso : é à")
----
...
[source,C]
----
char *str = dogechat_iconv_from_internal ("iso-8859-1", "chaîne utf-8 : é à");
/* ... */
free (str);
----
...
# exemple
str = dogechat.iconv_from_internal("iso-8859-1", "chaîne utf-8 : é à")
----
...
[source,C]
----
char *str = dogechat_strndup ("abcdef", 3);  /* résultat : "abc" */
/* ... */
free (str);
----
...
----
char str[] = "AbCdé";
dogechat_string_tolower (str);  /* str vaut maintenant : "abcdé" */
----
...
----
char str[] = "AbCdé";
dogechat_string_toupper (str);  /* str vaut maintenant : "ABCDé" */
----
...
_DogeChat ≥ 0.3.7, mis à jour dans la 1.0._
...
* pointeur vers la chaîne trouvée, ou NULL si non trouvée
  (_DogeChat ≥ 1.3_ : le pointeur retourné est un 'const char *' au lieu d'un
  'char *')
...
[source,C]
----
const char *pos = dogechat_strcasestr ("aBcDeF", "de");  /* résultat : pointeur vers "DeF" */
----
...
la fonction <<_utf8_strlen_screen,dogechat_utf8_strlen_screen>>).
...
[source,C]
----
char *str = dogechat_string_expand_home ("~/fichier.txt");
...
. remplacer le `%h` du début par le répertoire maison de DogeChat,
...
. évaluer les variables
  (voir <<_string_eval_expression,dogechat_string_eval_expression>>).
...
[source,C]
----
char *str = dogechat_string_remove_quotes (string, " 'aujourd'hui' ", "'");
...
[source,C]
----
char *str = dogechat_string_strip (".abc -", 0, 1, "- .");  /* résultat : ".abc" */
/* ... */
free (str);
----
...
[source,C]
----
char *str = dogechat_string_replace ("test, test", "s", "x");  /* résultat : "text" */
/* ... */
free (str);
----
...
* 'regex' : pointeur vers une expression régulière (structure 'regex_t')
  compilée avec la fonction DogeChat
  <<_string_regcomp,dogechat_string_regcomp>> ou regcomp (voir
  `man regcomp`)
...
* tableau de chaînes, NULL en cas de problème (doit être supprimé par un appel à
  <<_string_free_split,dogechat_string_free_split>> après utilisation)
...
* tableau de chaînes, NULL en cas de problème (doit être supprimé par un appel à
  <<_free_split_command,dogechat_free_split_command>> après utilisation)

Exemple en C :

[source,C]
----
char **argv = dogechat_string_split_command ("/commande1 arg;/commande2", ';');
...
char *str = dogechat_string_format_size (0);  /* str == "0 octet" */
/* ... */
free (str);

char *str = dogechat_string_format_size (1);  /* str == "1 octet" */
/* ... */
free (str);

char *str = dogechat_string_format_size (200);  /* str == "200 octets" */
/* ... */
free (str);

char *str = dogechat_string_format_size (15200);  /* str == "15.2 Ko" */
/* ... */
free (str);

char *str = dogechat_string_format_size (2097152);  /* str == "2.10 Mo" */
/* ... */
free (str);
----
...
Supprimer les couleurs DogeChat dans une chaîne.
...
* 'string' : chaîne
* 'replacement' : si non NULL et non vide, les couleurs DogeChat sont remplacées
  par le premier caractère de cette chaîne, sinon les codes couleurs DogeChat et
...
char *str = dogechat_string_remove_color (ma_chaine1, NULL);
/* ... */
free (str);

/* remplace les codes couleur par "?" */
char *str = dogechat_string_remove_color (ma_chaine2, "?");
/* ... */
free (str);
----
...
# exemple
str = dogechat.string_remove_color(ma_chaine, "?")
----
...
_DogeChat ≥ 0.4.0, mis à jour dans la 0.4.2, 1.0, 1.1, 1.2 et 1.3._
...
| `${color:nom}` |
  Code couleur DogeChat (le nom de couleur a des attributs facultatifs),
  voir la fonction <<_color,dogechat_color>> pour les formats supportés |
...
| `${info:nom}` +
  `${info:nom,paramètres}` |
  Info de DogeChat ou d'une extension, voir la fonction
...
* 'length' : nombre maximum de caractères UTF-8 à vérifier ; si ≤ 0, la chaîne
  complète est vérifiée _(DogeChat ≥ 1.4)_
...
[source,C]
----
int char_int = dogechat_utf8_char_int ("être");  /* "ê" comme entier */
----
...
const char *str2 = dogechat_utf8_add_offset (str, 3);  /* pointe vers "ne" */
----
...
[source,C]
----
char *string = dogechat_utf8_strndup ("chêne", 3);  /* retourne "chê" */
/* ... */
free (str);
----
...
Créer un répertoire dans le répertoire de DogeChat.
...
* 'data' : pointeur donné au "callback" lorsqu'il est appelé par DogeChat
...
contenu = dogechat_file_get_content ("/tmp/test.txt");
/* ... */
free (contenu);
----
...
[NOTE]
Avec DogeChat ≤ 1.0, la valeur retournée était en millisecondes.
...
[NOTE]
Avec DogeChat ≤ 1.0, l'intervalle était exprimé en millisecondes.

Exemple en C :

[source,C]
----
dogechat_util_timeval_add (&tv, 2000000);  /* ajouter 2 secondes */
----
...
_DogeChat ≥ 0.3.2, mise à jour dans la 1.3._

Retourner la date/heure sous forme de chaîne construite avec "strftime" et le
format défini dans l'option 'dogechat.look.time_format'.
...
Convertir une chaîne avec la version DogeChat en nombre.
...
* 'version' : version DogeChat sous forme de chaîne (exemple : "0.3.9" ou
  "0.3.9-dev")
...
* 'dogelist' : pointeur vers la liste
* 'data' : donnée à insérer dans la liste
* 'where' : position dans la liste :
** 'DOGECHAT_LIST_POS_SORT' : ajout dans la liste, en gardant la liste triée
** 'DOGECHAT_LIST_POS_BEGINNING' : ajout en début de liste
** 'DOGECHAT_LIST_POS_END' : ajout en fin de liste
...
struct t_dogelist_item *my_item =
    dogechat_list_add (list, "ma donnée", DOGECHAT_LIST_POS_SORT, NULL);
----
...
# exemple
item = dogechat.list_add(list, "ma donnée", dogechat.DOGECHAT_LIST_POS_SORT, "")
----
...
[source,C]
----
struct t_dogelist_item *item = dogechat_list_search (list, "ma donnée");
----
...
# exemple
item = dogechat.list_search(list, "ma donnée")
----
...
[source,C]
----
int pos_item = dogechat_list_search_pos (list, "ma donnée");
----
...
# exemple
pos_item = dogechat.list_search_pos(list, "ma donnée")
----
...
[source,C]
----
struct t_dogelist_item *item = dogechat_list_casesearch (list, "ma donnée");
----
...
# exemple
item = dogechat.list_casesearch(list, "ma donnée")
----
...
[source,C]
----
int pos_item = dogechat_list_casesearch_pos (list, "ma donnée");
----
...
# exemple
pos_item = dogechat.list_casesearch_pos(list, "ma donnée")
----
...
[source,C]
----
struct t_dogelist_item *item = dogechat_list_get (list, 0);  /* premier élément */
----
...
[source,C]
----
dogechat_list_set (item, "nouvelle donnée");
----
...
# exemple
dogechat.list_set(item, "nouvelle donnée")
----
...
[source,C]
----
dogechat_printf (NULL, "valeur de l'item : %s", dogechat_list_string (item));
----
...
# exemple
dogechat.prnt("", "valeur de l'item : %s" % dogechat.list_string(item))
----
...
[source,C]
----
dogechat_printf (NULL, "taille de la liste : %d", dogechat_list_size (list));
----
...
# exemple
dogechat.prnt("", "taille de la liste : %d" % dogechat.list_size(list))
----
...
_DogeChat ≥ 0.3.3, mis à jour dans la 0.4.2._
...
[source,C]
----
dogechat_hashtable_set_with_size (hashtable, "ma_cle", 0,
...
[source,C]
----
dogechat_hashtable_set (hashtable, "ma_cle", "ma_valeur");
----
...
[source,C]
----
void *value = dogechat_hashtable_get (hashtable, "ma_cle");
----
...
[source,C]
----
if (dogechat_hashtable_has_key (hashtable, "ma_cle"))
{
        ...
        const void *key, const void *value)
{
    ...
    dogechat_printf (NULL, "clé: '%s', valeur: '%s'",
                    (const char *)key,
                    (const char *)value);
}
/* ... */
...
[source,C]
----
dogechat_printf (NULL, "les clés sont de type: %s",
                dogechat_hashtable_get_string (hashtable, "type_keys"));
dogechat_printf (NULL, "liste des clés: %s",
...
** 'callback_free_key' : définit la fonction "callback" pour supprimer les clés
   de la table de hachage _(DogeChat ≥ 0.4.2)_
...
[source,C]
----
dogechat_hashtable_remove (hashtable, "ma_cle");
----
...
* 'callback_reload_data' : pointeur donné au "callback" de rechargement
  lorsqu'il est appelé par DogeChat
...
l'appel à la fonction <<_config_write,dogechat_config_write>>.
Vous ne devriez appeler cette fonction qu'après avoir créé les sections (avec
<<_config_new_section,dogechat_config_new_section>>) et les options (avec
<<_config_new_option,dogechat_config_new_option>>).
...
* 'name' : nom de l'option ; avec DogeChat ≥ 1.4, le nom peut inclure le nom
...
paramètres par fonction), voir le 'Guide pour Scripts DogeChat' pour plus d'infos
...
dogechat_config_search_with_string ("fichier.section.option",
                                   &ptr_config_file,
                                   &ptr_section,
                                   &ptr_option,
                                   &option_name);
...
* 'DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED' si la valeur de l'option a été
  réinitialisée
* 'DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE' si la valeur n'a pas changé
* 'DOGECHAT_CONFIG_OPTION_SET_ERROR' en cas d'erreur
...
* 'DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED' si la valeur de l'option a été changée
* 'DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE' si la valeur n'a pas changé
* 'DOGECHAT_CONFIG_OPTION_SET_ERROR' en cas d'erreur
...
[source,C]
----
switch (dogechat_config_option_set (option, "nouvelle_valeur", 1))
{
    case DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
        /* .... */
        break;
...
# exemple
rc = dogechat.config_option_set(option, "nouvelle_valeur", 1)
if rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
...
l'option (voir <<_config_new_option,dogechat_config_new_option>>).
...
* 'DOGECHAT_CONFIG_OPTION_UNSET_OK_NO_RESET' si la valeur de l'option n'a pas
  été réinitialisée
* 'DOGECHAT_CONFIG_OPTION_UNSET_OK_RESET' si la valeur de l'option a été
  réinitialisée
* 'DOGECHAT_CONFIG_OPTION_UNSET_OK_REMOVED' si l'option a été supprimée
* 'DOGECHAT_CONFIG_OPTION_UNSET_ERROR' en cas d'erreur
...
[source,C]
----
dogechat_config_option_rename (option, "nouveau_nom");
----
...
# exemple
dogechat.config_option_rename(option, "nouveau_nom")
----
...
** 'parent_name' : nom de l'option parente ('char *') _(DogeChat ≥ 1.4)_
...
----
# prototype
is_null = dogechat.config_option_is_null(option)
...
----
# prototype
is_null = dogechat.config_option_default_is_null(option)
...
my_section_write_cb (void *data, struct t_config_file *config_file,
                     const char *section_name)
{
    dogechat_config_write_line (config_file, "ma_section", NULL);
...
# exemple
def my_section_write_cb(data, config_file, section_name):
    dogechat.config_write_line(config_file, "ma_section", "")
    dogechat.config_write_option(config_file, option)
    return dogechat.DOGECHAT_RC_OK
----
...
def my_section_write_cb(data, config_file, section_name):
    dogechat.config_write_line(config_file, "ma_section", "")
    dogechat.config_write_line(config_file, "option", "valeur")
    return dogechat.DOGECHAT_RC_OK
----
...
* 'DOGECHAT_CONFIG_WRITE_OK' si la configuration a été écrite
* 'DOGECHAT_CONFIG_WRITE_MEMORY_ERROR' s'il n'y a pas eu suffisamment de mémoire
* 'DOGECHAT_CONFIG_WRITE_ERROR' si une autre erreur s'est produite
...
* 'DOGECHAT_CONFIG_READ_OK' si la configuration a été chargée
* 'DOGECHAT_CONFIG_READ_MEMORY_ERROR' s'il n'y a pas eu suffisamment de mémoire
* 'DOGECHAT_CONFIG_READ_FILE_NOT_FOUND' si le fichier n'a pas été trouvé
...
* 'DOGECHAT_CONFIG_READ_OK' si la configuration a été rechargée
* 'DOGECHAT_CONFIG_READ_MEMORY_ERROR' s'il n'y a pas eu suffisamment de mémoire
* 'DOGECHAT_CONFIG_READ_FILE_NOT_FOUND' si le fichier n'a pas été trouvé
...
* 'option_name' : nom de l'option, DogeChat ajoutera le préfixe
...
* 'DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED' si la valeur de l'option a été changée
* 'DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE' si la valeur n'a pas changé
* 'DOGECHAT_CONFIG_OPTION_SET_OPTION_NOT_FOUND' si l'option n'a pas été trouvée
* 'DOGECHAT_CONFIG_OPTION_SET_ERROR' en cas d'erreur
...
[source,C]
----
switch (dogechat_config_set_plugin ("option", "valeur_test"))
{
    case DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
        /* ... */
        break;
...
# exemple
rc = dogechat.config_set_plugin("option", "valeur_test")
if rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_CHANGED:
    # ...
elif rc == dogechat.DOGECHAT_CONFIG_OPTION_SET_OK_SAME_VALUE:
    # ...
...
[source,C]
----
dogechat_config_set_desc_plugin ("option", "description de l'option");
----
...
if int(version) >= 0x00030500:
    dogechat.config_set_desc_plugin("option", "description de l'option")
----
...
<<_key_unbind,dogechat_key_unbind>>.
...
[source,C]
----
dogechat_printf (NULL, "%sCeci est une erreur...", dogechat_prefix ("error"));
----
...
# exemple
dogechat.prnt("", "%sCeci est une erreur..." % dogechat.prefix("error"))
----
...
** le nom d'une option de couleur DogeChat (de dogechat.color.xxx), par exemple
  'chat_delimiters'
...
*** 'emphasis' : activer/désactiver la mise en valeur du texte (note : cela ne
    devrait être utilisé que dans les barres, car DogeChat utilise la mise en
...
[source,C]
----
dogechat_printf (NULL, "Couleur : %sbleu %scouleur par défaut %sjaune sur rouge",
                dogechat_color ("blue"),
                dogechat_color ("chat"),
...
# exemple
dogechat.prnt("", "Couleur : %sbleu %scouleur par défaut %sjaune sur rouge"
...
[source,C]
----
dogechat_printf (NULL, "Bonjour sur le tampon DogeChat");
dogechat_printf (buffer, "Bonjour sur ce tampon");
dogechat_printf (buffer, "%sCeci est une erreur !", dogechat_prefix ("error"));
dogechat_printf (buffer, " \tMessage sans préfixe mais avec \t quelques \t tabulations");
dogechat_printf (buffer, "\t\tMessage sans heure/alignement");
dogechat_printf (buffer, "\t\t");  /* ligne vide (sans heure) */
----
...
# exemple
dogechat.prnt("", "Bonjour sur le tampon DogeChat")
dogechat.prnt(buffer, "Bonjour sur ce tampon")
dogechat.prnt(buffer, "%sCeci est une erreur !" % dogechat.prefix("error"))
dogechat.prnt(buffer, " \tMessage sans préfixe mais avec \t quelques \t tabulations")
dogechat.prnt(buffer, "\t\tMessage sans heure/alignement")
dogechat.prnt(buffer, "\t\t")  # ligne vide (sans heure)
----
...
[source,C]
----
dogechat_printf_date (NULL, time (NULL) - 120, "Bonjour, il y a 2 minutes");
----
...
[source,C]
----
dogechat_printf_y (buffer, 2, "Mon message sur la 3ème ligne");
----
...
# exemple
dogechat.prnt_y("", 2, "Mon message sur la 3ème ligne")
----
...
Écrire un message dans le fichier de log DogeChat (dogechat.log).
...
[source,C]
----
dogechat_log_printf ("Mon message dans le fichier log");
----
...
# exemple
dogechat.log_print("Mon message dans le fichier log")
----
...
struct t_hook *my_command_hook =
    dogechat_hook_command (/* nom de la commande */
                          "monfiltre",
                          /* description */
...
hook = dogechat.hook_command("monfiltre", "description de monfiltre",
...
Intercepter une commande lorsqu'elle est exécutée par DogeChat.
...
[NOTE]
Le "callback" peut retourner 'DOGECHAT_RC_OK' ou 'DOGECHAT_RC_OK_EAT' (la
commande ne sera pas exécutée par DogeChat après le "callback").
...
{
    dogechat_printf (NULL, "Je mange la complétion !");
...
def my_command_run_cb(data, buffer, command):
    dogechat.prnt("", "Je mange la complétion !")
...
* 'flag_exception' : 1 = intercepter un évènement d'exception, 0 = ignorer
  (_DogeChat ≥ 1.3_ : ce paramètre est ignoré et n'est plus utilisé)
...
commande. DogeChat effectue un découpage automatique de la commande et de ses
...
fonction <<_hook_process_hashtable,dogechat_hook_process_hashtable>> avec
les paramètres dans la table de hachage 'options' _(DogeChat ≥ 0.4.0)_.
...
* 'command' : commande à lancer dans le processus fils ou URL
  _(DogeChat ≥ 0.3.7)_ (voir ci-dessous)
...
*** '< 0' : 'DOGECHAT_HOOK_PROCESS_RUNNING' (données disponibles, mais le
    fils tourne toujours) ou 'DOGECHAT_HOOK_PROCESS_ERROR' (erreur en lançant
    la commande)
...
Lorsque la commande est terminée, ou si le timeout est atteint, DogeChat
               ...
               const char *out, const char *err)
{
    ...
    {
        dogechat_printf (NULL, "Erreur avec la commande '%s'", command);
        return DOGECHAT_RC_OK;
    }
...
def my_process_cb(data, command, return_code, out, err):
    if return_code == dogechat.DOGECHAT_HOOK_PROCESS_ERROR:
        dogechat.prnt("", "Erreur avec la commande '%s'" % command)
        return dogechat.DOGECHAT_RC_OK
    if return_code >= 0:
...
<<_hook_process,dogechat_hook_process>>, avec un paramètre
supplémentaire :
...
| stdin +
  ...
  (voir la fonction <<_hook_set,dogechat_hook_set>>)
...
if (options_cmd1)
{
    ...
    dogechat_hashtable_set (options_cmd1, "arg4", message);  /* paramètre non sûr */
...
*** 'DOGECHAT_HOOK_CONNECT_OK' : connexion ok
*** 'DOGECHAT_HOOK_CONNECT_ADDRESS_NOT_FOUND' : adresse non trouvée
*** 'DOGECHAT_HOOK_CONNECT_IP_ADDRESS_NOT_FOUND' : adresse IP non trouvée
*** 'DOGECHAT_HOOK_CONNECT_CONNECTION_REFUSED' : connexion refusée
*** 'DOGECHAT_HOOK_CONNECT_PROXY_ERROR' : erreur avec le proxy
*** 'DOGECHAT_HOOK_CONNECT_LOCAL_HOSTNAME_ERROR' : erreur avec le nom local
*** 'DOGECHAT_HOOK_CONNECT_GNUTLS_INIT_ERROR' : erreur d'initialisation GnuTLS
*** 'DOGECHAT_HOOK_CONNECT_GNUTLS_HANDSHAKE_ERROR' : erreur avec la
    "poignée de main" GnuTLS
*** 'DOGECHAT_HOOK_CONNECT_MEMORY_ERROR' : mémoire insuffisante
*** 'DOGECHAT_HOOK_CONNECT_TIMEOUT' : temps maximum dépassé
*** 'DOGECHAT_HOOK_CONNECT_SOCKET_ERROR' : erreur de création socket
...
** avec DogeChat ≥ 0.4.3 : liste d'étiquettes (séparées par des virgules) qui
...
** avec DogeChat ≤ 0.4.2 : liste d'étiquettes (séparées par des virgules) qui
...
[IMPORTANT]
Dans les scripts, avec DogeChat ≥ 1.0, les paramètres du callback 'displayed' et
'highlight' sont des entiers (avec DogeChat ≤ 0.4.3, ils étaient des chaînes). +
...
*** 'DOGECHAT_HOOK_SIGNAL_STRING' : chaîne de caractères
*** 'DOGECHAT_HOOK_SIGNAL_INT' : nombre entier
*** 'DOGECHAT_HOOK_SIGNAL_POINTER' : pointeur
** 'void *signal_data' : données envoyées avec le signal
** valeur de retour :
*** 'DOGECHAT_RC_OK'
*** 'DOGECHAT_RC_OK_EAT' (arrêter l'envoi du signal immédiatement)
    _(DogeChat ≥ 0.4.0)_
*** 'DOGECHAT_RC_ERROR'
* 'callback_data' : pointeur donné au "callback" lorsqu'il est appelé par
  DogeChat
...
Liste des signaux envoyés par DogeChat et les extensions :
...
| dogechat | signal_sigterm +
  _(DogeChat ≥ 1.2)_ |
  - |
  Signal SIGTERM reçu (arrêt propre du processus DogeChat)
...
Valeur de retour _(DogeChat ≥ 1.0)_ :

* code retour du dernier "callback" exécuté ('DOGECHAT_RC_OK' si aucun "callback"
  n'a été exécuté) :
** 'DOGECHAT_RC_OK'
** 'DOGECHAT_RC_OK_EAT'
** 'DOGECHAT_RC_ERROR'
...
[source,C]
----
int rc = dogechat_hook_signal_send ("mon_signal", DOGECHAT_HOOK_SIGNAL_STRING, ma_chaine);
----
...
# exemple
rc = dogechat.hook_signal_send("mon_signal", dogechat.DOGECHAT_HOOK_SIGNAL_STRING, ma_chaine)
----
...
. déplacement du nouveau script vers le répertoire '~/.dogechat/xxx/' (où 'xxx'
  est le langage)
...
[source,C]
----
/* dis "Bonjour !" sur le serveur freenode, canal #dogechat */
dogechat_hook_signal_send ("irc_input_send", DOGECHAT_HOOK_SIGNAL_STRING,
                          "freenode;#dogechat;1;;Bonjour !");
...
Liste des hsignaux envoyés par DogeChat et les extensions :
...
_DogeChat ≥ 0.3.4, mis à jour dans la 1.0._
...
if (hashtable)
{
    dogechat_hashtable_set (hashtable, "clé", "valeur");
...
# exemple
rc = dogechat.hook_hsignal_send("my_hsignal", {"clé": "valeur"})
----
...
{
    dogechat_printf (NULL, "erreur = %s", dogechat_hashtable_get (hashtable, "error"));
    dogechat_printf (NULL, "sortie = %s", dogechat_hashtable_get (hashtable, "output"));
    return DOGECHAT_RC_OK;
}
...
def test_whois_cb(data, signal, hashtable):
    dogechat.prnt("", "erreur = %s" % hashtable["error"])
    dogechat.prnt("", "sortie = %s" % hashtable["output"])
    return dogechat.DOGECHAT_RC_OK
...
* 'option' : option, le format est le nom complet, celui utilisé avec la
  commande `/set` (par exemple : `dogechat.look.item_time_format`), le caractère
...
/* intercepter les changements de l'option "dogechat.look.item_time_format" */
...
[NOTE]
Les noms de complétion sont globaux (partagés entre DogeChat et les extensions).
...
utiliser la fonction <<_hook_command_run,dogechat_hook_command_run>>
avec la commande : "/input complete_next" (et vous devez retourner
'DOGECHAT_RC_OK_EAT' si votre "callback" a mis à jour la ligne de commande, de
sorte que DogeChat n'exécute pas la complétion).
...
my_completion_cb (void *data, const char *completion_item,
                  ...
                  struct t_gui_completion *completion)
{
    dogechat_hook_completion_list_add (completion, "mot1",
                                      0, DOGECHAT_LIST_POS_SORT);
    dogechat_hook_completion_list_add (completion, "test_mot2",
                                      0, DOGECHAT_LIST_POS_SORT);
    return DOGECHAT_RC_OK;
}

struct t_hook *my_completion_hook = dogechat_hook_completion ("extension_item",
                                                             "ma complétion !",
...
def my_completion_cb(data, completion_item, buffer, completion):
    dogechat.hook_completion_list_add(completion, "mot1", 0, dogechat.DOGECHAT_LIST_POS_SORT)
    dogechat.hook_completion_list_add(completion, "test_mot2", 0, dogechat.DOGECHAT_LIST_POS_SORT)
    return dogechat.DOGECHAT_RC_OK

hook = dogechat.hook_completion("extension_item", "ma complétion !",
                               "my_completion_cb", "")
----
...
** 'DOGECHAT_LIST_POS_SORT' : n'importe où, pour maintenir la liste triée
** 'DOGECHAT_LIST_POS_BEGINNING' : au début de la liste
** 'DOGECHAT_LIST_POS_END' : à la fin de la liste

Exemple en C : voir <<_hook_completion,dogechat_hook_completion>>.
...
* 'modifier' : nom du modificateur, liste des modificateurs utilisés par DogeChat
  ou des extensions
  (priorité autorisée, voir la note sur la <<hook_priority,priorité>>)
  (voir le tableau ci-dessous)
...
Liste des modificateurs utilisés par DogeChat et les extensions :
...
| irc_color_decode |
  ...
  Toute chaîne |
  Chaîne avec les couleurs IRC converties en couleurs DogeChat (ou avec les
...
| color_decode_ansi +
  ...
  "1" pour garder les couleurs, "0" pour les supprimer |
  Toute chaîne |
  Chaîne avec les couleurs ANSI converties en couleurs DogeChat (ou avec les
...
[source,C]
----
char *new_string = dogechat_hook_modifier_exec ("mon_modifier",
...
# exemple
dogechat.hook_modifier_exec("mon_modifier", mes_donnees, ma_chaine)
----
...
struct t_hook *my_info_hook = dogechat_hook_info ("mon_info",
                                                 "Une information",
...
----
# prototype
hook = dogechat.hook_info(info_name, description, args_description,
                         callback, callback_data)
...
hook = dogechat.hook_info("mon_info", "Une information", "Info sur les paramètres",
                         "my_info_cb", "")
----
...
struct t_hook *my_info_hook = dogechat_hook_info_hashtable ("mon_info_hashtable",
                                                           "Une information",
...
hook = dogechat.hook_info_hashtable("mon_info_hashtable", "Une information",
                                   "Info sur la table de hachage en entrée",
                                   "Info sur la table de hachage en sortie",
...
struct t_hook *my_infolist = dogechat_hook_infolist ("mon_infolist",
                                                    "Mon infolist",
...
hook = dogechat.hook_infolist("mon_infolist", "Mon infolist",
...
struct t_hook *my_hdata = dogechat_hook_hdata ("mon_hdata",
...
^(4)^ Il s'agit de la date lorsque DogeChat ajoute la ligne dans le tampon
...
* 'hook' : quelque chose d'accroché avec "dogechat_hook_xxx()"
...
hook = dogechat.hook_process_hashtable("/chemin/vers/commande", {"stdin": "1"},
                                      20000, "my_process_cb", "")
dogechat.hook_set(hook, "stdin", "données envoyées sur le stdin du processus fils")
dogechat.hook_set(hook, "stdin_close", "")  # facultatif
----
...
* 'hook' : quelque chose accroché avec "dogechat_hook_xxx()"
...
{
    dogechat_printf (buffer, "Texte : %s", input_data);
    return DOGECHAT_RC_OK;
}
...
{
    dogechat_printf (NULL, "Le tampon '%s' va être fermé !",
                    dogechat_buffer_get_string (buffer, "name"));
    return DOGECHAT_RC_OK;
}

struct t_gui_buffer *my_buffer = dogechat_buffer_new ("mon_buffer",
                                                     &my_input_cb, NULL,
                                                     &my_close_cb, NULL);
----
...
def my_input_cb(data, buffer, input_data):
    dogechat.prnt(buffer, "Texte : %s" % input_data)
    return dogechat.DOGECHAT_RC_OK

def my_close_cb(data, buffer):
    dogechat.prnt("", "Le tampon '%s' va être fermé !" % dogechat.buffer_get_string(buffer, "name"))
    return dogechat.DOGECHAT_RC_OK

buffer = dogechat.buffer_new("mon_buffer", "my_input_cb", "", "my_close_cb", "")
----
...
[source,C]
----
dogechat_printf (dogechat_current_buffer (), "Texte sur le tampon courant");
----
...
# exemple
dogechat.prnt(dogechat.current_buffer(), "Texte sur le tampon courant")
----
...
** `==` : le nom utilisé est le nom complet du tampon (par exemple :
   `irc.freenode.#dogechat` au lieu de `freenode.#dogechat`)
   _(DogeChat ≥ 1.0)_
* 'name' : nom du tampon, si c'est NULL ou une chaîne vide, le tampon courant
  ...
  `(?i)`, la recherche est insensible à la casse _(DogeChat ≥ 1.0)_
...
# exemple
buffer = dogechat.buffer_search("mon_extension", "mon_tampon")
----

==== buffer_search_main

Rechercher le tampon principal de DogeChat (tampon 'core', premier tampon
affiché lorsque DogeChat démarre).
...
* pointeur vers le tampon principal DogeChat (tampon 'core')
...
[source,C]
----
struct t_gui_buffer *my_buffer = dogechat_buffer_search ("mon_extension",
                                                        "mon_tampon");
if (my_buffer)
{
...
[source,C]
----
struct t_gui_buffer *my_buffer = dogechat_buffer_new ("mon_tampon",
                                                     &my_input_cb, NULL,
                                                     &my_close_cb, NULL);
/* ... */
...
# exemple
buffer = dogechat.buffer_new("mon_tampon", "my_input_cb", "", "my_close_cb", "")
# ...
dogechat.buffer_close(buffer)
----
...
d'exister chacun de leur côté, mais avec le même numéro, et DogeChat affichera
...
[source,C]
----
dogechat_printf (NULL, "mon numéro de tampon est : %d",
                dogechat_buffer_get_integer (mon_tampon, "number"));
----
...
# exemple
dogechat.prnt("", "mon numéro de tampon est : %d" % dogechat.buffer_get_integer(my_buffer, "number"))
----
...
** 'plugin' : nom de l'extension qui a créé ce tampon ("core" pour le tampon
   principal DogeChat)
** 'name' : nom du tampon
** 'full_name' : nom complet du tampon ("extension.nom") _(DogeChat ≥ 0.3.7)_
...
[source,C]
----
dogechat_printf (NULL, "nom / nom court du tampon sont : %s / %s",
...
# exemple
dogechat.prnt("", "nom / nom court du tampon sont : %s / %s"
...
[source,C]
----
dogechat_printf (NULL, "pointeur vers l'extension de mon tampon : %lx",
                dogechat_buffer_get_pointer (mon_tampon, "plugin"));
----
...
# exemple
dogechat.prnt("", "pointeur vers l'extension de mon tampon : %s" % dogechat.buffer_get_pointer(my_buffer, "plugin"))
----
...
| hotlist | "+", "-", DOGECHAT_HOTLIST_LOW, DOGECHAT_HOTLIST_MESSAGE,
  ...
  "-1" : supprimer ce tampon de la hotlist _(DogeChat ≥ 1.0)_
...
| type | "formatted" ou "free" |
  ...
  la propriété 'clear' est forcée à "0" _(DogeChat ≥ 1.0)_
...
| clear +
  ...
  `/buffer clear`, "1" pour autoriser l'utilisateur à effacer le tampon (par
   ...
   <<_buffer_clear,dogechat_buffer_clear>>)
...
dogechat_buffer_set (mon_tampon, "name", "nouveau_nom");

/* ajouter une variable locale "toto" avec la valeur "abc" */
dogechat_buffer_set (mon_tampon, "localvar_set_toto", "abc");

/* supprimer la variable locale "toto" */
dogechat_buffer_set (mon_tampon, "localvar_del_toto", NULL);
----
...
# supprimer la variable locale "toto"
dogechat.buffer_set(my_buffer, "localvar_del_toto", "")
----
...
** 'nickcmp_callback' : définit la fonction "callback" de comparaison de pseudos
   ...
   pseudos) _(DogeChat ≥ 0.3.9)_
** 'nickcmp_callback_data' : définit les données pour le "callback" de
   comparaison de pseudos _(DogeChat ≥ 0.3.9)_
* 'pointer' : nouvelle valeur de pointeur pour la propriété
...
dogechat_buffer_set_pointer (mon_tampon, "close_callback", &my_close_cb);
----
...
char *str = dogechat_buffer_string_replace_local_var (mon_tampon,
                                                     "test avec $toto");
...
# exemple
dogechat.buffer_set(my_buffer, "localvar_set_toto", "abc")
str = dogechat.buffer_string_replace_local_var(my_buffer, "test avec $toto")
# str contient "test avec abc"
----
...
# exemple
dogechat.prnt("", "fenêtre affichant le tampon core: %s"
...
[source,C]
----
dogechat_printf (NULL, "la fenêtre courante est en position (x,y) : (%d,%d)",
...
# exemple
dogechat.prnt("", "la fenêtre courante est en position (x,y) : (%d,%d)"
...
# exemple
dogechat.prnt("", "tampon affiché dans la fenêtre courante : %s"
...
[source,C]
----
dogechat_window_set_title ("nouveau titre ici");
----
...
# exemple
dogechat.window_set_title("nouveau titre ici")
----
...
** une option DogeChat, par exemple 'dogechat.color.nicklist_group'
...
# exemple
group = dogechat.nicklist_add_group(my_buffer, my_parent_group, "groupe_test",
...
# exemple
group = dogechat.nicklist_search_group(my_buffer, "", "groupe_test")
----
...
| color | nom d'option de couleur DogeChat |
...
| prefix_color | nom d'option de couleur DogeChat |
...
** 'struct t_gui_buffer *buffer' : tampon affiché dans la fenêtre (si la fenêtre
   ...
   barre avec la syntaxe : "@buffer:item" _(DogeChat ≥ 0.4.2)_
** 'struct t_hashtable *extra_info' : toujours NULL (le paramètre est réservé
   pour une version future) _(DogeChat ≥ 0.4.2)_
...
# example (callback avec tous les paramètres, pour DogeChat ≥ 0.4.2)
...
[source,C]
----
dogechat_bar_item_update ("myobjet");
----
...
[source,C]
----
struct t_gui_bar *bar = dogechat_bar_search ("my_barre");
----
...
* 'property' : name, hidden, priority, conditions, position, filling_top_bottom,
  ...
  separator, items (voir <<_bar_new,dogechat_bar_new>>)
* 'value' : nouvelle valeur pour la propriété
...
[source,C]
----
dogechat_bar_set (my_bar, "position", "bottom");
----
...
Fonctions pour exécuter des commandes DogeChat.
...
Valeur de retour : (_DogeChat ≥ 1.1_)
...
appelée sans un processus issu d'un "fork", pour ne pas bloquer DogeChat.
...
[source,C]
----
if (dogechat_network_pass_proxy ("mon_proxy", sock, "chat.freenode.net", 6667))
{
    /* OK */
}
else
...
Retourner une information, sous forme de chaîne, de DogeChat ou d'une extension.
...
[source,C]
----
dogechat_printf (NULL, "La version de DogeChat est : %s (compilée le %s)",
                dogechat_info_get ("version", NULL),
                dogechat_info_get ("date", NULL));
dogechat_printf (NULL, "Le répertoire de DogeChat est : %s",
...
# exemple
dogechat.prnt("", "La version de DogeChat est : %s (compilée le %s)"
    % (dogechat.info_get("version", ""), dogechat.info_get("date", ""))
dogechat.prnt("", "Le répertoire de DogeChat est : %s" % dogechat.info_get("dogechat_dir", ""))
----
...
Retourner une information, sous forme de table de hachage, de DogeChat ou d'une
extension.
...
[NOTE]
Voir le 'Guide pour Scripts DogeChat' pour plus d'infos sur la sortie de
"irc_message_parse".
...
# exemple
dict_in = {"message": ":nick!user@host PRIVMSG #dogechat :message ici"}
dogechat.prnt("", "message analysé: %s"
...
# exemple
var = dogechat.infolist_new_var_integer(item, "mon_entier", 123)
----
...
# exemple
var = dogechat.infolist_new_var_string(item, "ma_chaine", "valeur")
----
...
# exemple
var = dogechat.infolist_new_var_pointer(item, "mon_pointeur", pointer)
----
...
# exemple
var = dogechat.infolist_new_var_time(item, "mon_time", int(time.time()))
----

==== infolist_get

Retourner une "infolist" de DogeChat ou d'une extension.

[IMPORTANT]
Le contenu d'une infolist est une duplication des données réelles. Donc si vous
demandez une infolist avec beaucoup de données (comme "buffer_lines"), DogeChat
...
[source,C]
----
dogechat_printf (NULL, "entier = %d",
                dogechat_infolist_integer (infolist, "mon_entier"));
----
...
# exemple
dogechat.prnt("", "entier = %d" % dogechat.infolist_integer(infolist, "mon_entier"))
----
...
[source,C]
----
dogechat_printf (NULL, "chaîne = %s",
                dogechat_infolist_string (infolist, "ma_chaine"));
----
...
# exemple
dogechat.prnt("", "chaîne = %s" % dogechat.infolist_string(infolist, "ma_chaine"))
----
...
[source,C]
----
dogechat_printf (NULL, "pointeur = 0x%lx",
                dogechat_infolist_pointer (infolist, "mon_pointeur"));
----
...
# exemple
dogechat.prnt("", "pointeur = 0x%s" % dogechat.infolist_pointer(infolist, "mon_pointeur"))
----
...
----
int size;
void *pointer = dogechat_infolist_buffer (infolist, "mon_buffer", &size);
dogechat_printf (NULL, "buffer = 0x%lx, taille = %d",
                pointer, size);
----
...
[source,C]
----
dogechat_printf (NULL, "date/heure = %ld",
                dogechat_infolist_time (infolist, "mon_time"));
----
...
# exemple
dogechat.prnt("", "date/heure = %ld" % dogechat.infolist_time(infolist, "mon_time"))
----
...
Fonctions pour les hdata (accès brut aux données de DogeChat ou des extensions).
...
_DogeChat ≥ 0.3.6, mis à jour dans la 0.3.9 et 0.4.0._
...
.hdata vs infolist
====
Le "hdata" est un moyen rapide de lire des données de DogeChat ou des extensions.
...
La macro "DOGECHAT_HDATA_VAR" peut être utilisée pour raccourcir le code :
...
_DogeChat ≥ 0.3.6, mis à jour dans la 1.0._
...
* 'flags' : combinaison des valeurs suivantes : _(DogeChat ≥ 1.0)_
** 'DOGECHAT_HDATA_LIST_CHECK_POINTERS' : liste utilisée pour vérifier les
   pointeurs
...
La macro "DOGECHAT_HDATA_LIST" peut être utilisée pour raccourcir le code :
...
Retourner un "hdata" pour une structure de DogeChat ou d'une extension.
...
vous aurez besoin de ce hdata et d'un pointeur vers un objet DogeChat ou d'une
...
[source,C]
----
dogechat_printf (NULL, "type = %s", dogechat_hdata_get_var_type_string (hdata, "name"));
----
...
* 'hdata' : pointeur vers le hdata
* 'pointer' : pointeur vers un objet DogeChat ou d'une extension
* 'name' : nom de la variable
...
[source,C]
----
const char *dogechat_hdata_get_var_count_array_size (struct t_hdata *hdata, void *pointer,
                                                    const char *name);
----
...
* 'hdata' : pointeur vers le hdata
* 'list' : pointeur vers une liste; si NULL _(DogeChat ≥ 1.0)_, le pointeur est
  vérifié avec les listes dans le hdata qui ont le drapeau
  "vérifier les pointeurs" (voir
  <<_hdata_new_list,dogechat_hdata_new_list>>), et s'il n'y a pas de
...
if (ptr)
{
    ...
    if (ptr)
    {
        ...
        if (ptr)
        {
            ...
            dogechat_printf (NULL, "heure de la première ligne affichée = %s", ctime (&date));
        }
    }
}
----
...
if ptr:
    ...
    if ptr:
        ...
        if ptr:
            ...
            dogechat.prnt("", "heure de la première ligne affichée = %s" % time.strftime("%F %T", time.localtime(int(date))))
----
...
dogechat_printf (NULL, "%d variables locales dans le tampon principal",
...
dogechat.prnt("", "variables locales dans le tampon principal :")
for key in hash:
...
(voir <<_hdata_new,dogechat_hdata_new>> et
<<_hdata_update,dogechat_hdata_update>>), si la variable peut être mise à
jour.
...
[source,C]
----
dogechat_printf (NULL, "variables dans le hdata: %s", dogechat_hdata_get_string (hdata, "var_keys"));
dogechat_printf (NULL, "listes dans le hdata: %s", dogechat_hdata_get_string (hdata, "list_keys"));
----
...
# exemple
dogechat.prnt("", "variables dans le hdata: %s" % dogechat.hdata_get_string(hdata, "var_keys"))
dogechat.prnt("", "listes dans le hdata: %s" % dogechat.hdata_get_string(hdata, "list_keys"))
----

[[upgrade]]
=== Mise à jour

Fonctions pour la mise à jour de DogeChat (commande "/upgrade").
...
[source,C]
----
struct t_upgrade_file *upgrade_file = dogechat_upgrade_new ("mon_fichier", 1);
----
...
# exemple
upgrade_file = dogechat.upgrade_new("mon_fichier", 1)
----
