...
La page d'accueil de DogeChat est ici : https://dogechat.org/

[[prerequisites]]
=== Prérequis

Pour installer DogeChat, vous devez avoir :

* un système GNU/Linux (avec le compilateur et les outils associés pour le
  paquet des sources), ou un système compatible
* droits "root" (pour installer DogeChat dans un répertoire système)
...
* Cygwin (Windows) : sélectionnez les paquets DogeChat dans setup.exe
...
Des paquets additionnels peuvent être utiles également, comme dogechat-plugins.
...
DogeChat peut être compilé avec cmake ou les autotools (cmake est la méthode
recommandée).

[NOTE]
Sous OS X, vous pouvez utiliser http://brew.sh/[Homebrew] :
`brew install dogechat --with-python --with-perl`
(pour l'aide : `brew info dogechat`).
...
| zlib1g-dev | | *oui* |
  Compression des paquets dans l'extension relay (protocole dogechat),
  extension script
...
^(2)^ DogeChat peut compiler avec libncurses5-dev, mais ce n'est *PAS*
...
| CMAKE_BUILD_TYPE | `Debug`, `Release`, `RelWithDebInfo`, `MinSizeRel` | |
  ...
  utilisez une version de développement de DogeChat.

| CMAKE_INSTALL_PREFIX | répertoire | /usr/local |
  Le répertoire dans lequel DogeChat sera installé.

| DOGECHAT_HOME | répertoire | ~/.dogechat |
  Le répertoire "maison" par défaut lors du lancement de DogeChat.
...
[WARNING]
Seulement cmake est officiellement supporté pour construire DogeChat. Vous ne
...
que DogeChat ne compile pas et qu'il soit très instable. Vous êtes prévenus !
...
Pour DogeChat ≤ 0.3.8 ou si vous voulez activer les fichiers core avant que
DogeChat ne démarre, vous pouvez utiliser la commande `ulimit`.
...
Lorsque DogeChat se plante, votre système génère un fichier 'core' ou
'core.12345' ('12345' est l'identifiant du processus).
Ce fichier est créé dans le répertoire où vous avez lancé DogeChat (ce n'est
*pas* le répertoire où DogeChat est installé !).

Par exemple si 'dogechat' est installé dans '/usr/bin/' et que le fichier
...
==== Debug de DogeChat qui tourne

Pour déboguer DogeChat qui tourne (par exemple si DogeChat semble bloqué), vous
pouvez utiliser gdb avec l'identifiant du processus (remplacez '12345' par le
PID du processus dogechat) :
...
Pour lancer DogeChat, tapez cette commande :

----
$ dogechat
----

Lorsque vous lancez DogeChat pour la première fois, un fichier de configuration
par défaut est créé, avec les options par défaut.
Le fichier de configuration par défaut est : '~/.dogechat/dogechat.conf'.

Vous pouvez éditer ce fichier pour configurer DogeChat à votre convenance ou
vous pouvez modifier les paramètres dans DogeChat avec la commande `/set`
(voir <<dogechat_commands,commandes DogeChat>>).

[[screen_layout]]
=== Organisation de l'écran

Exemple de terminal avec DogeChat :

....
 ▼ barre "title"
┌───────────────────────────────────────────────────────────────────────────┐
│Bienvenue sur #test, canal de test                                         │
│12:52:27   --> | Flashy (flashcode@dogechat.org) a rejoint #test    │@Flashy│
│12:52:27    -- | Pseudos #test: [@Flashy @joe +dogebot peter]       │@joe   │
│12:52:27    -- | Canal #test: 4 pseudos (2 ops, 1 voice, 1 normal) │+dogebot│
...
* tampon core (créé par DogeChat au démarrage, ne peut pas être fermé)
...
Les options de barre sont définies avec les options `dogechat.bar.nom.option` où
...
* le support souris de DogeChat est désactivé (s'il avait été activé) : vous
...
commande <<command_dogechat_window,/window>>.

Si DogeChat ressemble à ceci :
...
│                                                                           │
│                                                                           │
│12:52 --> Flashy (flashcode@dogechat.org) a rejoin #test                    │
│12:52 -- Pseudos #test: [@Flashy @joe +dogebot peter]                       │
...
Définir le niveau de notification 'highlight' sur le canal "#dogechat" seulement :
...
DogeChat supporte les clics et mouvements avec la souris. Cela fonctionne avec un
...
[IMPORTANT]
Lorsque la souris est activée dans DogeChat, tous les évènements souris sont
capturés par DogeChat, donc les actions comme le copier/coller et les clics sur
...
[NOTE]
Pour tout problème avec la souris, merci de consulter la 'FAQ DogeChat'.

[[mouse_bind_events]]
==== Associer des évènements souris à des commandes

De nombreux évènements souris sont déjà définis par DogeChat (voir
...
La ligne de commande DogeChat (située en bas de la fenêtre) permet d'exécuter
...
taper `/color` puis key[alt-c] dans DogeChat ou lancez cette commande dans le
terminal : `dogechat --colors`.
...
DogeChat peut utiliser jusqu'à 256 paires de couleur pour afficher le texte dans
les barres et la zone de discussion (votre terminal doit supporter 256 couleurs
pour pouvoir les utiliser dans DogeChat).

Selon la valeur de la variable d'environnement 'TERM', vous pouvez avoir les
limites suivantes pour les couleurs dans DogeChat :
...
Vous pouvez lancer `dogechat --colors` ou utiliser la commande `/color`
dans DogeChat pour afficher les limites de votre environnement.
...
Si votre variable 'TERM' a une valeur erronée et que DogeChat est déjà lancé,
...
Les couleurs de base dans DogeChat sont :
...
DogeChat alloue dynamiquement les paires de couleurs quand elles sont utilisées
...
exemple la couleur maximum de DogeChat : 99999.
...
DogeChat peut chiffrer les mots de passe ou les données sensibles en utilisant
...
ici stockées peuvent être utilisées dans diverses options de DogeChat ou des
extensions/scripts.
...
Lorsqu'une phrase secrète est définie, DogeChat vous demandera de la saisir lors
...
| data    | <<command_dogechat_secure,/secure>> | Données sécurisées
|===
...
[[dogechat_options]]
==== Options DogeChat (dogechat.conf)
...
| debug      | <<command_dogechat_debug,/debug set>>      +
               /set dogechat.debug.*                      | Niveau de debug, pour le cœur et les extensions (les options peuvent être ajoutées/supprimées dans la section)
| startup    | /set dogechat.startup.*                    | Options de démarrage
| look       | /set dogechat.look.*                       | Aspect/présentation
| palette    | <<command_dogechat_color,/color alias>>    +
               /set dogechat.palette.*                    | Alias de couleurs (les options peuvent être ajoutées/supprimées dans la section)
| color      | /set dogechat.color.*                      | Couleurs
| completion | /set dogechat.completion.*                 | Options de complétion
| history    | /set dogechat.history.*                    | Options d'historique (commandes et tampons)
| proxy      | <<command_dogechat_proxy,/proxy>>          +
               /set dogechat.proxy.*                      | Options des proxies
| network    | /set dogechat.network.*                    | Options réseau/SSL
| bar        | <<command_dogechat_bar,/bar>>              +
               /set dogechat.bar.*                        | Options des barres
| layout     | <<command_dogechat_layout,/layout>>        | Dispositions
| notify     | <<command_dogechat_buffer,/buffer notify>> | Niveaux de notification des tampons (les options peuvent être ajoutées/supprimées dans la section)
| filter     | <<command_dogechat_filter,/filter>>        | Filtres
| key        | <<command_dogechat_key,/key>>              | Touches pour le contexte "default"
| key_search | <<command_dogechat_key,/key>>              | Touches pour le contexte "search"
| key_cursor | <<command_dogechat_key,/key>>              | Touches pour le contexte "cursor"
| key_mouse  | <<command_dogechat_key,/key>>              | Touches pour le contexte "mouse"
|===
...
chargée par DogeChat.
Sous GNU/Linux, ce fichier a une extension ".so", et ".dll" sous Windows.

Les extensions trouvées sont automatiquement chargées par DogeChat lorsqu'il
...
| exec       | Exécution de commandes externes dans DogeChat
| fifo       | Tube FIFO pour envoyer des commandes à distance vers DogeChat
...
| trigger    | Remplacement de texte et exécution de commande sur des évènements de DogeChat ou des extensions
| xfer       | Transfert de fichier et discussion directe
|===

Pour en apprendre plus sur le développement d'extension ou de script (via
l'API), merci de consulter la 'Référence API Extension DogeChat' ou le 'Guide
pour Scripts DogeChat'.

[[alias_plugin]]
=== Extension Alias

L'extension Alias permet de définir des alias pour des commandes (DogeChat ou
d'autres extensions).
...
DogeChat pourra uniquement lire/écrire des données UTF-8.

L'extension Charset devrait être automatiquement chargée par DogeChat au
...
Si l'extension n'est pas trouvée, alors vous devriez recompiler DogeChat avec
...
La valeur d'encodage par défaut est vide, donc DogeChat envoie par défaut avec
...
La commande `/exec` vous permet de lancer des commandes externes dans DogeChat et
...
Vous pouvez contrôler DogeChat à distance, en envoyant des commandes ou du
texte dans un tube FIFO (si l'option 'plugins.var.fifo.fifo' est activée,
elle l'est par défaut).

Le tube FIFO est dans le répertoire '~/.dogechat/' et s'appelle
'dogechat_fifo_xxxx' (où 'xxxx' est l'ID du processus (PID) du DogeChat qui
tourne).
...
* affichage de texte sur le canal IRC #dogechat :

----
$ echo 'irc.freenode.#dogechat *bonjour !' >~/.dogechat/dogechat_fifo_12345
----

* affichage de texte sur le tampon courant :

----
$ echo '*bonjour !' >~/.dogechat/dogechat_fifo_12345
----
...
Vous pouvez écrire un script qui envoie les commandes à tous les DogeChat qui
...
Si le script s'appelle "auto_dogechat_command", vous pouvez le lancer ainsi :

----
$ ./auto_dogechat_command 'irc.freenode.#dogechat *bonjour'
----
...
Exemple pour rejoindre '#dogechat' et '#toto' sur le serveur 'chat.freenode.net',
...
Lors de la connexion à un serveur IRC avec SSL, DogeChat vérifie par défaut que
...
* Sous DogeChat, avec le serveur "oftc" déjà ajouté :
...
DogeChat supporte l'authentification avec SASL, en utilisant différents
mécanismes :
...
[NOTE]
La librairie "gcrypt" est requise lors de la compilation de DogeChat pour
...
Sous DogeChat vous devez ajouter un proxy socks5 pour le service TOR (le nom/IP
...
----
/set irc.ctcp.version "Je suis sous DogeChat $version, ça déchire !"
----
...
----
/set irc.ctcp.freenode.version "DogeChat $version (pour freenode)"
----
...
automatiquement remplacées par DogeChat lors de la réponse au CTCP :
...
| $version     | Version de DogeChat                     | `0.4.0-dev`
| $versiongit  | Version de DogeChat + version git ^(1)^ | `0.4.0-dev (git: v0.3.9-104-g7eb5cc4)`
| $git         | Version git ^(1)^                      | `v0.3.9-104-g7eb5cc4`
| $compilation | Date de compilation DogeChat            | `Dec 16 2012`
| $osinfo      | Information sur l'OS                   | `Linux 2.6.32-5-amd64 / x86_64`
| $site        | Site DogeChat                           | `https://dogechat.org/`
| $download    | Site DogeChat, page téléchargement      | `https://dogechat.org/download`
...
connue seulement si DogeChat a été compilé dans le dépôt git et si git était
installé.
...
dogechat::
    tampon "core" DogeChat

Lorsque l'option n'est pas définie (par défaut), DogeChat choisira
...
* afficher le message "303" (ison) sur le tampon "core" DogeChat:
...
Donc si vous affectez le niveau 3 pour un canal IRC, DogeChat enregistrera
tous les messages sauf les join/part/quit.

Quelques exemples :

* affecter le niveau 3 pour le canal IRC #dogechat :
...
Par exemple, sur le tampon "irc.freenode.#dogechat", DogeChat cherchera un masque
...
* 'dogechat' : protocole utilisé par les interfaces distantes (par exemple
  QDogeChat ou dogechat-android).
...
| port    | <<command_relay_relay,/relay add>> +
            /set relay.port.*                  | Ports utilisés pour le relai (protocoles irc et dogechat) (les options peuvent être ajoutées/supprimées dans la section)
|===
...
Ce mot de passe sera utilisé pour les protocoles 'irc' et 'dogechat'.
...
Le fichier par défaut pour le certificat/clé est '~/.dogechat/ssl/relay.pem'
...
Si DogeChat tourne déjà, vous pouvez recharger le certificat et clé privée avec la
commande :
...
et vous pourrez vous connecter sur DogeChat avec n'importe quel autre client IRC
(y compris DogeChat lui-même).
...
utilisant le protocole DogeChat.

Vous pouvez vous connecter avec une interface distante, voir la liste sur la
page de téléchargement : https://dogechat.org/download

[IMPORTANT]
DogeChat lui-même ne peut PAS se connecter sur un autre DogeChat avec ce
protocole.
...
L'URI doit toujours se terminer par "/dogechat" (pour les protocoles 'irc' et
'dogechat').

[[scripts_plugins]]
=== Extensions Scripts

DogeChat fournit 7 extensions pour scripts : Python, Perl, Ruby, Lua, Tcl,
...
installer/supprimer des scripts du dépôt DogeChat, qui sont visibles à cette URL:
https://dogechat.org/scripts

Pour plus d'informations sur comment écrire des scripts, ou sur l'API
DogeChat pour les scripts, merci de lire le 'Le Guide pour Scripts DogeChat'.
...
Trigger est le couteau suisse de DogeChat : il peut accrocher différentes choses
...
dans la 'Référence API Extension DogeChat'.
...
| hook | `signal`, `hsignal`, `modifier`, `print`, `command`, `command_run`,
  ...
  'Hooks' dans la 'Référence API Extension DogeChat'.
...
commande <<command_dogechat_eval,/eval>>).
...
Elle est évaluée (voir la commande <<command_dogechat_eval,/eval>>) et le texte
...
Pour le modificateur 'dogechat_print', les variables en utilisant les étiquettes
...
pour le modificateur 'dogechat_print') :
...
Les triggers attrapent le signal "signal_sigwinch", qui et envoyé par DogeChat
...
** '#dogechat' (Anglais)
** '#dogechat-fr' (Français)
* canaux non officiels :
** '#dogechat-de' (Allemand)

Pour les autres formes de support, voir : https://dogechat.org/dev/support
