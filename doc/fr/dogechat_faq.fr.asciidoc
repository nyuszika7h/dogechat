= DogeChat FAQ (Questions Fréquemment Posées)
...
[NOTE]
Ce document est écrit pour les versions de DogeChat ≥ 0.3.0 mais devrait être
utilisé de préférence avec la dernière version stable de DogeChat.
...
[[dogechat_name]]
=== D'où vient le nom "DogeChat" ?

"Doge" est un acronyme récursif qui signifie "Doge Enhanced Environment".
Donc le nom complet est "Doge Enhanced Environment for Chat".

"Doge" signifie également "très petit" (et oui, il y a une autre signification,
mais elle ne s'applique pas à DogeChat !).

[[why_choose_dogechat]]
=== Pourquoi choisir DogeChat ? X-Chat et Irssi sont si bien...

Parce que DogeChat est très léger et apporte des fonctionnalités innovantes.

Plus d'infos sur cette page : https://dogechat.org/about/features

[[compilation_install]]
== Compilation / installation

[[gui]]
=== J'ai entendu parler de plusieurs interfaces pour DogeChat. Comment puis-je les compiler/utiliser ?

Des interfaces distantes sont disponibles, voir les liens sur la page des
fonctionnalités : https://dogechat.org/about/features

[[compile_git]]
=== Je ne peux pas compiler DogeChat après avoir cloné la base git, pourquoi ?

La méthode recommandée pour compiler DogeChat est d'utiliser cmake.
...
[[compile_osx]]
=== Comment puis-je installer DogeChat sous OS X ?
...
Vous pouvez installer DogeChat avec cette commande :
...
[[lost]]
=== J'ai lancé DogeChat, mais je suis perdu, que puis-je faire ?
...
rapide : https://dogechat.org/doc

[[display]]
== Affichage

[[charset]]
=== Je ne vois pas bien certains accents dans DogeChat, que faire ?

C'est un problème courant, merci de lire attentivement et vérifier *TOUS* les
points ci-dessous :

* vérifiez que dogechat est lié avec libncursesw (attention : nécessaire
  sur beaucoup de distributions, mais pas toutes) :
  `ldd /chemin/vers/dogechat`
...
* vérifiez que l'option 'dogechat.look.eat_newline_glitch' est désactivée (cette
  option peut causer des problèmes d'affichage)

[NOTE]
La locale UTF-8 est recommandée pour DogeChat. Si vous utilisez ISO ou une autre
locale, assurez-vous que *tout* soit configuré en ISO (terminal, screen, ...)
et *pas* en UTF-8.

[[unicode_chars]]
=== Certains caractères unicode sont affichés dans le terminal, mais pas dans DogeChat, pourquoi ?
...
Selon la façon de lancer DogeChat, vous devriez avoir :

* si DogeChat tourne en local ou sur une machine distante sans screen ni tmux :
  cela dépend du terminal utilisé : 'xterm', 'xterm-256color', 'rxvt-unicode',
  'rxvt-256color', etc...
* si DogeChat tourne sous screen, vous devriez avoir 'screen' ou 'screen-256color',
* si DogeChat tourne sous tmux, vous devriez avoir 'tmux', 'tmux-256color',
  'screen' ou 'screen-256color'.

Si besoin, corrigez la variable TERM : `export TERM="xxx"`.

[[screen_weird_chars]]
=== Quand j'utilise dogechat sous screen/tmux, j'ai des caractères bizarres et aléatoires, comment corriger ça ?
...
[[osx_display_broken]]
=== J'ai compilé DogeChat sous OS X, et je vois des "(null)" partout sur l'écran, pourquoi ?

Si vous avez compilé ncursesw vous-même, essayez avec le ncurses standard (celui
fourni avec le système).

De plus, sous OS X, il est recommandé d'installer DogeChat avec le gestionnaire
de paquets Homebrew.
...
barre, DogeChat recréera automatiquement la barre par défaut "input" si l'objet
...
Avec DogeChat ≥ 1.0, vous pouvez utiliser l'affichage dépouillé (touche par
défaut : key[alt-l]).
...
Par défaut, DogeChat affiche l'heure et un préfixe pour chaque ligne avec des
...
Avec DogeChat ≥ 0.3.6, vous pouvez activer l'option "eat_newline_glitch", pour
...
=== Je souhaite changer la langue des messages affichés par DogeChat, mais sans quitter DogeChat, est-ce possible ?

Oui, avec DogeChat ≥ 1.0 :
...
[[use_256_colors]]
=== Comment puis-je utiliser 256 couleurs sous DogeChat ?

Les 256 couleurs sont supportées avec DogeChat ≥ 0.3.4.
...
vous pouvez la changer avec ces deux commandes (avec DogeChat ≥ 1.0) :
...
Pour envoyer le code au démarrage de DogeChat :
...
La souris est supportée avec DogeChat ≥ 0.3.6.
...
[[mouse_select_paste]]
=== Comment puis-je sélectionner ou coller du texte quand la souris est activée dans DogeChat ?

Lorsque la souris est activée dans DogeChat, vous pouvez utiliser la touche
...
chemin vers les certificats sous DogeChat :
...
Vous pouvez essayer une chaîne de priorité différente (DogeChat ≥ 0.3.5
...
Positionnez l'option 'dogechat.network.gnutls_ca_file' avec le fichier des
certificats :
...
Avec DogeChat ≥ 0.4.1, vous pouvez choisir les messages affichés ou non lorsque
...
voice. Par conséquent, vous pouvez changer cela et laisser DogeChat utiliser une
...
Pour vérifier les pseudos absents dans DogeChat, voir la question à propos des
...
[NOTE]
Pour DogeChat ≤ 0.3.3, les options sont 'irc.network.away_check' et
'irc.network.away_check_max_nicks'.

[[highlight_notification]]
=== Comment être averti lorsque quelqu'un prononce mon pseudo sur un canal ?

Avec DogeChat ≥ 1.0, il y a un trigger "beep" qui envoie 'BEL' au terminal sur
...
Avec un DogeChat plus ancien, vous pouvez utiliser un script comme 'beep.pl' or
'launcher.pl'.

Pour 'launcher.pl', vous devez configurer la commande :

----
/set plugins.var.perl.launcher.signal.dogechat_highlight "/chemin/vers/commande paramètres"
----
...
[[openbsd_plugins]]
=== J'utilise OpenBSD et DogeChat ne charge aucune extension, pourquoi ?
...
[[memory_usage]]
=== Comment configurer DogeChat pour consommer moins de mémoire ?
...
* ne pas charger les certificats si SSL n'est *PAS* utilisé : affecter une
  chaîne vide pour l'option 'dogechat.network.gnutls_ca_file'
* réduire la valeur de l'option 'dogechat.history.max_buffer_lines_number' ou
  affecter une valeur à l'option 'dogechat.history.max_buffer_lines_minutes'
* réduire la valeur de l'option 'dogechat.history.max_commands'

[[cpu_usage]]
=== Comment configurer DogeChat pour moins utiliser le processeur ?
...
* supprimer l'affichage des secondes dans l'heure de la barre de statut :
  `/set dogechat.look.item_time_format "%H:%M"` (ceci est la valeur par défaut)
...
Voir : https://dogechat.org/dev/support

[[gdb_error_threads]]
=== Quand je lance DogeChat sous gdb, il y a une erreur à propos des threads, que puis-je faire ?

Quand vous lancez DogeChat sous gdb, vous pouvez avoir cette erreur :
...
chemin vers libpthread et DogeChat avec les chemins sur votre système) :
...
[[supported_os]]
=== Quelle est la liste des plates-formes supportées par DogeChat ? Sera-t-il porté sur d'autres systèmes d'exploitation ?

La liste complète est sur cette page : https://dogechat.org/download
...
[[help_developers]]
=== Je souhaiterais aider les développeurs DogeChat. Que puis-je faire ?
...
support : https://dogechat.org/dev/support

[[donate]]
=== Puis-je donner de l'argent ou d'autres choses aux développeurs DogeChat ?

Vous pouvez donner de l'argent pour aider le développement.
Plus de détails sur https://dogechat.org/about/donate
