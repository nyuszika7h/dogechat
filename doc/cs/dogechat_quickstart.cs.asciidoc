...
DogeChat má nápovědů pro všechny příkazy, postě vložte:
...
(kde `config` je jméno konfigurace (`dogechat` pro jádro, nebo jméno pluginu),
`section` sekce této konfigurace a `option` jméno volby).

DogeChat okamžitě použije novou hodnotu (*nikdy* nebudete potřebovat restart
DogeChatu po změně konfigurace).
...
* zobrazí všechny volby (DogeChatu a pluginů):

----
/set
----

* zobraz volby DogeChatu:
...
Všechna nastavení jsou uložena při ukončení DogeChatu (nebo použitím příkazu
...
"Jádro" DogeChatu je použito pouze k zobrazení dat na obrazovce a k interakci
s uživatelem, což znamená že jádro dogechatu je nepoužitelné bez pluginů.
...
DogeChat používá bezpečné výchozí volby pro všechny servery, pokud nenastavíte
specifické hodnoty pro volby serveru,
Výchozí volby jsou "irc.server_default.*"

Pro každou serverovou volbu DogeChat použije její hodnotu pokud je definována
(není "prázdná"). Jinak DogeChat použije výchozí hodnotu ("irc.server_default.xxx).
...
Ve výchozím nastavení jsou serverové bufery spojen s Dogechat 'core' buferem.
...
Dogechat používá ve výchozím nastavení mnoho klávesových zkratek, Všechny
...
balíček (něco jako dogechat-plugins).
Pluginy jsou automaticky nahrány při nálezení (prosím nahlédněte do
dokumentace DogeChatu jak nahrát/odebrat plugin nebo skript)

Mnoho externích skriptů (od přispěvovatelů) je dostupných pro DogeChat. můžete
...
A seznam skriptů je dostupný v DogeChatu s příkazem `/script` nebo na této
adrese https://dogechat.org/scripts

[[more_doc]]
== Další dokumentace

Nyní jste schopni používat DogeChat a můžete si přečíst FAQ/dokumentaci pro
jakékoliv další otázky: https://dogechat.org/doc

Ať máte radost z použití DogeChatu!
